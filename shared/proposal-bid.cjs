...
function computeProjectCosts(costing, totals, { revenueIssue = null } = {}) {
  ...
  const totalsSaveable = ['annualRecurring', 'monthlyEquivalent', 'oneTime'].every((key) => Number.isFinite(Number(totals?.[key] ?? 0)) && Number(totals?.[key] ?? 0) <= 99999999.99);
  const costsComplete = !revenueIssue && totalsSaveable && revenueYears != null && rows.length > 0 && rows.length <= COST_ROW_LIMITS.rowsMax && rows.every((row) => !costRowIssue(row));
  const byCategory = {};
