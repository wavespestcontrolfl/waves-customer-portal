...
describe('stackDiscounts', () => {
  ...
  test('cent-exact half-cent rounding: 5% of $20.70 is $1.04, not $1.03', () => {
    expect(stackDiscounts(20.7, [{ discountType: 'percentage', amount: 5 }]).items[0].dollars).toBe(1.04);
  });
});
...
describe('stackVisitDiscounts', () => {
  test('Silver on the pest line, Military scoped to it, membership line untouched', () => {
    ...
    expect(result.lines[0].lineDiscountDollars).toBe(11.1);
    expect(result.lines[0].net).toBe(99.9);
    expect(result.lines[1].lineDiscountDollars).toBe(0);
    expect(result.lines[1].net).toBe(60);
  ...
  // Codex pre-push audit P1, round 7 — carried into the client mirror: a
  // wider appointment credit must run BEFORE a narrower line credit when
  // both are fixed, never the reverse.
  test('a wider fixed appointment credit resolves before a narrower fixed line credit', () => {
    const result = stackVisitDiscounts({
      lines: [
        { gross: 50, lineDiscount: { discountType: 'fixed_amount', amount: 80, id: 'line-credit' }, eligible: true },
        { gross: 100, lineDiscount: null, eligible: true },
      ],
      appointmentDiscount: { discountType: 'fixed_amount', amount: 80, id: 'appt-credit' },
    });
