...
describe('stackDocumentDiscounts (client mirror)', () => {
  ...
  // Codex pre-push audit P1, round 7 — the fixed-document-term pass used
  // to iterate line terms unconditionally-first with no canonicalization,
  // so a WIDER document credit lost to a narrower line credit whenever the
  // line credit happened to resolve first.
  it('a wider document credit resolves before a narrower same-value line credit', () => {
    const res = stackDocumentDiscounts({
      lines: [
        { gross: 50, terms: [{ discountType: 'fixed_amount', amount: 80, id: 'line-credit' }] },
        { gross: 100, terms: [] },
      ],
      documentTerms: [{ discountType: 'fixed_amount', amount: 80, id: 'doc-credit' }],
    });
    const net = res.lines.reduce((sum, l) => sum + l.net, 0);
    expect(net).toBe(46.67);
  });
});
