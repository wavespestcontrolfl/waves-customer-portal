  ...
  return (
    <UiSurface>
          ...
          return (
            <div key={r.key}>
              <div className="mb-0.5 flex items-baseline justify-between gap-3 text-ui-caption">
                <span className={cn("min-w-0 truncate", emphasized ? "font-medium text-ink-secondary" : "text-ink-secondary")}>
                  {r.label}
                </span>
                <span className={cn("u-nums whitespace-nowrap", emphasized ? "font-medium" : "", tone.text, r.kind === "result" && "text-14")}>
                  ...
                  {r.marginPct != null && (
                    <span className="ml-1.5 text-ui-caption font-normal text-ink-secondary">{r.marginPct}%</span>
                  )}
                </span>
              </div>
              ...
              {r.key === "cogs" && Array.isArray(bridge.cogsDetail) && bridge.cogsDetail.length > 0 && (
                <div className="mt-1 pl-3 border-l border-hairline border-zinc-200 space-y-0.5">
                  {bridge.cogsDetail.map((d) => (
                    <div key={d.key} className="flex items-baseline justify-between gap-3 text-ui-caption text-ink-secondary">
      ...
      {!bridge.overheadEntered && (
        <div className="mt-3 rounded-sm border border-dashed border-zinc-300 px-3 py-2 text-ui-caption text-ink-secondary">
      ...
      {bridge.overheadEntered && bridge.overheadBasis === "pricing_defaults" && (
        <div className="mt-3 rounded-sm border border-zinc-300 bg-surface-sunken px-3 py-2 text-ui-caption text-ink-secondary">
      ...
      {bridge.overheadEntered && bridge.overheadBasis === "entered" && bridge.overheadEnteredAt && (
        <div className="mt-3 text-ui-caption text-ink-secondary">
