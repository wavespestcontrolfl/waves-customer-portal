...
const TYPE_COLORS = {
  percentage: { bg: "#F4F4F5", c: "#18181B" },
  fixed_amount: { bg: "#FEF3C7", c: "#A16207" },
  variable_amount: { bg: "#F4F4F5", c: "#3F3F46" },
  variable_percentage: { bg: "#DCFCE7", c: "#15803D" },
  free_service: { bg: "#DCFCE7", c: "#15803D" },
};
const TYPE_LABELS = {
...
function DiscountsSection() {
  ...
  const startEdit = (d) => {
    ...
    Object.keys(EMPTY).forEach((k) => {
      if (d[k] !== null && d[k] !== undefined) f[k] = d[k];
    });
    if (f.promo_code_expiry)
      f.promo_code_expiry = toETDateTimeLocal(f.promo_code_expiry);
    setForm(f);
    setEditing(d.id);
    setTab("form");
  };
  ...
  const runPreview = async () => {
    try {
      const r = await af("/admin/discounts/calculate", {
        method: "POST",
        body: JSON.stringify({
    ...
    <div>
        ...
        {tabs
          ...
          .map((t) => (
            <button
              ...
              style={{
                ...sBtn(
                  tab === t.key ? D.teal : "transparent",
                  tab === t.key ? D.white : D.muted,
                ),
                border: tab === t.key ? "none" : `1px solid ${D.border}`,
              }}
            >
              {t.label}
            </button>
      ...
      {tab === "catalog" &&
        (loading ? (
          ...
          >
            No discounts yet. Click <strong>+ New Discount</strong>to add your
            first one.
          </div>
        ) : (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {sortedDiscounts.map((d) => {
                  const tc =
                    TYPE_COLORS[d.discount_type] || TYPE_COLORS.percentage;
                  const rules = [
                    d.requires_military && "Military",
                    d.requires_senior && "Senior",
                    d.requires_multi_home && "Multi-home",
                    d.requires_new_customer && "New",
                    d.requires_prepayment && "Prepay",
                    d.requires_referral && "Referral",
                    d.requires_waveguard_tier && d.requires_waveguard_tier,
                  ].filter(Boolean);
                  const amount = d.discount_type.includes("percentage")
                    ? `${d.amount}%`
                    : d.discount_type.includes("amount") ||
                    ...
                    <tr key={d.id} style={{ opacity: d.is_active ? 1 : 0.45 }}>
                      ...
                      <td style={tdS}>
                        <span style={sBadge(tc.bg, tc.c)}>
                          {TYPE_LABELS[d.discount_type] || d.discount_type}
                        </span>
                      </td>{" "}
                      <td
                        style={{
                      ...
                      <td style={{ ...tdS, fontSize: 11 }}>
                        {rules.length ? (
                          ...
                          <span style={{ color: D.muted }}>None</span>
                        )}
                      </td>{" "}
                      <td style={{ ...tdS, fontSize: 11 }}>
                        {d.stack_group || (
                      ...
                      <td style={tdS}>
                        {d.is_auto_apply ? (
                          <span style={sBadge(D.green + "20", D.green)}>
                            Auto
                          </span>
                        ) : (
                      ...
                      <td style={{ ...tdS, textAlign: "right" }}>
                        ${Number(d.total_discount_given || 0).toFixed(2)}
                      </td>{" "}
                      <td style={{ ...tdS, textAlign: "center" }}>
                        ...
                        <button
                          style={{
                            ...sBtn(
                              d.is_active ? D.green + "20" : D.red + "20",
                              d.is_active ? D.green : D.red,
                            ),
                            fontSize: 11,
                            padding: "4px 10px",
                          }}
      ...
      {tab === "form" && (
        <div style={sCard}>
          ...
          >
            ...
            <label style={{ color: D.muted, fontSize: 11, gridColumn: "1/-1" }}>
              Description
              <input
                style={sInput}
            ...
            <label style={{ color: D.muted, fontSize: 11 }}>
              Amount {form.discount_type.includes("percentage") ? "(%)" : "($)"}
              {form.discount_type.startsWith("variable")
                ? " — base/default"
                : ""}
              <input
                type="number"
            ...
            <label style={{ color: D.muted, fontSize: 11 }}>
              ...
              <input
                ...
                onChange={(e) => upd("max_discount_dollars", e.target.value)}
              />
            </label>{" "}
            <label style={{ color: D.muted, fontSize: 11 }}>
            ...
            >
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 16 }}>
                {" "}
                <label style={{ color: D.text, fontSize: 12 }}>
                  {chk("requires_military")} Military
                </label>{" "}
                <label style={{ color: D.text, fontSize: 12 }}>
                  {chk("requires_senior")} Senior
                </label>{" "}
                <label style={{ color: D.text, fontSize: 12 }}>
                  {chk("requires_multi_home")} Multi-Home
                </label>{" "}
                <label style={{ color: D.text, fontSize: 12 }}>
                  {chk("requires_new_customer")} New Customer
                </label>{" "}
                <label style={{ color: D.text, fontSize: 12 }}>
                  {chk("requires_referral")} Referral
                </label>{" "}
                <label style={{ color: D.text, fontSize: 12 }}>
                  {chk("requires_prepayment")} Prepayment
                </label>{" "}
                <label style={{ color: D.text, fontSize: 12 }}>
                  {chk("is_waveguard_tier_discount")} Tier Discount
                </label>{" "}
              </div>{" "}
            </div>{" "}
            <label style={{ color: D.muted, fontSize: 11 }}>
              Requires WaveGuard Tier
              <select
                style={sInput}
                value={form.requires_waveguard_tier}
                onChange={(e) => upd("requires_waveguard_tier", e.target.value)}
              >
                {" "}
            ...
            <label style={{ color: D.muted, fontSize: 11 }}>
              ...
              <input
                ...
                onChange={(e) => upd("min_subtotal", e.target.value)}
              />
            </label>{" "}
            <label style={{ color: D.muted, fontSize: 11 }}>
              Min Service Count
              <input
                ...
                onChange={(e) => upd("min_service_count", e.target.value)}
              />
            </label>{" "}
            <label style={{ color: D.muted, fontSize: 11 }}>
              Service Key Filter
              <input
                ...
                onChange={(e) => upd("service_key_filter", e.target.value)}
              />
            </label>{" "}
            <div
              style={{
            ...
            <label style={{ color: D.muted, fontSize: 11 }}>
              ...
              <input
                ...
                onChange={(e) => upd("stack_group", e.target.value)}
              />
            </label>{" "}
            <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
              {" "}
              <label style={{ color: D.text, fontSize: 12 }}>
                {chk("is_stackable")} Stackable
              </label>{" "}
              <label style={{ color: D.text, fontSize: 12 }}>
                {chk("is_auto_apply")} Auto-Apply
              </label>{" "}
              <label style={{ color: D.text, fontSize: 12 }}>
            ...
            >
              {" "}
              <label style={{ color: D.text, fontSize: 12 }}>
                {chk("show_in_estimates")} Show in Estimates
              </label>{" "}
              <label style={{ color: D.text, fontSize: 12 }}>
                {chk("show_in_invoices")} Show in Invoices
              </label>{" "}
              <label style={{ color: D.text, fontSize: 12 }}>
                {chk("show_in_scheduling")} Show in Scheduling
              </label>{" "}
            </div>{" "}
            <div
              style={{
            ...
            <label style={{ color: D.muted, fontSize: 11 }}>
              ...
              <input
                ...
                onChange={(e) => upd("promo_code", e.target.value)}
            ...
            <label style={{ color: D.muted, fontSize: 11 }}>
              ...
              <input
                ...
                onChange={(e) => upd("promo_code_expiry", e.target.value)}
              />
            </label>{" "}
            <label style={{ color: D.muted, fontSize: 11 }}>
              Max Uses
              <input
                ...
                onChange={(e) => upd("promo_code_max_uses", e.target.value)}
              />
            </label>{" "}
            <label style={{ color: D.muted, fontSize: 11 }}>
              Color
              <input
                type="color"
                style={{ ...sInput, height: 36, padding: 2 }}
                value={form.color}
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
            {" "}
            <button style={sBtn(D.teal, D.white)} onClick={save}>
              {editing ? "Update" : "Create"}
            </button>{" "}
            <button
              style={sBtn("transparent", D.muted)}
              onClick={() => {
                setTab("catalog");
                setEditing(null);
              }}
        ...
        <div style={sCard}>
          ...
          >
                ...
                >
                  {customers.map((c) => (
                    ...
                    >
                      ...
                      {c.waveguard_tier || "No tier"}{" "}
                      {c.is_military ? "(Military)" : ""}{" "}
                      {c.is_senior ? "(Senior)" : ""}
                    </div>
                  ))}
                </div>
            ...
            <div style={{ alignSelf: "flex-end" }}>
              {" "}
              <button style={sBtn(D.teal, D.white)} onClick={runPreview}>
                Calculate
              </button>{" "}
            </div>{" "}
          </div>
          {previewResult && (
            <div>
              ...
              <div style={{ display: "flex", gap: 20, marginBottom: 16 }}>
                {" "}
                <div style={{ ...sCard, flex: 1, textAlign: "center" }}>
                  <div style={{ color: D.muted, fontSize: 11 }}>Subtotal</div>
                  <div
                    style={{ color: D.heading, fontSize: 20, fontWeight: 700 }}
                  >
                    ${previewResult.subtotal.toFixed(2)}
                  </div>
                </div>{" "}
                <div style={{ ...sCard, flex: 1, textAlign: "center" }}>
                  <div style={{ color: D.muted, fontSize: 11 }}>Discount</div>
                  <div style={{ color: D.red, fontSize: 20, fontWeight: 700 }}>
                ...
                <div style={{ ...sCard, flex: 1, textAlign: "center" }}>
                  ...
                  <div
                    style={{ color: D.green, fontSize: 20, fontWeight: 700 }}
                  >
                    ${previewResult.afterDiscount.toFixed(2)}
                  </div>
                </div>{" "}
              </div>
              {previewResult.discounts.length === 0 && (
                <div
                  style={{ color: D.muted, textAlign: "center", padding: 20 }}
                >
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
            ...
            <div style={{ ...sCard, flex: 1, textAlign: "center" }}>
              ...
              <div style={{ color: D.heading, fontSize: 24, fontWeight: 700 }}>
                {stats.totalApplied}
              </div>
            </div>{" "}
            <div style={{ ...sCard, flex: 1, textAlign: "center" }}>
              ...
              <div style={{ color: D.amber, fontSize: 24, fontWeight: 700 }}>
                  ...
                  <tr key={d.id}>
                    ...
                    <td style={tdS}>
                      {d.is_active ? (
                        ...
                        <span style={sBadge(D.red + "20", D.red)}>
                          Inactive
                        </span>
                      )}
                    </td>{" "}
