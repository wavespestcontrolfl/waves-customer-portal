...
function groupMultiServiceStops(services) {
  ...
  Object.values(groups).forEach((group) => {
    if (group.length === 1) {
      ...
      primary._extraServiceTypes = group
        .slice(1)
        .map((s) => sanitizeServiceTypeClient(s.serviceType));
      result.push(primary);
    }
  });
...
function LeadScoreBadge({ score }) {
  ...
  return (
    <Badge className="ml-1" tone={tone}>
      Lead {score}
    </Badge>
  );
}
    ...
    <div className="flex flex-wrap gap-1 mb-2">
      {alerts.map((a, i) => (
        <Badge key={i} tone="alert">
          {a.icon || "!"} {a.text || a.message || a.label || String(a)}
        </Badge>
      ))}
    </div>
  );
...
function ServiceCardV2({
  ...
  useEffect(() => {
    if (!editing || editQuery.length < 2) {
      setServiceResults([]);
      return;
    }
    const t = setTimeout(() => {
      adminFetch(
        `/admin/services?search=${encodeURIComponent(editQuery)}&is_active=true&limit=10`,
      )
  ...
  async function changeStatus(newStatus, notes) {
    ...
    try {
      await adminFetch(`/admin/schedule/${service.id}/status`, {
        method: "PUT",
        body: JSON.stringify(
          notes ? { status: newStatus, notes } : { status: newStatus },
        ),
      });
    ...
    try {
      await adminFetch(`/admin/schedule/${service.id}/update-details`, {
        ...
        body: JSON.stringify({
          ...
          estimatedDuration: parseInt(editDuration) || 30,
        }),
      });
  ...
  async function sendReview() {
    ...
    if (
      !window.confirm(
        `Send review request SMS to ${service.customerName || "customer"}?`,
      )
    )
      return;
    ...
    >
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-3">
          {" "}
          <span className="u-nums text-14 font-medium text-zinc-900">
            #{service.routeOrder}
          </span>{" "}
      ...
      <div className="flex items-center gap-2 mb-2">
        {" "}
        <span className="u-nums text-12 text-zinc-700">
        ...
        <button
          ...
          onClick={async () => {
            ...
            if (
              !window.confirm(
                `Call ${service.customerName || "customer"} at ${service.customerPhone}?\n\nWaves will call your phone first — press 1 to connect.`,
              )
            )
              return;
            try {
          ...
          <>
            ...
            <div className="relative">
              ...
              <input
                ...
                onBlur={() =>
                  setTimeout(() => setShowServiceResults(false), 150)
                }
                  ...
                  {serviceResults.map((svc) => (
                    <div
                      ...
                      onMouseDown={(e) => {
                        ...
                        if (svc.default_duration_minutes)
                          setEditDuration(String(svc.default_duration_minutes));
                    ...
                    >
                      ...
                      {svc.default_duration_minutes && (
                        <span className="u-nums text-11 text-ink-secondary whitespace-nowrap">
      ...
      <div className="flex gap-3 mb-2">
        ...
        <span
          className={cn(
            ...
            service.reminderSent ? "text-zinc-900" : "text-ink-tertiary",
          )}
        >
          {" "}
          <span
            className={cn(
              "u-dot",
              service.reminderSent ? "u-dot--filled" : "u-dot--hollow",
            )}
          />
          Reminder
        </span>{" "}
        <span
          className={cn(
            "flex items-center gap-1 text-11",
            service.customerConfirmed ? "text-zinc-900" : "text-ink-tertiary",
          )}
        >
          {" "}
          <span
            className={cn(
              "u-dot",
              service.customerConfirmed ? "u-dot--filled" : "u-dot--hollow",
            )}
          />
          Confirmed
        </span>{" "}
        <span
          className={cn(
            "flex items-center gap-1 text-11",
            service.enRouteSent ? "text-zinc-900" : "text-ink-tertiary",
          )}
        >
          {" "}
          <span
            className={cn(
              "u-dot",
              service.enRouteSent ? "u-dot--filled" : "u-dot--hollow",
            )}
          />
          En Route
        </span>{" "}
      ...
      {!service.lastServiceDate && !service.isNewCustomer && (
        <div className="text-11 italic text-ink-secondary mb-2">
        ...
        {status !== "completed" && status !== "skipped" && (
          <>
            ...
            <Button
              ...
              variant="secondary"
              onClick={sendReview}
              disabled={sendingReview || !service.customerId}
            >
            ...
            <div className="relative inline-block">
              ...
              <Button
                size="sm"
                variant="secondary"
                onClick={() => setShowSkipReasons((s) => !s)}
                disabled={updating}
              >
                Skip
              </Button>
        ...
        {isLawn && !lawnDone && (
          <>
            ...
            <Button
              size="sm"
              variant="secondary"
              onClick={() => lawnFileRef.current?.click()}
              disabled={lawnUploading}
            >
...
function TechSectionV2({
  ...
  const completedCount =
    ...
    tech.services.filter((s) => s.status === "completed").length;
  const totalHrs =
    Math.round(
      (((tech.estimatedServiceMinutes || 0) +
        (tech.estimatedDriveMinutes || 0)) /
        60) *
        10,
    ) / 10;
        ...
        <div className="flex-1 min-w-[140px]">
          ...
          <div className="text-14 font-medium text-zinc-900">
            ...
            <span className="u-nums text-12 font-normal text-ink-secondary ml-3">
              {completedCount}/{tech.totalServices || tech.services.length} done
              · ~{totalHrs}h
            </span>{" "}
          </div>
          {tech.zones && Object.keys(tech.zones).length > 0 && (
            <div className="flex gap-3 mt-1 flex-wrap">
              {Object.entries(tech.zones).map(([zone, count]) => (
                <span
                  key={zone}
                  className="flex items-center gap-1.5 text-11 text-ink-secondary"
                >
                  {" "}
                  <span
                    className="w-2 h-2 rounded-full"
                    style={{ background: zoneColors?.[zone] || "#71717A" }}
                  />
            ...
            <div key={svc.id}>
              {svc._extraServiceTypes && svc._extraServiceTypes.length > 0 && (
                <div className="flex gap-1 mb-1 flex-wrap pl-1">
                  ...
                  {[
                    sanitizeServiceTypeClient(svc.serviceType),
                    ...svc._extraServiceTypes,
                  ].map((t, i) => (
                    <Badge key={i} tone="neutral">
                      {t}
                    </Badge>
...
export default function DispatchPageV2({
  ...
  const handleDelete = useCallback(async (service) => {
    const name =
      service.customerName || service.customer_name || "this customer";
    if (
      !window.confirm(
        `Delete service for ${name}?\n\nThis will cancel the scheduled service.`,
      )
    )
      return;
    try {
