...
function SEODashboardTab() {
  ...
  useEffect(() => {
    setLoading(true);
    adminFetch(`/admin/seo/dashboard?period=${period}`)
      .then((d) => {
        setData(d);
        setLoading(false);
      })
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        ...
        <KpiCard
          ...
          sub={
            chg.impressions
              ? {
                  text: `${chg.impressions >= 0 ? "+" : ""}${chg.impressions}% vs prev`,
                  color: posColor(chg.impressions),
                }
              : null
          }
        />{" "}
        <KpiCard
          label="Avg CTR"
          value={(cur.ctr * 100).toFixed(2) + "%"}
          color={cur.ctr > 0.03 ? D.green : D.amber}
        />{" "}
        <KpiCard
          ...
          sub={
            chg.nonbrandClicks
              ? {
                  text: `${chg.nonbrandClicks >= 0 ? "+" : ""}${chg.nonbrandClicks}% vs prev`,
                  color: posColor(chg.nonbrandClicks),
                }
              : null
          }
          color={D.purple}
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 12, color: D.muted, marginBottom: 4 }}>
              Branded (people searching "Waves")
            </div>{" "}
            <div
              style={{
                fontSize: 20,
            ...
            >
              {cur.brandedClicks?.toLocaleString() || 0} clicks
            </div>{" "}
            <div style={{ fontSize: 12, color: D.muted, fontFamily: MONO }}>
              {cur.brandedImpressions?.toLocaleString() || 0} impressions
            </div>{" "}
          </div>{" "}
          <div
            style={{
          ...
          >
            ...
            >
              {cur.nonbrandClicks?.toLocaleString() || 0} clicks
            </div>{" "}
            <div style={{ fontSize: 12, color: D.muted, fontFamily: MONO }}>
              {cur.nonbrandImpressions?.toLocaleString() || 0} impressions
            </div>{" "}
          </div>{" "}
        </div>{" "}
      </Card>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            {data.devices.map((d, i) => {
              ...
              const totalClicks = data.devices.reduce(
                (s, x) => s + parseInt(x.clicks),
                0,
              );
              const pctOfTotal =
                totalClicks > 0
                  ? Math.round((parseInt(d.clicks) / totalClicks) * 100)
                  : 0;
              return (
                <div
                  key={i}
                ...
                >
                  ...
                  >
                    {parseInt(d.clicks).toLocaleString()}
                  </div>{" "}
                  <div style={{ fontSize: 11, color: D.muted }}>
                    {pctOfTotal}% of clicks
                  </div>{" "}
                </div>
              );
            })}
        ...
        <div style={{ overflowX: "auto" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <thead>
              ...
              <tr>
                {" "}
                <th style={thStyle}>Query</th> <th style={thR}>Clicks</th>{" "}
                <th style={thR}>Impr</th> <th style={thR}>CTR</th>{" "}
                <th style={thR}>Position</th> <th style={thStyle}>Service</th>{" "}
              ...
              {filteredQueries.slice(0, 25).map((q, i) => {
                ...
                return (
                  <tr key={i}>
                    ...
                    <td style={{ ...tdText, fontSize: 11, color: D.muted }}>
                      {q.service_category || "—"}
                    </td>{" "}
                    <td style={{ ...tdText, fontSize: 11, color: D.muted }}>
                      {q.city_target || "—"}
      ...
      {(data.opportunities || []).length > 0 && (
        <Card>
          ...
          <div style={{ fontSize: 12, color: D.muted, marginBottom: 16 }}>
            These queries have high impressions but rank on page 2 — push them
            to page 1 for significant traffic gains.
          </div>{" "}
          <div style={{ overflowX: "auto" }}>
            {" "}
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <thead>
                <tr>
                  ...
                  <th style={thR}>Potential Clicks</th>
                ...
                {data.opportunities.slice(0, 15).map((q, i) => (
                  <tr key={i}>
                    ...
                    <td style={{ ...tdR, color: D.amber }}>
                      {parseFloat(q.avg_position).toFixed(1)}
                    </td>{" "}
                    <td style={{ ...tdR, color: D.green }}>
                      {Math.round(parseInt(q.impressions) * 0.08)}
                    </td>{" "}
                    <td style={{ ...tdText, fontSize: 11, color: D.muted }}>
                      {q.service_category || "—"}
                    </td>{" "}
                ...
                {data.topPages.slice(0, 15).map((p, i) => {
                  const ctr =
                    parseInt(p.impressions) > 0
                      ? (
                          (parseInt(p.clicks) / parseInt(p.impressions)) *
                          100
                        ).toFixed(1)
                      : "0";
                  const shortUrl = p.page_url
                    .replace(/https?:\/\/(www\.)?/, "")
                    .replace(/\/$/, "");
                  return (
                    <tr key={i}>
                      ...
                      <td style={tdR}>
                        {parseInt(p.impressions).toLocaleString()}
                      </td>{" "}
                      <td style={tdR}>{ctr}%</td>{" "}
                      <td
                        style={{
                          ...tdR,
                          color:
                            parseFloat(p.avg_position) <= 10
                              ? D.green
                              : D.amber,
                        }}
                      >
                        {parseFloat(p.avg_position).toFixed(1)}
                      </td>{" "}
                      <td style={{ ...tdText, fontSize: 11, color: D.muted }}>
                        {p.page_type || "—"}
                      </td>{" "}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <thead>
                ...
                <tr>
                  {" "}
                  <th style={thStyle}>Page</th>
                  <th style={thStyle}>Device</th> <th style={thR}>LCP (ms)</th>
                  <th style={thR}>INP (ms)</th>
                  <th style={thR}>CLS</th>
                    ...
                    <tr key={i}>
                      ...
                      <td
                        style={{
                          ...tdR,
                          color:
                            parseFloat(c.lcp_p75) <= 2500 ? D.green : D.red,
                        }}
                      >
                        {c.lcp_p75}
                      </td>{" "}
                      <td
                        style={{
                          ...tdR,
                          color: parseFloat(c.inp_p75) <= 200 ? D.green : D.red,
                        }}
                      >
                        {c.inp_p75}
                      </td>{" "}
                      <td
                        style={{
                          ...tdR,
                          color: parseFloat(c.cls_p75) <= 0.1 ? D.green : D.red,
                        }}
                      >
                        {c.cls_p75}
                      </td>{" "}
      ...
      {(data.indexIssues || []).length > 0 && (
        <Card>
          ...
          >
            Indexing Issues ({data.indexIssues.length})
          </div>
          {data.indexIssues.slice(0, 10).map((iss, i) => (
            ...
            >
              ...
              <span style={{ color: D.red, fontSize: 12 }}>
        ...
        <>
          ...
          {data.key_metrics && (
            ...
            >
              ...
              <KpiCard
                label="Total Clicks"
                value={data.key_metrics.totalClicks?.toLocaleString() || "0"}
                sub={
                  data.key_metrics.clicksChange
                    ? {
                        text: data.key_metrics.clicksChange + " vs prev",
                        color: D.muted,
                      }
                    : null
                }
              ...
              <KpiCard
                label="Non-Brand Clicks"
                value={data.key_metrics.nonbrandClicks?.toLocaleString() || "0"}
                color={D.purple}
              />{" "}
              <KpiCard
                label="Impressions"
                value={data.key_metrics.impressionsChange || "—"}
              />{" "}
              <KpiCard
                label="Avg Position"
                value={data.key_metrics.avgPosition || "—"}
              />{" "}
            </div>
          )}
            ...
            <Card>
              ...
              {data.page2_opportunities.map((opp, i) => (
                ...
                >
                  ...
                  <div style={{ fontSize: 12, color: D.muted }}>
                    {opp.impressions} impressions • {opp.action}
                  </div>{" "}
                </div>
              ))}
            </Card>
            ...
            <Card>
              ...
              {data.declining_alerts.map((a, i) => (
                ...
                >
                  ...
                  <div style={{ fontSize: 13, color: D.heading }}>
                    "{a.query}" —{" "}
                    <span style={{ color: D.red }}>{a.drop_pct}%</span>decline
                  </div>
                  {a.action && (
                    <div style={{ fontSize: 12, color: D.muted }}>
                      {a.action}
              ...
              {data.technical_issues.map((t, i) => (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 8,
                    marginBottom: 6,
                    borderLeft: `3px solid ${t.severity === "high" ? D.red : D.amber}`,
                  }}
                >
                  {" "}
                  <div style={{ fontSize: 13, color: D.heading }}>
...
function RankingsTab() {
  ...
  useEffect(() => {
    setLoading(true);
    adminFetch(`/admin/seo/rankings?days=${days}`)
      .then((d) => {
        setData(d);
        setLoading(false);
      })
  ...
  if (!data)
    return (
      <Card style={{ padding: 40, textAlign: "center" }}>
        <div style={{ color: D.muted }}>
          No ranking data yet. Configure DataForSEO credentials and enable
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <thead>
              <tr>
                ...
                <th style={thStyle}>City</th> <th style={thR}>Position</th>
                <th style={thR}>Map Pack</th>
                <th style={thR}>Change</th> <th style={thStyle}>AIO</th>
              ...
              {(data.rankings || []).map((r, i) => (
                <tr key={i}>
                  ...
                  <td
                    style={{
                      ...tdR,
                      color: r.mapPackPosition ? D.green : D.muted,
                    }}
                  >
                    {r.mapPackPosition || "—"}
                  </td>{" "}
                  <td style={{ ...tdR, color: deltaColor(r.delta) }}>
                    {r.delta > 0 ? `+${r.delta}` : r.delta || "—"}
                  </td>{" "}
                  <td style={{ ...tdStyle, textAlign: "center" }}>
                    {r.aiOverviewCited ? "Yes" : "—"}
                  </td>{" "}
                  <td style={{ ...tdStyle, fontSize: 11, color: D.muted }}>
                    {r.serpFeatures
                      ? Object.entries(
                          typeof r.serpFeatures === "string"
                            ? JSON.parse(r.serpFeatures)
                            : r.serpFeatures,
                        )
                          .filter(([, v]) => v)
                          .map(([k]) => k)
...
function BacklinksTab() {
  ...
  if (!data)
    return (
      <Card style={{ padding: 40, textAlign: "center" }}>
        <div style={{ color: D.muted }}>No backlink data yet.</div>
      </Card>
    );
...
function ContentQATab() {
  ...
  if (!data || data.total === 0)
    return (
      <Card style={{ padding: 40, textAlign: "center" }}>
        <div style={{ color: D.muted }}>
          No QA scores yet. Run a batch score from the API.
        </div>
      </Card>
    );
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <thead>
              <tr>
                ...
                <th style={thR}>Grade</th>
                <th style={thR}>Tech</th>
                <th style={thR}>OnPage</th>
                <th style={thR}>E-E-A-T</th>
                <th style={thR}>Local</th>
                <th style={thR}>Brand</th>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {(data.quickWins || []).length > 0 && (
        <Card>
          ...
          >
            Quick Wins — AIO exists but Waves not cited
          </div>
          {data.quickWins.map((r, i) => (
            ...
            >
              ...
              <div style={{ fontSize: 13, color: D.heading }}>
                "{r.keyword}"{" "}
                <span style={{ color: D.muted, fontSize: 11 }}>({r.city})</span>
              </div>{" "}
              <div style={{ fontSize: 11, color: D.muted }}>
                Currently cited:{" "}
                {r.sources.map((s) => s.domain).join(", ") || "unknown"}
              </div>{" "}
            </div>
          ))}
        </Card>
            ...
            <tbody>
              {(data.results || []).map((r, i) => (
                <tr key={i}>
                  ...
                  <td style={{ ...tdR }}>{r.aioPresent ? "Yes" : "—"}</td>{" "}
                  <td
                    style={{ ...tdR, color: r.wavesCited ? D.green : D.muted }}
                  >
                    {r.wavesCited ? "Yes" : "—"}
                  </td>{" "}
                  <td style={{ ...tdStyle, fontSize: 11, color: D.muted }}>
...
function SEOFunnelTab() {
  ...
  useEffect(() => {
    setLoading(true);
    adminFetch(`/admin/seo/funnel?days=${days}`)
      .then((d) => {
        setData(d);
        setLoading(false);
      })
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        <KpiCard
          ...
          value={e.booked || 0}
          color={D.green}
          sub={{ text: `${e.conversionRate || 0}% rate` }}
        />{" "}
        <KpiCard
          label="Revenue"
            ...
            <tbody>
              {(data.funnelByPage || []).slice(0, 20).map((f, i) => (
                <tr key={i}>
                  ...
                  >
                    {(f.landingPage || "").replace(/https?:\/\/[^/]+/, "")}
                  </td>{" "}
                  <td style={tdR}>{f.impressions.toLocaleString()}</td>{" "}
                  <td style={tdR}>{f.clicks}</td> <td style={tdR}>{f.ctr}%</td>{" "}
                  <td
                    style={{
                      ...tdStyle,
                      fontFamily: "inherit",
...
function CitationsTab() {
  ...
  if (!data)
    return (
      <Card style={{ padding: 40, textAlign: "center" }}>
        <div style={{ color: D.muted }}>No citation data.</div>
      </Card>
    );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {data.canonicalNAP && (
        <Card style={{ padding: 16 }}>
          ...
          <div style={{ fontSize: 12, color: D.text }}>
            {data.canonicalNAP.name} · {data.canonicalNAP.phone} ·{" "}
            {data.canonicalNAP.website}
          </div>{" "}
        </Card>
      )}
      <Card>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <Card style={{ padding: 16, textAlign: "center" }}>
          ...
          {run.score_delta != null && (
            <div
              style={{
                fontSize: 11,
                color: parseFloat(run.score_delta) >= 0 ? D.green : D.red,
              }}
            >
              {parseFloat(run.score_delta) >= 0 ? "+" : ""}
              {parseFloat(run.score_delta).toFixed(1)} vs last
            </div>
          )}
        </Card>{" "}
        <KpiCard
          label="Healthy (80+)"
          value={run.pages_healthy || 0}
          color={D.green}
        />{" "}
        <KpiCard
          label="Warning (50-79)"
          value={run.pages_warning || 0}
          color={D.amber}
        />{" "}
        <KpiCard
          label="Critical (<50)"
          value={run.pages_critical || 0}
          color={D.red}
        />{" "}
      </div>{" "}
      <div
      ...
      >
        ...
        <KpiCard
          label="Broken Links"
          value={run.pages_with_broken_links || 0}
          color={run.pages_with_broken_links > 0 ? D.red : D.green}
        />{" "}
        <KpiCard
          label="Missing Schema"
          value={run.pages_missing_schema || 0}
          color={run.pages_missing_schema > 0 ? D.amber : D.green}
        />{" "}
        <KpiCard
          label="Thin Content"
          value={run.pages_thin_content || 0}
          color={run.pages_thin_content > 0 ? D.amber : D.green}
        />{" "}
        <KpiCard
          label="Failing CWV"
          value={run.pages_failing_cwv || 0}
          color={run.pages_failing_cwv > 0 ? D.red : D.green}
        />{" "}
      </div>
      {/* Issues by category */}
          ...
          {data.issues.map((issue, i) => (
            <div
              ...
              style={{
                ...
                borderRadius: 6,
                marginBottom: 4,
                borderLeft: `3px solid ${sevColor[issue.severity] || D.muted}`,
              }}
            >
              {" "}
              <div
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <thead>
                <tr>
                  ...
                  <th style={thR}>Critical</th>
                  <th style={thR}>Warning</th>
                ...
                {data.pages.slice(0, 30).map((p, i) => (
                  <tr key={i}>
                    ...
                    <td
                      style={{
                        ...tdR,
                        color: scoreColor(p.technical_health_score),
                        fontWeight: 700,
                      }}
                    >
                    ...
                    <td
                      style={{
                        ...tdR,
                        color: p.issue_count_critical > 0 ? D.red : D.muted,
                      }}
                    >
                      {p.issue_count_critical}
                    </td>{" "}
                    <td
                      style={{
                        ...tdR,
                        color: p.issue_count_warning > 0 ? D.amber : D.muted,
                      }}
                    >
                      {p.issue_count_warning}
                    </td>{" "}
                    <td style={{ ...tdStyle, fontSize: 11 }}>
                      {p.has_local_business_schema ? "Yes" : "—"}{" "}
                      {p.has_faq_schema ? "FAQ" : ""}
                    </td>{" "}
                    <td style={{ ...tdStyle }}>
                      {p.nap_present ? "Yes" : "—"}
                    </td>{" "}
