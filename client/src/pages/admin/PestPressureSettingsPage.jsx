...
export default function PestPressureSettingsPage() {
  ...
  const submitOverride = async () => {
    ...
    // Reject blank explicitly — Number("") === 0, which would silently
    // submit a 0 override on an empty field.
    const trimmedScore = String(overrideScore).trim();
    const num = Number(trimmedScore);
    if (trimmedScore === "" || !Number.isFinite(num) || num < 0 || num > 5) {
        ...
        <Card>
          ...
          <div style={{ marginTop: 16 }}>
            <Toggle
              ...
              description="When on, services explicitly labelled 'one-time' are skipped — the model needs a recurring plan to compare against. Unknown-frequency labels (e.g. 'General Pest Control') are treated as recurring."
