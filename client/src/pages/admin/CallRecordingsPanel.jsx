...
function getClassification(recording) {
  ...
  if (recording.processing_status === "spam" || extraction?.is_spam)
    return { label: "Spam", color: D.gray, bg: `${D.gray}22` };
  if (recording.processing_status === "voicemail" || extraction?.is_voicemail)
    return { label: "Voicemail", color: D.amber, bg: `${D.amber}22` };
  if (recording.customer_id && recording.processing_status === "processed") {
    ...
    if (
      extraction?.lead_quality === "cold" ||
      extraction?.lead_quality === "warm"
    )
      return { label: "Existing Customer", color: D.green, bg: `${D.green}22` };
    return { label: "New Lead", color: D.blue, bg: `${D.blue}22` };
  }
  if (recording.processing_status === "processed")
    return { label: "New Lead", color: D.blue, bg: `${D.blue}22` };
  return null;
}
...
function getActionStatus(recording) {
  ...
  if (recording.customer_id && extraction.appointment_confirmed)
    return { text: "Lead created", icon: " \u2713", color: D.green };
  if (recording.customer_id && !extraction.appointment_confirmed)
    return { text: "Follow-up needed", icon: " \u26A0\uFE0F", color: D.amber };
  if (!recording.customer_id && !extraction.is_spam && !extraction.is_voicemail)
    return { text: "Follow-up needed", icon: " \u26A0\uFE0F", color: D.amber };
...
function parseExtraction(recording) {
  ...
  try {
    return typeof recording.ai_extraction === "string"
      ? JSON.parse(recording.ai_extraction)
      : recording.ai_extraction;
  } catch {
    return null;
  }
}
...
export default function CallRecordingsPanel() {
  ...
  const loadData = useCallback(async () => {
    const [s, r] = await Promise.all([
      ...
      adminFetch("/admin/call-recordings/recordings?limit=30").catch(() => ({
        recordings: [],
      })),
    ]);
    setStats(s);
    ...
    <div>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            {" "}
            <div style={{ fontSize: 20, fontWeight: 700, color: D.heading }}>
              Call Recording Processor
            </div>{" "}
            <div style={{ fontSize: 13, color: D.muted, marginTop: 4 }}>
              AI transcription + customer extraction
            </div>{" "}
          </div>{" "}
          <span
            style={{
              ...sBadge(`${D.green}22`, D.green),
              fontSize: 11,
              padding: "4px 10px",
            }}
          >
        ...
        <button
          onClick={processAll}
          disabled={processing}
          style={{ ...sBtn(D.teal, D.white), opacity: processing ? 0.5 : 1 }}
        >
                    ...
                    >
                      ...
                      {r.sentiment &&
                        (() => {
                          ...
                          return (
                            <span style={sBadge(`${sc.color}22`, sc.color)}>
                              {r.sentiment}
                            </span>
                          );
                        })()}
          ...
          >
            {stats.sourceBreakdown.map((s, i) => (
              ...
              >
                ...
                {i < stats.sourceBreakdown.length - 1 && (
...
function RecordingDetail({ recording, onClose, onUpdate }) {
  ...
  const handleGenerateSynopsis = async () => {
    ...
    try {
      const result = await adminFetch(
        `/admin/call-recordings/synopsis/${r.twilio_call_sid}`,
        { method: "POST" },
      );
      if (result?.synopsis) {
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith("## ")) {
        ...
        elements.push(
          ...
          >
            {" "}
            <span style={{ position: "absolute", left: 0, color: D.muted }}>
              -
            </span>
            {parts.map((part, i) =>
        ...
        elements.push(
          <div
            key={key++}
            style={{ fontSize: 12, color: D.text, lineHeight: 1.6 }}
          >
            {parts.map((part, i) =>
              part.startsWith("**") && part.endsWith("**") ? (
                <strong key={i} style={{ color: D.heading, fontWeight: 600 }}>
  ...
  const canProcess =
    ...
    r.processing_status === "no_transcription";
  const canGenerateSynopsis =
    r.transcription && r.processing_status === "processed";
  ...
  return (
    ...
    >
      ...
      <div style={sCard}>
        ...
        >
          {classification && (
            <span style={sBadge(classification.bg, classification.color)}>
              {classification.label}
            </span>
          )}
          {action && (
            <span
              style={sBadge(
                action.color === D.green ? `${D.green}22` : `${action.color}22`,
                action.color,
              )}
            >
              {action.text}
        ...
        >
          {canProcess && (
            <button
              ...
              disabled={processingOne}
              style={{
                ...sBtn(D.teal, D.white),
                opacity: processingOne ? 0.5 : 1,
              }}
            >
          ...
          {canGenerateSynopsis && (
            <button
              ...
              style={{
                ...
                opacity: generatingSynopsis ? 0.5 : 1,
              }}
            >
              {generatingSynopsis
                ? "Generating..."
          ...
          <>
            ...
            >
              {[
                [
                  "Name",
                  `${extraction.first_name || ""} ${extraction.last_name || ""}`,
                ],
                ...
                [
                  "Service",
                  extraction.matched_service || extraction.requested_service,
                ],
                [
                  "Appointment",
                  extraction.appointment_confirmed
                    ? `Yes -- ${extraction.preferred_date_time}`
                    : "No",
                ],
                ...
                ([label, value]) =>
                  value && (
                    <div key={label} style={{ display: "contents" }}>
                      {" "}
                      <span style={{ color: D.muted, padding: "4px 0" }}>
                        {label}
                      </span>{" "}
                      <span style={{ color: D.heading, padding: "4px 0" }}>
                        {value}
                      </span>{" "}
                    </div>
                  ),
