...
function RepeatCommsNotification() {
  const navigate = useNavigate();
  return <button onClick={() => navigate('/admin/customers?customerId=customer-a&tab=comms')}>Open SMS notification</button>;
}
...
describe('CustomersPageV2 workflow state', () => {
  ...
  it('reopens Comms when the same notification URL is visited after a local tab change', async () => {
    vi.stubGlobal('fetch', vi.fn((url) => String(url).includes('/admin/customers?') ? response(list) : response({})));
    render(<MemoryRouter initialEntries={['/admin/customers?customerId=customer-a&tab=comms']}>
      <RepeatCommsNotification /><CustomersPageV2 />
    </MemoryRouter>);
