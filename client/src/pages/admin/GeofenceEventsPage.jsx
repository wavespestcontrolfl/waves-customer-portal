...
import { Badge, Card, CardBody, Select, cn } from "../../components/ui";
...
function actionVariant(action) {
  ...
  if (action === "timer_started" || action === "timer_stopped")
    return "success";
  return "muted";
}
...
function variantClasses(variant) {
  switch (variant) {
    case "alert":
      return "bg-alert-fg/10 text-alert-fg border-alert-fg/30";
    case "success":
...
export default function GeofenceEventsPage() {
  ...
  const groupTotals = useMemo(() => {
    const totals = {};
    GROUPS.forEach((g) => {
      totals[g.key] = g.actions.reduce(
        (sum, a) => sum + (countMap.get(a) || 0),
        0,
      );
    });
    return totals;
    ...
    <div className="max-w-[1300px] mx-auto">
      ...
      {error && (
        <Card className="mb-6 border-alert-fg/40">
          ...
          <CardBody>
            {" "}
            <div className="text-14 text-alert-fg">
                ...
                {g.actions.map((a) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setActionFilter(actionFilter === a ? "" : a)
                      }
                      className={cn(
              ...
              <table className="w-full text-14">
                ...
                <thead>
                  ...
                  <tr className="border-b border-hairline text-ink-tertiary">
                    {" "}
                    <th className="text-left font-medium py-2 px-2 uppercase text-12 tracking-label">
                      When
                    </th>{" "}
                    <th className="text-left font-medium py-2 px-2 uppercase text-12 tracking-label">
                      Type
                ...
                <tbody>
                  {events.map((e) => (
                    <tr
                      key={e.id}
                      className="border-b border-hairline hover:bg-zinc-50"
                    >
                      {" "}
