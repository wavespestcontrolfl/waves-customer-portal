...
function KpiCard({ label, value, sub, color }) {
  return (
    ...
    >
      {" "}
      <div style={{ fontSize: 13, color: D.muted, fontWeight: 500 }}>
        {label}
      </div>{" "}
      <div
        style={{
...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ color: D.teal, fontFamily: MONO }}>
        {fmt(payload[0].value)} revenue
      </div>
      {payload[1] && (
        <div style={{ color: D.muted, fontFamily: MONO }}>
          {fmt(payload[1].value)} cost
        </div>
      )}
    </div>
  );
...
function AdAttributionSection({ period }) {
  ...
  useEffect(() => {
    adminFetch(`/admin/ads/revenue-attribution?period=${period}`)
      .then((d) => setAttr(d))
      .catch(() => {});
  }, [period]);
      ...
      >
        ...
        {attr.blendedROAS && (
          <div style={{ fontSize: 13, color: D.muted }}>
            Blended ROAS:{" "}
            <span style={{ color: D.teal, fontFamily: MONO, fontWeight: 700 }}>
              {attr.blendedROAS}x
            </span>
          </div>
        )}
...
export default function RevenuePage() {
  ...
  if (!data)
    return (
      <div style={{ color: D.red, padding: 60, textAlign: "center" }}>
  ...
  const avgRevenuePerJob =
    t.totalServices > 0 ? Math.round(t.totalRevenue / t.totalServices) : 0;
  ...
  const kpis = [
    {
      ...
      sub:
        vs.revenueChange != null
          ? {
              text: `${vs.revenueChange >= 0 ? "+" : ""}${vs.revenueChange.toFixed(1)}% vs last period`,
              color: vs.revenueChange >= 0 ? D.green : D.red,
            }
          : null,
    },
    {
      label: "Gross Margin %",
      value: `${t.grossMarginPct}%`,
      color: marginColor(t.grossMarginPct),
    },
    {
      label: "Revenue / Man-Hour",
      value: fmt(t.revenuePerManHour),
      color: rpmhColor(t.revenuePerManHour),
    },
            ...
            <tbody>
              {(data.byTier || []).map((t, i) => (
                <tr key={i}>
                  ...
                  <td style={{ ...tdStyle, textAlign: "right" }}>
                    {t.customers > 0
                      ? fmt(Math.round(t.revenue / t.customers))
                      : "$0"}
                  </td>{" "}
                  <td style={{ ...tdStyle, textAlign: "right" }}>
                    {t.services}
