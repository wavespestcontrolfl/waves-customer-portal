...
function StatCard({ label, value, color, sub }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 22, fontWeight: 700, color: color || D.heading }}>
        {value}
      </div>
      {sub && (
...
export default function EquipmentMaintenancePage() {
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (sortBy === "condition")
        return (a.condition_rating || 0) - (b.condition_rating || 0);
      if (sortBy === "cost")
        return (b.avg_maintenance_cost || 0) - (a.avg_maintenance_cost || 0);
      return 0;
    });
    return list;
  ...
  const categories = useMemo(
    () => [...new Set(equipment.map((e) => e.category).filter(Boolean))].sort(),
    [equipment],
  );
  ...
  const dismissAlert = async (id) => {
    try {
      await af(`/admin/equipment-maintenance/alerts/${id}`, {
        method: "PUT",
        body: JSON.stringify({ status: "resolved", resolved_by: "admin" }),
      });
    ...
    <>
      ...
      {overview && (
        ...
        >
          ...
          <StatCard
            ...
            value={overview.overdue_maintenance}
            color={overview.overdue_maintenance > 0 ? D.red : D.green}
          />{" "}
          <StatCard
            label="YTD Maintenance"
            value={fmt(overview.ytd_maintenance_spend)}
          />{" "}
          <StatCard
            label="YTD Mileage"
            value={fmtN(Math.round(overview.ytd_total_miles))}
            sub="miles"
          />{" "}
          <StatCard label="YTD Fuel" value={fmt(overview.ytd_fuel_cost)} />{" "}
          <StatCard
            label="YTD IRS Deduction"
            value={fmt(overview.ytd_irs_deduction)}
            color={D.green}
          />{" "}
        </div>
      )}
      ...
      >
        ...
        <select
          ...
          style={{ ...sInput, width: "auto", minWidth: 140 }}
        >
          {" "}
        ...
        <select
          ...
          style={{ ...sInput, width: "auto", minWidth: 130 }}
        >
          {" "}
      ...
      <div
        style={{
          ...
          gridTemplateColumns: isMobile
            ? "1fr"
            : "repeat(auto-fill, minmax(340, 1fr))",
          gap: 12,
        }}
      >
        {filtered.map((eq) => (
  ...
  useEffect(() => {
    if (isExpanded && !detail) {
      Promise.all([
        ...
        eq.category === "vehicle"
          ? af(`/admin/equipment-maintenance/${eq.id}/mileage?limit=30`)
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            {" "}
            <span style={{ fontSize: 15, fontWeight: 700, color: D.heading }}>
              {eq.name}
            </span>{" "}
          ...
          <div style={{ fontSize: 11, color: D.muted, marginTop: 2 }}>
            {eq.asset_tag && (
              <span style={{ marginRight: 12 }}>{eq.asset_tag}</span>
            )}
            {eq.make && (
          ...
          >
            {eq.assigned_tech_name !== "Unassigned" && (
              <span>Assigned: {eq.assigned_tech_name}</span>
            )}
            {eq.current_miles > 0 && <span>{fmtN(eq.current_miles)} mi</span>}
            {parseFloat(eq.current_hours) > 0 && (
              <span>{fmtN(eq.current_hours)} hrs</span>
            )}
          </div>{" "}
        </div>{" "}
      </div>
      ...
      {isExpanded && detail && (
        ...
        >
          ...
          >
            ...
            <InfoRow
              label="Serial"
              value={detail.equipment.serial_number}
            />{" "}
            ...
            <InfoRow
              ...
              value={
                detail.equipment.purchase_price
                  ? fmt(detail.equipment.purchase_price)
                  : null
              }
            />{" "}
            <InfoRow
            ...
            <InfoRow
              label="Depreciation"
              value={detail.equipment.depreciation_method}
            />{" "}
          </div>
              ...
              >
                ...
                <tbody>
                  {(detail.schedules || []).map((s) => {
                    const intervals = [];
                    if (s.interval_miles)
                      intervals.push(`${fmtN(s.interval_miles)} mi`);
                    if (s.interval_hours)
                      intervals.push(`${s.interval_hours} hrs`);
                    if (s.interval_days)
                      intervals.push(`${s.interval_days} days`);
                    if (s.interval_months)
                      intervals.push(`${s.interval_months} mo`);
                    return (
                      <tr
                        key={s.id}
                        style={{
                          borderBottom: `1px solid ${D.border}`,
                          background: s.is_overdue
                            ? "rgba(239,68,68,0.1)"
                            : "transparent",
                        }}
                      >
                        ...
                        <td style={{ padding: "6px 8px", color: D.muted }}>
                          {intervals.join(" / ") || "--"}
                        </td>{" "}
                        <td
                          style={{
          ...
          >
            ...
            {eq.category === "vehicle" && (
              <button
                onClick={() => setMileageForm(!mileageForm)}
          ...
          {detail.costOfOwnership && (
            <div style={{ ...sCard, background: D.darkCard }}>
              ...
              >
                ...
                <div>
                  <div style={{ color: D.muted }}>Total Cost</div>
                ...
                {detail.costOfOwnership.total_irs_deduction > 0 && (
                  <div>
                    <div style={{ color: D.muted }}>IRS Deduction</div>
                    <div style={{ color: D.green, fontWeight: 700 }}>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              >
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 16, fontWeight: 700, color: D.heading }}
                  >
                ...
                >
                  ...
                  <div style={{ fontSize: 16, fontWeight: 700, color: D.teal }}>
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 16, fontWeight: 700, color: D.amber }}
                  >
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 16, fontWeight: 700, color: D.green }}
                  >
              ...
              <div
                style={{ overflowX: "auto", maxHeight: 300, overflowY: "auto" }}
              >
                {" "}
                <table
                  style={{
    ...
    try {
      await af(`/admin/equipment-maintenance/${equipmentId}/records`, {
        ...
        body: JSON.stringify({
          ...
          milesAtService: form.milesAtService
            ? parseInt(form.milesAtService)
            : null,
          hoursAtService: form.hoursAtService
            ? parseFloat(form.hoursAtService)
            : null,
          conditionBefore: form.conditionBefore
            ? parseInt(form.conditionBefore)
            : null,
          conditionAfter: form.conditionAfter
            ? parseInt(form.conditionAfter)
            : null,
    ...
    <div style={{ ...sCard, background: D.darkCard, marginBottom: 16 }}>
      ...
      >
        ...
        <div>
          ...
          <input
            type="number"
            value={form.milesAtService}
            onChange={(e) => set("milesAtService", e.target.value)}
            style={sInput}
          />{" "}
        </div>{" "}
        <div>
          ...
          <input
            ...
            onChange={(e) => set("hoursAtService", e.target.value)}
            style={sInput}
          />{" "}
        </div>{" "}
        <div>
          ...
          <input
            ...
            onChange={(e) => set("conditionBefore", e.target.value)}
            style={sInput}
          />{" "}
        </div>{" "}
        <div>
          ...
          <input
            ...
            onChange={(e) => set("conditionAfter", e.target.value)}
            style={sInput}
          />{" "}
        </div>{" "}
        <div>
        ...
        >
          ...
          >
            ...
            <input
              ...
              onChange={(e) => set("warrantyClaim", e.target.checked)}
            />
            Warranty claim
          </label>{" "}
        </div>
...
function MileageForm({ vehicleId, currentMiles, onDone }) {
  ...
  const totalMiles =
    (parseInt(form.odometerEnd) || 0) - (parseInt(form.odometerStart) || 0);
  const irsDeduction =
    totalMiles > 0
      ...
      >
        ...
        <div>
          ...
          <input
            type="number"
            step="0.1"
            value={form.personalMiles}
            onChange={(e) => set("personalMiles", e.target.value)}
            style={sInput}
          />{" "}
        </div>{" "}
        <div>
        ...
        <div>
          ...
          <input
            type="number"
            value={form.jobsServiced}
            onChange={(e) => set("jobsServiced", e.target.value)}
            style={sInput}
          />{" "}
        </div>{" "}
        <div>
...
function AnalyticsTab({
  ...
  return (
    <>
              ...
              {costs.map((c) => (
                ...
                >
                  ...
                  <td style={{ padding: "8px", color: D.text }}>
                    ...
                    {c.asset_tag && (
                      <span
                        style={{ color: D.muted, fontSize: 10, marginLeft: 6 }}
                      >
                        {c.asset_tag}
                      </span>
                    )}
      ...
      {mileageSummary &&
        mileageSummary.vehicles &&
        mileageSummary.vehicles.length > 0 && (
          <div style={{ ...sCard, marginBottom: 16 }}>
            {" "}
            <div
              style={{
                ...
                {mileageSummary.fleet_totals && (
                  <tfoot>
                    ...
                    <tr style={{ borderTop: `2px solid ${D.border}` }}>
                      ...
                      >
                        {fmtN(
                          Math.round(mileageSummary.fleet_totals.total_miles),
                        )}
                      </td>{" "}
                      <td
                        style={{
                      ...
                      >
                        {fmtN(
                          Math.round(
                            mileageSummary.fleet_totals.business_miles,
                          ),
                        )}
                      </td>{" "}
                      <td
                ...
                {dueSchedules.map((s) => (
                  ...
                  >
                    ...
                    >
                      {s.is_overdue && (
                        <span
                          style={{ ...sBadge(D.red, D.white), marginRight: 4 }}
                        >
                          OVERDUE
                        </span>
                      )}
