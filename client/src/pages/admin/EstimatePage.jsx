  ...
  async function doGenerate(overrides = {}) {
    ...
    if (enrichedProfile || canUseServerForBedBug || hasServerOnlyService || canUseServerForCommercial) {
      try {
        ...
        // injection branch: with both services selected the injection line
        // can legitimately use its own treatment count while the recurring
        // line silently clamps. Same 1–200 contract as the public route and
        // the intent schema.
  ...
  async function doSend(id, method) {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        throw new Error(smsError || `Send failed: ${r.status}`);
      }
      const label =
        sendMethod === "sms"
          ? "SMS"
