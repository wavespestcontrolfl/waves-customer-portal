    ...
    >
      ...
      {onRemove && (
        <span
          ...
          style={{ cursor: "pointer", marginLeft: 2, fontSize: 12 }}
        >
          x
        </span>
      )}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <input
              ...
              style={inputStyle}
              placeholder="VIP, referral_machine"
            />{" "}
          </div>{" "}
          ...
          {existingMatch && (
            ...
            >
              ...
              <button
                type="button"
                onClick={() => {
                  onOpenExisting?.(existingMatch.id);
                  onClose();
                }}
                style={{
                  padding: "6px 14px",
  ...
  return (
    ...
    >
      ...
      {confirming && (
        ...
        >
          {" "}
          <div style={{ fontSize: 12, color: D.red, marginBottom: 8 }}>
        ...
        >
          ...
          {data.pendingOutreach.map((o) => (
            ...
            >
              ...
              >
                {o.first_name} {o.last_name} — {o.outreach_type?.toUpperCase()}{" "}
                ({o.outreach_strategy?.replace(/_/g, " ")})
              </div>{" "}
              <div
                style={{
                  fontSize: 12,
              ...
              <tbody>
                {data.upsells.slice(0, 15).map((u) => (
                  <tr key={u.id}>
                    ...
                    >
                      {(u.recommended_service || "").replace(/_/g, " ")}
                    </td>{" "}
                    <td
                      style={{
                        padding: "8px 12px",
                    ...
                    >
                      +${parseFloat(u.estimated_monthly_value || 0).toFixed(0)}
                    </td>{" "}
                    <td
                      style={{
                        padding: "8px 12px",
                    ...
                    >
                      {Math.round(parseFloat(u.confidence || 0) * 100)}%
                    </td>{" "}
                    <td
                      style={{
                        padding: "8px 12px",
...
function CustomerTimeline({ customerId }) {
  ...
  if (loading)
    return (
      <div style={{ color: D.muted, fontSize: 12, padding: 12 }}>
        Loading timeline...
      </div>
    );
  ...
  return (
    <div style={{ gridColumn: "1 / -1", marginTop: 16 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
        {items.map((item, i) => (
          <div
            key={item.id || `${item.type}-${item.date}-${i}`}
            style={{
              display: "flex",
              gap: 10,
              padding: "6px 0",
              borderBottom:
                i < items.length - 1 ? `1px solid ${D.border}22` : "none",
              fontSize: 12,
            }}
          >
            {" "}
...
const STAGE_PIN_COLORS = {
  active_customer: "#16A34A",
  won: "#16A34A",
  new_lead: "#0A7EC2",
  contacted: "#0A7EC2",
  estimate_sent: "#F0A500",
  at_risk: "#C0392B",
  churned: "#C0392B",
};
...
function CustomerMap({ customers: _ignored, onSelect }) {
  ...
  useEffect(() => {
    try {
      ...
      script.onerror = () =>
        setMapError("Google Maps script failed to load. Check API key.");
  ...
  const filtered = useMemo(() => {
    ...
    return customers.filter((c) => {
      if (
        filterTier !== "all" &&
        (c.tier || null) !== (filterTier === "none" ? null : filterTier)
      )
        return false;
      if (filterStage !== "all" && c.pipelineStage !== filterStage)
        return false;
      return true;
    });
  ...
  useEffect(() => {
    ...
    try {
      ...
      mappable.forEach((c, idx) => {
        const address = [c.address, c.city, c.state, c.zip]
          .filter(Boolean)
          .join(", ");
        ...
        if (c.lat && c.lng) {
          ...
          // limit geocoding to 200 to avoid quota
      ...
      function placeMarker(c, position) {
        const color =
          TIER_PIN_COLORS[c.tier] ||
          STAGE_PIN_COLORS[c.pipelineStage] ||
          "#0A7EC2";
      ...
      console.error("[CustomerMap] Error:", e);
      setMapError(e.message);
    }
  }, [mapReady, filtered]);
          ...
          >
            ...
            >
              ...
              <div>
                ...
                >
                  {selectedPin.firstName} {selectedPin.lastName}
                </div>{" "}
