    ...
    <div>
      ...
      >
        {[
          ...
          {
            label: "LTV:CAC Ratio",
            value: d.ltvCacRatio ? `${d.ltvCacRatio.toFixed(1)}x` : "—",
            color:
              d.ltvCacRatio >= 3
                ? D.green
                : d.ltvCacRatio >= 2
      ...
      <div style={sCard}>
        ...
        >
          $100M Money Model — Revenue by Stage
        </div>{" "}
        <div
          style={{
            display: "grid",
        ...
        >
          {[
            {
              stage: "Stage I: Attraction",
              desc: "First service / one-time",
              value: stages.attraction,
              color: D.blue,
              icon: "",
            },
            {
              stage: "Stage II: Core",
              desc: "WaveGuard recurring",
              value: stages.core,
              color: D.teal,
              icon: "",
            },
            {
              stage: "Stage III: Upsell",
              desc: "Add-ons & upgrades",
              value: stages.upsell,
              color: D.purple,
              icon: "",
            },
            {
              stage: "Stage IV: Continuity",
              desc: "Retention & renewals",
              value: stages.continuity,
              color: D.green,
              icon: "",
            },
          ].map((s) => (
          ...
          {[
            ...
            <div key={s.label} style={{ textAlign: "center", minWidth: 0 }}>
              ...
              {i < 4 && (
                ...
                >
                  {s.value && funnel.leads
                    ? `${Math.round((s.value / funnel.leads) * 100)}%`
                    : ""}
                </div>
              )}
            </div>
          ...
          {d.upsellOpportunities.slice(0, 5).map((o, i) => (
            ...
            >
              ...
              <div>
                ...
                <span style={{ color: D.muted, marginLeft: 8, fontSize: 12 }}>
                  Currently: {o.currentTier} ({o.serviceCount} services)
                </span>{" "}
              </div>{" "}
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                {" "}
                <span
                  style={{ color: D.green, fontFamily: MONO, fontSize: 13 }}
                >
                  +{fmt(o.potentialAdd)}/mo
                </span>{" "}
...
function ValueEquationTab() {
  ...
  const calculate = async () => {
    try {
      const r = await adminFetch("/admin/pricing/calculate-value", {
        method: "POST",
      ...
      /* calculate locally */
      const v =
        (inputs.dreamOutcome * inputs.perceivedLikelihood) /
        (inputs.timeDelay * inputs.effortSacrifice);
      const score = Math.round(v * 10);
      setResult({
        valueScore: score,
        priceRecommendation:
          score > 70 ? "Premium" : score > 40 ? "Market Rate" : "Needs Work",
        positioning:
          score > 70
            ? "You can charge 2-3x market rate"
            : score > 40
              ? "Competitive pricing is appropriate"
              : "Improve the offer before raising prices",
      });
    }
  };
  ...
  const Slider = ({
    ...
    <div style={{ marginBottom: 20 }}>
      ...
      >
        ...
        <span
          style={{
            ...
            fontSize: 18,
            fontWeight: 700,
            color: value >= 7 ? D.green : value >= 4 ? D.amber : D.red,
          }}
        >
          {value}
        </span>{" "}
      ...
      >
        <span>{decreaseLabel || "Low"}</span>
        <span>{increaseLabel || "High"}</span>
      </div>{" "}
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div style={sCard}>
        ...
        <Slider
          label="Dream Outcome"
          desc="How life-changing is the result?"
          value={inputs.dreamOutcome}
          onChange={(v) => setInputs((p) => ({ ...p, dreamOutcome: v }))}
          decreaseLabel="Minor improvement"
          increaseLabel="Life-changing"
        />{" "}
        <Slider
          label="Perceived Likelihood"
          desc="Do they believe it'll work?"
          value={inputs.perceivedLikelihood}
          onChange={(v) => setInputs((p) => ({ ...p, perceivedLikelihood: v }))}
          decreaseLabel="Skeptical"
          increaseLabel="Guaranteed"
        />{" "}
        <div
          style={{
            fontSize: 12,
        ...
        <Slider
          label="Time Delay"
          desc="How long until they see results?"
          value={inputs.timeDelay}
          onChange={(v) => setInputs((p) => ({ ...p, timeDelay: v }))}
          decreaseLabel="Instant"
          increaseLabel="Months/years"
        />{" "}
        <Slider
          label="Effort & Sacrifice"
          desc="How much work for the customer?"
          value={inputs.effortSacrifice}
          onChange={(v) => setInputs((p) => ({ ...p, effortSacrifice: v }))}
          decreaseLabel="Done-for-you"
          increaseLabel="DIY"
        />{" "}
      </div>{" "}
      <div>
        ...
        <div style={sCard}>
          ...
          >
            Waves Pest Control Value Levers
          </div>
          {[
            {
              lever: "Dream Outcome",
              current: "Pest-free, healthy lawn, protected home",
              improve:
                'Frame as "protecting your family\'s health and your biggest investment"',
            },
            {
              lever: "Likelihood",
              current: "Licensed, insured, local reputation",
              improve:
                "Add guarantee language, show review count, before/after photos",
            },
            {
              lever: "Time Delay",
              current: "Results within first treatment",
              improve:
                'Emphasize "same-week service" and "immediate protection"',
            },
            {
              lever: "Effort",
              current: "Fully done-for-you",
              improve:
                'Highlight: "We handle everything — you just unlock the gate"',
            },
          ].map((l) => (
            <div
              key={l.lever}
            ...
            >
              ...
              <div style={{ color: D.amber, marginTop: 2 }}>
                ↑ {l.improve}
              </div>{" "}
            </div>
          ))}
...
function OfferBuilderTab({ showToast }) {
  ...
  useEffect(() => {
    adminFetch("/admin/pricing/offers")
  ...
  return (
    <div>
      ...
      >
        {[
          {
            name: "WaveGuard Bronze",
            services: 1,
            discount: "0%",
            price: "$49-89/mo",
            anchor: "$120+/mo",
            guarantee: "Satisfaction Guarantee",
            bonuses: ["Digital Service Reports"],
          },
          {
            ...
            price: "$85-140/mo",
            anchor: "$190+/mo",
            guarantee: "100% Satisfaction + Free Re-treat",
            bonuses: [
              "Digital Reports",
              "Priority Scheduling",
          ...
          },
          {
            ...
            price: "$130-200/mo",
            anchor: "$280+/mo",
            guarantee: "100% Satisfaction + Free Re-treat + Money Back",
            bonuses: [
              "All Silver perks",
          ...
          {
            name: "WaveGuard Platinum",
            services: "4+",
            discount: "20%",
            price: "$180-280/mo",
            anchor: "$400+/mo",
            guarantee: "Unconditional Money Back",
            bonuses: [
              "All Gold perks",
              "Dedicated Tech",
          ...
          <div key={o.name} style={{ ...sCard, marginBottom: 0 }}>
            ...
            <div style={{ fontSize: 12, marginBottom: 8 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ color: D.muted }}>Price</span>
                <span
                  style={{ color: D.green, fontFamily: MONO, fontWeight: 700 }}
                >
                  {o.price}
                </span>
              </div>{" "}
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ color: D.muted }}>Anchor (without us)</span>
                <span
                  style={{
                    color: D.red,
        ...
        <div style={{ marginTop: 20 }}>
          ...
          {offers.map((o) => (
            <div key={o.id} style={{ ...sCard, marginBottom: 8 }}>
              ...
              {o.conversion_rate > 0 && (
                <div style={{ fontSize: 11, color: D.green, marginTop: 4 }}>
...
function UpsellEngineTab({ showToast }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      adminFetch("/admin/pricing/upsell-opportunities").catch(() => ({
        opportunities: [],
      })),
    ]).then(([r, o]) => {
      setRules(r.rules || []);
  ...
  return (
    <div>
      ...
      <div style={sCard}>
        ...
        >
          Upsell Opportunities ({opportunities.length})
        </div>
          ...
          opportunities.slice(0, 10).map((o, i) => (
            ...
            >
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: D.muted }}>
                  Currently: {o.currentTier} · {o.serviceCount} service
                  {o.serviceCount !== 1 ? "s" : ""} · {fmt(o.monthlyRate)}/mo
                </div>{" "}
              </div>{" "}
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                {" "}
                <div style={{ textAlign: "right" }}>
                  ...
                  <div style={{ fontSize: 10, color: D.purple }}>
                    {o.suggestedService}
                  </div>{" "}
                </div>{" "}
                <button
                  onClick={() => triggerUpsell(o.customerId)}
          ...
          rules.map((r) => (
            ...
            >
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: D.muted }}>
                  Trigger: {r.trigger_event} · Offer: {r.offer_service}
                </div>{" "}
              </div>{" "}
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                {" "}
                <span style={{ fontSize: 11, color: D.muted }}>
                  {r.times_triggered || 0} triggered · {r.times_converted || 0}{" "}
                  converted
                </span>{" "}
                <span
                  style={sBadge(
                    r.enabled ? `${D.green}22` : `${D.muted}22`,
                    r.enabled ? D.green : D.muted,
                  )}
                >
...
function LTVAnalysisTab() {
  ...
  if (!data)
    return (
      ...
      >
        No LTV data yet. Click "Recalculate" to generate.
      </div>
    );
    ...
    <div>
      ...
      >
        ...
        <button
          ...
          style={{ ...sBtn(D.teal, D.white), opacity: recalculating ? 0.5 : 1 }}
        >
      ...
      >
        {[
          ...
          {
            label: "LTV:CAC",
            value: data.ltvCacRatio ? `${data.ltvCacRatio.toFixed(1)}x` : "—",
            color: data.ltvCacRatio >= 3 ? D.green : D.amber,
          },
          {
            label: "Best Channel",
          ...
          },
          {
            ...
            value: data.retention12mo ? `${data.retention12mo}%` : "—",
            color: D.purple,
          },
        ].map((s) => (
          <div
            ...
            style={{
              ...sCard,
              flex: isMobile ? "1 1 calc(50% - 6px)" : "1 1 130px",
            ...
            >
              ...
              <div style={{ display: "flex", gap: 16 }}>
                ...
                <span style={{ color: D.green, fontFamily: MONO }}>
                  LTV: {fmt(stats.avgLTV)}
                </span>{" "}
                <span style={{ color: D.amber, fontFamily: MONO }}>
                  CAC: {fmt(stats.avgCAC)}
                </span>{" "}
                <span
                  style={{
                    color: stats.ratio >= 3 ? D.green : D.amber,
                    fontFamily: MONO,
                  }}
                >
