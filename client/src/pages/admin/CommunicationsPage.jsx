...
const TYPE_META = {
  ...
  completion: { icon: "Yes", color: D.green },
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showDot && (
          <span
            title={isHealthy ? "Active" : "No activity detected"}
            style={{
              width: 7,
              height: 7,
              borderRadius: "50%",
              ...
              >
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: 10,
                      color: isOutbound ? "rgba(255,255,255,0.7)" : D.muted,
                    }}
                  >
...
function CallLogTab() {
  ...
  const handleDisposition = async (callId, value) => {
    if (
      value === "spam" &&
      !confirm(
        "Block this number and delete the call log? This cannot be undone.",
      )
    )
      return;
  ...
  const thisMonthCalls = calls.filter((c) => {
    ...
    return (
      cd.getMonth() === now.getMonth() && cd.getFullYear() === now.getFullYear()
    );
  });
  ...
  const sortedSources = Object.entries(sourceNumberCounts).sort(
    (a, b) => b[1] - a[1],
  );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        {[
          ...
          <div
            key={i}
            onClick={() =>
              setCallFilter((prev) => (prev === s.filter ? "all" : s.filter))
            }
            style={{
              flex: isMobile ? "1 1 calc(50% - 6px)" : "1 1 100px",
              background: D.card,
              borderRadius: 10,
              padding: "12px 14px",
              border: `1px solid ${callFilter === s.filter && s.filter !== "all" ? s.color : D.border}`,
              textAlign: "center",
              minWidth: 0,
              cursor: "pointer",
              transition: "border-color 0.15s",
      ...
      >
        ...
        <button
          ...
          style={{
            padding: "10px 24px",
            background: calling ? D.muted : D.green,
            border: "none",
            borderRadius: 8,
            color: D.heading,
            fontSize: 14,
        ...
        {callResult && (
          <div
            style={{
              marginTop: 10,
              fontSize: 12,
              color: callResult.ok ? D.green : D.red,
            }}
          >
            {callResult.text}
          </div>
                    ...
                    >
                      ...
                      <span
                        style={{
                          ...
                          width: 20,
                          textAlign: "center",
                          color: c.direction === "inbound" ? D.teal : D.green,
                        }}
                      >
                        {c.direction === "inbound" ? "↓" : "↑"}
                      </span>{" "}
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11, color: D.muted }}>
                          {c.from_phone} {c.to_phone ? ` >${c.to_phone}` : ""} ·{" "}
                          {dur} ·{" "}
                          {c.caller_city
                    ...
                    >
                      ...
                      <select
                        ...
                        style={{
                          ...
                          border: `1px solid ${D.border}`,
                          borderRadius: 6,
                          color: currentDisp ? D.text : D.muted,
                          fontSize: 11,
                          padding: "4px 8px",
                          fontFamily: "Roboto, Arial, sans-serif",
                          outline: "none",
                      ...
                      {isMissed && c.from_phone && (
                        <button
                          onClick={() => handleCallBack(c.from_phone)}
                          style={{
                            padding: "4px 12px",
                            borderRadius: 6,
                            border: "none",
        ...
        <div style={{ overflowX: "auto" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <thead>
              ...
              <tr>
                {" "}
                <th style={thSt}>CSR</th> <th style={thR}>Calls</th>{" "}
            ...
            <tbody>
              {csrs.map((c) => (
                <tr key={c.name}>
                  {" "}
                  <td style={tdT}>{c.name}</td> <td style={tdR}>{c.calls}</td>{" "}
                  <td
                    style={{ ...tdR, color: rateColor(c.firstCallBookingRate) }}
                  >
                    {c.firstCallBookingRate}%{" "}
                    {c.firstCallBookingRate >= 60
                      ? "Yes"
                      : c.firstCallBookingRate >= 40
                        ? ""
                        : ""}
                  </td>{" "}
                  <td style={{ ...tdR, color: scoreColor(c.avgScore) }}>
                    {c.avgScore}/15
                  </td>{" "}
                  <td style={{ ...tdR, color: rateColor(c.followUpRate) }}>
                    {c.followUpRate}%{" "}
                    {c.followUpRate >= 80
                      ? "Yes"
                      : c.followUpRate >= 60
                ...
                >
                  ...
                  <div
                    style={{
                      height: "100%",
                      width: `${r.pct}%`,
                      background: isCsr ? D.red : D.muted,
                      borderRadius: 4,
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: 12, color: D.muted }}>
            ...
            <span style={{ color: tasks?.overdue > 0 ? D.red : D.muted }}>
              {tasks?.overdue || 0}
            </span>{" "}
          </div>{" "}
        </div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderRadius: 8,
                  marginBottom: 8,
                  borderLeft: `3px solid ${isOverdue ? D.red : D.amber}`,
                }}
              >
                {" "}
                <div
          ...
          {(leaderboard.categories || []).map((cat, i) => (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: D.teal }}>
                  {cat.value}
                </div>{" "}
              </div>{" "}
              <div
...
function getHealthStatus(lastInboundDate, isUnassigned) {
  if (isUnassigned)
    return { color: "#6b7280", label: "Unassigned", key: "gray" };
  if (!lastInboundDate)
    return { color: "#C0392B", label: "Dormant (no inbound)", key: "red" };
  const now = new Date();
  ...
  if (daysSince <= 60)
    return {
      color: "#f59e0b",
      label: `Low activity (${daysSince}d ago)`,
      key: "amber",
    };
...
const CHANNEL_TYPE_META = {
  ...
  auto_completion: { icon: "Yes", label: "Auto Completion", color: D.green },
  auto_review: { icon: "", label: "Auto Review", color: D.amber },
  auto_estimate: { icon: "", label: "Auto Estimate", color: D.teal },
  auto_payment: { icon: "Card", label: "Auto Payment", color: "#8b5cf6" },
  ...
  completion: { icon: "Yes", label: "Completion", color: D.green },
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          ...
          <button
            ...
            style={{
              ...
              color: filter === "all" ? D.white : D.muted,
            }}
          >
            All
          </button>
          {categories.map((c) => (
            <button
              ...
              style={{
                ...
                color: filter === c ? D.white : D.muted,
                textTransform: "capitalize",
              }}
            >
              {c}
      ...
      {filtered.map((t) => (
        <div
          ...
          style={{
            ...
            border: `1px solid ${D.border}`,
            marginBottom: 8,
            borderLeft: `3px solid ${catColors[t.category] || D.muted}`,
          }}
        >
          {" "}
          <div
            ...
            <div>
              ...
              <span
                style={{
                  ...
                  padding: "2px 6px",
                  borderRadius: 4,
                  background: (catColors[t.category] || D.muted) + "22",
                  color: catColors[t.category] || D.muted,
                  textTransform: "capitalize",
                }}
              >
                {t.category}
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              ...
              <button
                ...
                style={{
                  ...
                  border: "none",
                  cursor: "pointer",
                  background: t.is_active ? D.green : "#475569",
...
function PhoneNumbersTab({ channelStats, maxChannel, stats }) {
  ...
  const groupCount = (name) =>
    ALL_NUMBERS.find((g) => g.group === name)?.numbers.length || 0;
  ...
  const totalChannelMessages = channelStats.reduce(
    (s, c) => s + (c.sent || 0),
    0,
  );
  const manualMessages =
    channelStats.find((c) => c.type === "manual")?.sent || 0;
  const automatedMessages = totalChannelMessages - manualMessages;
  const automationRate =
    totalChannelMessages > 0
      ? Math.round((automatedMessages / totalChannelMessages) * 100)
      : 0;
  ...
  channelStats.forEach((c) => {
    channelStatsMap[c.type] = c.sent || 0;
  });
  // Also map legacy types
  const expandedChannels = allChannelTypes
    .map((type) => ({
      type,
      count:
        channelStatsMap[type] ||
        channelStatsMap[type.replace("auto_", "")] ||
        0,
    }))
    .filter((c) => c.count > 0);
  ...
  const displayChannels =
    expandedChannels.length > 0
      ? expandedChannels
      : channelStats.map((c) => ({ type: c.type, count: c.sent || 0 }));
  const maxDisplayChannel =
    displayChannels.length > 0
      ? Math.max(...displayChannels.map((c) => c.count), 1)
      : 1;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        ...
        <StatCard
          label="GBP Locations"
          value={groupCount("GBP Locations")}
          color={D.green}
        />{" "}
        <StatCard
          label="Pest Domains"
          value={groupCount("Pest Control Domains")}
          color={D.teal}
        />{" "}
        <StatCard
          label="Paid"
          value={groupCount("Paid Campaigns")}
          color={D.amber}
        />{" "}
        <StatCard
          label="Lawn Domains"
          value={groupCount("Lawn Care Domains")}
          color={D.green}
        />{" "}
        <StatCard
          label="Operations"
          value={groupCount("Operations")}
          color={D.muted}
        />{" "}
      </div>
      {/* Number Groups */}
            ...
            >
              {group.numbers.map((n, i) => {
                ...
                const daysSinceInbound = ns?.lastInboundDate
                  ? Math.floor(
                      (new Date() - new Date(ns.lastInboundDate)) /
                        (1000 * 60 * 60 * 24),
                    )
                  : null;
                const showDormancyWarning =
                  !isUnassignedGroup &&
                  (daysSinceInbound === null || daysSinceInbound >= 60);
                const smsEnabled =
                  ns?.smsEnabled !== undefined ? ns.smsEnabled : true;
                const voiceEnabled =
                  ns?.voiceEnabled !== undefined ? ns.voiceEnabled : true;
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {" "}
                        <span
                          title={smsEnabled ? "SMS enabled" : "SMS disabled"}
                          style={{
                            fontSize: 9,
                            padding: "1px 5px",
                            borderRadius: 4,
                    ...
                    {/* Label */}
                    <div
                      style={{ fontSize: 12, color: D.muted, marginBottom: 8 }}
                    >
                      {n.label}
                    </div>
                    {/* Activity metrics */}
                      ...
                      >
                        ...
                        {ns?.lastInboundDate ? (
                          <span style={{ color: health.color }}>
                            {new Date(ns.lastInboundDate).toLocaleDateString(
                              [],
                    ...
                    {isUnassignedGroup && (
                      <div
                        style={{
                          ...
                          padding: "6px 10px",
                          borderRadius: 6,
                          background: D.muted + "12",
                          borderLeft: `3px solid ${D.muted}`,
                          fontSize: 11,
                          color: D.muted,
                          lineHeight: 1.4,
                        }}
                      >
                        Unassigned — assign to a domain or campaign to start
                        tracking
                      </div>
                    )}
                  </div>
      ...
      >
        ...
        {displayChannels.length > 0 ? (
          ...
          >
            No channel data yet. Analytics will appear as messages are sent and
            received across your numbers.
          </div>
        )}
      </div>{" "}
    </div>
