...
function TierDotV2({ active, label }) {
  return (
    <span className="inline-flex items-center gap-1 mr-2">
      ...
      <span
        className={cn(
          "text-11 font-medium u-label",
          active ? "text-zinc-900" : "text-ink-tertiary",
        )}
      >
        {label}
      </span>{" "}
    ...
    <Card className="overflow-hidden mb-2">
      ...
      <div className="px-4 py-3 bg-zinc-50 border-b border-hairline border-zinc-200 flex justify-between items-center flex-wrap gap-2">
        ...
        <div>
          ...
          <div className="text-11 text-ink-tertiary mt-1 u-label">
            {trackName}
          </div>{" "}
        </div>{" "}
        <TierDotsV2
        ...
        {warnings.length > 0 && (
          <div className="mb-3">
            {warnings.map((w, i) => (
              ...
              >
                {" "}
                <span className="text-alert-fg text-13 flex-shrink-0">
                  !
                </span>{" "}
                <span className="text-12 text-alert-fg leading-normal">
                  {w}
                </span>{" "}
              </div>
            ))}
        ...
        <div className="rounded-md border border-hairline border-alert-fg/30 bg-alert-bg px-3 py-2">
          {plan.warnings.map((w) => (
            <div key={w.code} className="text-12 text-alert-fg leading-normal">
              {" "}
              <span className="font-medium">{w.code.replace(/_/g, " ")}:</span>
              {w.message}
            </div>
          ))}
        </div>
        ...
        <div className="px-4 py-3 border-b border-hairline border-zinc-200 bg-zinc-50 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {plan.equipment?.carrierGalPer1000 && (
              <Badge tone="neutral">
                {fmtNumber(plan.equipment.carrierGalPer1000, " gal/1K")}
              </Badge>
            )}
          </div>{" "}
        </div>{" "}
            ...
            plan.mixingOrder.map((step) => (
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-12 text-ink-secondary leading-normal mt-1">
                    {step.instruction}
                  </div>{" "}
                </div>{" "}
              </div>
...
export default function ProtocolReferenceTabV2() {
  ...
  useEffect(() => {
    ...
    if (selectedConditionalIds.length)
      params.set(
        "selectedConditionalProductIds",
    ...
    <div className="flex flex-col gap-4">
      ...
      {trackData && (
        <div className="flex flex-col gap-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    ...
                    >
                      ...
                      {equipmentOptions.map((row) => (
                        <option key={row.id} value={row.equipment_system_id}>
                          {row.system_name} ·{" "}
                          {fmtNumber(row.carrier_gal_per_1000, " gal/1K")}
                        </option>
                      ))}
                    </Select>{" "}
                  </div>{" "}
                  ...
                  <div className="rounded-sm border-hairline border-zinc-200 bg-zinc-50 px-3 py-2">
                    ...
                    <div className="u-nums text-14 font-medium text-zinc-900">
                      {fmtNumber(
                        mixPlan?.equipment?.tankCoverageSqft,
                        " sq ft",
                      )}
                    </div>{" "}
                  </div>{" "}
          ...
          <div className="flex gap-3 flex-wrap items-center px-3.5 py-2 bg-white border border-hairline border-zinc-200 rounded-md">
            {" "}
            <span className="text-11 font-medium u-label text-ink-tertiary">
              Tier Legend
            </span>{" "}
            <span className="text-12 text-ink-secondary inline-flex items-center gap-1">
              {" "}
              <span className="inline-block w-2 h-2 rounded-full bg-zinc-900" />
              included
            </span>{" "}
            <span className="text-12 text-ink-tertiary inline-flex items-center gap-1">
              {" "}
              <span className="inline-block w-2 h-2 rounded-full border-hairline border-zinc-400 box-border" />
              not included
            </span>{" "}
      ...
      {!selectedTrack && (
        <Card className="px-10 py-10 text-center">
          {" "}
          <div className="text-14 font-medium text-ink-primary mb-1">
            Select a program above
          </div>{" "}
          <div className="text-13 text-ink-tertiary">
            View the full visit-by-visit protocol with products, rates, costs,
