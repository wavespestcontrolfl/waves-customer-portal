...
function DashboardTab({ data }) {
  if (!data)
    return <div style={{ color: COLORS.textMuted }}>Loading dashboard...</div>;
  ...
  const metricCards = [
    ...
    {
      label: "30-Day Churn Forecast",
      value: data.predictedChurns,
      color: COLORS.amber,
    },
  ];
    ...
    <div>
      ...
      >
        {["low", "moderate", "high", "critical"].map((risk) => (
          <Card
            key={risk}
            style={{ borderLeft: `3px solid ${RISK_COLORS[risk]}` }}
          >
            {" "}
            <div
              ...
              {(data.atRiskCustomers || []).map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: "8px 12px" }}>
                      <GradeCircle grade={c.score_grade} size={26} />
                    </td>{" "}
                    <td style={{ padding: "8px 12px" }}>
                      <Badge
                        label={RISK_LABELS[c.churn_risk]}
                        color={RISK_COLORS[c.churn_risk]}
                      />
                    </td>{" "}
                    <td
                      style={{
          ...
          >
            ...
            <div
              style={{
                ...
                height: 32,
                borderRadius: 2,
                backgroundColor: RISK_COLORS[a.severity] || COLORS.textMuted,
                flexShrink: 0,
              }}
            />{" "}
...
function CustomerDetail({ customerId, onClose }) {
  ...
  if (loading)
    return (
      <div style={{ padding: 20, color: COLORS.textMuted }}>Loading...</div>
    );
  if (!data || !data.score)
    return (
  ...
  const subScores = [
    {
      key: "payment_score",
      label: "Payment",
      detail: s.payment_details,
      color: COLORS.green,
    },
    {
      key: "service_score",
      label: "Service",
      detail: s.service_details,
      color: COLORS.teal,
    },
    {
      key: "engagement_score",
      label: "Engagement",
      detail: s.engagement_details,
      color: COLORS.purple,
    },
    {
      key: "satisfaction_score",
      label: "Satisfaction",
      detail: s.satisfaction_details,
      color: COLORS.amber,
    },
    {
      key: "loyalty_score",
      label: "Loyalty",
      detail: s.loyalty_details,
      color: "#0A7EC2",
    },
    {
      key: "growth_score",
      label: "Growth",
      detail: s.growth_details,
      color: "#16A34A",
    },
  ];
  ...
  function detailString(detail) {
    if (!detail) return "--";
    const parts = [];
    if (detail.onTimeRate !== undefined && detail.onTimeRate !== null)
      parts.push(`On-time: ${Math.round(detail.onTimeRate * 100)}%`);
    if (
      detail.daysSinceLastService !== undefined &&
      detail.daysSinceLastService !== null
    )
      parts.push(`Last: ${detail.daysSinceLastService}d ago`);
    if (detail.adherenceRate !== undefined && detail.adherenceRate !== null)
      parts.push(`Adherence: ${Math.round(detail.adherenceRate * 100)}%`);
    if (detail.smsInbound !== undefined)
      parts.push(`SMS in: ${detail.smsInbound}`);
    if (detail.avgRating !== undefined && detail.avgRating !== null)
      parts.push(`Rating: ${detail.avgRating}/5`);
    if (detail.tenureMonths !== undefined)
      parts.push(`${detail.tenureMonths}mo tenure`);
    if (detail.tier) parts.push(`Tier: ${detail.tier}`);
    if (detail.distinctServices !== undefined)
      parts.push(`${detail.distinctServices} service types`);
    if (detail.monthlyRate !== undefined && detail.monthlyRate !== null)
      parts.push(`$${detail.monthlyRate}/mo`);
    return parts.join(" | ") || "--";
  }
      ...
      >
        ...
        >
          ...
          <div
            style={{
              ...
              height: 80,
              borderRadius: "50%",
              border: `3px solid ${GRADE_COLORS[s.score_grade]}`,
              display: "flex",
              flexDirection: "column",
              alignItems: "center",
              justifyContent: "center",
          ...
          >
            {" "}
            <Badge
              label={`${RISK_LABELS[s.churn_risk]} Risk`}
              color={RISK_COLORS[s.churn_risk]}
            />{" "}
            <Badge
              label={`${TREND_ARROWS[s.score_trend] || ""} ${s.score_trend}`}
              color={TREND_COLORS[s.score_trend]}
            />{" "}
          </div>{" "}
        </div>
        {/* Sub-scores + signals */}
        <div>
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      borderRadius: "50%",
                      backgroundColor:
                        RISK_COLORS[sig.severity] || COLORS.textMuted,
                      flexShrink: 0,
                    }}
                  />{" "}
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button
              onClick={() =>
                adminFetch(`/admin/health/rescore/${customerId}`, {
                  method: "POST",
    ...
    <div>
      ...
      >
        ...
        <select
          ...
          onChange={(e) => {
            setFilters((f) => ({ ...f, churn_risk: e.target.value }));
            setPage(0);
          }}
          style={selectStyle}
        >
            ...
            <tbody>
              ...
              {!loading && scores.length === 0 && (
                <tr>
                  ...
                  >
                    No scores found. Run a batch rescore to generate data.
                  </td>
                </tr>
              )}
              {!loading &&
                scores.map((c) => (
                  <React.Fragment key={c.customer_id}>
                    ...
                    <tr
                      onClick={() =>
                        setExpandedId(
                          expandedId === c.customer_id ? null : c.customer_id,
                        )
                      }
                      style={{
                        borderBottom: `1px solid ${COLORS.border}22`,
                    ...
                    >
                      ...
                      <td style={{ padding: "10px 12px" }}>
                        ...
                        <ScoreBar
                          ...
                          color={GRADE_COLORS[c.score_grade]}
                          width={50}
                        />{" "}
                      </td>{" "}
                      <td style={{ padding: "10px 12px" }}>
                        <GradeCircle grade={c.score_grade} size={24} />
                      </td>{" "}
                      <td style={{ padding: "10px 12px" }}>
                        {" "}
                      ...
                      >
                        ...
                        {c.score_change_30d != null && (
                          <span style={{ fontSize: 11, marginLeft: 4 }}>
                            {c.score_change_30d > 0 ? "+" : ""}
                            {c.score_change_30d}
                          </span>
                        )}
                      </td>{" "}
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              backgroundColor: COLORS.card,
              color:
                (page + 1) * limit >= total ? COLORS.textMuted : COLORS.text,
...
function AlertsTab() {
  ...
  async function handleAction(alertId, actionIndex) {
    ...
    try {
      await adminFetch(`/admin/health/alerts/${alertId}/action`, {
        method: "POST",
        body: { actionIndex },
      });
      fetchAlerts();
  ...
  async function handleStatusChange(alertId, newStatus) {
    try {
      await adminFetch(`/admin/health/alerts/${alertId}`, {
        method: "PUT",
  ...
  return (
    <div>
      ...
      {loading && (
        <div style={{ color: COLORS.textMuted, padding: 20 }}>
          Loading alerts...
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {alerts.map((a) => {
          const actions =
            ...
            (typeof a.recommended_actions === "string"
              ? JSON.parse(a.recommended_actions)
              : []) ||
            [];
          return (
            <Card
                ...
                {actions.length > 0 && (
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {actions.map((act, idx) => {
                      ...
                      const btnColor = isExecuted
                        ? COLORS.textMuted
                        : actionColors[act.type] || COLORS.teal;
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            fontSize: 11,
                            cursor:
                              isExecuted || isLoading ? "default" : "pointer",
                            fontWeight: 500,
...
function SequencesTab() {
  ...
  async function handleAction(seqId, action, outcome) {
    try {
      await adminFetch(`/admin/health/sequences/${seqId}`, {
        method: "PUT",
        body: { action, outcome, notes: "" },
      });
      fetchSequences();
    } catch {
      /* ignore */
            ...
            <Card key={seq.id}>
              ...
              >
                ...
                <div>
                  ...
                  >
                    {seq.first_name} {seq.last_name}
                  </span>{" "}
                  <span style={{ marginLeft: 8 }}>
                    <Badge
                      label={seq.sequence_type.replace("_", " ")}
                      color={COLORS.purple}
                    />
                  </span>
                  {seq.waveguard_tier && (
                    <span style={{ marginLeft: 8 }}>
                      <Badge label={seq.waveguard_tier} color={COLORS.teal} />
                    </span>
                  )}
                </div>{" "}
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  {seq.status === "active" && (
                    <>
                      ...
                      <button
                        onClick={() =>
                          handleAction(seq.id, "complete", "customer_saved")
                        }
                        style={{
                          padding: "4px 10px",
                          borderRadius: 6,
              ...
              <div
                style={{ fontSize: 11, color: COLORS.textMuted, marginTop: 10 }}
              >
    ...
    >
      ...
      >
        ...
        <div>
          ...
          >
            Monitor customer health scores, detect churn risk, and automate
            retention
          </p>{" "}
        </div>{" "}
        <button
