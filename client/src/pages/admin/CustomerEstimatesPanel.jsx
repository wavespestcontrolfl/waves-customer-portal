...
export default function CustomerEstimatesPanel({ customerId, onClose }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [summaryRes, commsRes] = await Promise.all([
        ...
        adminFetch(`/admin/customers/${customerId}/comms?limit=15`).catch(
          () => null,
        ),
      ]);
  ...
  const prefillHref = c
    ? `/admin/estimates?${new URLSearchParams({
        address:
          `${c.address_line1 || ""}${c.city ? ", " + c.city : ""}${c.state ? ", " + c.state : ""}${c.zip ? " " + c.zip : ""}`.trim(),
        ...
        {!loading && !err && c && (
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
            ...
            <section className="border border-hairline border-zinc-200 rounded-sm p-3 space-y-1.5 text-13">
              ...
              {c.company_name && (
                <div className="text-ink-secondary pl-[22px]">
                  {c.company_name}
                </div>
              )}
              {c.phone && (
              ...
              <div className="flex items-center gap-1.5 pt-2 flex-wrap">
                {c.waveguard_tier && (
                  <Badge tone="neutral">{c.waveguard_tier}</Badge>
                )}
              ...
              {c.phone && (
                <button
                  ...
                  onClick={async () => {
                    if (
                      !window.confirm(
                        `Call ${c.first_name || ""} ${c.last_name || ""}`.trim() +
                    ...
                    try {
                      ...
                      if (!res?.success)
                        alert(
                          "Call failed: " + (res?.error || "unknown error"),
                        );
                    } catch (e) {
            ...
            {stats && stats.total > 0 && (
              <section className="grid grid-cols-3 gap-2 text-center">
                ...
                <StatCell
                  ...
                  accent={stats.accepted > 0 ? "good" : undefined}
                />{" "}
                <StatCell
                  label="Conversion"
                  value={
                  ...
                  sub={
                    stats.conversionRate != null
                      ? `${stats.accepted}/${stats.accepted + stats.declined} decided`
                      : null
                  }
                />{" "}
              </section>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-right flex-shrink-0 flex flex-col items-end gap-1">
                          ...
                          <div
                            className={cn(
                              "text-14 font-medium u-nums",
                              Number(e.monthly_total || 0) > 0
                                ? "text-zinc-900"
                                : "text-ink-tertiary",
                            )}
                          >
              ...
              <section className="border-t border-hairline border-zinc-200 pt-3">
                ...
                {comms.length > 0 ? (
                  ...
                  <div className="text-12 text-ink-secondary">
                    ...
                    (
                    {lastContact.direction === "inbound"
                      ? "from customer"
                      : "from us"}
                    ) · {timeAgo(lastContact.at)}
                    {lastContact.preview && lastContact.channel === "sms" && (
                      <div className="mt-1 text-ink-secondary italic truncate">
                        "{lastContact.preview}"
                      </div>
                    )}
                  </div>
...
function StatCell({ label, value, sub, accent }) {
  return (
    <div className="border border-hairline border-zinc-200 rounded-sm p-2">
      ...
      <div
        className={cn(
          "text-18 u-nums font-medium",
          accent === "good" ? "text-zinc-900" : "text-zinc-900",
        )}
      >
        {value}
      </div>{" "}
...
function CommsRow({ m }) {
  ...
  const subtitle = isVoice
    ? m.aiSummary ||
      (m.durationSeconds ? `${Math.round(m.durationSeconds / 60)} min` : null)
    : m.body;
  return (
    <div
      className={cn(
    ...
    >
      ...
      <Icon
        size={13}
        strokeWidth={1.75}
        className="text-ink-tertiary mt-0.5 flex-shrink-0"
      />{" "}
      <div className="min-w-0 flex-1">
        {" "}
        ...
        {subtitle && (
          <div className="text-ink-secondary mt-0.5 line-clamp-2">
            {subtitle}
          </div>
        )}
      </div>{" "}
