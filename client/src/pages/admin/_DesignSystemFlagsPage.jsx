...
export default function DesignSystemFlagsPage() {
  ...
  const toggle = async (userId, flagKey, next) => {
    ...
    try {
      ...
      /* legacy bare-id format — skip refetch */
    }
  };
    ...
    <div className="bg-surface-page min-h-full p-6 font-sans text-zinc-900">
        ...
        <CardBody>
          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex-1 min-w-[180px]">
              {" "}
              <label className="u-label block mb-1 text-ink-secondary">
                User
              </label>{" "}
              <Select
                value={newFlagUser}
            ...
            <Button
              onClick={addFlag}
              disabled={!newFlag.trim() || !newFlagUser}
            >
              Enable
            </Button>{" "}
          </div>{" "}
      ...
      <Card>
        ...
        <CardBody className="p-0">
          {err && <div className="p-4 text-13 text-alert-fg">Error: {err}</div>}
          {loading ? (
        ...
        <ul className="divide-y divide-zinc-200">
          {flagKeys.map((k) => {
            ...
            return (
              ...
              >
                {" "}
                <span className="text-14 text-ink-primary break-all">
                  {k}
                </span>{" "}
                <Switch
                  checked={on}
