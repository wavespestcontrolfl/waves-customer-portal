...
const fmt = (n) =>
  n != null
    ...
    : "--";
const fmtHrs = (min) =>
  min != null ? (parseFloat(min) / 60).toFixed(1) + "h" : "--";
...
function DashboardTab({ showToast }) {
  ...
  if (!data)
    return (
      <div style={{ color: D.red, padding: 40, textAlign: "center" }}>
        Failed to load
      </div>
    );
  ...
  const todayJobMin = todaySummaries.reduce(
    (s, d) => s + parseFloat(d.total_job_minutes || 0),
    0,
  );
  const todayRevenue = todaySummaries.reduce(
    (s, d) => s + parseFloat(d.revenue_generated || 0),
    0,
  );
  ...
  const todayLaborCost = todaySummaries.reduce((s, d) => {
    ...
    const rate =
      tech && parseFloat(tech.pay_rate) > 0
        ? parseFloat(tech.pay_rate)
        : LABOR_RATE;
  ...
  // This week totals
  const weekShiftMin = weekDailies.reduce(
    (s, d) => s + parseFloat(d.total_shift_minutes || 0),
    0,
  );
  const weekRevenue = weekDailies.reduce(
    (s, d) => s + parseFloat(d.revenue_generated || 0),
    0,
  );
      ...
      >
        {allTechs
          ...
          .map((tech) => {
            ...
            const shiftDur = active
              ? (Date.now() - new Date(active.clock_in).getTime()) / 60000
              : 0;
            const dayHrs = summary
              ? parseFloat(summary.total_shift_minutes || 0) / 60
              : 0;
            const dayJobs = summary ? summary.job_count : 0;
            const dayRev = summary
              ? parseFloat(summary.revenue_generated || 0)
              : 0;
            const dayUtil = summary
              ? parseFloat(summary.utilization_pct || 0)
              : 0;
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div
                    style={{
                      ...
                      height: 10,
                      borderRadius: "50%",
                      background: active ? D.green : D.muted,
                  ...
                  <span
                    style={sBadge(
                      active ? D.green + "22" : D.muted + "22",
                      active ? D.green : D.muted,
                    )}
                  >
...
function StatCard({ label, value, color }) {
  return (
    ...
    >
      {" "}
      <div style={{ fontSize: 20, fontWeight: 700, color, fontFamily: MONO }}>
        {value}
      </div>{" "}
...
function WeekBarChart({ weekDailies, weekStart }) {
  ...
  return (
    <svg
      width="100%"
      height={barH + 30}
      viewBox={`0 0 ${7 * (barW + 12) + 20} ${barH + 30}`}
    >
      {DAYS.map((day, i) => {
        const dateStr = addDays(weekStart, i);
        const dayData = weekDailies.filter(
          (d) =>
            d.work_date === dateStr ||
            (d.work_date && d.work_date.split("T")[0] === dateStr),
        );
        const shiftMin = dayData.reduce(
          (s, d) => s + parseFloat(d.total_shift_minutes || 0),
          0,
        );
      ...
      <rect
        ...
        rx={2}
      />{" "}
      <text x={7 * (barW + 12) - 48} y={10} fontSize={9} fill={D.muted}>
        Reg
      </text>{" "}
      <rect
        x={7 * (barW + 12) - 60}
        ...
        rx={2}
      />{" "}
      <text x={7 * (barW + 12) - 48} y={22} fontSize={9} fill={D.muted}>
        OT
      </text>{" "}
    </svg>
  );
...
function TimesheetTab({ showToast }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [dailyRes, dashRes] = await Promise.all([
        adminFetch(
          `/admin/timetracking/daily?startDate=${weekStart}&endDate=${weekEnd}`,
        ),
    ...
    async (techId, date) => {
      ...
      try {
        const res = await adminFetch(
          `/admin/timetracking/entries?technicianId=${techId}&startDate=${date}&endDate=${date}`,
        );
  ...
  const byTech = useMemo(() => {
    const map = {};
    techs.forEach((t) => {
      map[t.id] = { tech: t, days: {} };
    });
    dailies.forEach((d) => {
      const tid = d.technician_id;
      if (!map[tid])
        map[tid] = {
          tech: { id: tid, name: d.tech_name || "Unknown" },
          days: {},
        };
      const dateKey =
        typeof d.work_date === "string"
          ? d.work_date.split("T")[0]
          : d.work_date;
        ...
        >
              ...
              {DAYS.map((d, i) => (
                <th key={d} style={thStyle}>
                  ...
                  <span style={{ fontSize: 9, color: D.muted }}>
                    {addDays(weekStart, i).slice(5)}
                  </span>
                </th>
              ))}
              <th style={thStyle}>Total</th> <th style={thStyle}>OT</th>{" "}
          ...
          <tbody>
            {byTech.map(({ tech, days }) => {
              ...
              const weekStatus = allStatuses.includes("disputed")
                ? "disputed"
              ...
              return (
                ...
                >
                  ...
                  {DAYS.map((_, i) => {
                    ...
                    const d = days[dt];
                    const hrs = d
                      ? (parseFloat(d.total_shift_minutes || 0) / 60).toFixed(1)
                      : "--";
                    const color = !d
                      ? D.muted
                      : d.status === "approved"
                        ? D.green
                        : d.status === "edited" || d.status === "pending"
                          ? D.amber
                          : D.red;
                    const isExp =
                      expanded &&
                      expanded.techId === tech.id &&
                      expanded.date === dt;
                    return (
                      <td
                        key={dt}
                        style={{
                          ...tdStyle,
                          cursor: "pointer",
                          background: isExp ? D.input : "transparent",
                        }}
                  ...
                  <td
                    style={{
                      ...
                      fontFamily: MONO,
                      fontWeight: 700,
                      color: totalOT > 0 ? D.red : D.muted,
                    }}
                  >
                    {(totalOT / 60).toFixed(1)}
                  </td>{" "}
                  <td style={tdStyle}>
                    {" "}
                    <span
                      style={sBadge(
                        (STATUS_COLORS[weekStatus] || D.muted) + "22",
                        STATUS_COLORS[weekStatus] || D.muted,
                      )}
                    >
              ...
              <thead>
                ...
                <tr>
                  {[
                    ...
                    <th key={h} style={{ ...thStyle, fontSize: 10 }}>
                      {h}
                    </th>
                  ))}
                </tr>{" "}
                ...
                {cellEntries.map((e) => (
                  ...
                  >
                    ...
                    <td style={tdStyle}>
                      <span
                        style={sBadge(
                          (TYPE_COLORS[e.entry_type] || D.muted) + "22",
                          TYPE_COLORS[e.entry_type] || D.muted,
                        )}
                      >
                        {e.entry_type}
                      </span>
                    ...
                    <td style={tdStyle}>
                      {e.customer_first_name
                        ? `${e.customer_first_name} ${e.customer_last_name || ""}`.trim()
                        : "--"}
                    </td>{" "}
                    <td style={tdStyle}>
                      <span
                        style={sBadge(
                          (STATUS_COLORS[e.status] || D.muted) + "22",
                          STATUS_COLORS[e.status] || D.muted,
                        )}
                      >
                        {e.status}
                      </span>
...
function EntriesTab({ showToast }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await adminFetch(
        `/admin/timetracking/entries?${params.toString()}`,
      );
  ...
  const handleVoid = useCallback(
    async (id) => {
      ...
      try {
        await adminFetch(`/admin/timetracking/entries/${id}`, {
          method: "DELETE",
          body: { reason: "Admin voided from UI" },
        });
  ...
  const handleEdit = useCallback(
    async (formData) => {
      try {
        await adminFetch(`/admin/timetracking/entries/${formData.id}`, {
          method: "PUT",
          body: formData,
        });
    ...
    <div>
      ...
      >
        ...
        <div style={{ minWidth: 100 }}>
          ...
          <select
            ...
            onChange={(e) => {
              setFilters((f) => ({ ...f, entryType: e.target.value }));
              setPage(0);
            }}
            style={sInput}
          >
            {" "}
            <option value="">All</option>
            {["shift", "job", "break", "drive", "admin_time"].map((t) => (
        ...
        <div style={{ minWidth: 100 }}>
          ...
          >
            {" "}
            <option value="">All</option>
            {["active", "completed", "edited", "voided"].map((s) => (
          ...
          <tbody>
            {loading ? (
              <tr>
                <td
                  colSpan={11}
                  style={{ ...tdStyle, textAlign: "center", color: D.muted }}
                >
                  Loading...
                </td>
              </tr>
              ...
              entries.map((e) => (
                ...
                >
                  ...
                  <td style={tdStyle}>
                    {e.service_type || e.job_service_type || "--"}
                  </td>{" "}
                  <td
                    style={{
                      ...tdStyle,
      ...
      {totalPages > 1 && (
        ...
        >
          ...
          <span style={{ color: D.muted, fontSize: 12, padding: "8px 0" }}>
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          >
            {" "}
            <button onClick={onClose} style={sBtn(D.card, D.muted)}>
              Cancel
            </button>{" "}
            <button
...
function AnalyticsTab() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      adminFetch(
        `/admin/timetracking/analytics?startDate=${range.start}&endDate=${range.end}`,
      ),
      adminFetch(
        `/admin/timetracking/analytics/comparison?startDate=${range.start}&endDate=${range.end}`,
      ),
    ])
      .then(([ana, cmp]) => {
        setData(ana);
  ...
  (rpmhByTech || []).forEach((r) => {
    if (!rpmhMap[r.technician_id])
      rpmhMap[r.technician_id] = { name: r.tech_name, weeks: [] };
  ...
  (overtimeTrend || []).forEach((o) => {
    if (!utilTrend[o.week_start])
      utilTrend[o.week_start] = { week: o.week_start, util: [], ot: 0 };
    utilTrend[o.week_start].util.push(parseFloat(o.utilization_pct || 0));
    utilTrend[o.week_start].ot += parseFloat(o.overtime_minutes || 0);
  });
  const utilWeeks = Object.values(utilTrend).sort((a, b) =>
    a.week < b.week ? -1 : 1,
  );
  ...
  return (
    <div>
          ...
          >
            ...
            <tbody>
              {serviceTypeStats.map((s) => {
                ...
                const accuracy =
                  est > 0 ? (1 - Math.abs(variance) / est) * 100 : 0;
                return (
                  <tr
                    key={s.svc_type}
                    style={{ borderBottom: `1px solid ${D.border}` }}
                  >
                    ...
                    <td
                      style={{
                        ...tdStyle,
                        fontFamily: MONO,
                        color: variance > 0 ? D.red : D.green,
                      }}
                    >
                      {variance > 0 ? "+" : ""}
                      {variance.toFixed(0)}
      ...
      >
        Per-Tech Service Time Comparison
      </div>{" "}
      <div style={sCard}>
        {comparison.length === 0 ? (
            ...
            <tbody>
              {Object.values(rpmhMap).map(({ name, weeks }) => {
                ...
                const avg4 =
                  weeks.length > 0
                    ? weeks.reduce(
                        (s, w) => s + parseFloat(w.avg_rpmh || 0),
                        0,
                      ) / weeks.length
                    : 0;
                const vsTarget = avg4 - LABOR_RATE;
                const color =
                  vsTarget >= 0 ? D.green : vsTarget >= -10 ? D.amber : D.red;
                return (
                  <tr
                    key={name}
                    style={{ borderBottom: `1px solid ${D.border}` }}
                  >
                    ...
                    <td style={{ ...tdStyle, fontFamily: MONO, color }}>
                      {vsTarget >= 0 ? "+" : ""}
                      {fmt(vsTarget)}
                    </td>{" "}
                    <td style={tdStyle}>
                      <span style={sBadge(color + "22", color)}>
                        {vsTarget >= 0 ? "PROFITABLE" : "BELOW TARGET"}
                      </span>
                    </td>{" "}
                  </tr>
                );
...
function UtilizationLineChart({ weeks }) {
  ...
  const points = weeks.map((wk, i) => {
    const avgUtil =
      wk.util.length > 0
        ? wk.util.reduce((a, b) => a + b, 0) / wk.util.length
        : 0;
  ...
  return (
    ...
    >
      ...
      <line
        x1={pad.l}
        y1={pad.t + innerH - (70 / maxUtil) * innerH}
        x2={w - pad.r}
        y2={pad.t + innerH - (70 / maxUtil) * innerH}
        stroke={D.green}
        strokeWidth={1}
        strokeDasharray="4 3"
        opacity={0.5}
      ...
      <text
        x={w - pad.r + 2}
        y={pad.t + innerH - (70 / maxUtil) * innerH + 3}
        fontSize={8}
        fill={D.green}
      >
        70% target
    ...
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
      ...
      <tbody>
        {Object.entries(byTech).map(([name, rows]) =>
          rows
            .filter((r) => parseFloat(r.overtime_minutes || 0) > 0)
            .map((r, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td style={tdStyle}>
                    {otHrs > 5 ? (
                      <span style={sBadge(D.red + "22", D.red)}>HIGH OT</span>
                    ) : (
                      <span style={sBadge(D.amber + "22", D.amber)}>OT</span>
                    )}
                  </td>{" "}
                </tr>
              );
        ...
        {Object.values(byTech).every((rows) =>
          rows.every((r) => parseFloat(r.overtime_minutes || 0) === 0),
        ) && (
          <tr>
            <td
              colSpan={4}
...
function TeamTab({ showToast }) {
  ...
  const handlePhotoSelected = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/admin/timetracking/technicians/${techId}/photo`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("waves_admin_token")}`,
  ...
  const startEdit = (tech) => {
    ...
    setShowPayrollSection(
      !!(
        tech.pay_rate ||
        tech.hire_date ||
        tech.job_title ||
        tech.dob ||
        tech.emergency_contact_name
      ),
    );
    setShowAdd(true);
  };
        ...
        <div style={{ ...sCard, marginBottom: 16 }}>
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                placeholder="john@wavespestcontrol.com"
                style={sInput}
              />{" "}
            </div>{" "}
          </div>
          ...
          >
            ...
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, autoFlipEnabled: e.target.checked }))
              }
            ...
            <span style={{ fontSize: 11, color: D.muted }}>
              (when off, EXIT events for this tech skip the en-route auto-flip
              pipeline)
            </span>{" "}
          </label>
          ...
          >
            {showPayrollSection ? "Hide" : "Show"} Payroll & personal info{" "}
            {showPayrollSection ? "" : "(optional)"}
          </button>
          {showPayrollSection && (
            <div
              style={{
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  >
                    {" "}
                    <option value="">--</option> <option value="w2">W-2</option>{" "}
      ...
      <div style={sCard}>
        ...
        >
          ...
          <thead>
            {" "}
            <tr>
              {["Photo", "Name", "Phone", "Email", "Status", "Actions"].map(
                (h) => (
                  <th
                    key={h}
                    style={{
          ...
          <tbody>
                ...
                >
                  ...
                  {(t.job_title || t.pay_rate) && (
                    ...
                    >
                      ...
                      {t.job_title && t.pay_rate ? " · " : ""}
                      {t.pay_rate
                        ? `$${parseFloat(t.pay_rate).toFixed(2)}/hr`
                        : ""}
                      {t.employment_type
                        ? ` · ${t.employment_type === "w2" ? "W-2" : "1099"}`
                        : ""}
                    </div>
                  )}
                </td>{" "}
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        color: D.teal,
                        fontSize: 11,
                        cursor: uploadingId !== null ? "wait" : "pointer",
                        opacity:
                          uploadingId !== null && uploadingId !== t.id
                            ? 0.4
                            : uploadingId === t.id
                              ? 0.6
                              : 1,
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border: `1px solid ${D.border}`,
                        borderRadius: 6,
                        color: t.active ? D.amber : D.green,
                        fontSize: 11,
                        cursor: "pointer",
                      }}
                    >
            ...
            {techs.length === 0 && (
              <tr>
                <td
                  colSpan={6}
                  style={{ padding: 20, textAlign: "center", color: D.muted }}
                >
                  No technicians found. Add your first one above.
                </td>
              </tr>
            )}
          </tbody>{" "}
    ...
    (async () => {
      ...
      try {
        ...
        const [a, b] = await Promise.all([
          adminFetch(
            `/admin/timetracking/technicians/${tech.id}/earnings?period=this-week`,
          ),
          adminFetch(
            `/admin/timetracking/technicians/${tech.id}/earnings?period=last-week`,
          ),
        ]);
        if (!cancelled) {
          setThisWeek(a);
    ...
    >
      ...
      >
        ...
        {!loading && thisWeek && thisWeek.payRateSource === "default" && (
          ...
          >
            No pay rate configured — using fallback $
            {thisWeek.payRate.toFixed(2)}/hr. Set one in this tech's profile for
            accurate numbers.
          </div>
        )}
      </div>{" "}
...
function EarningsBlock({ label, data }) {
  ...
  return (
    <div
      style={{ border: `1px solid ${D.border}`, borderRadius: 8, padding: 12 }}
    >
      {" "}
      <div
        style={{
      ...
      >
        ...
        <span style={{ fontSize: 11, color: D.muted, fontFamily: MONO }}>
          {data.from} → {data.to}
        </span>{" "}
      </div>{" "}
      <div
      ...
      >
        {" "}
        <div>
          Reg: {fmt(data.regularPay)} · OT (1.5×): {fmt(data.overtimePay)}
        </div>{" "}
        <div
          style={{
            marginTop: 4,
...
function DocumentsTab({ showToast }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (techFilter && techFilter !== "all")
        params.set("technicianId", techFilter);
      const res = await adminFetch(
        `/admin/timetracking/documents?${params.toString()}`,
      );
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (uploadForm.technicianId)
        fd.append("technicianId", uploadForm.technicianId);
      if (uploadForm.expirationDate)
        fd.append("expirationDate", uploadForm.expirationDate);
  ...
  const handleDownload = async (doc) => {
    try {
      const res = await adminFetch(
        `/admin/timetracking/documents/${doc.id}/download`,
      );
  ...
  const handleDelete = async (doc) => {
    ...
    try {
      await adminFetch(`/admin/timetracking/documents/${doc.id}`, {
        method: "DELETE",
      });
  ...
  const handleEditSave = async () => {
    try {
      await adminFetch(`/admin/timetracking/documents/${editingId}`, {
        method: "PUT",
        ...
        <div style={{ ...sCard, borderColor: D.teal + "44" }}>
          ...
          >
            ...
            <div>
              ...
              <select
                value={uploadForm.technicianId}
                onChange={(e) =>
                  setUploadForm((f) => ({ ...f, technicianId: e.target.value }))
                }
                style={sInput}
              >
                {" "}
        ...
        {docs.map((doc) => {
          ...
          if (isEditing) {
            return (
              <div
                key={doc.id}
                style={{ ...sCard, borderColor: D.teal + "44" }}
              >
                {" "}
                <div
                  style={{
                ...
                <div style={{ marginBottom: 8 }}>
                  {" "}
                  <div
                    style={{ fontSize: 11, color: D.muted, marginBottom: 3 }}
                  >
                    Title
                  </div>{" "}
                  <input
                ...
                <div style={{ marginBottom: 8 }}>
                  ...
                  <select
                    ...
                    style={sInput}
                  >
                    {DOC_CATEGORIES.filter((c) => c.value !== "all").map(
                      (c) => (
                        <option key={c.value} value={c.value}>
                          {c.label}
                        </option>
            ...
            <div key={doc.id} style={sCard}>
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    {" "}
                    <span style={sBadge(catColor + "22", catColor)}>
                      {(
                    ...
                    >
                      ...
                      {doc.expiration_date &&
                        (() => {
                          ...
                          const expStr = String(doc.expiration_date).split(
                            "T",
                          )[0];
                          ...
                          return (
                            <span
                              style={{ fontSize: 11, color, fontFamily: MONO }}
                            >
                              {expired ? "Expired " : "Expires "}
                              {fmtYmdDate(doc.expiration_date)}
                              {expired
                                ? ` (${Math.abs(days)}d ago)`
                                : soon
                                  ? ` (${days}d)`
                  ...
                  onClick={() => {
                    ...
                    setEditForm({
                      ...
                      expirationDate: doc.expiration_date
                        ? String(doc.expiration_date).split("T")[0]
                        : "",
                    });
                  }}
                  style={{
      ...
      {docs.length === 0 && !showUpload && (
        <div style={{ ...sCard, textAlign: "center", padding: 40 }}>
          ...
          <div style={{ fontSize: 13, color: D.muted, marginBottom: 16 }}>
            Upload SOPs, onboarding packets, offer letters, and other internal
            documents.
          </div>{" "}
          <button
            onClick={() => setShowUpload(true)}
...
function ApprovalsTab({ showToast }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await adminFetch(
        `/admin/timesheets/pending?weekStart=${weekStart}`,
      );
      setTechs(data.techs || []);
      setSelected(new Set());
    } catch (e) {
  ...
  const loadDetail = async (techId) => {
    ...
    try {
      const d = await adminFetch(
        `/admin/timesheets/week-detail?technicianId=${techId}&weekStart=${weekStart}`,
      );
      setDetail(d);
    } catch (e) {
  ...
  const bulkApprove = async () => {
    ...
    if (
      !confirm(
        `Approve ${selected.size} tech week${selected.size === 1 ? "" : "s"}?`,
      )
    )
      return;
    try {
      ...
      showToast(`Bulk approve failed: ${e.message}`);
    }
  };
  ...
  const disputeEntry = async (entryId) => {
    ...
    try {
      ...
      showToast(`Dispute failed: ${e.message}`);
    }
  };
  ...
  const exportCsv = () => {
    ...
    fetch(`${API_BASE}/admin/timesheets/export?weekStart=${weekStart}`, {
      ...
      .then((blob) => {
        ...
        a.href = url;
        a.download = `payroll_week_${weekStart}.csv`;
        a.click();
  ...
  const totalOT =
    techs.reduce((s, t) => s + parseFloat(t.overtime_minutes || 0), 0) / 60;
  ...
  return (
    <div>
      ...
      >
        ...
        <button
          onClick={() => setWeekStart(lastMonday)}
          style={sBtn(D.card, D.muted)}
        >
          Last Week
        </button>{" "}
        <div style={{ flex: 1 }} />{" "}
        <button onClick={exportCsv} style={sBtn(D.purple, D.white)}>
          Export Payroll CSV
        </button>{" "}
      </div>
      {/* Overtime banner */}
      {totalOT > 0 && (
        ...
        >
          ...
          <div style={{ fontSize: 13, color: "#78350F" }}>
            {" "}
            <strong>{totalOT.toFixed(1)}h overtime</strong>across the team this
            week. Review before approving.
          </div>{" "}
        </div>
      )}
      ...
      >
        ...
        <div style={{ fontSize: 13, color: D.muted }}>
          {loading
            ? "Loading…"
            : `${techs.length} tech${techs.length === 1 ? "" : "s"} · ${pendingCount} pending`}
        </div>{" "}
        <div style={{ flex: 1 }} />
        {pendingCount > 0 && (
          <button onClick={selectAllPending} style={sBtn(D.card, D.teal)}>
            Select all pending
          </button>
        )}
        {selected.size > 0 && (
          ...
          <div key={t.technician_id} style={sCard}>
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 180 }}>
                {" "}
                <div
                  style={{ fontSize: 15, fontWeight: 700, color: D.heading }}
                >
                  {t.tech_name}
                </div>{" "}
                <div
                ...
                >
                  ...
                  <span
                    style={sBadge(
                      approved ? "#DCFCE7" : "#FEF3C7",
                      approved ? D.green : D.amber,
                    )}
                  >
                    {t.status || "pending"}
                  </span>
                  ...
                  {!approved &&
                    (t.tech_signed_at ? (
                      <span
                        style={sBadge("#DCFCE7", D.green)}
                        title={
                          t.tech_signature
                            ? `Signed by ${t.tech_signature}`
                            : ""
                        }
                      >
                        Tech signed
                      ...
                      <span style={sBadge(D.border, D.muted)}>
                        Awaiting tech sign-off
                      </span>
                    ))}
                </div>{" "}
              </div>{" "}
              ...
              <button
                onClick={() => loadDetail(t.technician_id)}
                style={sBtn(D.card, D.teal)}
              >
                {isExpanded ? "Hide" : "Details"}
              </button>
              {approved ? (
                <button
                  onClick={() => unlock(t.technician_id)}
                ...
                <button
                  onClick={() => approveOne(t.technician_id)}
                  style={sBtn(D.green, D.white)}
                >
                  Approve
                </button>
                    ...
                    <div style={{ overflowX: "auto" }}>
                      ...
                      >
                        ...
                        <thead>
                          {" "}
                          <tr
                            style={{ background: "#F1F5F9", textAlign: "left" }}
                          >
                            {" "}
                        ...
                        <tbody>
                          {detail.dailies.map((d) => (
                            ...
                            >
                              {" "}
                              <td style={{ padding: 8, fontFamily: MONO }}>
                                {d.work_date}
                              </td>{" "}
                              <td style={{ padding: 8 }}>
                                {fmtHrs(d.total_shift_minutes)}
                              </td>{" "}
                              ...
                              <td
                                style={{
                                  padding: 8,
                                  color:
                                    parseFloat(d.overtime_minutes) > 0
                                      ? D.amber
                                      : D.muted,
                                }}
                              >
                              ...
                              <td style={{ padding: 8 }}>
                                {" "}
                                <span style={sBadge("#F1F5F9", D.text)}>
                                  {d.status || "pending"}
                                </span>{" "}
                              </td>{" "}
                            </tr>
                    ...
                    {detail.entries &&
                      detail.entries.some(
                        ...
                        <div style={{ marginTop: 16 }}>
                          ...
                          {detail.entries
                            .filter((e) => e.approval_status === "disputed")
                            .map((e) => (
                              <div
                                key={e.id}
                                style={{
