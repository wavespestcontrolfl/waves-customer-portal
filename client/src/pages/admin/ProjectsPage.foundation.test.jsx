...
describe("Project photo caption dirty signal", () => {
  ...
  const renderDetail = (onDirtyChange, canAdminActions = false) => render(<MemoryRouter><ProjectDetail
    projectId="project-1" typesRegistry={types} onClose={vi.fn()} onDirtyChange={onDirtyChange} canAdminActions={canAdminActions}
  /></MemoryRouter>);
  ...
  it("hydrates canonical project values after send autosaves pending edits", async () => {
    let saved = false;
    fetch.mockImplementation(async (url, options = {}) => {
      const path = String(url).replace(/^\/api/, "");
      if (path === "/admin/projects/project-1" && options.method === "PUT") saved = true;
      if (saved && path === "/admin/projects/project-1" && (!options.method || options.method === "GET")) {
        return response({ ...fixtureFor(url, options), project: { ...project, title: "Canonical sent title" } });
      }
      return response(fixtureFor(url, options));
    });
    renderDetail(vi.fn(), true);

    fireEvent.change(await screen.findByLabelText("Report title"), { target: { value: "  Pending send title  " } });
    fireEvent.click(screen.getByRole("button", { name: "Send report" }));
    fireEvent.click(await screen.findByRole("button", { name: "Send", exact: true }));

    await screen.findByText(/Report delivered/);
    expect(screen.getByLabelText("Report title")).toHaveValue("Canonical sent title");
  });

  it("hydrates canonical project values after AI autosave", async () => {
    let saved = false;
    fetch.mockImplementation(async (url, options = {}) => {
      const path = String(url).replace(/^\/api/, "");
      if (path === "/admin/projects/project-1/ai-write") return response({ report: "Raw AI draft" });
      if (path === "/admin/projects/project-1" && options.method === "PUT") saved = true;
      if (saved && path === "/admin/projects/project-1" && (!options.method || options.method === "GET")) {
        return response({ ...fixtureFor(url, options), project: { ...project, recommendations: "Canonical AI draft" } });
      }
      return response(fixtureFor(url, options));
    });
    renderDetail(vi.fn(), true);

    fireEvent.change(await screen.findByLabelText("Recommendations / notes"), { target: { value: "" } });
    fireEvent.click(screen.getByRole("button", { name: "Write with AI" }));

    await screen.findByText("AI draft saved.");
    expect(screen.getByLabelText("Recommendations / notes")).toHaveValue("Canonical AI draft");
  });

  it("does not overwrite edits made while an AI draft is in flight", async () => {
    let finishDraft;
    const draft = new Promise((resolve) => { finishDraft = resolve; });
    fetch.mockImplementation(async (url, options = {}) => {
      const path = String(url).replace(/^\/api/, "");
      if (path === "/admin/projects/project-1/ai-write") return draft;
      return response(fixtureFor(url, options));
    });
    renderDetail(vi.fn(), true);
    ...
    fireEvent.change(screen.getByLabelText("Report title"), { target: { value: "Newer title" } });
    finishDraft(response({ report: "Stale AI draft" }));

    await screen.findByText("AI draft was not applied because the report changed while it was being drafted.");
    expect(screen.getByLabelText("Report title")).toHaveValue("Newer title");
    expect(screen.getByLabelText("Recommendations / notes")).toHaveValue("");
    expect(fetch.mock.calls.some(([url, options]) => url === "/api/admin/projects/project-1" && options?.method === "PUT")).toBe(false);
  });
