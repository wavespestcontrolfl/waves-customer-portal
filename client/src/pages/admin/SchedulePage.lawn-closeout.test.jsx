...
let delayReport;
let planRate;
const generatedReport = 'WHAT WE DID\nApplied Test liquid fertilizer and Test micronutrients.\n\nWHAT WE FOUND\nRecorded the confirmed lawn assessment.';
beforeEach(async () => {
  ...
  vi.stubGlobal('fetch', vi.fn(async (url, options = {}) => {
    ...
    if (url.includes('treatment-plans')) {
      ...
      if (defaultsEnabled) {
        const items = (withdrawDefaults ? [] : catalog).map((product, index) => ({ product, selected: true, applicationMethod: 'broadcast_spray',
          mix: { ratePer1000: index === 0 ? planRate : 2, rateUnit: 'fl_oz', amount: sqft ? sqft * (index === 0 ? planRate : 2) / 1000 : null, amountUnit: 'fl_oz', treatedSqft: sqft } }));
    ...
    if (url.endsWith('/schedule/generate-report')) {
      if (delayReport) await new Promise(resolve => { reportResolver = resolve; });
...
it.each(['removal', 'rate', 'visit area'])('clears an untouched generated report after a plan %s changes actuals', async (change) => {
  enableDefaults();
  mount();
  await waitFor(() => expect(totals()).toHaveLength(2));
  fireEvent.click(screen.getByRole('button', { name: 'Generate AI Service Report' }));
  await screen.findByDisplayValue(/Applied Test liquid fertilizer and Test micronutrients\./);
  if (change === 'removal') catalog = products;
  if (change === 'rate') planRate = 4;
  if (change === 'visit area') fireEvent.change(screen.getByLabelText('Area for this visit (sq ft)'), { target: { value: '4000' } });
  else fireEvent.click(screen.getByRole('button', { name: 'Refresh plan' }));
  await waitFor(() => expect(screen.queryByDisplayValue(/Applied Test liquid fertilizer and Test micronutrients\./)).toBeNull());
  fireEvent.click(await screen.findByRole('button', { name: /complete & send recap/i }));
  await waitFor(() => expect(submit).toHaveBeenCalledOnce());
  const body = submit.mock.calls[0][1];
  expect(JSON.stringify(body)).not.toContain('Applied Test liquid fertilizer and Test micronutrients.');
  if (change === 'removal') expect(body.products.map(p => p.productId)).toEqual(['test-k']);
  if (change === 'rate') expect(Number(body.products[0].rate)).toBe(4);
  if (change === 'visit area') expect(body.lawnProtocolCompletion).toEqual({ treatedSqft: 4000 });
});

it('discards a generated response when products changed while generation was in flight', async () => {
  enableDefaults();
  mount();
  await waitFor(() => expect(totals()).toHaveLength(2));
  delayPlan = true;
  ...
  delayReport = true;
  fireEvent.click(screen.getByRole('button', { name: 'Generate AI Service Report' }));
  await waitFor(() => expect(reportResolver).toBeTypeOf('function'));
  catalog = products;
  await act(async () => { planResolvers[0](); });
  await waitFor(() => expect(totals()).toHaveLength(1));
  await act(async () => { reportResolver(); });
  expect(screen.queryByDisplayValue(/Applied Test liquid fertilizer and Test micronutrients\./)).toBeNull();
  fireEvent.click(screen.getByRole('button', { name: /complete & send recap/i }));
  await waitFor(() => expect(submit).toHaveBeenCalledOnce());
  expect(JSON.stringify(submit.mock.calls[0][1])).not.toContain('Applied Test liquid fertilizer and Test micronutrients.');
});

it('keeps technician-edited prose when a plan refresh changes products', async () => {
  enableDefaults();
  mount();
  await waitFor(() => expect(totals()).toHaveLength(2));
  fireEvent.click(screen.getByRole('button', { name: 'Generate AI Service Report' }));
  const report = await screen.findByDisplayValue(/Applied Test liquid fertilizer and Test micronutrients\./);
  fireEvent.change(report, { target: { value: 'Technician reviewed actual application notes.' } });
  catalog = products;
  fireEvent.click(screen.getByRole('button', { name: 'Refresh plan' }));
  await waitFor(() => expect(totals()).toHaveLength(1));
  expect(screen.getByDisplayValue('Technician reviewed actual application notes.')).toBeTruthy();
});

it('retains a restored report for unchanged actuals then clears it when the current plan withdraws a product', async () => {
  enableDefaults();
  const view = mount();
  await waitFor(() => expect(totals()).toHaveLength(2));
  fireEvent.click(screen.getByRole('button', { name: 'Generate AI Service Report' }));
  await screen.findByDisplayValue(/Applied Test liquid fertilizer and Test micronutrients\./);
  await waitFor(() => expect(JSON.parse(localStorage.getItem(`waves_completion_draft_${service.id}`)).generatedReportText).toBe(generatedReport));
  view.unmount();
  mount();
  fireEvent.click(await screen.findByRole('button', { name: 'Restore', exact: true }));
  await screen.findByDisplayValue(/Applied Test liquid fertilizer and Test micronutrients\./);
