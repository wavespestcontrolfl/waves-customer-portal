...
function StatButton({ label, value, filter, active, onClick, alert }) {
  return (
    ...
    >
      ...
      <div
        className={cn(
          "text-22 font-mono u-nums",
          alert && value > 0 ? "text-alert-fg" : "text-ink-primary",
        )}
      >
        {value}
      </div>{" "}
...
export function SandyTransferCard({ handoff }) {
  ...
  const facts = Object.entries(handoff.facts_collected || {}).filter(([, v]) => v != null && String(v).trim() !== "");
  const tools = Array.isArray(handoff.tools) ? handoff.tools : [];
  const commitments = Array.isArray(handoff.commitments) ? handoff.commitments : [];
  const Row = ({ label, children }) => (
    <div className="flex gap-2 text-14 md:text-12">
      <span className="w-28 flex-shrink-0 text-ink-tertiary">{label}</span>
    ...
    <div className="mt-1.5 ml-8 p-2 bg-zinc-50 border-hairline rounded-md" data-testid="sandy-transfer-card">
      ...
      {noContext ? (
        <div className="text-14 md:text-12 text-ink-secondary">
          Context unavailable — the summary could not be saved before the transfer. Ask the caller to recap.
        </div>
      ) : (
        <div className="flex flex-col gap-1">
          {handoff.intent && <Row label="Intent">{handoff.intent}</Row>}
          {handoff.summary && <Row label="Summary">{handoff.summary}</Row>}
          {handoff.unresolved_question && <Row label="Unresolved">{handoff.unresolved_question}</Row>}
          {facts.length > 0 && (
            <Row label="Collected">{facts.map(([k, v]) => `${k.replace(/_/g, " ")}: ${v}`).join(" · ")}</Row>
          )}
          {tools.length > 0 && (
            <Row label="Tools">
              {tools.map((t, i) => (
                <span key={`${t.name}-${i}`} className={cn("inline-block mr-2", t.ok ? "" : "text-alert-fg")}>
          ...
          {commitments.length > 0 && (
            <Row label="Promised">{commitments.map((c) => `${String(c.kind || "").replace(/_/g, " ")}${c.verdict === true ? "" : " (not promised)"}`).join(" · ")}</Row>
          )}
          <Row label="Turns">{Number.isFinite(handoff.turn_count) ? handoff.turn_count : "—"}</Row>
        </div>
      )}
    </div>
  );
...
export default function CallLogTabV2() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await adminFetch(`/admin/call-recordings/process/${sid}`, {
          method: "POST",
        });
      } catch {
    ...
    <div className="flex min-w-0 max-w-full flex-col gap-4 overflow-x-hidden">
      ...
      <div className="hidden md:flex gap-2 flex-wrap">
        ...
        <StatButton
          ...
          value={answered}
          filter="answered"
          active={callFilter === "answered"}
          onClick={() =>
            setCallFilter((p) => (p === "answered" ? "all" : "answered"))
          }
        />{" "}
        <StatButton
          label="Voicemail"
        ...
        <StatButton
          ...
          value={missed}
          filter="missed"
          active={callFilter === "missed"}
          alert
          onClick={() =>
            setCallFilter((p) => (p === "missed" ? "all" : "missed"))
          }
        />{" "}
      </div>
      {/* Voicemail queue */}
      ...
      {sortedSources.length > 0 && (
        <Card className="hidden md:block">
          ...
          <CardBody>
            {" "}
            <div className="text-11 uppercase tracking-label text-ink-tertiary mb-3">
            ...
            <div className="flex flex-wrap gap-2">
              {sortedSources.slice(0, 12).map(([label, count]) => (
                <div
                  key={label}
                  className="flex items-center gap-2 px-3 py-1 bg-zinc-50 border-hairline rounded-md"
                >
                  {" "}
      ...
      <Card>
        ...
        <CardBody>
          {" "}
          <div className="text-14 md:text-11 font-medium md:font-normal md:uppercase tracking-normal md:tracking-label text-zinc-900 md:text-ink-tertiary mb-3">
            Make a Call
          </div>{" "}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
            ...
            <div>
              {" "}
              <label className="block text-13 md:text-11 font-medium md:font-normal md:uppercase tracking-normal md:tracking-label text-zinc-900 md:text-ink-tertiary mb-1">
                From
              </label>{" "}
              <Select
                value={callFrom}
          ...
          {callResult && (
            <div
              className={cn(
                "mt-3 text-12",
                callResult.ok ? "text-ink-secondary" : "text-alert-fg",
              )}
            >
              {callResult.text}
            </div>
                  ...
                  >
                    ...
                    {((isUnknown && c.from_phone) ||
                      ...
                      <div className="mt-2 ml-8 flex gap-2 flex-wrap items-center">
                        ...
                        {c.twilio_call_sid &&
                          (() => {
                            ...
                            const isManual =
                              processingCallSid === c.twilio_call_sid;
                            const isProcessed =
                              c.processing_status === "processed";
                            const hasTranscript = !!(
                              c.transcription && c.transcription.length > 0
                            );
                            if (isAuto || isManual) {
                              return (
                        ...
                        {processResult &&
                          processResult.callSid === c.twilio_call_sid && (
                            <span
                              className={cn(
                                "text-12",
                                PROCESS_RESULT_CLASS[processResult.severity],
                        ...
                        return (
                          <div className="mt-1.5 ml-8 bg-zinc-50 border-hairline rounded-md">
                            ...
                            >
                              ...
                              <span
                                aria-hidden
                                className="text-12 text-ink-tertiary"
                              >
                                {open ? "▾" : "▸"}
                              </span>{" "}
                            </button>{" "}
                    ...
                    <SandyTransferCard handoff={parseJsonObject(c.metadata)?.relay_handoff} />
