  ...
  return (
    <Card>
      ...
      <CardBody className="space-y-5">
        ...
        <Table className="min-w-[900px]" aria-label="Pest calibration samples"><THead><TR><TH>Date</TH><TH>Customer</TH><TH align="right">Pool</TH><TH align="right">Lot</TH><TH align="right">Pred</TH><TH align="right">Actual</TH><TH align="right">Delta</TH><TH align="right">Confidence</TH><TH>Reasons</TH></TR></THead><TBody>{records.slice(0, 50).map((row) => { const reasons = Array.isArray(row.review_reasons) ? row.review_reasons.join(", ") : ""; return <TR key={row.id || row.scheduled_service_id}><TD nums>{String(row.service_date || "").slice(0, 10) || "-"}</TD><TD className="font-medium">{row.customer_name || row.address_line1 || "Unknown"}</TD><TD align="right">{row.pool_cage_size || "-"}</TD><TD align="right" nums>{row.lot_sqft ? Number(row.lot_sqft).toLocaleString() : "-"}</TD><TD align="right" nums>{Number(row.predicted_minutes || 0).toFixed(1)}</TD><TD align="right" nums>{Number(row.actual_minutes || 0).toFixed(1)}</TD><TD align="right" nums>{formatMinutes(row.delta_minutes || 0)}</TD><TD align="right">{row.pricing_confidence || "-"}</TD><TD className="max-w-[220px] truncate text-ink-secondary" title={reasons}>{reasons || "-"}</TD></TR>; })}{!loading && records.length === 0 && <TR><TD colSpan="9" className="py-6 text-center text-ink-secondary">No calibration samples yet. Completed pest jobs need an accepted estimate link and a completed job timer.</TD></TR>}</TBody></Table>
      </CardBody>
    </Card>
  );
}
