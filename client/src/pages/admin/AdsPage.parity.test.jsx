  ...
  vi.stubGlobal(
    ...
    vi.fn(async (url) => {
      const data = url.endsWith("/campaigns")
        ? { campaigns: [] }
        : url.includes("/call-bridge?")
          ? { summary: {}, matches: [] }
          : url.includes("/service-lines?")
            ? { totalLeads: 0 }
            : url.endsWith("/advisor/history")
              ? { reports: [] }
              : url.endsWith("/advisor")
                ? { report: null }
                : url.endsWith("/capacity-heatmap")
                  ? { heatmap: {} }
                  : null;
...
it("retains all six PPC workflows and their data reads without starting an action", async () => {
  ...
  expect(
    screen.getByRole("heading", { name: "PPC", level: 1 }),
  ).toBeInTheDocument();
  expect(
    screen.getByRole("navigation", { name: "PPC section" }),
  ).toBeInTheDocument();
  expect(await screen.findByText("Existing PPC dashboard")).toBeInTheDocument();
  for (const [label, endpoint] of [
    ...
    await waitFor(() =>
      expect(fetch.mock.calls.some(([url]) => url.endsWith(endpoint))).toBe(
        true,
      ),
    );
    await waitFor(() =>
  ...
  expect(
    fetch.mock.calls.some(([url]) =>
      url.endsWith("/admin/ads/advisor/history"),
    ),
  ).toBe(true);
  ...
  expect(
    fetch.mock.calls.every(
      ([url, options]) =>
        !url.includes("/seo") && (!options.method || options.method === "GET"),
    ),
  ).toBe(true);
  fireEvent.click(
