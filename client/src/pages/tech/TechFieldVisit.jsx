...
export default function TechFieldVisit({ stop, loading, error, onBack, onRetry, busy, enRouteState, onSiteState, onEnRoute, onSite, onSync, onMove, children }) {
  ...
  if (error) return <>{back}<div className="tf-alert tf-error" role="alert">{error}<div><button type="button" className="tf-button" onClick={onRetry}>Retry route</button></div></div></>;
  ...
  return <>
    ...
    <div className="tf-actions">
      ...
      {['pending', 'confirmed', 'rescheduled'].includes(status) && <button type="button" className="tf-button" disabled={pending} onClick={() => onEnRoute(service.id)}>En route</button>}
      {status === 'en_route' && <button type="button" className="tf-button" disabled={pending} onClick={() => onSite(service.id)}><MapPin aria-hidden="true" />On site</button>}
