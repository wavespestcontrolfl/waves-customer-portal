 ...
 * at call time.) A DEPLOY-WIDE release gate, not a per-user capability —
 * those go through useFeatureFlag / user_feature_flags.
 *
 ...
 * is always what the server will save. Fails closed — an unreachable API
 * leaves the surface exactly as it was before this lane.
 *
 * A transient probe failure is NOT the same as a confirmed answer: caching
 * `false` for the rest of the SPA session would strand a surface previewing
 * single-discount math while the real (unreachable) gate is ON and the
 * server compounds on save. So a failure clears the cache instead of
 * pinning it, and callers get a `known` flag to refuse acting on an
 * unconfirmed answer instead of confidently computing the wrong one.
 *
 * Codex #4405 P1: even a SUCCESSFUL probe was cached for the rest of the SPA
 * session, so an open tab kept the old semantics across a mid-session gate
 * flip while the server's money endpoints read the live env var on every
 * request. A confirmed value now expires after CACHE_TTL_MS and reads as
 * unconfirmed until revalidated. A TTL alone only helps surfaces that MOUNT
 * after it expires, so the hook also re-probes on an interval and on tab
 * focus, and exports ensureStackingFresh() to await immediately before
 * posting money.
 *
 * Codex P1 (an older head, useDiscountStacking.js:160): ensureStackingFresh
 * trusting an already-fresh cache is still up to CACHE_TTL_MS behind a flip
 * between the last poll and the submit click. A submit-time check needs the
 * LIVE answer, not "recently confirmed" — so it always issues (or joins) a
 * real probe, bypassing the cacheIsFresh() short-circuit loadStacking() uses
 * for routine mounts and polls.
 */
...
// ensureStackingFresh() never consults this — see the header note above.
...
// Issues (or joins an already-inflight) real probe, ignoring whether the
// cache is currently fresh. `loadStacking` wraps this with a freshness
// short-circuit for routine mounts/polls; `ensureStackingFresh` calls this
// directly so a submit-time check always sees the live answer.
function probeStacking() {
...
/**
 * Revalidate before POSTING money. The polling in useDiscountStackingState
 * narrows the window but cannot close it: a gate flip between the last
 * probe and this click would still submit under the old semantics. Money
 * surfaces await this and compare `enabled` against the value their preview
 * used — if it moved, the totals on screen are not the totals the server
 * will save, so the submission must stop rather than silently bill the
 * other regime.
 *
 * Always issues (or joins) a live probe — never trusts an already-fresh
 * cache, even seconds old (see the header's older-head P1 note): a submit
 * needs the LIVE answer, not "recently confirmed."
 *
