...
export function usePublishIntelligenceBarPageData({ customer_id = null, appointment_id = null, product_id = null, viewed_date = null, overlay = false }) {
  const setScopes = useContext(PageDataContext)?.setScopes;
  const owner = useRef({});
  useEffect(() => {
    if (!setScopes) return undefined;
    const scope = { owner: owner.current, overlay, data: { customer_id, appointment_id, product_id, viewed_date } };
  ...
  }, [setScopes, customer_id, appointment_id, product_id, viewed_date, overlay]);
}
