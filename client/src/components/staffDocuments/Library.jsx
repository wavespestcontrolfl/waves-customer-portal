...
export default function StaffDocumentLibrary({ manage = false }) {
  ...
  useEffect(() => {
    ...
    request('/people', undefined, controller.signal).then(setProfile).catch(failure => { if (failure.name !== 'AbortError') setError(failure.message); });
    return () => controller.abort();
  }, []);
  useEffect(() => {
    if (!canManage) return undefined;
    const controller = new AbortController();
    request('/starters', undefined, controller.signal).then(result => setStarters(result.starters)).catch(failure => { if (failure.name !== 'AbortError') setError(failure.message); });
    return () => controller.abort();
  }, [canManage]);
  useEffect(() => {
    const controller = new AbortController();
    const timer = setTimeout(() => {
      ...
      request(`/?${query}`, undefined, controller.signal).then(result => { setItems(result.documents); setError(''); }).catch(failure => { if (failure.name !== 'AbortError') setError(failure.message); });
    }, 200);
  ...
  useEffect(() => {
    ...
    request(`/${selected}?${query}`, undefined, controller.signal).then(setDetail).catch(failure => { if (failure.name !== 'AbortError') setError(failure.message); });
    return () => controller.abort();
  ...
  function select(id, version) {
    ...
    setParams(next); setEditing(null); setPolicyEditor(false); setError('');
  }
