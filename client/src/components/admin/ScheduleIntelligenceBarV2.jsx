...
const FALLBACK_ACTIONS = [
  {
    ...
    group: "Plan",
    label: "Day Briefing",
    prompt: "Give me a full briefing for today",
  },
  {
    id: "gaps",
    group: "Plan",
    label: "Gaps This Week",
    prompt: "Where do we have open capacity this week?",
  },
  {
    id: "optimize",
    group: "Optimize",
    label: "Optimize Routes",
    prompt: "Optimize all routes for today",
  },
  {
    id: "zone_density",
    group: "Optimize",
    label: "Zone Density",
    prompt: "Analyze zone density — any consolidation opportunities?",
  },
  {
    id: "far_out",
    group: "Optimize",
    label: "Far-Out Appts",
    prompt: "Find appointments more than 30 days out that we could move sooner",
  },
  {
    id: "unassigned",
    group: "Fix",
    label: "Unassigned",
    prompt: "Show me unassigned stops and suggest tech assignments",
  },
];
...
export default function ScheduleIntelligenceBarV2({
  date,
  scheduleData,
  onRefresh,
}) {
  const buildPageData = useCallback(() => {
    const pd = {
      current_date: date,
      current_date_formatted: new Date(date + "T12:00:00").toLocaleDateString(
        "en-US",
        {
          weekday: "long",
          month: "long",
    ...
    if (scheduleData) {
      ...
      pd.completed =
        scheduleData.services?.filter((s) => s.status === "completed").length ||
        0;
  ...
  const handleAfterSubmit = useCallback(
    (data) => {
      const didWrite = (data.toolCalls || []).some((tc) =>
        WRITE_TOOLS.includes(tc.name),
      );
  ...
  const completedCount =
    scheduleData?.services?.filter((s) => s.status === "completed").length || 0;
      ...
      headerSlot={({ expanded }) =>
        totalServices > 0 && !expanded ? (
          <div className="flex gap-3 u-nums text-11 text-ink-secondary flex-shrink-0">
            ...
            {unassignedCount > 0 && (
              <span>
                <strong className="text-alert-fg font-medium">
                  {unassignedCount}
                </strong>
                unassigned
              </span>
