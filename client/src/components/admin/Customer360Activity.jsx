...
export default function Customer360Activity({ timeline, filter, onFilter, search = "", onSearch, error, retrying, onRetry, missingSources = [], loading = false, hasMore = false, onLoadOlder }) {
  ...
  return <section className="c360-activity" aria-labelledby={`${id}-heading`}>
    <div className="mb-3 flex items-center justify-between gap-3">
      ...
      {!error && <span className="text-14 text-ink-secondary">{timeline.length} {timeline.length === 1 ? "event" : "events"} loaded</span>}
    </div>
    ...
    <Card>
      <div className="c360-activity-toolbar">
        <label className="c360-search-field c360-activity-search">
          ...
          <Input type="search" value={search} onChange={(event) => onSearch(event.target.value)} maxLength={200} aria-label="Search activity" placeholder="Search activity…" className="!pl-9 !text-16" />
        </label>
        <Select className="c360-activity-filter !w-auto !max-w-full" aria-label="Filter activity" value={filter} onChange={(event) => onFilter(event.target.value)}>
      ...
      <div className="c360-activity-list" role="region" aria-label="Customer activity history" tabIndex={0}>
        {error && <div className="flex flex-col items-center gap-3 p-5"><p role="alert" className="text-14 text-alert-fg">Could not load customer history.</p><Button variant="secondary" aria-label="Retry customer history" onClick={onRetry} disabled={retrying}>{retrying ? "Retrying…" : "Retry"}</Button></div>}
        {loading && <p role="status" className="p-5 text-14 text-ink-secondary">Loading history…</p>}
        ...
        {!error && !loading && entries.length === 0 && <p className="p-5 text-14 text-ink-secondary">{terms.length ? "No matching activity." : "No activity in this category."}</p>}
      </div>
      <div className="c360-activity-footer flex flex-wrap items-center justify-between gap-3">
        <span>{entries.length} loaded</span>
        {hasMore && !error && <Button variant="secondary" onClick={onLoadOlder} disabled={loading}>Load older activity</Button>}
      </div>
    </Card>
  </section>;
}
