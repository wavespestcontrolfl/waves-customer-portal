...
// Shared ET formatter (lib/timezone.js — the client companion of the
// server's datetime-et); only the null / unparseable guard is local.
function fmtWhen(value) {
  if (!value) return null;
  if (Number.isNaN(new Date(value).getTime())) return String(value);
  return formatETDateTime(value, { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" });
}
...
// (a relink / unlink, an adopted recording) so the parent list can reload
// the row — its audio player and transcript would otherwise keep showing
// the superseded recording beside the new intelligence (Codex r7 P1).
// `refreshKey` changes whenever the parent's call row was reloaded with a
...
export default function CallIntelligencePanel({ callId, onJumpToQuote, onCallChanged, refreshKey = null }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setState({ status: "ready", data: body.intelligence, error: null, canEdit: body.features?.commitments === true, isAdmin: body.features?.admin === true });
    } catch (err) {
  ...
  // Reload an already-loaded panel when the parent's row moved on; the
  // first render of a key is never a reload (the idle→ready effect owns it).
  const seenKey = useRef(refreshKey);
  useEffect(() => {
    if (seenKey.current === refreshKey) return;
    seenKey.current = refreshKey;
    if (open && state.status === "ready") load();
  }, [refreshKey]);

  // Each customer search carries its sequence number; a response that is
  // not the latest is dropped, so buttons never name customers from an
  // obsolete query (Codex r8 P2).
  ...
  const act = async (label, fn) => {
    ...
    try {
      await fn();
      await load();
      if (label === "adopt" || label === "relink" || label === "unlink") await onCallChanged?.();
    } catch (err) {
            ...
            <div className="space-y-1.5">
              <Input
                ...
                onChange={async (e) => {
                  ...
                  setRelink((r) => ({ ...r, query }));
                  // A query too short to search shows NO results: stale
                  // buttons from the previous query could relink the call
                  // to a customer the field no longer names (Codex r7 P2).
                  const seq = ++searchSeq.current;
                  if (query.trim().length < 2) { setRelink((r) => (r ? { ...r, results: [] } : r)); return; }
                  try {
          ...
          <div>
            ...
            <ul className="mt-1 space-y-1.5">
              {view.commitments.map((c) => {
                // Stale = not seen by the NEWEST pass that recorded commitments
                // (the max last_seen_generation over the call's AI rows) — never
                // the row's processing_generation, which a pass bumps on claim
                // before it has recorded anything (parity with the Owed queue's
                // staleAiRowSql, #3725 r15).
                const latestSeen = Math.max(...view.commitments.filter((x) => x.source === "ai" && x.last_seen_generation != null).map((x) => Number(x.last_seen_generation)), -Infinity);
                const stale = Number.isFinite(latestSeen) && c.source === "ai" && c.last_seen_generation != null && Number(c.last_seen_generation) < latestSeen;
                ...
                return (
                  <li key={c.id} className="border-hairline rounded-md bg-white p-2 space-y-1">
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      {stale && <span className="text-12 md:text-11 text-ink-tertiary">not detected on pass {latestSeen}</span>}
