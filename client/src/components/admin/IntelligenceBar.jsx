  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^[-•*]\s/)) {
      ...
      elements.push(
        ...
        >
          {" "}
          <span style={{ color: D.teal, fontSize: 10, marginTop: 5 }}>
            ●
          </span>{" "}
...
export default function IntelligenceBar({ onSelectCustomer }) {
    ...
    adminFetch("/admin/intelligence-bar/quick-actions")
      ...
      .catch(() => {
        ...
        setQuickActions([
          {
            id: "missing_city",
            label: "Missing Cities",
            prompt: "Show me customers with no city on their profile",
            icon: "",
          },
          {
            id: "pest_overdue",
            label: "Pest Overdue",
            prompt:
              "Which quarterly pest control customers are overdue for service?",
            icon: "",
          },
          {
            id: "lawn_overdue",
            label: "Lawn Overdue",
            prompt: "Which monthly lawn care customers are overdue?",
            icon: "",
          },
          {
            id: "at_risk",
            label: "At Risk",
            prompt: "Show me customers with health scores below 40",
            icon: "",
          },
          {
            id: "high_balance",
            label: "Balances",
            prompt: "Who has an outstanding balance over $100?",
            icon: "",
          },
          {
            id: "duplicates",
            label: "Duplicates",
            prompt: "Find duplicate customers by phone number",
            icon: "",
          },
          {
            id: "tech_perf",
            label: "Tech Stats",
            prompt: "Compare technician performance this month",
            icon: "",
          },
          {
            id: "win_back",
            label: "Win Back",
            prompt:
              "Show churned Gold/Platinum customers from the last 6 months",
            icon: "",
          },
        ]);
      });
  ...
  const submit = useCallback(
    async (text) => {
      ...
      try {
        ...
        setResponse(
          ` Error: ${err.message}. Make sure the intelligence-bar route is registered and ANTHROPIC_API_KEY is set.`,
        );
      }
  ...
  const customerList =
    structuredData?.customers || structuredData?.overdue_customers || null;
        ...
        <div style={{ padding: "0 18px 14px" }}>
          ...
          {recentPrompts.map((p, i) => (
            <div
              ...
              onMouseEnter={(e) => {
                e.target.style.background = `${D.teal}11`;
                e.target.style.color = D.text;
              }}
              onMouseLeave={(e) => {
                e.target.style.background = "transparent";
                e.target.style.color = D.muted;
              }}
            >
              {p}
            </div>
          ...
          {customerList && customerList.length > 0 && (
            ...
            >
              ...
              >
                ...
                {structuredData?.total_matching != null &&
                  structuredData.total_matching > customerList.length && (
                    <span
                      style={{
                        color: D.teal,
                        textTransform: "none",
                    ...
                    >
                      showing {customerList.length} of{" "}
                      {structuredData.total_matching}
                    </span>
                  )}
              </div>
