...
export default function JobCostCard({ scheduledServiceId, dark = true }) {
  ...
  async function load() {
    ...
    try {
      const [cRes, sRes] = await Promise.all([
        adminFetch(
          `/admin/job-costs?scheduled_service_id=${scheduledServiceId}&limit=1`,
        )
          .then((r) => r.json())
          .catch(() => ({})),
        adminFetch(
          `/admin/job-forms/submissions?scheduled_service_id=${scheduledServiceId}`,
        )
          .then((r) => r.json())
          .catch(() => ({})),
      ]);
  ...
  async function recalc() {
    setRecalcing(true);
    try {
      await adminFetch(`/admin/job-costs/recalc/${scheduledServiceId}`, {
        method: "POST",
      });
      await load();
    } finally {
  ...
  if (loading)
    return (
      <div style={{ fontSize: 12, color: P.muted }}>Loading job detail…</div>
    );

  const fmt = (n) => (n == null ? "—" : `$${Number(n).toFixed(2)}`);
  const marginColor =
    cost?.margin_pct == null
      ? P.muted
      : cost.margin_pct >= 40
        ? P.green
        : cost.margin_pct >= 20
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {recalcing ? "Recalculating…" : "Recalc"}
        </button>{" "}
      </div>
      {!cost ? (
        <div style={{ fontSize: 12, color: P.muted }}>
          No cost data yet. Click Recalc.
        </div>
      ) : (
        <div
          style={{
        ...
        >
          ...
          <Stat
            P={P}
            label="Profit"
            value={fmt(cost.gross_profit)}
            color={(cost.gross_profit || 0) >= 0 ? P.green : P.red}
          />{" "}
          <Stat
            P={P}
            label="Margin"
            value={
              cost.margin_pct == null
                ? "—"
                : `${Number(cost.margin_pct).toFixed(1)}%`
            }
            color={marginColor}
          />{" "}
        </div>
      ...
      {cost && (
        ...
        >
          ...
          {cost.equipment_cost > 0 && (
            <span>Equipment: {fmt(cost.equipment_cost)}</span>
          )}
        </div>
      )}
      {submission && (
          ...
          >
            ...
            <span
              style={{
                ...
                color:
                  submission.completion_percent === 100 ? P.green : P.amber,
              }}
            >
              {submission.completion_percent ?? 0}%
            </span>{" "}
          </div>{" "}
        </div>
      )}
