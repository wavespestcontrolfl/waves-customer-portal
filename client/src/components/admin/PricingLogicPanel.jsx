...
function ChangelogTab() {
  ...
  useEffect(() => {
    ...
    adminFetch(`/admin/pricing-config/changelog${query}`)
  ...
  return (
    <Card>
      <CardHeader className="flex flex-wrap items-end justify-between gap-3">
        <div><CardTitle className="text-16">Pricing changelog</CardTitle><p className="mt-1 text-ui-body text-ink-secondary">Review deployed pricing changes and their rationale.</p></div>
        <Field label="Category"><Select value={filter} onChange={(event) => setFilter(event.target.value)}>{filterOptions.map((option) => <option key={option} value={option}>{option === "all" ? "All categories" : option}</option>)}</Select></Field>
      </CardHeader>
      <CardBody>
...
function ProposalsTab() {
  ...
  const loadProposals = useCallback(() => {
    setLoading(true);
    adminFetch(`/admin/pricing-proposals?status=${encodeURIComponent(statusFilter)}&limit=50`)
  ...
  const submitAction = async (action) => {
    ...
    setSubmitting(true);
    try {
      const response = await adminFetch(`/admin/pricing-proposals/${selected.id}/${action}`, {
        method: "POST", body: JSON.stringify({ review_notes: reviewNotes || null }),
      });
  ...
  return (
    <Card>
      <CardHeader className="flex flex-wrap items-center justify-between gap-3">
        <div><CardTitle className="text-16">Pricing proposals</CardTitle><p className="mt-1 text-ui-body text-ink-secondary">Review proposed config changes before they reach the pricing engine.</p></div>
      ...
      <Dialog open={Boolean(selected)} onClose={submitting ? undefined : () => setSelected(null)} size="md">
        {selected && <>
          ...
          <DialogBody className="space-y-4">
            ...
            <Field label="Review notes"><Textarea value={reviewNotes} onChange={(event) => setReviewNotes(event.target.value)} disabled={selected.status !== "pending" || submitting} placeholder="Optional notes captured with approval/rejection (included in changelog rationale on approve)" rows={4} /></Field>
          </DialogBody>
          <DialogFooter>
...
function ConfigCard({ config, onUpdate }) {
  ...
  return <Card>
    ...
    {expanded && <CardBody className="space-y-3">{error && <ActionFeedback error>{error}</ActionFeedback>}{rawEdit ? <><Field label="Configuration JSON"><Textarea value={rawText} onChange={(event) => setRawText(event.target.value)} rows={Math.min(20, rawText.split("\n").length + 1)} className="u-nums" /></Field><div className="flex gap-2"><Button onClick={handleRawSave} loading={saving}>Save</Button><Button variant="secondary" onClick={() => setRawEdit(false)}>Cancel</Button></div></> : isObject ? <div>{Object.entries(data).map(([key, value]) => renderValue(key, value))}</div> : Array.isArray(data) ? renderArray(data) : <pre className="overflow-auto rounded-md bg-zinc-50 p-3 text-ui-body text-zinc-700 u-nums">{JSON.stringify(data, null, 2)}</pre>}</CardBody>}
  </Card>;
}
...
function LawnBracketsTab() {
  ...
  useEffect(() => { adminFetch("/admin/pricing-config/lawn-brackets").then((data) => setTracks(data.tracks || {})).catch(() => {}).finally(() => setLoading(false)); }, []);
  const handleCellUpdate = async (squareFeet, tier, price) => {
    const updated = (tracks[activeTrack] || []).map((row) => row.sqft_bracket === squareFeet && row.tier === tier ? { ...row, monthly_price: price } : row);
    setTracks((previous) => ({ ...previous, [activeTrack]: updated })); setSaving(true); setError("");
    try { await adminFetch(`/admin/pricing-config/lawn-brackets/${activeTrack}`, { method: "PUT", body: JSON.stringify({ brackets: [{ sqft_bracket: squareFeet, tier, monthly_price: price }] }) }); }
    catch (nextError) { setError(`Save failed: ${nextError.message}`); }
    finally { setSaving(false); }
  };
  ...
  return <Card><CardHeader className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle className="text-16">Monthly price brackets</CardTitle><p className="mt-1 text-ui-body text-ink-secondary">Set monthly lawn prices by turf track and annual application count.</p></div>{saving && <Badge tone="strong">Saving...</Badge>}</CardHeader><CardBody className="space-y-4">
...
function DiscountRulesTab() {
  ...
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  useEffect(() => { adminFetch("/admin/pricing-config/discount-rules").then((data) => setRules(data.rules || [])).catch(() => {}).finally(() => setLoading(false)); }, []);
  const handleUpdate = async (serviceKey, field, value) => {
    ...
    try { await adminFetch(`/admin/pricing-config/discount-rules/${serviceKey}`, { method: "PUT", body: JSON.stringify({ [field]: value }) }); }
    catch (nextError) { setError(`Save failed: ${nextError.message}`); }
  };
...
function ProductsTab() {
  ...
  const [loading, setLoading] = useState(true);
  useEffect(() => { adminFetch("/admin/inventory?limit=200").then((data) => setProducts(data.products || [])).catch(() => {}).finally(() => setLoading(false)); }, []);
...
export default function PricingLogicPanel() {
  ...
  const [loading, setLoading] = useState(true);
  useEffect(() => { adminFetch("/admin/pricing-config").then((data) => setConfigs(data.configs || [])).catch(() => {}).finally(() => setLoading(false)); }, []);
