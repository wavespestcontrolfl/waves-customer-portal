 ...
 * plus the tech and Bouncie IMEI mapping table and an event log.
 *
...
export default function GeofenceSettings() {
  ...
  async function saveSettings(partial) {
    ...
    setSettings(next);
    try {
      await api("/admin/geofence/settings", {
        method: "PUT",
  ...
  async function saveVehicle(techId) {
    try {
      await api(`/admin/geofence/vehicles/${techId}`, {
        method: "PUT",
        body: JSON.stringify({
          bouncie_imei: imeiInput.trim(),
          bouncie_vin: vinInput.trim(),
          vehicle_name: vehNameInput.trim(),
        }),
      });
      setEditingTech(null);
      await load();
  ...
  if (!settings)
    return (
      <div style={{ color: D.muted, padding: 24 }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      {!hasMapping && (
        <Card style={{ borderLeft: `4px solid ${D.amber}` }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 600, color: D.heading }}>
            Connect your Bouncie devices
          </div>{" "}
          <div style={{ fontSize: 13, color: D.muted, marginTop: 4 }}>
            Geofence auto-timers need at least one tech mapped to a Bouncie
            IMEI. Add a mapping below to enable.
          </div>{" "}
        </Card>
      )}
      ...
      <Card>
        ...
        <ToggleField
          ...
          help={
            settings.auto_complete_on_exit
              ? " Leaving the property will mark jobs complete. Careful with supply runs and van moves."
                ...
                {editingTech === t.id ? (
                  <>
                    ...
                    <Td>
                      <input
                        value={vehNameInput}
                        onChange={(e) => setVehNameInput(e.target.value)}
                        placeholder="Transit Van #1"
                        style={inputStyle}
                      />
                    </Td>{" "}
                    <Td>
                  ...
                  <>
                    ...
                    <Td style={{ fontFamily: MONO, fontSize: 12 }}>
                      {t.bouncie_imei || (
                        <span style={{ color: D.muted }}>Not set</span>
                      )}
                    </Td>{" "}
                    <Td style={{ fontFamily: MONO, fontSize: 12 }}>
                      {t.bouncie_vin || (
                    ...
                    <Td>
                      <Badge
                        color={t.bouncie_imei ? D.green : D.muted}
                        label={t.bouncie_imei ? "Active" : "Unmapped"}
                      />
                    </Td>{" "}
                    <Td>
                      <button
                        onClick={() => startEditVehicle(t)}
                        style={btnGhost}
                      >
                        Edit
                      </button>
      ...
      <Card>
        ...
        {events.length === 0 ? (
          <div style={{ padding: 24, textAlign: "center", color: D.muted }}>
            No events yet.
          </div>
        ) : (
          <table
            ...
            <tbody>
              {events.map((e) => (
                ...
                >
                  ...
                  <Td>
                    <Badge
                      color={e.event_type === "ENTER" ? D.green : D.red}
                      label={e.event_type}
                    />
                  </Td>{" "}
                  <Td>
                    {[e.customer_first_name, e.customer_last_name]
                      .filter(Boolean)
                      .join(" ") || <span style={{ color: D.muted }}>—</span>}
                  </Td>{" "}
                  <Td>
                    <Badge
                      color={ACTION_COLORS[e.action_taken] || D.muted}
...
function Td({ children, style }) {
  return (
    <td style={{ padding: "10px 8px", color: D.text, ...style }}>{children}</td>
  );
}
...
function NumberField({ label, suffix, value, min, max, step, onChange, help }) {
  return (
    <div style={{ padding: "12px 0", borderBottom: `1px solid ${D.border}` }}>
      ...
      {help && (
        <div style={{ fontSize: 12, color: D.muted, marginTop: 6 }}>{help}</div>
      )}
    </div>
  );
}
...
function ToggleField({ label, checked, onChange, help, warn }) {
  return (
    <div style={{ padding: "14px 0" }}>
      ...
      {help && (
        <div
          style={{ fontSize: 12, color: warn ? D.red : D.muted, marginTop: 6 }}
        >
          {help}
        </div>
      )}
