...
export default function JobFormSection({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    adminFetch(
      `/admin/job-forms/templates/by-service/${encodeURIComponent(serviceType)}`,
    )
      .then((r) => r.json())
      .then((data) => {
        if (cancel) return;
        if (data?.template) {
          const sections =
            typeof data.template.sections === "string"
              ? JSON.parse(data.template.sections)
