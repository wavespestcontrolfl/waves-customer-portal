...
function AnnualPrepayModal({ customer, activeTerm, onClose, onSaved }) {
  const suggestedAmount = Number(customer?.annualValue || 0) > 0
    ? Number(customer.annualValue)
    : Number(customer?.monthlyRate || 0) * 12;
  ...
  const submitDisabled = saving || !(Number(amount) > 0) || !termStart || !termEnd || termEnd <= termStart;
  ...
  return (
    ...
    >
      <div
        className="bg-white w-full max-w-[520px] rounded-sm border-hairline border-zinc-300 my-4"
