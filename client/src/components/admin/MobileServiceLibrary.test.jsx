...
describe("MobileServiceLibrary", () => {
  it.each([false, true])("saves only intentionally changed durations (edited: %s)", async (editDuration) => {
    const service = { id: "fixture-service", name: "Fixture Pest Service", category: "pest_control",
      is_active: true, default_duration_minutes: 30, base_price: "89.00", pricing_type: "variable" };
    vi.stubGlobal("fetch", vi.fn(async () => ({ ok: true, json: async () => ({ services: [service] }) })));
    render(<MobileServiceLibrary initialView="services" />);
    fireEvent.click(await screen.findByRole("button", { name: /Fixture Pest Service/ }));
    ...
    expect(payload).toMatchObject({ name: "Renamed Service", base_price: "89.00" });
