...
export default function PushSettings() {
  ...
  const togglePush = async () => {
    ...
    try {
      if (pushOn) {
        await disablePush({ apiBase: API_BASE });
        setPushOn(false);
        showToast("Push disabled on this device");
      } else {
        await ensurePushSubscription({ apiBase: API_BASE });
        setPushOn(true);
  ...
  const updatePref = (key, field, value) => {
    setPrefs((cur) =>
      cur.map((p) => (p.key === key ? { ...p, [field]: value } : p)),
    );
  };
    ...
    >
      ...
      >
        ...
        <button
          ...
          style={{
            padding: "8px 16px",
            background: pushOn ? D.red : D.green,
            color: D.white,
            border: "none",
            borderRadius: 8,
            fontSize: 13,
      ...
      >
        {" "}
        <div>Event</div> <div style={{ textAlign: "center" }}>Push</div>{" "}
            ...
            {list.map((p, i) => (
              ...
              >
                ...
                <div style={{ display: "flex", justifyContent: "center" }}>
                  ...
                  <Toggle
                    on={p.push_enabled}
                    onChange={(v) => updatePref(p.key, "push_enabled", v)}
                  />{" "}
                </div>{" "}
                <div style={{ display: "flex", justifyContent: "center" }}>
                  {" "}
                  <Toggle
                    on={p.bell_enabled}
                    onChange={(v) => updatePref(p.key, "bell_enabled", v)}
                    color={D.green}
                  />{" "}
                </div>{" "}
                <div style={{ display: "flex", justifyContent: "center" }}>
                  ...
                  <Toggle
                    on={p.sound_enabled}
                    onChange={(v) => updatePref(p.key, "sound_enabled", v)}
                    color={D.amber}
                  />{" "}
                </div>{" "}
              </div>
