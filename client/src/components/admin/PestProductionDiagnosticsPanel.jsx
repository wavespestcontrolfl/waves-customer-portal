...
export default function PestProductionDiagnosticsPanel({ diagnostics }) {
  ...
  const confidence = String(diagnostics.pricingConfidence || diagnostics.confidence || 'high').toLowerCase();
  const confidenceStyle = confidenceMeta(confidence);
  const reasons = [...new Set(diagnostics.reviewReasons || diagnostics.manualReviewReasons || [])];
  const breakdown = Object.entries(diagnostics.breakdown || {})
    .filter(([, value]) => Number(value) !== 0);
  const poolCageSize = diagnostics.poolCageSize || 'none';
  const poolCageSizeInferred = diagnostics.poolCageSizeInferred || reasons.includes('pool_cage_size_inferred');
  const poolCageSource = diagnostics.poolCageSizeSource || (poolCageSize === 'none' ? 'none' : poolCageSizeInferred ? 'inferred' : 'explicit');

  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
        <div style={{ fontSize: 12, fontWeight: 800, color: COLORS.ink, textTransform: 'uppercase', letterSpacing: 0.8 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
          <Metric label="Estimated service time" value={`${diagnostics.estimatedMinutes || 0} min`} strong />
          <Metric label="Pricing confidence" value={confidenceStyle.label} color={confidenceStyle.color} bg={confidenceStyle.bg} />
          <Metric label="Pricing mode" value="Shadow only" />
          <Metric
            label="Pool cage size"
            value={poolCageSize === 'none' ? 'None' : `${titleize(poolCageSize)} (${poolCageSource})`}
        ...
        {breakdown.length > 0 && (
          <div style={{ marginTop: 8 }}>
            <div style={{ fontSize: 11, fontWeight: 800, color: COLORS.muted, textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 6 }}>
              Minute Breakdown
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', columnGap: 16, rowGap: 4, fontSize: 12, color: COLORS.muted }}>
              {breakdown.map(([key, value]) => (
                <div key={key} style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                  <span>{BREAKDOWN_LABELS[key] || titleize(key)}</span>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", color: COLORS.ink }}>{formatBreakdownValue(value)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ marginTop: 12, fontSize: 11, color: COLORS.muted, lineHeight: 1.4 }}>
          These minutes are for calibration and manual review only. They do not drive the customer price yet.
        </div>
      </div>
    </div>
  );
...
function Metric({ label, value, strong = false, color = COLORS.ink, bg = '#FFFFFF' }) {
  return (
    <div style={{
      border: `1px solid ${COLORS.line}`,
      background: bg,
