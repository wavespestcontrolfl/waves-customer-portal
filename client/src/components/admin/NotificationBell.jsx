...
export default function NotificationBell() {
  ...
  const loadList = async () => {
    try {
      const r = await adminFetch("/admin/notifications?limit=25");
  ...
  const enablePush = async () => {
    ...
    try {
      ...
      alert("Push not enabled: " + e.message);
    } finally {
      setEnabling(false);
    }
  };
  ...
  const handleClickItem = async (n) => {
    ...
    if (!isLive) {
      try {
        await adminFetch(`/admin/notifications/${n.id}/read`, {
          method: "PUT",
        });
      } catch {}
    }
  ...
  const markAll = async () => {
    try {
      await adminFetch("/admin/notifications/read-all", { method: "PUT" });
    } finally {
      loadList();
      loadCount();
    }
    ...
    <div ref={panelRef} style={{ position: "relative" }}>
      ...
      <button
        ...
        style={{
          ...
          height: 40,
          borderRadius: 10,
          background: open ? `${D.teal}22` : "transparent",
          border: `1px solid ${open ? D.teal : D.border}`,
          color: D.text,
          cursor: "pointer",
          display: "flex",
          alignItems: "center",
