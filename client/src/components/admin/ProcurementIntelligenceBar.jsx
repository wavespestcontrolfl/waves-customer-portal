  ...
  for (const line of lines) {
    ...
    if (line.match(/^[-•*]\s/)) {
      elements.push(
        ...
        >
          <span style={{ color: D.purple, fontSize: 10, marginTop: 5 }}>●</span>
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: "5px 12px",
        borderRadius: 9999,
        background: hover ? `${D.purple}22` : D.card,
        ...
        setQuickActions([
          {
            id: "unpriced",
            label: "Unpriced",
            prompt: "What products need pricing?",
            icon: "?",
          },
          {
            id: "compare",
            label: "Compare Vendors",
            prompt: "Compare SiteOne vs LESCO on our top products",
            icon: "",
          },
          {
            id: "approvals",
            label: "Approvals",
            prompt: "Any pending price approvals?",
            icon: "Yes",
          },
          {
            id: "margins",
            label: "Margins",
            prompt: "Margin analysis by service type",
            icon: "",
          },
          {
            id: "cheapest",
            label: "Best Deals",
            prompt: "Where are we getting the best deals?",
            icon: "",
          },
          {
            id: "price_check",
            label: "Price Check",
            prompt: "Run a price check on Demand CS",
            icon: "",
          },
        ]);
      });
  ...
  const submit = useCallback(
    async (text) => {
      ...
      try {
        ...
        if (
          (data.toolCalls || []).some((tc) => writeTools.includes(tc.name)) &&
          onRefresh
        ) {
      ...
      >
        ...
        <div style={{ flex: 1, position: "relative" }}>
          ...
          <input
            ...
            onFocusCapture={(e) =>
              (e.target.style.borderColor = D.purple + "66")
            }
        ...
        {stats && !expanded && (
          ...
          >
            {" "}
            <span>
              <strong style={{ color: D.heading }}>{stats.total}</strong>
              products
            </span>
            {stats.needsPrice > 0 && (
              <span>
                <strong style={{ color: D.amber }}>{stats.needsPrice}</strong>
                unpriced
              </span>
            )}
            {stats.pendingApprovals > 0 && (
              <span>
                <strong style={{ color: D.purple }}>
      ...
      {loading && (
        <div style={{ padding: "8px 16px 16px" }}>
          ...
          <div style={{ fontSize: 11, color: "#000", marginTop: 4 }}>
            Price lookups use web search and may take 15-30 seconds...
          </div>{" "}
        </div>
      )}
      {/* Response */}
