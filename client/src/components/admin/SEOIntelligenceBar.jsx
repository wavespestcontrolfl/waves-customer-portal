...
function QuickChip({ icon, label, onClick }) {
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} style={{
      ...
      background: hover ? `${D.teal}22` : D.card, border: `1px solid ${hover ? D.teal + '55' : D.border}`,
...
export default function SEOIntelligenceBar({ context = 'seo', activeDomain, onRefresh }) {
  ...
  }, [prompt, loading, conversationHistory, context, buildPageData]);
      ...
      <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          <div style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)' }}>
            {loading ? (
              <div style={{ padding: '4px 10px', borderRadius: 6, background: `${D.teal}22`, color: D.teal, fontSize: 11, fontWeight: 600, fontFamily: 'JetBrains Mono, monospace', animation: 'pulse 1.5s ease infinite' }}>analyzing...</div>
            ) : (
        ...
        {activeDomain && (
          <div style={{ padding: '4px 10px', borderRadius: 8, background: `${D.teal}15`, border: `1px solid ${D.teal}33`, color: D.teal, fontSize: 11, fontWeight: 600, whiteSpace: 'nowrap' }}>
            {activeDomain}
          </div>
        )}
        {response && (
