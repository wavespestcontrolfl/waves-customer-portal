...
describe('PropertySelectionRevalidator', () => {
  ...
  it('a STALLED re-read is bounded — the shared cycle completes and later refreshes still run (uncapped codex r1p P1)', async () => {
    vi.useFakeTimers();
    try {
      let calls = 0;
      const refresh = vi.fn(() => { calls += 1; return calls === 1 ? new Promise(() => {}) : Promise.resolve(true); });
      render(
        <PortalReadProvider enabled>
          <PropertySelectionRevalidator active refresh={refresh} />
          <Probe />
      ...
      let settled = false;
      const first = portal.refresh().then(() => { settled = true; });
