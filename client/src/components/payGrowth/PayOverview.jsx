...
export default function PayOverview({ view, manage: canManage, onSaved }) {
  ...
  return <>{error && <p role="alert" className="pg-error">{error}</p>}
    ...
    {editor && <BusinessEditor key={editor.initial?.id || 'new-business'} technicianId={view.person.id} month={view.month} initial={editor.initial} onCancel={() => setEditor(null)} onSaved={saved} />}
