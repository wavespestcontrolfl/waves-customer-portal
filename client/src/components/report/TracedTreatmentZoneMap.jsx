      ...
      <div className="service-coverage-map traced-zone-map" ref={mapRef} style={{ position: 'relative' }}>
        ...
        {sprayLive && pathD && (
          ...
          >
            ...
            `}</style>
            {/* wide soft mist band + tighter dense core — both draw in once */}
            <path
            ...
            {puffs.map((p, i) => (
              ...
              >
                <circle cx={Math.round(p.x)} cy={Math.round(p.y)} r="24" fill="#2FA89D" fillOpacity="0.5" />
                <circle cx={Math.round(p.x) + 14} cy={Math.round(p.y) - 10} r="16" fill="#7CD6CB" fillOpacity="0.45" />
                <circle cx={Math.round(p.x) - 13} cy={Math.round(p.y) + 6} r="13" fill="#2FA89D" fillOpacity="0.4" />
              </g>
            ))}
            ...
            <g className="traced-spray-emitter" filter="url(#tracedMistBlur)">
              <circle r="20" fill="#7CD6CB" fillOpacity="0.8">
