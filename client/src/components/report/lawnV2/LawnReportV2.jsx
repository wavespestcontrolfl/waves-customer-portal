  ...
  return (
    <Card style={compact ? { marginBottom: 0, padding: 16 } : undefined}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} role="group" aria-label={`${title} trend`} onMouseLeave={() => setActive(null)} style={{ touchAction: 'pan-y' }}>
        ...
        {pts.map((p, i) => (
          <text key={i} x={x(i)} y={H - 6} textAnchor={i === 0 ? 'start' : i === pts.length - 1 ? 'end' : 'middle'} style={{ fontFamily: FONTS.body, fill: active === i ? TEXT : MUTED, fontWeight: active === i ? 700 : 400 }} fontSize="10.5">{p.label}</text>
        ))}
      </svg>
