...
// `rig=<calibration id>` in place of gallons, for a full tank of that rig
// on its own carrier and volume. Same gate; pure read. Registered BEFORE /:serviceId so the
...
router.get('/job-card/mix', async (req, res, next) => {
  try {
    ...
    if (rig && !UUID_RE.test(rig)) return res.status(400).json({ error: 'rig must be a calibration id' });
    ...
    const mix = await jobCard.mixForProduct(productId, gallons, { serviceId, includePricing: viewerSeesPricing(req), ...(rig ? { calibrationId: rig } : {}) });
