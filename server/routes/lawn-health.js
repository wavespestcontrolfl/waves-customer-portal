...
async function signedUrl(s3Key) {
  ...
  try {
    return await PhotoService.getViewUrl(s3Key, PhotoService.CUSTOMER_DWELL_TTL_SECONDS);
  } catch {
    return null;
  }
}

// Scoring math + lawn-program predicate live in lawn-health-shared.js (one
// copy, shared with the Property Score service).
const {
  lawnStressDamage,
  lawnOverall,
  applyLawnServiceFilter,
  hasCustomerLawnCare,
} = require('../services/lawn-health-shared');
