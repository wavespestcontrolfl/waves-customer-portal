...
async function ensureTable() {
  if (!(await db.schema.hasTable('sms_templates'))) {
    ...
    const templates = [
      ...
      { template_key: 'invoice_sent', name: 'Invoice Sent', category: 'billing', body: 'Hi {first_name}! Your invoice for {service_type} completed on {service_date} is ready: {pay_url} — Waves', variables: JSON.stringify(['first_name','service_type','service_date','pay_url']), sort_order: 10 },
