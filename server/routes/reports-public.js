...
async function buildServiceReportV1ResponseData(service, token, { mode = 'live' } = {}) {
  ...
  // excludes this report). When that's the case, the dynamic-context
  // builders that read service_records.pressure_index must also be
  // suppressed — otherwise pressureTrend, sinceLastVisit.pressureLine,
  // neighborhoodPressure, aiSummary, and premiumExperience would still
  // leak the score outside the new card.
  ...
  if (omitPestPressureContext) {
    // Top-level pressureIndex + the buildMetrics-emitted pressure_index
    // entry are built in report-data.js (separate code path from the
    // dynamic-context builders), so still scrub them here.
    const scrubbedMetrics = Array.isArray(data.metrics)
      ? data.metrics.filter((m) => m && m.key !== 'pressure_index')
      : data.metrics;
    return {
      ...data,
      pressureIndex: null,
      metrics: scrubbedMetrics,
      dynamicContext,
    };
  }
