...
7. For a recognized customer, list what they currently buy and spend per application, then put ONLY requested additions in engineInputs.services. Never reprice, discount, or duplicate an active service: its key establishes the starting tier, but its existing paid price stays unchanged. Apply the combined tier only to the newly quoted services. The returned presentation metadata must match that new-service mix (for example lawn_care + tree_shrub uses the multi-service bundle with both service sections).
