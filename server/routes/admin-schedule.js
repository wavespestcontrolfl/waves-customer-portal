        ...
        switch (action) {
          ...
          case 'reschedule': {
            ...
            if (reminderSyncTime) {
              try {
                const AppointmentReminders = require('../services/appointment-reminders');
                // Suppressed reschedules no longer claim a still-pending
                // creation confirmation (handleReschedule only claims the
                // slot when its notice actually goes out), so a deferred
                // confirmation stays armed and renders the new time.
