  ...
  return {
    ...
    quoteApi: {
      ...
      optionalFields: {
        ...
        lotSizeConfirmed: 'boolean — set true when the customer stated or verified the lot size themselves; a confirmed lot takes precedence over property-record data. REQUIRED for an instant mosquito price on this endpoint: mosquito and oneTimeMosquito price only from a lot the customer confirmed (this endpoint never reads the server property record), so send lotSqFt WITH lotSizeConfirmed:true; an unconfirmed lotSqFt returns a manual-review outcome for them and is not stored as the customer\'s lot.',
      },
