...
// ── GET /fleet-stats — aggregated fleet health overview ──────────────

router.get('/fleet-stats', async (req, res) => {
  try {
    await ensureTable();
    const stats = await wpManager.getFleetStats();
    res.json(stats);
  } catch (err) {
...
// ── POST /sites/:id/quick-scan — scan site for pages, posts, WP version, schema ─

router.post('/sites/:id/quick-scan', async (req, res) => {
  try {
    await ensureTable();
    const result = await wpManager.quickScan(req.params.id);
    res.json(result);
  } catch (err) {
...
// ── POST /quick-scan-all — scan ALL connected sites ─────────────────

router.post('/quick-scan-all', async (req, res) => {
  try {
    await ensureTable();
    res.json({ status: 'scanning', message: 'Scanning all connected sites...' });
    wpManager.quickScanAll().then(results => {
      console.log(`[wp-fleet] Quick scan complete: ${results.length} sites scanned`);
    }).catch(err => {
      console.error(`[wp-fleet] Quick scan failed: ${err.message}`);
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// ── POST /check-uptime — check all sites uptime ────────────────────

router.post('/check-uptime', async (req, res) => {
  try {
    const results = await wpManager.checkAllUptime();
    res.json({ results });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// ── POST /sites/:id/check-llms — check llms.txt on a site ──────────

router.post('/sites/:id/check-llms', async (req, res) => {
  try {
    await ensureTable();
    const result = await wpManager.checkLlmsTxt(req.params.id);
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// ── POST /sites/:id/check-robots — check robots.txt for AI crawlers ─

router.post('/sites/:id/check-robots', async (req, res) => {
  try {
    await ensureTable();
    const result = await wpManager.checkRobotsTxt(req.params.id);
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// ── POST /sites/:id/update-field — update any field on a site ───────

router.post('/sites/:id/update-field', async (req, res) => {
  try {
    await ensureTable();
    const { field, value } = req.body;
    const allowedFields = [
      'content_status', 'hub_type', 'nearest_hub_domain', 'schema_deployed',
      'schema_type', 'llms_txt_deployed', 'ga4_active', 'search_console_verified',
      'gbp_listing', 'tracking_phone', 'schema_phone', 'target_city', 'county',
      'neighborhoods', 'pagespeed_mobile', 'pagespeed_desktop',
    ];
    if (!allowedFields.includes(field)) {
      return res.status(400).json({ error: `Field '${field}' not allowed` });
    }
    await db('wordpress_sites').where({ id: req.params.id }).update({ [field]: value, updated_at: new Date() });
