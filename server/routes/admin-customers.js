...
router.post('/:id/annual-prepay', requireAdmin, async (req, res, next) => {
  try {
    ...
    const tierLabel = customer.waveguard_tier || 'Membership';
    const invoiceNotes = [
    ...
    const invoice = await InvoiceService.create({
      customerId: customer.id,
      title: `WaveGuard ${tierLabel} - Annual Prepay (12 months)`,
      lineItems: [{
        description: 'WaveGuard Membership - 12 months prepaid',
        quantity: 1,
        unit_price: amount,
        category: 'Annual prepay',
      }],
    ...
    await db.transaction(async (trx) => {
      ...
      const term = await AnnualPrepayRenewals.createTermForAnnualPrepay({
        ...
        planLabel: `WaveGuard ${tierLabel} Annual Prepay`,
      ...
      await trx('activity_log').insert({
        ...
        description: `Annual prepay recorded for ${termStart} through ${termEnd}: $${Number(updatedInvoice.total).toFixed(2)} via ${method.replace(/_/g, ' ')}`,
        metadata: JSON.stringify({
          invoice_id: updatedInvoice.id,
