...
async function ensureTable() {
  if (!(await db.schema.hasTable('pricing_config'))) {
    ...
    const configs = [
      ...
      { config_key: 'rodent_trapping', name: 'Rodent Trapping (Standard — flat, 2 included callbacks)', category: 'rodent', sort_order: 6, data: JSON.stringify({ standard_price: 350, base: 350, floor: 350, ceiling_before_custom: 795, included_followups: 2, active_window_days: null, additional_followup_rate: 125, emergency_multiplier: 1.20, emergency_minimum_surcharge: 75, home_size_adjustments: [{max_sqft:1200,adjustment:0},{max_sqft:2500,adjustment:0},{max_sqft:4000,adjustment:0},{max_sqft:6000,adjustment:0},{max_sqft:'Infinity',adjustment:0,custom_recommended:true}], lot_adjustments: [{max_lot_sqft:10000,adjustment:0},{max_lot_sqft:20000,adjustment:0},{max_lot_sqft:43560,adjustment:0},{max_lot_sqft:'Infinity',adjustment:0,custom_recommended:true}], pressure_adjustments: { light:0, normal:0, moderate:0, heavy:0, severe:0 } }) },
