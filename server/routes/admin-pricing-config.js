...
async function ensureTable() {
  if (!(await db.schema.hasTable('pricing_config'))) {
    ...
    const configs = [
      ...
      { config_key: 'pest_features', name: 'Pest Feature Modifiers', category: 'pest', sort_order: 3, data: JSON.stringify({ indoor:15,pool_cage:10,pool_no_cage:5,shrubs_heavy:12,shrubs_moderate:5,trees_heavy:12,trees_moderate:5,landscape_simple:-5,landscape_moderate:0,landscape_complex:5,near_water:5,large_driveway:5 }) },
      ...
      { config_key: 'rodent_trapping', name: 'Rodent Trapping (setup + 2 follow-ups)', category: 'rodent', sort_order: 6, data: JSON.stringify({ base: 395, floor: 350, ceiling_before_custom: 795, included_followups: 2, additional_followup_rate: 95, emergency_multiplier: 1.20, emergency_minimum_surcharge: 75, home_size_adjustments: [{max_sqft:1200,adjustment:-25},{max_sqft:2500,adjustment:0},{max_sqft:4000,adjustment:50},{max_sqft:6000,adjustment:95},{max_sqft:'Infinity',adjustment:150,custom_recommended:true}], lot_adjustments: [{max_lot_sqft:10000,adjustment:0},{max_lot_sqft:20000,adjustment:35},{max_lot_sqft:43560,adjustment:75},{max_lot_sqft:'Infinity',adjustment:125,custom_recommended:true}], pressure_adjustments: { light:-25, normal:0, moderate:35, heavy:75, severe:150 } }) },
      ...
      { config_key: 'rodent_waveguard', name: 'Rodent WaveGuard Rules', category: 'rodent', sort_order: 10, data: JSON.stringify({ tier_qualifier: false, exclude_from_pct_discount: true, setup_credit: 50 }) },
      ...
      { config_key: 'onetime_exclusion', name: 'Exclusion Point Pricing + Access Multipliers', category: 'one_time', sort_order: 8, data: JSON.stringify({ simple: 50, moderate: 95, advanced: 195, specialty_minimum: 275, inspection: 125, inspection_waived_with_service_optin: true, minimums_by_home_sqft: [{max_sqft:1500,minimum:395},{max_sqft:2500,minimum:595},{max_sqft:4000,minimum:895},{max_sqft:'Infinity',minimum:1295,custom_recommended:true}], story_multipliers: { one: 1.00, two: 1.15, three: 1.30 }, roof_multipliers: { shingle: 1.00, flat: 1.00, metal: 1.15, tile: 1.25, steep_or_fragile: 1.35 }, construction_multipliers: { block: 1.00, stucco: 1.05, frame: 1.10, mixed: 1.10 } }) },
