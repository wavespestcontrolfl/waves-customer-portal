...
// Re-service ranking profile (GATE_RESERVICE_RANK_AFTER_NEW, owner ruling
// 2026-09-24: "prefer new customers over existing — new-customer bookings
// get first pick of open time; re-service/callback pickers rank after").
// Values are minutes-equivalent, the same units as compareRankedSlots'
// `score` (detour minutes + 0.5/day-out — scheduling/find-time.js), so they
// combine with it directly.
//
// An empty tech-day (no stops yet) is exactly the room a brand-new customer
// at an unproven address needs — nothing to anchor a route estimate
// against. 240 minutes (4 hours) comfortably exceeds any single day's
// realistic detour/days-out spread, so an empty day only wins when nothing
// packed is feasible anywhere in range.
const RESERVICE_EMPTY_DAY_PENALTY_MINUTES = 240;
// idle_minutes (idleMinutesAgainst) already measures the hole a candidate
// leaves against its neighbours. Weight it below 1:1 against detour so a
// slot that saves a little drive time but opens a big hole still loses to a
// tightly packed one, without letting idle alone override a much
// better-routed candidate.
const RESERVICE_IDLE_WEIGHT = 0.5;
// No re-service SLA is enforced anywhere in code — the only reference is a
// "5-business-day callback protocol" comment (routes/booking.js). Used only
// as this ranking guard's horizon, never as a commitment the engine keeps.
const RESERVICE_LATENCY_BUSINESS_DAYS = 5;

function reserviceAdjustedScore(candidate) {
  const base = candidate.score ?? candidate.rank ?? 999999;
  const emptyDayPenalty = (candidate.stops_that_day || 0) === 0 ? RESERVICE_EMPTY_DAY_PENALTY_MINUTES : 0;
  const idlePenalty = (candidate.idle_minutes || 0) * RESERVICE_IDLE_WEIGHT;
...
async function createSelfBooking(payload = {}) {
    ...
    if (customersOnly && !custId) {
      ...
      if (pricing_estimate_id && verifyEstimateHandoffToken(pricing_estimate_id, estimate_token)) {
        ...
        let gateShapeOk = wizardDraftSelfServeBookable(handoffRow);
        if (!gateShapeOk && handoffRow?.archived_at
            && wizardDraftSelfServeBookable({ ...handoffRow, archived_at: null })) {
          // Consumed-by-booking replay: the first self-booking retired the
          // draft, so an IDENTICAL signed retry (committed but response
          // lost) must still reach createSelfBooking's replay path instead
          // of 403ing at the gate. Bound to the ORIGINAL booking tuple —
          // the consuming booking's own date + start (the same tuple the
          // downstream replay matches) — so a consumed link submitted for
          // a DIFFERENT slot is refused here rather than creating a second
          // appointment (there is no generic lane dedupe for ordinary
          // bookings). A staff-archived draft (no consuming booking) stays
          // dead, and the fee stamp re-checks the un-relaxed predicate
          // under its row lock either way.
          const consumedBy = await db('scheduled_services as ss')
            .join('self_booked_appointments as sba', 'sba.id', 'ss.self_booking_id')
            .where('ss.source_estimate_id', handoffRow.id)
    ...
    if (shouldSeedQuarterlyPestFollowUps && !pestDuplicateKeptAtBooking) {
      try {
        const outcome = await runSeriesTxWithOwnerRetry(db, async (trx) => {
          ...
          // or duplicate-skipped seeding leaves the parent fee-less, so the
          // completion mint can never bill a fee for a plan that was never
          // created. Everything runs in a SAVEPOINT (nested transaction):
          // a statement failure rolls back the stamp alone, never the
          // seeding (a bare try/catch would leave this trx aborted). Under
          // it, FOR UPDATE re-reads serialize with the /calculate in-place
          // draft refresh and with concurrent plan activation, so the
          // FROZEN amount stamped is the fee the committed draft actually
          // discloses and the customer is still not a member.
          // Fee eligibility is decided HERE, independent of the best-effort
          // visit-pricing block (whose catch deliberately lets a booking
          // proceed price-less — a pricing hiccup must not silently drop a
          // disclosed fee): a valid wizard-handoff token is re-verified
...
module.exports._internals = {
  ...
  // Re-service rank profile (GATE_RESERVICE_RANK_AFTER_NEW) primitives,
