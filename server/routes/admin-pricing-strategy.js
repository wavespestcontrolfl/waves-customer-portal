...
router.post('/trigger-upsell/:customerId', async (req, res, next) => {
  try {
    ...
    // At least one marketing consent column must be EXPLICITLY true (the
    // policy's any-of set: seasonal_tips OR marketing_offers) — a NULL
    // (default-seeded or backfilled row — customer never asked) is not
    // captured consent, and a promotions-only opt-in is honored.
