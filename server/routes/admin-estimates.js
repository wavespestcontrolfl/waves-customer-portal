...
const { proposalExpiry, hasFixedBidValidity, assertBidSendDate, assertBidScheduleDate, earliestScheduledDelivery, validateBidFields, FIXED_BID_VALIDITY_ABSENT_SQL } = require('../services/proposal-bid');
      ...
      const scheduleOutcome = await db.transaction(async (trx) => {
        ...
        const claimed = await trx('estimates')
          ...
          .update({
            ...
            expires_at: estimateExpiresAt(() => scheduledTime, lockedRow),
  ...
  try {
    ...
    res.json({
      ...
      estimate: {
        ...
        fixedBidValidity: hasFixedBidValidity(estimate),
...
router.put('/:id/proposal', async (req, res, next) => {
  try {
    ...
    const expiredRecovery = estimate.status === 'expired' && (hasFixedBidValidity(estimate) || expiredRowRecoverableUnderGate(estimate));
