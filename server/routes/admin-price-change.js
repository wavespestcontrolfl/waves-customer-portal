...
router.post('/send', async (req, res) => {
  try {
    ...
    const failNote = result.failed ? ` ⚠️ ${result.failed} FAILED — check logs; re-running is safe (notices are idempotent per batch).` : '';
    res.json({
      ...result,
      message: `${result.created} notices created — ${result.emailed} emailed, ${result.texted} texted${result.unreachable ? `, ${result.unreachable} unreachable (no email or phone)` : ''}.${failNote}`,
    });
  } catch (err) {
    if (err.status) return res.status(err.status).json({ error: err.message });
