...
router.post('/prospects/:id/reconcile-backlink', async (req, res, next) => {
  try {
    ...
    const result = await require('../utils/cron-lock').runExclusive('backlink-scan', () => require('../services/seo/link-prospect-verifier').reconcileOutreach({ ownerMatch: { prospectId: req.params.id, backlinkId: req.body.backlink_id, actorId: req.technician?.id || null } }), { recordHealth: false });
