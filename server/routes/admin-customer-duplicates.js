...
async function handleMerge(req, res, { linkAsProperty }) {
  ...
  try {
    ...
    const conflict = err.mergeConflictCode === 'inactive_primary_property_conflict'
      ...
      || err.mergeConflictCode === 'billing_delivery_channels_conflict'
  ...
  try {
    ...
    if (allJournaledInvoiceIds.length) {
      try {
        ...
        for (const childTable of ['customer_credit_ledger', 'payment_plans', 'invoice_followup_sequences', 'stripe_orphan_charges', 'invoice_attachments']) {
