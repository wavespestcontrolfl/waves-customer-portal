...
const DELETABLE_CHILD_STATUSES = ['pending', 'confirmed'];
...
async function buildPlan(conn) {
  ...
  const cleanupEstimates = estimates;
  ...
  const childRows = estimateIds.length
    ? await conn('scheduled_services')
      .whereIn('source_estimate_id', estimateIds)
      .whereNotNull('recurring_parent_id')
      .whereIn('status', DELETABLE_CHILD_STATUSES)
      .where('scheduled_date', '>', TODAY_ET)
      .where((q) => {
        if (hasColumn(ssColumns, 'is_recurring')) q.where('is_recurring', true);
