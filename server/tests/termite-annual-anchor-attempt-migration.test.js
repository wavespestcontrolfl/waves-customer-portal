/**
 * Real PostgreSQL, real DDL: 20260925000007_termite_annual_anchor_attempt
 ...
 * 20260925000001..000006 migration tests.
 *
 * Self-skips without REPAIR_TEST_DATABASE_URL, e.g.:
 *   REPAIR_TEST_DATABASE_URL=postgresql://waves_user@localhost:5432/waves_test \
 *     npx jest --runInBand server/tests/termite-annual-anchor-attempt-migration.test.js
 */
const knexLib = require('knex');
const { randomUUID } = require('crypto');
const migration = require('../models/migrations/20260925000007_termite_annual_anchor_attempt');
...
async function createScratchDb() {
  ...
  const schema = `termite_anchoratt_${randomUUID().replace(/-/g, '')}`;
...
describeOrSkip('20260925000007_termite_annual_anchor_attempt — real Postgres DDL', () => {
  let fixture;
  ...
  test('up() adds the nullable attempt column', async () => {
    ...
    expect(cols.installation_anchor_attempted_at).toMatchObject({ type: 'timestamp with time zone', nullable: true });
  });
  ...
  test('up() and down() are idempotent; down() removes the column', async () => {
    ...
    expect(await db('annual_prepay_terms').columnInfo()).not.toHaveProperty('installation_anchor_attempted_at');
  });
