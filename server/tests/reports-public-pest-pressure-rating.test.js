...
describe('POST /reports/:token/pest-pressure/client-rating', () => {
  ...
  test('409 when a rating has already been submitted', async () => {
    ...
    await withServer(async (baseUrl) => {
      ...
      expect(runAndSwallowErrors).not.toHaveBeenCalled();
    });
  });
  ...
  test('happy path: writes rating, runs orchestrator, returns updated pestPressure', async () => {
    ...
    await withServer(async (baseUrl) => {
      ...
      expect(runAndSwallowErrors).toHaveBeenCalledWith('svc-1', db);
    });
  });

  test('rounds a fractional rating to the nearest whole number', async () => {
    const initialServiceRow = {
      id: 'svc-1',
      customer_id: 'cust-1',
      service_type: 'Quarterly Pest Control',
    ...
    const initialQuery = chain({ first: jest.fn().mockResolvedValue(initialServiceRow) });
    const updateQuery = chain({ update: jest.fn().mockResolvedValue(1) });
    const reloadQuery = chain({ first: jest.fn().mockResolvedValue({ ...initialServiceRow, client_pest_rating: 3 }) });
    let call = 0;
    db.mockImplementation(() => {
      call += 1;
      if (call === 1) return initialQuery;
    ...
    loadScoreForServiceRecord.mockResolvedValueOnce({
      displayed_score: 2.5, label_name: 'Moderate', trend: 'first_marker',
      data_completeness: 'complete', service_date: '2026-05-17',
    });
    ...
    await withServer(async (baseUrl) => {
      ...
      expect(res.status).toBe(200);
      const body = await res.json();
      expect(body.submittedRating).toBe(3);
      expect(updateQuery.update).toHaveBeenCalledWith(expect.objectContaining({ client_pest_rating: 3 }));
    });
  });
});
