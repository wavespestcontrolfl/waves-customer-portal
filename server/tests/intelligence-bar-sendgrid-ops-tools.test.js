...
describe('intelligence bar SendGrid ops tools', () => {
  ...
  test('get_email_suppressions merges the three lists with truncated reasons', async () => {
    process.env.SENDGRID_API_KEY = 'SG.test';
  ...
  test('check_email_suppression reports the lists an address is on; 404 means not listed', async () => {
    ...
    global.fetch
      .mockResolvedValueOnce(jsonResponse([{ email: 'jane@example.com', created, reason: 'mailbox full', status: '4.2.2' }]))
