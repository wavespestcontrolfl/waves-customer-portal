...
describe('public estimate one-time breakdown', () => {
  ...
  test('server-rendered bundled estimate showcases per-application prices by service', () => {
    ...
    expect((html.match(/Choose Your WaveGuard Payment Option/g) || []).length).toBe(1);
    expect(html).not.toContain('Choose how you want to pay');
    expect(html).toContain('Your custom plan includes 9 lawn care visits + 4 pest control visits for your property, with locked-in pricing for 12 months and a 90-day money-back guarantee.');
    expect(html).toContain('data-pay-pref="card_on_file"');
    ...
    expect(html).toContain('Setup included at no charge');
    expect(html).toContain('Pay in Full &amp; Save $99');
    expect(html).toContain('Included with both options');
    expect(html).toContain('90-day money-back guarantee');
    expect(html).toContain('Text-your-tech access');
