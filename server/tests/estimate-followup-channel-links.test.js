 ...
 *
 * The dual-channel stages used to mint ONE short code tagged
...
function makeBuilder(table, cfg = {}) {
  ...
  for (const m of [
    ...
    'orWhereNull', 'andWhere', 'orWhere', 'whereNotExists',
  ]) {
    b[m] = jest.fn(() => b);
  }
...
// Fixed clock (checkAll reads the real clock, so fake timers pin it).
const NOW = new Date('2026-06-10T15:00:00Z');
const H = 3600000;

// A questions-touch candidate: sent 60h ago (inside the 48-120h window),
// never opened. The unviewed SMS variant renders "price locked until
// {expires_at}", so the fixture carries a live expiry + address.
function unviewedEstimate(overrides = {}) {
  return {
    ...
    sent_at: new Date(NOW.getTime() - 60 * H),
    viewed_at: null,
    expires_at: new Date(NOW.getTime() + 7 * 24 * H),
    created_at: new Date(NOW.getTime() - 61 * H),
    ...overrides,
  };
}
...
describe('checkAll stage wiring — one tracked code per channel leg', () => {
  test('both handles: sms-tagged link rides the SMS template, email-tagged link rides the email payload', async () => {
    enqueue('estimates', { rows: [] }); // expiring touch runs first — no candidates
    enqueue('estimates', { rows: [unviewedEstimate()] }); // questions-touch candidates
...
describe('mintStageLinks — legs the estimate cannot receive are never minted', () => {
  ...
  test('phone-only estimate: no email mint; email slot falls back to the long URL', async () => {
    ...
    expect(shortenOrPassthrough).toHaveBeenCalledWith(
      'https://portal.wavespestcontrol.com/estimate/tok-xyz',
      expect.objectContaining({ channel: 'sms', purpose: 'estimate_followup_expiring' }),
    );
