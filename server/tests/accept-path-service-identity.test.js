...
// The ADOPTED-appointment path (codex #3328 r4 P0). A structurally one-time
// estimate defaults the REQUEST's serviceMode to 'recurring', so resolving
// identity from the raw request values yields the recurring profile
// (pest_control) instead of pre_slab_termiticide — no seeded row matches,
// service_id stays null, and the billing/compliance failure this PR closes
// stays open on that path.
describe('adopted appointments resolve identity in the ACCEPTED mode', () => {
  const { catalogServiceIdForAcceptedEstimate } = require('../services/slot-reservation');

  // Structurally one-time: the accepted service mix is a single one-time line.
  const structurallyOneTimeEstimate = {
    id: 'est-adopt',
    ...
    estimate_data: {
      result: {
        oneTime: {
          specItems: [{ service: 'pre_slab_termiticide', name: 'Pre-Slab Termiticide Treatment', price: 276 }],
          total: 276,
        },
      },
    },
  ...
  const connReturning = (rows) => {
    const builder = () => ({
      whereRaw: () => ({ andWhere: () => ({ limit: () => ({ select: async () => rows }) }) }),
    });
    builder.transaction = async (cb) => cb(builder);
    return builder;
  };

  test("resolves the one-time catalog row when the mode is 'one_time'", async () => {
    const id = await catalogServiceIdForAcceptedEstimate(
      connReturning([{ id: 'svc-preslab' }]),
      structurallyOneTimeEstimate,
      { serviceMode: 'one_time', requireSingleService: true },
    );
    expect(id).toBe('svc-preslab');
  });

  test("the RAW 'recurring' request mode resolves a DIFFERENT (unseeded) identity", async () => {
    // Pins the P0's mechanism: same estimate, wrong mode, no one-time profile
    // to resolve — so nothing is stamped.
    const id = await catalogServiceIdForAcceptedEstimate(
      connReturning([]),
      structurallyOneTimeEstimate,
      { serviceMode: 'recurring', requireSingleService: true },
    );
    expect(id).toBeNull();
  });

  test('the adoption call site passes the ACCEPTED scheduling inputs, not raw request values', () => {
    // Source-level contract: this exact drift is what shipped the P0, and it is
    // invisible to unit tests of the helper itself.
    const fs = require('fs');
    const src = fs.readFileSync(path.join(__dirname, '..', 'routes', 'estimate-public.js'), 'utf8');
    const idx = src.indexOf('catalogServiceIdForAcceptedEstimate(trx');
    expect(idx).toBeGreaterThan(-1);
    const callSite = src.slice(idx, idx + 400);
    expect(callSite).toContain('acceptedEstimateForScheduling');
    expect(callSite).toContain("treatAsOneTime ? 'one_time' : serviceMode");
    expect(callSite).toContain('acceptedSchedulingFrequencyKey');
  });
});
