...
describe('irrigation_week_plans table', () => {
  test('creates once with a unique (customer_id, week_ending)', async () => {
    const cols = [];
    const builder = {
      uuid: (n) => { cols.push(n); return chain; }, date: (n) => { cols.push(n); return chain; },
    ...
    expect(cols).toEqual(expect.arrayContaining(['customer_id', 'week_ending', 'plan_as_of', 'weather_inputs', 'restriction_policy', 'week_plan', 'sent_at']));
