...
describe('safetyGate integration (real guard, mocked db)', () => {
  ...
  test('unconverted customer still passes the gate', async () => {
    // The table-keyed stub isn't filter-aware, so answer customers reads by
    // call order: isLiveCustomer's two live-stage lookups (by id, then the
    // phone/email contact fallback) must MISS before the guard's stage read
    // returns the non-live row.
    let customerReads = 0;
    wireDb({
      invoices: { first: null },
      scheduled_services: { first: null },
      customers: {
        first: () =>
          ++customerReads <= 2 ? null : { pipeline_stage: 'estimate_sent' },
      },
...
describe('claimStage', () => {
  test('re-checks archived_at at claim time (archive landing between the candidate read and the claim blocks the send)', async () => {
    ...
    const won = await _private.claimStage('est-1', 'followup_questions_sent_at');
  ...
  test('winning claim returns true on exactly one affected row', async () => {
    ...
    await expect(
      _private.claimStage('est-1', 'followup_questions_sent_at'),
    ).resolves.toBe(true);
  });
});
