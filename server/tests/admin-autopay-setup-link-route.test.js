...
// delivery defaults to inline (no comm), 'sms' is the only other value, and
...
test('defaults to inline delivery, trigger admin, and echoes the outcome + link', async () => {
  ...
  expect(await res.json()).toEqual({ requested: true, action: 'link_created', reason: 'created', secureUrl: 'https://p/secure/tok', expiresAt: '2026-10-01T00:00:00Z' });
  expect(mockRequest).toHaveBeenCalledWith({ customerId: 'cust-1', delivery: 'inline', trigger: 'admin' });
});

test('sms delivery passes through; anything else collapses to inline', async () => {
  mockRequest.mockResolvedValue({ requested: true, action: 'sent', reason: 'sent', secureUrl: 'https://p/secure/tok' });
  ...
  await fetch(`${base}/customers/cust-1/autopay-setup-link`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ delivery: 'email' }) });
...
test('skips are reported verbatim with null link fields', async () => {
  ...
  const res = await fetch(`${base}/customers/cust-1/autopay-setup-link`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' });
  expect(await res.json()).toEqual({ requested: false, action: 'skipped', reason: 'gate_off', secureUrl: null, expiresAt: null });
});
