...
test.each(['QUIET_HOURS_HOLD', 'PUSH_IN_FLIGHT', 'APP_DELIVERY_HOLD', 'APP_PROVIDER_RETRY', 'BILLING_PREFERENCES_CHANGED', 'SUPPRESSION_LOOKUP_FAILED'])('setup failure deferred by %s queues the same event identity and allows a later event', async (code) => {
