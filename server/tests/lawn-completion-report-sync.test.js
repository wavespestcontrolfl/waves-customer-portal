...
test('the customer payload preserves submitted lawn findings, work, quantities and areas without publishing internal notes as findings', async () => {
  const governedFinding = completionObservationCatalog.lawn[1][1];
  const rows = {
  ...
  const data = await buildReportV1Data({
    ...
    structured_notes: { formObservations: [finding, governedFinding], observations: [finding, governedFinding, 'Internal access instruction'], protocolActionsCompleted: ['Tested irrigation coverage'], techTips: tips },
    service_data: {},
