...
describe('buildCallIntelligence', () => {
  ...
  test('a call with no V2 extraction still renders from the legacy fields without throwing', () => {
    const view = buildCallIntelligence({ call: { ...CALL, ai_extraction_enriched: null, ai_extraction: JSON.stringify({ call_summary: 'legacy summary', first_name: 'Test', quote_promised: true, matched_service: 'General Pest Control' }), transcript_structured: null }, commitments: [] });
    expect(view.summary).toBe('legacy summary');
    expect(view.intent.specific_service_name).toBe('General Pest Control');
    expect(view.prices.quote_promised).toBe(true);
