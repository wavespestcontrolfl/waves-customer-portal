...
const { ESTIMATE_SEND_EXPIRY_DAYS } = require('../services/admin-estimate-persistence');
...
describe('runEstimateExpiration Rule 1 extension carve-out', () => {
  ...
  test('Rule 1 skips only rows whose deadline was EXTENDED beyond the send stamp', () => {
    ...
    // The standard send stamp (expires_at = send + ESTIMATE_SEND_EXPIRY_DAYS,
    // now 10 to match the collapsed follow-up cadence) does NOT suppress the
    // age rule — only a deadline pushed past the send window does.
    const orRaw = nested.find(([n]) => n === 'orWhereRaw');
    expect(orRaw).toBeDefined();
    expect(orRaw[1]).toContain('sent_at');
    expect(orRaw[2]).toEqual([ESTIMATE_SEND_EXPIRY_DAYS]);
  });
