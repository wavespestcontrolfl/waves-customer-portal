...
function makeConn(handler, { hasCardHoldTable = true } = {}) {
  const buildTable = (table) => {
    ...
    for (const m of ['where', 'orWhere', 'whereIn', 'whereNotIn', 'whereBetween', 'whereNull', 'whereNotNull', 'orWhereIn', 'whereNot', 'orderBy', 'count', 'select', 'del', 'update', 'limit', 'leftJoin', 'whereRaw', 'forUpdate']) {
      b[m] = record(m);
    }
    b.first = (...args) => {
