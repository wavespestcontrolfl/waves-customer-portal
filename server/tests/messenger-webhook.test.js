...
describe('messenger webhook channel mapping', () => {
  ...
  test('non-Messenger addresses return null (ignored, never mis-tagged)', () => {
    // Instagram is not a Twilio Messaging channel — handled via a separate
    // integration, not this webhook — so instagram: is ignored here too.
    expect(router.channelForAddress('instagram:17841400000000000')).toBeNull();
    ...
    expect(router.channelForAddress('rcs:+19415551234')).toBeNull();
  });
});
