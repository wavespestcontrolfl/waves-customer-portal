...
describe('reopenAnnualPrepayCoveredInvoicesForTerm', () => {
  ...
  test('inside a caller transaction → invoices reopen but re-arm is deferred (warned)', async () => {
    const prepaidInv = { id: 'i1', status: 'prepaid', prepaid_prev_status: 'sent', payment_recorded_at: null, invoice_number: 'A' };
    const reopenChain = chain({ returning: [1] });
    reopenChain.update = jest.fn(() => Promise.resolve(1));
    const trx = Object.assign(jest.fn(), { isTransaction: true, fn: { now: () => 'NOW' } });
    trx.mockReturnValueOnce(chain({ rows: [prepaidInv] }))
