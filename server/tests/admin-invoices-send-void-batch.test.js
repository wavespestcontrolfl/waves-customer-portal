...
describe('POST / accepted-estimate billing lock and conflict response', () => {
  ...
  test('packet ownership refusal is an operator-visible 409 with a code', async () => {
    InvoiceService.create.mockRejectedValue(Object.assign(new Error('Resume that closeout.'), {
      status: 409, statusCode: 409, isOperational: true, code: 'VISIT_PACKET_OWNS_BILLING',
    }));
    await withServer(async (baseUrl) => {
      const response = await post(baseUrl, '/', createBody('Ordinary manual invoice'));
