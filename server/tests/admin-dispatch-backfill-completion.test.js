...
describe('completion route wiring (source contracts)', () => {
  ...
  test('backfill never auto-applies account credit — invoice stays open for operator review', () => {
    ...
    // r16: the appointment lane's frozen cap also rides INTO the credit
    // apply itself (locked re-check) — the call carries the conditional
  ...
  test('job costing runs on RESUMED retries too — a released mint failure cannot finalize with financials missing (Codex P2, fix round 13)', () => {
    ...
    const activityGuardAt = before.lastIndexOf('if (!resumingCommittedCompletion || firstPacketEffectsRun) {');
  ...
  test('the closeout-requirements freeze is computed from the LOCKED row inside the trx and lands in structuredNotes', () => {
    // Same staleness rule as the tier snapshot: the frozen catalog identity
