...
describe('footprint gate — parity pre-push hardening (mid-fragment conjunctions, object binding)', () => {
  ...
  test('trailing qualifiers on list fragments rejoin; editorial modifiers stay excluded', () => {
    const qualified = guardrails.evaluate({ body: 'We serve Sarasota; Venice and Naples year-round.' }, {});
    expect(qualified.findings.some((f) => f.code === 'OFF_FOOTPRINT_CITY_CLAIM')).toBe(true);
    const factual = guardrails.evaluate({ body: 'We serve Sarasota; Tampa mosquito season starts earlier than ours.' }, {});
    expect(factual.findings.some((f) => f.code === 'OFF_FOOTPRINT_CITY_CLAIM')).toBe(false);
  ...
  test('a claim-object city cannot ride the long disclaimer-list exemption', () => {
  ...
  test('round 4: natural qualifiers, educational bare titles, referential on/for', () => {
    for (const body of [
      'We serve Sarasota; Naples every week.',
      'We serve Sarasota; Venice and Naples year round.',
      'Cape Coral pest control services',
    ]) {
      const r = guardrails.evaluate({ body }, {});
      expect(r.findings.some((f) => f.code === 'OFF_FOOTPRINT_CITY_CLAIM')).toBe(true);
    ...
    for (const body of [
      ...
      'Naples pest control services guide',
      'We provide guidance on pest control services in Naples.',
      'We provide checklists for pest control services in Naples.',
    ]) {
      const r = guardrails.evaluate({ body }, {});
      expect(r.findings.some((f) => f.code === 'OFF_FOOTPRINT_CITY_CLAIM')).toBe(false);
    }
  ...
  test('round 11: bare operate, archival determiners, city-preposition demand spans, singular mouse', () => {
    for (const body of [
      'Our technicians operate in Naples.',
      'Waves techs operate in Tampa.',
      'Our customers in Naples ask about WaveGuard.',
      'Need mouse control in Naples?',
      'Mouse control in Tampa',
    ]) {
      const r = guardrails.evaluate({ body }, {});
      expect(r.findings.some((f) => f.code === 'OFF_FOOTPRINT_CITY_CLAIM')).toBe(true);
    }
    for (const body of [
      'We work across the Naples records to compare seasons.',
      'We operate around the Tampa regulations in this article.',
    ]) {
      const r = guardrails.evaluate({ body }, {});
      expect(r.findings.some((f) => f.code === 'OFF_FOOTPRINT_CITY_CLAIM')).toBe(false);
    }
