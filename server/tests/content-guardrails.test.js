...
describe('angle-bracket link destinations vs raw tags (merge of #3125)', () => {
  const f = guardrails._internals.externalLinkFinding;

  test('a CommonMark angle destination is a link, not a closing tag', () => {
    expect(f('[svc](</pest-control/>)', {})).toBeNull();
    expect(f('[svc](<https://www.wavespestcontrol.com/x>)', {})).toBeNull();
    expect(guardrails.repairInventedInternalRoutes('[svc](</pest-control/>)').body)
      .toBe('[svc](</pest-control-services/>)');
  });

  test('…but the destination position grants no exemption to real tags', () => {
    expect(f('[x](<script src="y">)', {})?.code).toBe('DISALLOWED_EXTERNAL_LINK');
    expect(f('[x](<iframe src="y">)', {})?.code).toBe('DISALLOWED_EXTERNAL_LINK');
    expect(f('<script>x()</script>', {})?.code).toBe('DISALLOWED_EXTERNAL_LINK');
  });
});
...
describe('deterministic repair of invented internal routes (owner ruling 2026-08-01)', () => {
  ...
  test('an absolute hub link stays absolute when aliased (r3: spoke contract)', () => {
    ...
    // Noncanonical hub origins are canonicalized, not preserved — keeping
    // the draft's scheme/port would alias one dead link into another.
    for (const bad of ['http://www.wavespestcontrol.com', 'https://www.wavespestcontrol.com:444']) {
      const r = repairInventedInternalRoutes(`[services](${bad}/pest-control/)`);
      expect(r.body).toBe('[services](https://www.wavespestcontrol.com/pest-control-services/)');
    }
