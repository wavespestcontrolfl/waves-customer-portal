const { inspectEmailReplyMonetaryPricing } = require('../services/email/email-reply-monetary-verifier');

// Preserve every frozen policy assertion while checking the new no-approval contract.
const verifyEmailReplyMonetaryPricing = (input) => {
  const result = inspectEmailReplyMonetaryPricing(input);
...
describe('email reply monetary visit-pricing policy', () => {
  ...
  test.each([
    ...
    '$98 per 30-min visit', '$98 for each 1-hr visit',
  ])('consumes cent amounts and abbreviated visit durations: %s', rejected);

  test.each([
    'Each application costs $98', "This application's price is $98",
    'Each visit costs 98 mins', 'Each visit runs 1 hr',
  ])('preserves application-first prices and abbreviated measurements: %s', (text) => {
...
// Known R5 false negatives are positive evidence, not exemptions.
test.each([
  'Each visit costs no more than $98', 'Each visit costs not less than $98',
  'Each visit has been billed $98', 'Each visit has not been billed $98',
  'For each visit, we charge you $98', 'For each visit, a $98 fee applies',
])('retains R5 pricing findings without authorizing a reply: %s', (text) => {
  expect(inspectEmailReplyMonetaryPricing({ text })).toEqual({
    disposition: 'needs_review', violations: ['customer_copy_compliance'],
  });
});
