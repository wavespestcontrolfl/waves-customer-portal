/**
 * Real PostgreSQL, real DDL: 20260926000101_termite_annual_notice_45_claim_column
 ...
 *     npx jest --runInBand server/tests/termite-annual-notice-45-claim-column-migration.test.js
 */
const knexLib = require('knex');
const { randomUUID } = require('crypto');
const migration = require('../models/migrations/20260926000101_termite_annual_notice_45_claim_column');
...
describeOrSkip('20260926000101_termite_annual_notice_45_claim_column — real Postgres DDL', () => {
  let fixture;

  beforeEach(async () => { fixture = await createScratchDb(); });
  afterEach(async () => { if (fixture) await fixture.destroy(); });

  test('up() adds the nullable timestamptz claim column', async () => {
    const { db } = fixture;
    await migration.up(db);
    const cols = await db('annual_prepay_terms').columnInfo();
    expect(cols.notice_45_claimed_at).toMatchObject({ type: 'timestamp with time zone', nullable: true });

    const [term] = await db('annual_prepay_terms').insert({ notice_45_claimed_at: new Date() }).returning('*');
    expect(term.notice_45_claimed_at).toBeInstanceOf(Date);
  });
  ...
  test('up() is idempotent', async () => {
    ...
    const cols = await db('annual_prepay_terms').columnInfo();
    expect(cols.notice_45_claimed_at).toBeTruthy();
  });
  ...
  test('down() removes the column and is idempotent (reversible)', async () => {
    ...
    const cols = await db('annual_prepay_terms').columnInfo();
    expect(cols).not.toHaveProperty('notice_45_claimed_at');
  });
