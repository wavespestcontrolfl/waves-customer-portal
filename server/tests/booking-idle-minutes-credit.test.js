...
test('BEFORE side: the PRIOR stop\'s own credit already reduces idle time regardless of the candidate\'s own expectedMinutes (unaffected by this fix)', () => {
  ...
  // picks the earlier-ending side for the padding credit).
  const before = [{ startMin: 540, endMin: 600, lat: HERE.lat, lng: HERE.lng, windowMinutes: 60, expectedMinutes: 45 }];
  // Candidate at 11:00-12:00 (660-720): raw gap = 660 - 600 = 60.
  // requiredGap = drive(0) + max(0, 15 - padding(15)) = 0. idle = 60 - 0 = 60.
  ...
  expect(idleWithCandidateCredit).toBe(60);
});
