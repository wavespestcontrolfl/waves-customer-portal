 ...
 * gets 200 (control). The test FAILS on current code if the bug is real.
 *
 * Runs against a private clone only:
 *   DATABASE_URL=postgres://wavespestcontrol@localhost:5432/waves_audit_r1_platform_3
 ...
 * requireTechOrAdmin / requireAdmin are the REAL middleware.
 */
...
if (!/waves_audit_/.test(process.env.DATABASE_URL || '')) {
  throw new Error('r1-platform-3 repro must run against a private waves_audit_* clone (DATABASE_URL)');
}
...
describe('r1-platform-3: /api/admin/ical-history is owner-only', () => {
