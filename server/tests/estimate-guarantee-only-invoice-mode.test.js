...
describe('resolveDepositPolicy — RETIRED (owner ruling 2026-08-10)', () => {
  // The noVisit/slotRequired ladder this block used to pin belonged to the
  // enforcement era. Deposits are permanently not-enforced now — no accept
  // shape (plan customer, renewal, one-time, noVisit) produces a deposit or
  // a deposit-driven booking gate. The renewal accept's commitment is the
  // invoice minted at accept; one-time commitment is the card hold.
  const { resolveDepositPolicy } = require('../services/estimate-deposits');

  test('every shape resolves not-enforced, even with the dead env flag set', () => {
    const prevFlag = process.env.ESTIMATE_DEPOSIT_REQUIRED;
    process.env.ESTIMATE_DEPOSIT_REQUIRED = 'true';
    try {
      for (const args of [
        { estimate: {}, membership: { isExistingCustomer: true }, oneTime: true, noVisit: true },
        { estimate: {}, membership: { isExistingCustomer: true }, oneTime: true },
        { estimate: {}, membership: null, oneTime: true, noVisit: true },
      ]) {
