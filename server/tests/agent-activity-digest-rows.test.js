...
function builder(rows) {
  ...
  for (const m of ['select', 'where', 'orWhere', 'whereNull', 'andWhereRaw', 'whereRaw', 'orderBy', 'limit']) {
    q[m] = jest.fn((...args) => { q._ops.push([m, ...args.map((a) => (typeof a === 'function' ? 'fn' : a))]); return q; });
  }
...
test('pinned rows are loaded without the window, merged with windowed rows, deduped by id', async () => {
  const old = { id: 'old-fix', title: 'FIX: ancient', created_at: '2026-06-01T00:00:00Z', read_at: '2026-06-02T00:00:00Z', metadata: { opsKey: 'k' } };
