...
maybeDescribe('triage auto-resolve sweep (live Postgres)', () => {
  let db;
  let sweep;
  const ids = { customer: null, calls: [], visits: [] };
  ...
  beforeAll(async () => {
    ...
    const [cust] = await db('customers').insert({
      first_name: 'Sweep', last_name: 'Fixture', phone: PHONE, email: 'sweep-fixture@example.invalid',
  ...
  afterAll(async () => {
    ...
    if (ids.customer) await db('customers').where({ id: ids.customer }).del();
  ...
  async function seedCall(sid, { cardAgeMin, bookingAgeMin }) {
    ...
    const [call] = await db('call_log').insert({
      ...
      customer_id: ids.customer, review_status: 'open', created_at: callAt,
    ...
    const [visit] = await db('scheduled_services').insert({
      customer_id: ids.customer, service_type: 'Quarterly Pest Control', status: 'confirmed',
  ...
  test('booking after the card resolves it as auto; booking before the card leaves it open', async () => {
