/**
 * Real PostgreSQL, real DDL: 20260926000104_termite_annual_notice_45_late_column
 ...
 * migration tests (e.g. termite-annual-install-anchor-migration.test.js).
 *
 * Self-skips without REPAIR_TEST_DATABASE_URL, e.g.:
 *   REPAIR_TEST_DATABASE_URL=postgresql://waves_user@localhost:5432/waves_test \
 *     npx jest --runInBand server/tests/termite-annual-notice-45-late-column-migration.test.js
 */
const knexLib = require('knex');
const { randomUUID } = require('crypto');
const migration = require('../models/migrations/20260926000104_termite_annual_notice_45_late_column');
...
async function createScratchDb() {
  ...
  const schema = `termite_notice45late_${randomUUID().replace(/-/g, '')}`;
...
describeOrSkip('20260926000104_termite_annual_notice_45_late_column — real Postgres DDL', () => {
  let fixture;

  beforeEach(async () => { fixture = await createScratchDb(); });
  afterEach(async () => { if (fixture) await fixture.destroy(); });

  test('up() adds the nullable timestamptz late-notice column', async () => {
    const { db } = fixture;
    await migration.up(db);
    const cols = await db('annual_prepay_terms').columnInfo();
    expect(cols.notice_45_late_sent_at).toMatchObject({ type: 'timestamp with time zone', nullable: true });

    const [term] = await db('annual_prepay_terms').insert({ notice_45_late_sent_at: new Date() }).returning('*');
    expect(term.notice_45_late_sent_at).toBeInstanceOf(Date);
  });
  ...
  test('up() is idempotent', async () => {
    ...
    const cols = await db('annual_prepay_terms').columnInfo();
    expect(cols.notice_45_late_sent_at).toBeTruthy();
  });
  ...
  test('down() removes the column and is idempotent (reversible)', async () => {
    ...
    const cols = await db('annual_prepay_terms').columnInfo();
    expect(cols).not.toHaveProperty('notice_45_late_sent_at');
  });
