...
describe('completion attempts', () => {
  ...
  test('a generic intervening release preserves marker-bound definite-rejection proof until a newer rejection replaces it', async () => {
    ...
    // The completion SMS lane rejected definitively; a LATER, unrelated
    // release (e.g. a card-hold failure) must not erase that durable proof
    // — a resume matches its marker against structured_notes and only
    // repeats the send if the marker is gone.
    ...
    // A NEWER definite rejection legitimately replaces the older proof.
    ...
    // An attempt with no prior marker-bound proof still records the
    // ordinary release error verbatim.
    const plainKnex = makeKnex([{ returning: [{ id: 'attempt-1' }] }]);
    await expect(releaseCompletionAttemptForResume(
      { id: 'attempt-1', error: 'earlier plain failure' },
      new Error('transient DB error'),
      plainKnex,
    )).resolves.toBe(true);
    expect(plainKnex.calls[0].op.updatePayload.error).toBe('transient DB error');
  });
  ...
  test('mode fields (backfill/timeOnSite) split the FULL hash but never the CORE segment (Codex P1, fix round 10)', () => {
    ...
    // NORMAL completions post the panel's auto-elapsed timer as a STRING —
    ...
    expect(hashCompletionRequest({ ...base, timeOnSite: '1:01:01' }))
      .toBe(hashCompletionRequest({ ...base, timeOnSite: '1:01:02' }));
    expect(hashCompletionRequest({ ...base, timeOnSite: '1:01:01' })).toBe(flagless);
    // A NUMBER is operator-TYPED minutes — a backfill's or the live admin
    // override's — stable and consequential, so it binds the MODE segment
    // in any mode (the CORE segment still never sees it).
    expect(hashCompletionRequest({ ...base, timeOnSite: 61 }))
      .not.toBe(hashCompletionRequest({ ...base, timeOnSite: 62 }));
  ...
  test('pre-commit same-key retry may NOT flip the mode: failed attempt + flagless retry → idempotency_key_mismatch (Codex P1, fix round 10)', () => {
    ...
    return (async () => {
      ...
      // panel's NEXT timer tick (a string) — that is noise, not a payload
      // change: the failed row resets to pending and the retry proceeds
      // (fix round 13; pre-fix this 409'd idempotency_key_mismatch on the
      // next tick).
      const normalStored = hashCompletionRequest({ notes: 'done', visitOutcome: 'routine', timeOnSite: '1:01:05' });
      ...
      const tickRetry = await claimCompletionAttempt({
        serviceId: 'svc-1',
        idempotencyKey: 'key-1',
        requestHash: hashCompletionRequest({ notes: 'done', visitOutcome: 'routine', timeOnSite: '1:02:08' }),
      }, tickKnex);
