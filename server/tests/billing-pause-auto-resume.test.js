...
describe('off-Stripe payment paths honor the same contract', () => {
  ...
  test('reconcile passes the Stripe charge\'s own settlement time when it has one', () => {
    ...
    // The balance transaction is created at charge SUCCESS — settlement for
    // ACH — and is fetched via expand on the existing retrieve.
    expect(rs).toContain("expand: ['balance_transaction']");
    expect(rs).toContain('chargeDetails?.balance_transaction?.created || chargeDetails?.created');
