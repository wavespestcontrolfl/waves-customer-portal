/**
 * Real PostgreSQL, real DDL: 20260926000106_termite_annual_notice_30_late_and_missed_columns
 ...
 *     npx jest --runInBand server/tests/termite-annual-notice-30-late-and-missed-columns-migration.test.js
 */
const knexLib = require('knex');
const { randomUUID } = require('crypto');
const migration = require('../models/migrations/20260926000106_termite_annual_notice_30_late_and_missed_columns');
...
async function createScratchDb() {
  ...
  const schema = `termite_notice30late_${randomUUID().replace(/-/g, '')}`;
...
describeOrSkip('20260926000106_termite_annual_notice_30_late_and_missed_columns — real Postgres DDL', () => {
  let fixture;

  beforeEach(async () => { fixture = await createScratchDb(); });
  afterEach(async () => { if (fixture) await fixture.destroy(); });

  test('up() adds both nullable timestamptz columns', async () => {
    const { db } = fixture;
    await migration.up(db);
    const cols = await db('annual_prepay_terms').columnInfo();
    expect(cols.notice_30_late_sent_at).toMatchObject({ type: 'timestamp with time zone', nullable: true });
    expect(cols.notice_missed_escalated_at).toMatchObject({ type: 'timestamp with time zone', nullable: true });

    const [term] = await db('annual_prepay_terms').insert({
      notice_30_late_sent_at: new Date(),
      notice_missed_escalated_at: new Date(),
    }).returning('*');
    expect(term.notice_30_late_sent_at).toBeInstanceOf(Date);
    expect(term.notice_missed_escalated_at).toBeInstanceOf(Date);
  });
  ...
  test('up() is idempotent', async () => {
    ...
    const cols = await db('annual_prepay_terms').columnInfo();
    expect(cols.notice_30_late_sent_at).toBeTruthy();
    expect(cols.notice_missed_escalated_at).toBeTruthy();
  });
  ...
  test('down() removes both columns and is idempotent (reversible)', async () => {
    ...
    const cols = await db('annual_prepay_terms').columnInfo();
    expect(cols).not.toHaveProperty('notice_30_late_sent_at');
    expect(cols).not.toHaveProperty('notice_missed_escalated_at');
  });
