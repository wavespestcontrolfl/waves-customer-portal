  ...
  describe('click auto-link (GATE_REVIEW_CLICK_AUTOLINK)', () => {
    ...
    test('correlates on the LIVE row, not the collector payload: a reviewer_name rewritten by a newer runner reaches the matcher and the bell (GH codex r4 P1)', async () => {
      ...
      const matcher = jest.fn(async () => ({
        ...
        rung: 'click_near',
        evidence: 'the nearest click at this location before the review; no other clicker at this location tapped before it in the window',
      }));
      jest.doMock('../services/review-click-correlation', () => ({
        AUTO_LINK_MAX_BEFORE_MS: 12 * 3600 * 1000,
    ...
    test('a click_near match links with link_source click_auto and carries the matcher\'s own evidence into the bell', async () => {
      process.env.GATE_REVIEW_CLICK_AUTOLINK = 'true';
      jest.doMock('../services/review-click-correlation', () => ({
        AUTO_LINK_MAX_BEFORE_MS: 12 * 3600 * 1000,
      ...
      expect(notifs[0].body).toContain('(the nearest click at this location before the review; the next-nearest clicker at this location tapped 1d 15h before)');
      expect(notifs[0].body).not.toContain('other clicks in the window were other names');
    });

    test('one click, one review: a second unlinked review at the SAME location inside the click\'s forward window refuses; alone it links; a review at ANOTHER location never blocks (pre-push P1 r6, GH codex r2 P1)', async () => {
      process.env.GATE_REVIEW_CLICK_AUTOLINK = 'true';
      ...
      // Two unlinked reviews inside the click's forward window at the SAME
      // GBP location: the click cannot say which one the customer wrote, so
      // neither may claim it — whichever synced first would otherwise take
      // the customer. Every match is stamped for the review's own location,
      // so the guard scopes to that GBP.
      ...
      // Alone in its forward window at its location: links.
      ...
      // The guard is scoped to the review's own GBP: another location's
      // review inside the window does not block.
