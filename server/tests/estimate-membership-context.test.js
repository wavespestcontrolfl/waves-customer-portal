...
describe('preservesMonthlyBilling lane stamp (codex P1 on #2978)', () => {
  test('current monthly member (active stage, monthly rate, legacy/monthly lane) stamps true', async () => {
    for (const billingMode of [null, undefined, 'monthly_membership']) {
      const database = fakeDb({
        customer: {
          id: 'cust-1', first_name: 'Don', active: true, waveguard_tier: 'Bronze',
          pipeline_stage: 'active_customer', monthly_rate: 95,
          ...(billingMode !== undefined ? { billing_mode: billingMode } : {}),
        },
        scheduledRows: futurePestRows(),
      });
      const ctx = await computeMembershipContext(database, {
      ...
      expect(ctx.preservesMonthlyBilling).toBe(true);
    }
  });

  test('per-application lane customer stamps false — the estimate card may suppress the monthly note', async () => {
    const database = fakeDb({
      customer: {
        id: 'cust-1', first_name: 'Don', active: true, waveguard_tier: 'Bronze',
        pipeline_stage: 'active_customer', monthly_rate: 95, billing_mode: 'per_application',
      },
      scheduledRows: futurePestRows(),
    });
    const ctx = await computeMembershipContext(database, {
    ...
    expect(ctx.preservesMonthlyBilling).toBe(false);
  });

  test('no monthly rate stamps false regardless of stage', async () => {
    const database = fakeDb({
      customer: {
        id: 'cust-1', first_name: 'Don', active: true, waveguard_tier: 'Bronze',
        pipeline_stage: 'active_customer', monthly_rate: null,
      },
      scheduledRows: futurePestRows(),
    });
    const ctx = await computeMembershipContext(database, {
