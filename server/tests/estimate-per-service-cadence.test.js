...
describe('billedPerApplication vs preserved monthly membership (codex P1 on #2978)', () => {
  // A CURRENT monthly member adding on keeps monthly membership billing at
  // accept (estimate-converter preservesExistingMembership) — for exactly that
  // audience the "Billed $X/mo" note is the truthful disclosure, so the
  // buildPricingBundle wrapper strips the flag when the frozen
  // membershipSnapshot recorded that lane at save time.
  function lawnEstimateRow(membershipSnapshot) {
    return {
      id: `estimate-preserved-member-${membershipSnapshot ? 'snap' : 'lead'}-${JSON.stringify(membershipSnapshot || {}).length}`,
      status: 'sent',
      monthly_total: 55.5,
      annual_total: 666,
      onetime_total: 0,
  ...
  test('lead estimate (no snapshot): flag survives — leads are the per-application audience', async () => {
    const bundle = await buildPricingBundle(lawnEstimateRow(null));
    expect(collectFlags(bundle).length).toBeGreaterThan(0);
  });

  test('snapshot records preservesMonthlyBilling: true → every flag stripped, monthly note stays', async () => {
    const bundle = await buildPricingBundle(lawnEstimateRow({ isExistingCustomer: true, preservesMonthlyBilling: true }));
    expect(collectFlags(bundle)).toEqual([]);
  });

  test('pre-field snapshot (existing customer, lane unrecorded) strips conservatively', async () => {
    const bundle = await buildPricingBundle(lawnEstimateRow({ isExistingCustomer: true }));
    expect(collectFlags(bundle)).toEqual([]);
  });

  test('snapshot records preservesMonthlyBilling: false (per-application lane) → flag survives', async () => {
    const bundle = await buildPricingBundle(lawnEstimateRow({ isExistingCustomer: true, preservesMonthlyBilling: false }));
    expect(collectFlags(bundle).length).toBeGreaterThan(0);
  });
});
