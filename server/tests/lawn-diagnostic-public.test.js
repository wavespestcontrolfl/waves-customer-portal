...
describe('buildPublicLawnReport whitelisting', () => {
  ...
  const GOVERNED_CAUSE = /\b(chinch(?:[\s‐‑‒–—-]*bugs?)?|caterpillars?|army[\s‐‑‒–—-]*worms?|sod[\s‐‑‒–—-]*webworms?|grubs?|large[\s‐‑‒–—-]*patch(?:es)?|brown[\s‐‑‒–—-]*patch(?:es)?|gr[ae]y[\s‐‑‒–—-]*leaf(?:[\s‐‑‒–—-]*spots?)?|dollar[\s‐‑‒–—-]*spots?|fungus|fungi|fungal|leaf[\s‐‑‒–—-]*spots?|mold(?:s|y)?|mildew(?:s|ed|y)?|rhizoctonia|take[‐‑‒–—-]all(?:[\s‐‑‒–—-]*root[\s‐‑‒–—-]*rot)?|take[\s‐‑‒–—-]*all[\s‐‑‒–—-]*root[\s‐‑‒–—-]*rot|insects?|infestations?|droughts?|water[\s‐‑‒–—-]*stress|under[\s‐‑‒–—-]*water(?:ed|ing)?|wilt(?:ed|ing)?|nutsedges?|sedges?|crabgrass|dollarweeds?|clovers?|spurges?|chlorosis|(?:iron|nitrogen|magnesium)[\s‐‑‒–—-]*deficienc(?:y|ies))\b/i;
