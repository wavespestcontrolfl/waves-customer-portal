...
const LARGE_SCOPE_RE = new RegExp(`\\b(both sides of (?:the |my |our )?${PROPERTY_SIZED}|around the (?:house|property)|front and back (?:of (?:the |my |our )?${PROPERTY_SIZED}|yards?)|(?:entire|whole) (?:\\w+ )?${PROPERTY_WIDE}|(?:all (?:of )?(?:my|our|the)|every) (?:\\w+ )?${SCOPE_SUBJECT})\\b`, 'i');
...
// A recurrence phrase needs PAST treatment language, not just a remedy noun
// nearby — "keeps coming back; should I spray it?" asks about a future
// attempt (codex #4810 r16). "didn't work" is itself a past verdict, so it
// keeps the plain subject.
const PAST_TREATMENT = '(?:sprayed|treated|applied|put down|tried (?:everything|\\w+ing)|(?:used|tried) (?:a |the |some )?(?:spray|product|remedy|pesticide|fungicide|insecticide|granules?|fertiliz\\w*|sevin|neem|soap)|(?<!(?:should|can|could|do|would) )(?:we|i|they) (?:spray|treat|use|apply)|(?:last|previous) (?:visit|treatment|application))';
const PRIOR_TREATMENT_RE = new RegExp(`\\b(?:${TREATMENT_SUBJECT}\\b[^.!?]{0,40}\\b(?:didn.?t|did not) work|${PAST_TREATMENT}\\b[^.!?]{0,40}\\b${RECURRENCE}|${RECURRENCE}\\b[^.!?]{0,40}\\b${PAST_TREATMENT}|couldn.?t (?:fix|kill|stop|control|treat|clear) ${FAILED_OBJECT})\\b`, 'i');
