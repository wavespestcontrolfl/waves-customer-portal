...
function ownershipFamiliesFromText(raw) {
  ...
  // "Recurring Termite Foam Service"; legacy rows still say "Recurring Foam
  // Treatment", which carries no termite token). Its OWN family,
  ...
  if (/recurring[\s_-]*(?:termite[\s_-]*)?foam|foam[\s_-]*recurring/.test(s)) add('termite_foam');
  return keys;
}
