...
class SearchConsoleService {
   ...
   */
  async syncDailyData(daysBack = 3) {
    ...
    logger.info(`GSC sync: ${startStr} to ${endStr}`);

    try {
      // 1. Query-level data (all devices)
      await this.syncQueries(startStr, endStr);

      // 2. Page-level data
      await this.syncPages(startStr, endStr);

      // 3. Device breakdown (sitewide)
      await this.syncDeviceBreakdown(startStr, endStr);

      // 4. Sitewide totals
      await this.syncSitewideTotals(startStr, endStr);

      logger.info('GSC sync complete');
      return { synced: true, period: { start: startStr, end: endStr } };
    } catch (err) {
  ...
  async syncQueries(startDate, endDate) {
    ...
    for (const row of rows) {
      ...
      await db('gsc_queries')
        ...
        .onConflict(db.raw('(query, date)'))
        .merge();
    }

    logger.info(`GSC queries synced: ${rows.length} rows`);
  }

  async syncPages(startDate, endDate) {
    const response = await this.webmasters.searchanalytics.query({
      siteUrl: SITE_URL,
      requestBody: {
        startDate,
    ...
    logger.info(`GSC pages synced: ${rows.length} rows`);
  }

  async syncDeviceBreakdown(startDate, endDate) {
    const response = await this.webmasters.searchanalytics.query({
      siteUrl: SITE_URL,
      requestBody: {
        startDate,
  ...
  async syncSitewideTotals(startDate, endDate) {
   ...
   */
  async getPerformanceSummary(periodDays = 28) {
