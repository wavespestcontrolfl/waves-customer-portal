...
class DataForSEO {
  ...
  async getBacklinks(target, limit = 1000, { dofollowOnly = true, offset = 0, searchAfterToken = null } = {}) {
    ...
    // Paging: prefer the result's search_after_token (no ceiling); plain offset
    // is capped by DataForSEO at 20,000 and kept only for small explicit skips.
    if (searchAfterToken) task.search_after_token = searchAfterToken;
    else if (offset) task.offset = offset;
