      ...
      try {
        ...
        await db.transaction(async (trx) => {
          ...
          if (decideState) {
            ...
            // …and the same for ANY active path this pass left unverified
            // (an unstamped write, or a path outside the fetch budget) —
            // zero-confidence included: a route the pass could not read is
            // not proof of absence, so the close waits for the rotated
            // retry instead of standing terminal while the backoff re-spends
            if (verdict.verdict === 'not_reproducible' && (best || unstampedIds.size > 0 || activeLeftUnstamped)) { effectiveVerdict = 'watching'; downgradeNote = best ? 'terminal verdict deferred: an uncovered active path remains' : 'terminal verdict deferred: an active path was left unverified this pass'; tailDeferred = true; }
            ...
            if (verdict.verdict === 'not_reproducible' && effectiveVerdict === 'not_reproducible' && probesRemain) {
              if (probeProgress) {
