...
const CARD_STATUSES = Object.freeze([PARKED, CHECKOUT, CONTACTED, 'negotiating', ...PLACED_STATUSES]);
...
async function listOwnerQueue(db) {
  ...
  const parked = candidates.filter((p) => (p.quality_signals?.outreach_match_ambiguous || (p.status === PARKED ? p.parked_from_status === PARKABLE
