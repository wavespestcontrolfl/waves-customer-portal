...
// Identity is location_key alone: step 2's contract migration re-ran the backfill
// from quality_signals.location, so no row placed during the step-1 rolling
// deploy is still unstamped (the former '-' + legacy-location fallback is gone).
const LOCATION_MATCH_SQL = 'location_key = ?';
async function alreadyPlacedAt(domain, locId) {
  ...
  const row = await db('seo_link_prospects')
    ...
    .whereRaw(LOCATION_MATCH_SQL, [locationKeyOf(locId)])
    .first();
  return !!row;
}
...
async function leaseGuardedReclassify(p, patch) {
  ...
  // This IS a lease release: like worker.report/releaseClaims, the placement
  // settles onto its live path now if the investigator superseded it while
  // this action was in flight (best-effort inside the worker helper).
