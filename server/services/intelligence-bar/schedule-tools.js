...
const SCHEDULE_TOOLS = [
  ...
  },
  {
    ...
    description: `Inspect schedule gaps for a date or date range. When schedule-quality measurements are enabled, reports stored service minutes, modeled driving/waiting, overlaps and unknown capacity; gross gaps or minute budgets never prove an extra visit fits. candidate_service_id tests a specific existing visit against the saved route and known constraints; suggested times still require staff review of commitments/access and a locked save recheck. Supply the operator's workday/break allowance if known; never invent it. include_recurring measures recurring spacing and continuation. Otherwise returns legacy stop-count estimates, which are not bookable capacity.`,
    input_schema: {
      type: 'object',
      properties: {
        ...
        include_recurring: { type: 'boolean', description: 'Include stored intervals and future continuation for recurring plans.' },
        recurring_customer_id: { type: 'string', format: 'uuid', description: 'Optional customer for recurring coverage.' },
        coverage_offset: { type: 'integer', minimum: 0, description: 'Recurring coverage page offset; use hasMore to page the complete book.' },
      },
    },
  },
  {
