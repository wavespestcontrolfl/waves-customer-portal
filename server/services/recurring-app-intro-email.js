 ...
 *   - Recurring members only (is_recurring + waveguard_tier) — mirrors the
 *     welcome-SMS guard in appointment-tagger.
...
async function maybeSendOnEnRoute(svc) {
  try {
    ...
    if (!svc.is_recurring || !svc.waveguard_tier) {
      return { sent: false, skipped: true, reason: 'not_recurring_member' };
    }
