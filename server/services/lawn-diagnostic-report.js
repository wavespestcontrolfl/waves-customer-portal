...
// Every weed SPECIES the CONDITION_LABELS weed entry recognises is here too
// (sedge / nutsedge, crabgrass, dollarweed, clover, spurge): the species is
// governed even though its label ("weed pressure") is not a cause label, so
// a low-confidence finding never lets prose name the species (Codex #4149
// r11 — sedge, clover and spurge were missing).
const SUMMARY_CAUSE_RE = /\b(chinch|large patch(?:es)?|brown patch(?:es)?|gr[ae]y leaf|dollar spots?|rhizoctonia|take[-\s]?all|fungus|fungal|diseases?|leaf spots?|mold|mildew|insects?|pests?|grubs?|caterpillars?|worms?|armyworms?|sod\s?webworms?|nutsedge|sedges?|crabgrass|dollarweed|clover|spurge|drought|water stress|chlorosis|iron deficiency|nitrogen deficiency|magnesium deficiency)\b/i;
...
const CONFIRMABLE_CONDITION = 'chinch(?:\\s*bugs?)?|fungus|fungal|diseases?|large patch(?:es)?|brown patch(?:es)?|gray leaf spots?|grey leaf spots?|leaf spots?|dollar spots?|mold|mildew|drought|water stress|grubs?|army\\s?worms?|sod\\s?webworms?|caterpillars?|worms?|insects?|pests?|infestations?';
