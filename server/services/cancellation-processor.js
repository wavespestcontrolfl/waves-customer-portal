...
async function processCancellationRequest({
  ...
  if (!scoped) try {
    ...
    if (customer) {
      ...
      // A row still in a CUSTOMER stage is live no matter what stamp it
      // carries (customer-stages clears churned_at on reactivation, but a
      // legacy or admin-edited row can keep a stale one) — a live customer
      // cancelling now starts a fresh episode; only the archival non-live
      // stages preserve the old one.
      const liveStage = CUSTOMER_STAGES.includes(customer.pipeline_stage);
      firstChurn = !wasChurnedStage && (liveStage || !customer.churned_at);
