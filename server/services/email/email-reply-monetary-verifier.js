...
const PRICE_UNIT_SRC = `(?:(?:(?:for|on)\\s+)?${VISIT_UNIT_SRC}|for\\s+${VISIT_SRC}|at\\s+(?:each|every)\\s+${VISIT_SRC}|per[\\s-]+${VISIT_SRC}|/\\s*${VISIT_SRC})`;
...
const VISIT_MONETARY_PRICE_RE = new RegExp([
  ...
  `\\b${VISIT_REFERENCE_SRC}\\s+has\\s+(?:a\\s+)?${AMOUNT_SRC}\\s+${PRICE_NOUN_SRC}\\b`,
