...
const PRICE_PREDICATE = `(?:(?:<modal> )?(?:${BILL}(?: at)?|cost|run|incur|generate)|<be> ${BILL}(?: at)?)`;
...
const PRICE_CLAUSES = [
  ...
  `${MONEY}(?: ${NOUN}(?: (?:<be>|(?:<modal> )?apply))?)?(?: (?:<be> )?${BILL})?${TAX}${SEPARATOR} (?:${UNIT}|${VISIT})`,
  `${MONEY} ${VISIT} ${NOUN}`,
  ...
  `${VISIT}(?: <modal>)? has (?:a |an )?${NOMINAL_PRICE}`,
