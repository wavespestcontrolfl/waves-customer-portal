  ...
  async autoSync() {
    ...
    try {
      ...
      const pricingContent = [
        ...
        `Roach: recurring roach percentages are retired. Recurring pest with native roach adds a one-time ${PEST.pestInitialRoach?.display?.regular?.name || 'Cockroach Treatment'} (${roachTierText(PEST.pestInitialRoach?.regular)}); German adds ${PEST.pestInitialRoach?.display?.german?.name || 'German Cockroach Treatment'} (${roachTierText(PEST.pestInitialRoach?.german)}). These first-visit fees are not waived with annual prepay and do not receive the recurring-customer one-time discount. Standalone regular roach uses ${roachTierText(PEST.pestInitialRoach?.regular_standalone)} by footprint | WaveGuard Membership: flat $${PEST.initialFee} (waived with annual prepay)`,
        '',
