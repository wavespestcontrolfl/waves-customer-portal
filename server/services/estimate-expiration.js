...
async function runEstimateExpiration() {
  ...
  const agedResult = await db('estimates')
    ...
    // every successful send writes expires_at = send time +
    // ESTIMATE_SEND_EXPIRY_DAYS (estimateExpiresAt, admin-estimate-
    // persistence.js) — by exceeding sent_at + that send window (+1h clock
    // slack). The whereNull('expires_at') filter above already keeps every
    // stamped expiry out of the age rule; this nested block stays as the
    // #2445 contract and a second fence should that filter ever loosen.
    // Passed expires_at rows age out via Rule 2 regardless.
    .where(function () {
