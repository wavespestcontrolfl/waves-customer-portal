...
<a href="${esc(feedbackReactionUrl(token, r.key))}" class="dm-gold" style="display:block;background:#F5B520;background-image:linear-gradient(135deg,#FFDE78 0%,#F4B014 100%);border:1px solid #FFEEB4;color:#1B2C5B;text-decoration:none;border-radius:10px;padding:12px 8px;font-weight:700;font-size:16px;text-align:center;font-family:Inter,Arial,sans-serif;white-space:nowrap;">${esc(r.label)}</a>
</td>`).join('');
...
<p style="margin:0 0 14px 0;font-size:18px;font-weight:700;color:${C.navy};font-family:Inter,Arial,sans-serif;text-align:center;">${esc(FEEDBACK_QUESTION)}</p>
<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="width:100%;border-collapse:separate;">
<tr>${buttons}</tr>
</table>
<p style="margin:12px 0 0 0;font-size:14px;color:${C.muted};text-align:center;font-family:Inter,Arial,sans-serif;">One tap — it shapes next week's issue.</p>
</div>`;
}
...
function renderFeedbackNeutralHtml() {
  // Chips mirror the live buttons — label only (owner call 2026-07-17:
  // no emojis ON the buttons; the question line and text part keep them).
  const chips = REACTIONS.map((r) =>
    `<span class="dm-chip-lite" style="display:inline-block;margin:4px;padding:9px 14px;background:#ffffff;border:1px solid ${C.rule};border-radius:10px;font-size:14px;color:${C.navy};font-family:Inter,Arial,sans-serif;">${esc(r.label)}</span>`,
  ).join('');
  return `<div class="dm-box" style="margin:24px 0 0 0;padding:20px;background:${C.cardBg};border:1px solid ${C.rule};border-radius:12px;text-align:center;">
<p style="margin:0 0 12px 0;font-size:18px;font-weight:700;color:${C.navy};font-family:Inter,Arial,sans-serif;">${esc(FEEDBACK_QUESTION)}</p>
<div>${chips}</div>
</div>`;
}
