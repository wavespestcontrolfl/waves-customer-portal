...
const REPORT_TRAILING_UNCERTAINTY_RE = new RegExp(
  `^\\s*(?:,\\s*)?(?:${REPORT_COMPLETION_TIME}\\s*,?\\s*)?(?:(?:(?:i\\s+am|we\\s+are|i['’]m|we['’]re)\\s+(?:not\\s+(?:sure|certain)|${vocabAlt(EPISTEMIC_DENIAL_WORDS)}))(?:\\s+(?:of|about)\\s+(?:it|this|that))?|(?:i|we)\\s+${EPISTEMIC_HEDGE_PREFIX_SOURCE}\\s+(?:it|this|that)|maybe|perhaps|possibly|potentially|probably|i\\s+`
