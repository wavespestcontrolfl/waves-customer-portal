...
const CALLBACK_RECIPIENT_END = `(?=\\s*(?:[.!?,;:—–]|$|and\\b|but\\b|so\\b|back\\b|again\\b|directly\\b|personally\\b|anyway\\b|in\\b|today\\b|tomorrow\\b|tonight\\b|later\\b|${CALLBACK_TIMING_ADVERB}\\b|next\\s+(?:week|${WEEKDAYS})\\b|at\\b|on\\b|by\\b|before\\b|after\\b|if\\b|unless\\b|when\\b|once\\b|provided\\b|because\\b|to\\b|about\\b|regarding\\b|with\\b|(?:the|an?|this|that|these|those|some)\\b))`;
...
function no_account_holder_callback(value, record, { spoken }) {
  ...
  const re = new RegExp(
    ...
    + `|${CALLBACK_DELEGATION_FINITE}\\s+${contactFinite})|${recipientFirst})`,
    'gi',
  );
  for (const text of spoken) {
    re.lastIndex = 0;
