...
const REPORT_INSTRUCTION_RE = /(?:^\s*|[,:]\s*)(?:please\s+)?(?:apply|use|put|treat|spray|place|have(?!\s+(?:(?:not\s+(?:only|just|merely|simply)|already|also|just|now|\w+ly)\s+)*(?:had|been|applied|used|sprayed|treated|placed|put|got|received|succeeded|finished|completed|managed|confirmed|verified|checked)\b)|get|(?:(?:\w+ly)\s+)*(?:confirm|verify|check)|tell\s+me|let\s+me\s+know)\b|\b(?:(?:i|we|he|she|they|(?:the\s+)?(?:customer|client|homeowner|caller|technician|tech))(?:\s+(?:need(?:s|ed)?|want(?:s|ed)?|ask(?:s|ed)?|request(?:s|ed)?)|['’]d\s+like|\s+would\s+like|(?:\s+(?:am|is|are|was|were)|['’](?:m|re|s))\s+(?:asking|requesting))\s+(?:me|us|you|him|her|them|(?:(?:the|our|your|their)\s+)?(?:[\w'’-]+\s+){0,3}(?:technician|tech|crew|team)|[\w'’-]+(?:\s+[\w'’-]+){0,2})\s+to\s+(?:(?:\w+ly)\s+)*(?:confirm|verify|check|tell)|(?:i|we|he|she|they|(?:the\s+)?(?:customer|client|homeowner|caller|technician|tech))\s+(?:need(?:s|ed)?|want(?:s|ed)?|request(?:s|ed)?|ask(?:s|ed)?\s+for)\s+(?:(?:a|your)\s+)?(?:confirmation|verification)\s+(?:that|whether|if)|please\s+let\s+me\s+know\s+(?:whether|if|that)|(?:ask(?:s|ed|ing)?|request(?:s|ed|ing)?)\s+that(?=\s+(?:[\w'’-]+\s+){1,6}(?:be|get|have|apply|use|treat|spray|place|put|receive)\b)|(?:(?:am|is|are|was|were)\s+|(?:has|have|had)\s+been\s+)(?:(?:\w+ly)\s+)*(?:asked|requested)\s+to\s+(?:(?:\w+ly)\s+)*(?:confirm|verify|check|tell)|make sure|ensure|remember to|please\s+(?:(?:\w+ly)\s+)*(?:confirm|verify|check|tell))\b/i;
...
function reportFindingIsInstruction(affirmed, subjectAt, locationAt, findingVerb, findingEvidenceEnd) {
  ...
  const subjectAside = /(^|,\s*)(\s*(?:i|we|you|he|she|they|it|(?:(?:the|our|your|their)\s+)?(?:(?:[\w'’-]+(?:\s+and\s+[\w'’-]+)?\s+){0,4})(?:technician|tech|customer|client|homeowner|caller|crew|team))(?:\s+(?:(?:am|is|are|was|were)\s+(?:asked|requested)|(?:has|have|had)\s+been\s+(?:asked|requested)|am|is|are|was|were|do|does|did|has|have|had|will|would|should|can|could|may|might|must|shall))?),\s*[^,]+,\s*/gi;
  const completedAssurance = /\b(?:i|we|you|he|she|they|(?:(?:the|our|your|their)\s+)?(?:technician|tech|customer|client|homeowner|caller|crew|team))\s+did\s+(?:(?:already|also|just|now|\w+ly)\s+)*(?:make\s+sure|ensure)\b/i;
  ...
  for (const comma of commaEvidence.matchAll(/,/g)) {
    ...
    if (new RegExp(`^\\s*(?:[\\w'’-]+\\s+)+${REPORT_FINITE_PREDICATE_RE.source}`, 'i').test(
      clauseRight,
