...
async function sendCancellationReceived({
  ...
  // and the churn episode it was decided in. The end-of-term email is sent
  // once per (term, episode), not per request — a repeat commit on the same
  // decided term after the admin latch's 24h echo window opens a NEW
  // request; a won-back customer churning again is a new episode.
  // priorRequestIds: earlier requests of the same episode for this term —
  // their request-keyed sends (from before the term key shipped) count.
  prepayTermId = null,
  termEpisodeKey = null,
  priorRequestIds = [],
  ...
  if (!idempotencyKey) {
    ...
    if (priorIds.length) {
      try {
        ...
        const priorSend = await acceptedUnderKeys(keys);
        if (priorSend) return { ok: true, deduped: true, priorRequest: true };
      } catch (probeErr) {
        logger.warn(`[account-membership-email] prior-request cancellation-key probe failed for ${request.id}: ${probeErr.message}`);
      }
    }
  }
  return sendTemplate({
