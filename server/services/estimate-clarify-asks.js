...
function unitAskFlags({ callLogId, unitAskBuilding, leadId, customerId, quotePromised, quoteRequested }) {
  return {
    ...
    // Write-back targets + the call's own quote signals, bound to the unit
    // item (a later merged ask for another lead on the same phone cannot
    // re-point them). ALWAYS emitted — null when the producer has none — so
    // a newer unit ask with a deliberately-null customer (ambiguous shared
    // phone) CLEARS a prior ask's target instead of inheriting it (codex r1
    // P1 on #3785).
...
// P1 on #3785). The call's quote signals are recovered the same way.
...
async function applyUnitWriteback(trx, { unitLine, flags, targets }) {
  ...
  if (leadId) {
    ...
    if (leadRow && !leadAddress) {
      ...
      out.lead = 'filled';
    } else if (leadRow && !splitStreetLineUnit(leadAddress).unit && sameBuilding(leadAddress, building)) {
  ...
  if (customerId) {
    if (customerRow) {
      ...
      const atBuilding = (props || []).filter((p) => sameBuilding(customerAddressLine(p), building));
      const unitAlreadyOnFile = atBuilding.some((p) => propUnit(p) && unitLineValueKey(propUnit(p)) === wanted);
      if (ownAddress && sameBuilding(customerAddressLine(customerRow), building)) {
        ...
        if (!String(customerRow.address_line2 || '').trim() && !splitStreetLineUnit(ownAddress).unit) {
...
// The call's engine draft that a unit reply should supersede: unsent, still
// a draft, not archived or invalidated (same predicate the engine's own
// existing-draft lookups use). Taken FOR UPDATE: every caller writes the
// row next (the re-price guard), and the invalidation finalizer's order
// is estimates → leads → call_log — so the reply handler locks this row
// BEFORE its customer/lead rows, never after (codex r4 P1 on #3785).
async function unsentDraftForCall(trx, callLogId) {
...
async function handleClarifyReply({ phone, body }) {
  try {
    ...
    const locked = await withClarifyLock(digits, async (trx) => {
      ...
      if (recorded.includes('unit_number')) {
        ...
        if (unitWritebackEnabled()) {
          if (!cardStamped) {
            ...
            // Re-draft target: the call's live-unsent building-level draft,
            // guarded exactly like the bedroom re-price (the send guard is
            // stamped in THIS locked phase so the stale draft cannot go out
            // first). Looked up — and row-locked — BEFORE the CRM rows
            // below: the invalidation finalizer holds the estimate and then
            // locks its leads, so taking the lead first could deadlock and
            // roll the whole reply back (codex r4 P1 on #3785). Quote
            ...
            if (freshFlags.unit_call_log_id && (targets.quotePromised || targets.quoteRequested)) {
              const existing = await unsentDraftForCall(trx, freshFlags.unit_call_log_id);
              unitRedraft = {
                callLogId: String(freshFlags.unit_call_log_id),
                quotePromised: targets.quotePromised === true,
                supersedeEstimateId: existing?.id ? String(existing.id) : null,
      ...
      // item's target is the call's live-unsent draft; the bedroom item's
      // is its own unit draft. Both are guarded here; when they are the
      // same row one re-run carries both overrides, when they differ the
      // unit re-run supersedes ITS target and the bedroom target is handed
      // to the operator (unscheduled + bell) — never left sendable.
    ...
    const unitSupersede = locked.unitRedraft?.supersedeEstimateId || null;
    const repriceTarget = locked.estimateId || null;
    const repriceReason = unitSupersede && repriceTarget === unitSupersede ? 'clarify_unit_reply' : 'clarify_bedroom_reply';
    if (repriceTarget) {
    ...
    const repricePromise = (async () => {
      ...
      try {
        ...
        if (unitRedraft) {
          ...
          // the service and quote evidence — superseding the unsent
          // building-level draft when one exists (same supersede path and
          // send guard as the bedroom re-price), else drafting fresh. The
          // engine's dedupe guards make the re-entry safe.
          ...
          if (estimatorEngineEnabled()) {
            repriceOutcome = await maybeDraftEstimateForCall({
              ...
              // SAME estimate (codex r2 P1) — a bedroom count bound to another
              // property's draft never reaches this call's re-draft.
              ...(recorded.includes('bedroom_count') && bedroomCount !== null && !locked.extraBedroomTarget
                && locked.estimateId && String(locked.estimateId) === String(unitSupersede || '')
      ...
      // Race with the call's ORIGINAL detached composer (codex r1 P1 on
      // #3785): the reply can land before that run commits, so the locked
      // lookup found no target and this re-run had nothing to supersede —
      // and the engine's call-draft dedupe then blocks it once the original
      // unitless draft inserts. If a live-unsent draft exists NOW and ours
      // did not replace it, guard it and hand it to the operator; never
      // leave a building-level draft sendable.
      if (locked.unitRedraft && !locked.unitRedraft.supersedeEstimateId && repriceOutcome?.created !== true) {
        // Under the SAME phone dedupe lock the engine's creators hold while
        // inserting (codex r2 P1): the original composer either committed
        // before we look (we see and guard its row) or serializes after
        // us — never a window where it inserts unguarded right after an
        // unlocked read.
        const { withAutomatedEstimatePhoneLock } = require('./estimate-automation-duplicates');
        const raceAttempt = require('crypto').randomUUID();
        const runGuard = () => withAutomatedEstimatePhoneLock(`+1${digits}`, async (trx) => {
          const appeared = await unsentDraftForCall(trx, locked.unitRedraft.callLogId);
          if (!appeared?.id) return null;
          const ok = await setEstimateRepricePending(trx, String(appeared.id), new Date().toISOString(), raceAttempt);
          return ok ? String(appeared.id) : null;
        });
        // This transaction is the ONLY thing that makes a late-appearing
        // unitless draft unsendable, so a transient failure must not
        // degrade to "nothing happened" (codex r4 P1 on #3785): retry once,
        // then leave a durable failure marker on the ask and bell the
        // operator so the call's draft is checked before it goes out.
        let guardedId = null;
        let guardError = null;
        for (let attempt = 0; attempt < 2 && !guardedId && !guardError; attempt += 1) {
          try {
            guardedId = await runGuard();
          } catch (err) {
            if (attempt === 0) { logger.warn(`[estimate-clarify] race guard failed, retrying: ${err.message}`); continue; }
            guardError = err;
          }
        }
        if (guardError) {
          logger.error(`[estimate-clarify] race guard failed twice — the call's draft may be sendable: ${guardError.message}`);
          await stampClarifyFlags(digits, awaiting.id, {
            reprice_pending: { call_log_id: locked.unitRedraft.callLogId, unit: unitLine, at: new Date().toISOString(), race: true, guard_failed: true },
          });
          try {
            await require('./notification-service').notifyAdmin(
              'lead',
              'Unit number received — check the call\'s estimate before sending',
              `The customer texted their unit (${unitLine}) while the call's estimate was still being drafted, and the hold on that draft could not be recorded. If a whole-building estimate exists for this call, re-draft it before sending.`,
              { link: '/admin/estimates', metadata: { estimate_clarify: true, reprice_pending: true, guard_failed: true, draftId: awaiting.id, callLogId: locked.unitRedraft.callLogId, unit: unitLine } },
            );
          } catch (bellErr) {
            logger.warn(`[estimate-clarify] race guard-failed bell failed: ${bellErr.message}`);
          }
        }
        const appeared = guardedId ? { id: guardedId } : null;
        if (appeared?.id) {
          const guarded = true;
          if (guarded) {
            await withClarifyLock(digits, (trx) => unscheduleForOperatorReprice(trx, String(appeared.id), raceAttempt))
              .catch((err) => logger.warn(`[estimate-clarify] race unschedule failed: ${err.message}`));
            await stampClarifyFlags(digits, awaiting.id, {
              reprice_pending: { estimate_id: String(appeared.id), unit: unitLine, at: new Date().toISOString(), race: true },
            });
            try {
              await require('./notification-service').notifyAdmin(
                'lead',
                'Unit number received — re-draft the estimate',
                `The customer texted their unit (${unitLine}) while the call's estimate was still being drafted. That draft describes the whole building and is now held — re-draft it before sending.`,
                { link: `/admin/estimates/${appeared.id}`, metadata: { estimate_clarify: true, reprice_pending: true, draftId: awaiting.id, estimateId: String(appeared.id), unit: unitLine } },
              );
            } catch (bellErr) {
              logger.warn(`[estimate-clarify] race bell failed (guard stands): ${bellErr.message}`);
            }
          }
        }
      }
      ...
      if (repriceOutcome?.created === true) {
        ...
        // guard is moot. The ask row records the outcome — and a bedroom
        // item still OPEN on the same ask re-points to the replacement, or
        // its later answer would guard the archived row (codex r2 P1).
        ...
        try {
          await require('./notification-service').notifyAdmin(
            ...
            repriceReason === 'clarify_unit_reply'
              ? `The customer texted their unit (${unitLine}) but the automated re-draft did not produce a replacement. The draft still describes the whole building — re-draft it before sending.`
              : `The customer answered the bedroom question (${bedroomCount === 0 ? 'studio' : `${bedroomCount} bedroom${bedroomCount === 1 ? '' : 's'}`}) but the automated re-draft did not produce a replacement. The draft still carries its fallback price — re-price it before sending.`,
            {
              ...
              metadata: { estimate_clarify: true, reprice_pending: true, draftId: awaiting.id, estimateId: repriceTarget, ...(repriceReason === 'clarify_unit_reply' ? { unit: unitLine } : { bedroomCount }) },
            },
          );
        } catch (bellErr) {
