...
async function getScheduleQualityMeasurements(input = {}, conn = require('../../models/db'), now = new Date()) {
  ...
  if (input.include_recurring === true) {
    result.recurringCoverage = await require('../recurring-schedule-audit').auditRecurringScheduleCoverage({
      customerId: input.recurring_customer_id, offset: input.coverage_offset,
    }, conn);
  }
  return result;
}
