...
const BANNED_RE = new RegExp([
  ...
  `\\b(?:(?:${DURATION_COUNT})[- ]+(?:minutes?|mins?|hours?|hrs?|days?)|(?:one[- ]|a\\s+)?half[- ]?(?:an?[- ])?hour|(?:three[- ])?quarter[- ]hour|\\d+[- ]?(?:min|hr)s?)\\b[^.]{0,30}\\b(?:before|until|till|then|and\\s+then|prior\\s+to|ahead\\s+of)\\b[^.]{0,30}\\b(?:access\\w*|reopen\\w*|resum\\w*|return\\w*|re-?enter\\w*|back|ready|usable|available|open|normal)\\b`,
  `\\b(?:once|after|when|as\\s+soon\\s+as)\\s+(?:(?:${DURATION_COUNT})[- ]+(?:minutes?|mins?|hours?|hrs?|days?)|(?:one[- ]|a\\s+)?half[- ]?(?:an?[- ])?hour|(?:three[- ])?quarter[- ]hour|\\d+[- ]?(?:min|hr)s?)\\s+(?:had\\s+|have\\s+|has\\s+)?(?:passed|elapsed|went\\s+by|gone\\s+by|were\\s+up|was\\s+up|are\\s+up|is\\s+up)\\b`,
  // "<duration> later / afterwards" is a fixed interval in any position (codex r56).
  `\\b(?:(?:${DURATION_COUNT})[- ]+(?:minutes?|mins?|hours?|hrs?|days?)|(?:one[- ]|a\\s+)?half[- ]?(?:an?[- ])?hour|(?:three[- ])?quarter[- ]hour|\\d+[- ]?(?:min|hr)s?)\\s+(?:later|afterwards?|on|thereafter|down\\s+the\\s+(?:road|line))\\b`,
  ...
  `\\b(?:ready|back\\s+to\\s+normal|good\\s+to\\s+go|usable|clear|settled|set|kick(?:ed|s)?\\s+in|t(?:ake|akes|ook)\\s+effect|effective|working|results?|reopen\\w*|resum\\w*|access\\w*|open\\s+again|open|available(?:\\s+again)?|back\\s+in\\s+use|allowed\\s+back|let\\s+back|welcome\\s+back|back\\s+in|back\\s+out|back\\s+on|back\\s+home|back\\s+inside|back\\s+outside|return\\w*|went\\s+back|came\\s+back|come\\s+back|go\\s+back|re-?enter\\w*)\\b[^.]{0,30}\\b(?:after|within|in|inside\\s+of|at\\s+the|by(?:\\s+the)?|no\\s+later\\s+than|before|under|ahead\\s+of|past)\\s+(?:(?:about|around|roughly|approximately|nearly|almost|just\\s+under|just\\s+over|some|maybe|only|barely|under|over|close\\s+to|less\\s+than|more\\s+than|as\\s+little\\s+as|as\\s+few\\s+as|the\\s+first)\\s+)?(?:(?:${DURATION_COUNT})[- ]+(?:minutes?|mins?|hours?|hrs?|days?)|(?:one[- ]|a\\s+)?half[- ]?(?:an?[- ])?hour|(?:three[- ])?quarter[- ]hour|\\d+[- ]?(?:min|hr)s?)\\b`,
  `\\b(?:after|within|in|inside\\s+of|by|no\\s+later\\s+than|before|under|ahead\\s+of|past)\\s+(?:(?:about|around|roughly|approximately|nearly|almost|just\\s+under|just\\s+over|some|maybe|only|barely|under|over|close\\s+to|less\\s+than|more\\s+than|as\\s+little\\s+as|as\\s+few\\s+as|the\\s+first)\\s+)?(?:(?:${DURATION_COUNT})[- ]+(?:minutes?|mins?|hours?|hrs?|days?)|(?:one[- ]|a\\s+)?half[- ]?(?:an?[- ])?hour|(?:three[- ])?quarter[- ]hour|\\d+[- ]?(?:min|hr)s?)\\b[^.]{0,30}\\b(?:ready|back\\s+to\\s+normal|good\\s+to\\s+go|usable|clear|settled|re-?enter\\w*|go\\s+back|be\\s+back|let\\s+\\w+\\s+(?:out|back|in)|return\\w*|reopen\\w*|resum\\w*|access\\w*|open\\s+again|allowed\\s+back|welcome\\s+back)\\b`,
