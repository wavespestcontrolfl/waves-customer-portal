...
const BANNED_RE = new RegExp([
  ...
  '\\b(?:okay|ok|fine|gentle|mild|appropriate|acceptable|friendly|compatible|approved|comfortable|harmless|tolerable|tolerated|recommended|designed|formulated|intended|made|meant|no\\s+(?:problem|issue|worry|worries|concern)s?)(?:\\s+(?:choices?|options?|products?|treatments?|solutions?|picks?|selections?|fits?))?\\s+(?:to\\s+use\\s+|for\\s+use\\s+|to\\s+apply\\s+)?(?:around|for|with|near|on|by|in\\s+homes\\s+with)\\s+(?:your\\s+|the\\s+|our\\s+|all\\s+|small\\s+|young\\s+)?(?:pets?|dogs?|cats?|puppies|kittens|kids?|children|babies|toddlers|infants|newborns|family|families|grandkids|grandchildren|animals|birds|fish|reptiles|plants|garden|people|humans|everyone|pregnan\\w+|elderly|seniors)\\b',
