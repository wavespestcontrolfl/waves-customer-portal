...
const MODE_RULES = {
  no_text: {
    maxWords: 40,
    guidance: 'The reviewer left a rating with no comment. One or two short sentences: thank them for the rating in plain words. Vary the wording; do not use a fixed line about being their team, and do not invent what they liked or where they live.',
  },
  tech_praise: {
    maxWords: 90,
...
// compound fractions ("two and a half", "3 and three quarters" — codex r73),
// qualified dozens ("a couple dozen", "several dozen" — codex r74),
// fractions, "a dozen", and EVERY spelled-out number 1–99 (thirty-five,
// forty-two), not an enumerated few.
const DURATION_COUNT_BASE = '\\d*\\s?[½¼¾⅓⅔⅛⅜⅝⅞]|\\d+\\s+\\d\\s*\\/\\s*\\d|\\d\\s*\\/\\s*\\d|\\d+(?:\\.\\d+)?|a\\s+few|a\\s+couple(?:\\s+of)?|several|(?:a\\s+few|a\\s+couple(?:\\s+of)?|several|a|an|one|half\\s+a|two|three|four|five|six|\\d+)\\s+dozen|dozens\\s+of|(?:one[- ]|a\\s+)?half(?:\\s+of)?\\s+an?|one[- ]half|(?:three[- ])?quarters?\\s+of\\s+an?|(?:\\d+|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)[- ]and[- ](?:(?:an?|one)[- ](?:half|quarter|third)|three[- ](?:quarters|fourths)|two[- ]thirds)|(?:one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety|hundred)(?:[- ](?:one|two|three|four|five|six|seven|eight|nine))?|an?(?:\\s+(?:full|whole|good|solid|mere|quick|short|brief|single))?';
...
const BANNED_RE = new RegExp([
  ...
  '\\b(?:earn|earning|deserv|merit|consider|reconsider|revisit|upgrad|bump|improv|round\\s+up|hop\\w+\\s+for|aim\\w*\\s+for|shoot\\w*\\s+for|work\\w*\\s+(?:toward|towards|for)|go\\w*\\s+for)\\w*\\b[^.]{0,40}\\b(?:stars?|rating|★|five|score|marks?|grade|the\\s+fifth|5\\s*\\/\\s*5|10\\s*\\/\\s*10)\\b',
  // "perfect score / full marks / top marks / a fifth star" (codex r64).
  '\\b(?:perfect|full|top|highest|maximum|max|complete)\\s+(?:score|marks?|grade|rating|review)s?\\b', '\\b(?:the|that|a|one)\\s+(?:fifth|5th|last|missing|extra|final)\\s+star\\b', '\\b(?:5|10)\\s*\\/\\s*(?:5|10)\\b',
  ...
  "\\b(?:won't|will\\s+not|doesn't|does\\s+not|don't|do\\s+not|didn't|did\\s+not|hasn't|has\\s+not|haven't|have\\s+not|hadn't|wasn't|weren't|never|no|not|nothing|none|nobody|neither|nor|cannot|can't|can\\s+not|couldn't|could\\s+not|shouldn't|should\\s+not|wouldn't|would\\s+not|isn't|is\\s+not|aren't|are\\s+not|unable\\s+to|without)(?:\\s+[\\w'-]+){0,3}?\\s+(?:harm|hurt|affect|effect|bother|endanger|poison|sicken|threaten|injur|impact|upset|disturb|caus|lead\\s+to|result\\s+in|trigger|expos|kill|die|lethal|fatal|deadly|damag|distress|irritat|react|compromis|jeopard|undermin|interfer|worr|concern|troubl|faze|inconvenienc|alarm|scar|spook|startl|stress|notic|realiz|realis|mind|blink|flinch|care|aware|knew|know|slow|hold\\s+back|held\\s+back|keep\\s+down|dampen|hinder|hamper|set\\s+back|knock|wear\\s+down|drag\\s+down|weigh\\s+down|throw\\s+off|put\\s+off|deter|discourag|sideline|ground|faze|get\\s+to)\\w*\\b",
  ...
  // "left your pets no worse off", "no worse for it", "not any worse" (codex r68).
  '\\bworse\\s+off\\b', '\\b(?:no|none|not\\s+(?:any|one\\s+bit|a\\s+bit)|never)\\s+(?:the\\s+)?worse\\b', '\\bleft\\s+(?:[\\w-]+\\s+){0,3}?(?:no|none|not)\\s+(?:[\\w-]+\\s+){0,2}?(?:worse|harmed|affected|bothered|fazed|troubled|hurt|different)\\b',
  ...
  '\\bin\\s+(?:their|his|her|its)?\\s*stride\\b', '\\blike\\s+(?:a\\s+)?champ\\w*\\b', '\\blike\\s+(?:a\\s+)?troopers?\\b', '\\bwithout\\s+(?:a\\s+|any\\s+)?(?:fuss|hitch|hiccup|complaints?|issues?|problems?|incident)\\b',
  ...
  `(?<![\\w½¼¾⅓⅔⅛⅜⅝⅞])(?:(?:${DURATION_COUNT})[- ]+(?:seconds?|secs?|minutes?|mins?|hours?|hrs?|days?)|(?:one[- ]|a\\s+)?half[- ]?(?:an?[- ])?hour|(?:three[- ])?quarter[- ]hour|\\d+[- ]?(?:sec|min|hr)s?)\\b[^.]{0,30}\\b(?:before|until|till|then|and\\s+then|prior\\s+to|ahead\\s+of)\\b[^.]{0,30}\\b(?:access\\w*|reopen\\w*|resum\\w*|return\\w*|re-?enter\\w*|back|ready|usable|available|open|normal)\\b`,
  `\\b(?:once|after|when|as\\s+soon\\s+as)\\s+(?:(?:${DURATION_COUNT})[- ]+(?:seconds?|secs?|minutes?|mins?|hours?|hrs?|days?)|(?:one[- ]|a\\s+)?half[- ]?(?:an?[- ])?hour|(?:three[- ])?quarter[- ]hour|\\d+[- ]?(?:sec|min|hr)s?)\\s+(?:had\\s+|have\\s+|has\\s+)?(?:passed|elapsed|went\\s+by|gone\\s+by|were\\s+up|was\\s+up|are\\s+up|is\\s+up)\\b`,
  // "<duration> later / afterwards" is a fixed interval in any position (codex r56).
  `(?<![\\w½¼¾⅓⅔⅛⅜⅝⅞])(?:(?:${DURATION_COUNT})[- ]+(?:seconds?|secs?|minutes?|mins?|hours?|hrs?|days?)|(?:one[- ]|a\\s+)?half[- ]?(?:an?[- ])?hour|(?:three[- ])?quarter[- ]hour|\\d+[- ]?(?:sec|min|hr)s?)\\s+(?:later|afterwards?|on|thereafter|down\\s+the\\s+(?:road|line))\\b`,
  ...
  // Promise-shaped permanence claims (codex r68): "we promise the ants will
  // not return", "will stay gone permanently", "gone for good".
  '\\bpromis\\w*\\b', '\\bassur\\w*\\b', '\\bpermanent\\w*\\b', '\\bfor\\s+good\\b', '\\bforever\\b', '\\bonce\\s+and\\s+for\\s+all\\b', '\\bfrom\\s+now\\s+on\\b', '\\bfor\\s+the\\s+long\\s+haul\\b', '\\bnever\\s+(?:again|to\\s+return|come\\s+back|coming\\s+back|return)\\b',
  '\\b(?:will|would|shall|.ll|going\\s+to|gonna)\\s+(?:not\\s+|never\\s+)?(?:stay|remain|keep|be)\\s+(?:gone|away|out|clear|off|free|pest[- ]free|bug[- ]free|ant[- ]free)\\b', '\\b(?:will|shall|.ll)\\s+(?:not|never)\\s+(?:return|come\\s+back|be\\s+back|reappear|resurface|recur|see|have|bother|trouble)\\b', '\\bwon.t\\s+(?:return|come\\s+back|be\\s+back|reappear|resurface|recur|bother|trouble|see\\s+(?:them|those|any|another))\\b', '\\b(?:stays?|remains?|kept|keeps?)\\s+(?:them\\s+|those\\s+|the\\s+\\w+\\s+)?(?:gone|away|out|clear|off)\\s+(?:for\\s+good|forever|permanently|indefinitely|from\\s+now\\s+on|for\\s+the\\s+long\\s+haul)\\b', '\\b(?:no|zero)\\s+(?:more|further|future)\\s+(?:[\\w-]+\\s+){0,2}?(?:ever|again|from\\s+now\\s+on)\\b', '\\b(?:you|they|it)\\s+(?:will|.ll)\\s+never\\b',
  ...
  '\\b(?:wait|stay\\s+off|keep\\s+off|stay\\s+out|keep\\s+out|avoid)\\b[^.]{0,30}\\b(?:seconds?|secs?|minutes?|mins?|hours?|hrs?|days?)\\b',
  ...
  '\\b(?:seconds?|secs?|minutes?|mins?|hours?|hrs?|days?)\\b[^.]{0,30}\\b(?:waits?|waiting|wait\\s+time|downtime|down\\s+time|delay|hold[- ]?off|keep[- ]?out|stay[- ]?out|keep[- ]?off|stay[- ]?off|off\\s+the\\s+(?:lawn|grass|yard|turf|floors?)|out\\s+of\\s+the\\s+(?:house|home|yard|rooms?|area)|away\\s+from|before\\s+(?:re-?entering|going\\s+back|letting|coming\\s+back))\\b',
...
const OUTCOME_TERM_RE = /^(?:behind (?:you|us|them)|(?:a )?thing of the past|in the past|in the rearview|(?:a )?distant memory|history|over and done|solved?|resolv\w*|clear(?:ed)? up|took care of|take care of|taken care of|dealt with|deal with|fix(?:ed|ing)?|sorted|got rid of|get rid of|wiped out|knocked out|under control|no more|gone|eliminat\w*|exterminat\w*|eradicat\w*|protect\w*|remov(?:ed|al|ing)?|controlled)$/i;
...
const EXPERIENCE_CLAIM_RE = /\b(?:stop(?:ped|s)? by|came out|come out|coming out|came by|dropped by|swung by|on[- ]site|was there|were there|made it out|got out to|sent (?:someone|a tech\w*|the tech\w*|our tech\w*)|left (?:everything|it|things|the (?:place|house|home|yard)|no mess)|as (?:we|they) found (?:it|them)|put (?:everything|things|it) back|cleaned up|tidied|booties|shoe covers|no mess|spotless|honest|efficient(?:ly)?|reliable|dependable|responsive|diligent|hard-?working|trustworthy|affordable|fair|reasonable|excellent|outstanding|amazing|wonderful|fantastic|great|awesome|superb|effective(?:ly)?|spotless|tidy|neat|on[- ]time|arrived|arrival|showed up|show up|quick(?:ly)?|fast|prompt(?:ly)?|same[- ]day|next[- ]day|right away|punctual|early|kept (?:you|them) (?:informed|updated|posted)|respectful|clean(?:ed)? up)\b/gi;
...
function verifyReplyDetailed(text, grounding, { recentReplies = [], mode } = {}) {
  ...
  const ROLE_FOLLOWERS = new Set(['wants', 'want', 'wanted', 'hopes', 'hope', 'hoped', 'asked', 'asks', 'ask', 'apologizes', 'apologises', 'apologized', 'apologised', 'personally', 'himself', 'herself', 'themselves', 'needs', 'wishes', 'cares', 'looks', 'looked', 'promised', 'promises', 'likes', 'loves', 'stands', 'believes', 'knows', 'thinks', 'feels', 'sends', 'offered', 'offers', 'invites', 'welcomes', 'reads', 'read', 'reached', 'reaches', 'spoke', 'speaks', 'saw', 'sees', 'called', 'calls', 'met', 'meets', 'goes', 'tries', 'tried', 'agrees', 'agreed', 'would', 'is', 'was', 'did', 'who', 'that', 'will', 'has', 'had', 'came', 'and', 'or', 'from', 'at', 'on', 'to', 'for', 'in', 'of', 'the', 'a', 'an', 'we', 'you', 'your', 'our', 'they', 'he', 'she', 'it', 'got', 'took', 'made', 'handled', 'said', 'can', 'could', 'would', 'should', 'does', 'do', 'went', 'gets', 'get', 'visit', 'visits', 'visited', 'team', 'today', 'again', 'also', 'even', 'always', 'never', 'really', 'very', 'just', 'still', 'back', 'out', 'over', 'there', 'here', 'when', 'while', 'after', 'before', 'since', 'so', 'as', 'but', 'if', 'then', 'with', 'about', 'himself', 'herself']);
  let rn;
  ...
  // Non-repetition against the location's recent posted replies. The
...
async function draftReviewReply({ grounding, recentReplies = [] }) {
  ...
  for (const step of ladder) {
    ...
    if (!res.ok) {
      // Providers down. The runner retries on a backoff for a tailored
      // reply; once its retries are exhausted it parks the row as
      // provider_down rather than posting a template — owner ruling
      // 2026-09-24: a template reply is worse than a parked row. This
      // supersedes the 2026-09-03 "never parks for want of copy" directive.
  ...
  // Every ladder attempt was rejected: park the row for a human rather than
  // post a canned template — owner ruling 2026-09-24 (a template reply is
  // worse than a parked row; supersedes the 2026-09-03 "never parks for want
  // of copy" directive that used to fall back to a deterministic safe copy
  // here).
