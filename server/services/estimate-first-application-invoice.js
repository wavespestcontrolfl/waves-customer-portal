...
async function findFirstApplicationInvoiceForEstimateService(svc, conn = db) {
  ...
  // non-refunded vocabulary and keep scanning; a refunded match still wins
  // its position (newest matching row semantics otherwise unchanged).
  const InvoiceService = require('./invoice');
  const skipStatuses = InvoiceService.CANCELLED_SERVICE_RESOLVED_STATUSES.filter((s) => s !== 'refunded');
  return (rows || []).find((row) => isAutoGeneratedPayPerApplicationInvoice(row)
    && !skipStatuses.includes(row.status)) || null;
}
