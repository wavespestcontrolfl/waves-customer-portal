...
// Pest Control" or any sign-off). The prompt forbids it; this strips a
// trailing sign-off the model adds anyway. Only a TRAILING segment set off
// by a dash or its own line counts, so "Waves Pest Control here" mid-message
// and a sentence ending "...choosing Waves Pest Control" stay intact.
  ...
  switch (toolName) {
    ...
    case 'send_lead_response': {
      ...
      const result = await sendCustomerMessage({
        ...
        withSmsHandoff: dispatch => withSmsConsentLock(db, { phone: customer.phone, customerId: context.customerId }, async trx => {
      ...
      //
      //   Retryable authority/database blocks are infrastructure failures too.
      ...
      if (result.blocked) {
        return {
          ...
          ...(result.retryable ? { retryable: true, failed: true, error: result.reason || result.code || 'SMS authority unavailable' } : {}),
          name: customer.first_name,
        };
      }
      return {
...
module.exports = { executeLeadTool, recordLeadAutoReplyDelivered, stripTrailingSignature };
