...
// email") stay licensed.
const CONDITIONAL_CHANGE_RE = /\b(?:if|unless|in\s+case|assuming|once|should)\s+(?:i|we|they|he|she|it|the|my|that|things?|everything)\b|\b(?:if|unless|in\s+case|assuming|once)\s+[\p{L}][^,;.!?\n]{0,40}(?:,|\bthen\b)|\b(?:after|when|whenever|as\s+soon\s+as)\s+(?:i|we|they|he|she|it|my|our|the\s+(?:sale|closing|wedding|move|divorce|paperwork|account|transfer)|that|things?|everything)\b(?!\s+(?:signed|called|texted|emailed|messaged|spoke|talked|moved|booked|scheduled|was|were|had|did|got|first)\b)|\b(?:if|unless|once|assuming)\s+(?:it\s+is\s+|it'?s\s+|all\s+is\s+|everything\s+is\s+)?(?:approved|accepted|confirmed|completed|finali[sz]ed|done|signed|processed|paid|settled|closed|granted)\b/iu;
...
async function applyContactCorrections({ customerId, corrections, source, sourceId = null, knex = db, postApply = null, moveContext = false, expectedValues = null, senderPhone = null }) {
  ...
  try {
    ...
    let line2WasStated = byField.has('address_line2') && byField.get('address_line2').newValue !== '';
    ...
    if (stagedAddressFields.length) {
      ...
      if (parsed.unitConflict || (inlineSplit.unit && explicitUnitClear)) {
        ...
        if (inlineSplit.unit) {
          ...
          // A unit promoted OUT of the street line was stated by the
          // customer just as surely as a dedicated line2 (codex #3413
          // r58): if validation later drops it, the whole group must
          // fail closed rather than commit the street and clear the old
          // unit.
          line2WasStated = true;
        }
    ...
    await knex.transaction(async (trx) => {
      ...
      if (addressChanged) {
        ...
        addressApplied = true;
        geoComponentApplied = geoComponentApplied
          || ['address_line1', 'city', 'state', 'zip'].some((f) => updates[f] !== undefined);
      }
    ...
    // Unit-only corrections preserve coordinates in-transaction (the
    // building did not move) — re-geocoding would spend a provider call to
    // rewrite an unchanged point (codex #3413 r59), so the continuation
    // runs only when a GEOCODED component (street/city/state/zip) changed.
    if (geoComponentApplied) {
