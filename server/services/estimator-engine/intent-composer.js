...
- flea: flea/tick treatment — a ONE-TIME engagement (single knockdown visit or two-visit elimination package), not a recurring program. fleaComplexity REQUIRED (light = recent/localized activity | moderate = established activity across rooms or with pets untreated | heavy = heavy/longstanding infestation) — if the call did not establish how bad it is, skip with reason. Options: offerKey (flea_knockdown_single ONLY when the caller explicitly wants a single visit with no follow-up; omit for the standard two-visit elimination), fleaExterior (true when the caller describes a yard/outdoor flea source or wants the exterior treated; omit otherwise).
