...
class LinkedInService {
  ...
  async _uploadImageFromUrl(imageUrl, token) {
    ...
    // ALWAYS re-encode to a clean JPEG before the PUT, for two reasons:
    // (1) LinkedIn's Images API accepts JPG/PNG/GIF only, and the admin
    // BlogPage share passes the published .webp hero URL straight through —
    // unconverted bytes get rejected and the post silently ships without its
    // thumbnail. (2) Even accepted formats must not PUT the fetched bytes
    // unchanged: the re-encode drops every metadata block (EXIF, XMP, and
    // C2PA/AI-provenance segments), matching the uploadImageToS3 hygiene the
    // other platforms get, and .rotate() bakes EXIF orientation into pixels
    // first so nothing publishes sideways. A conversion failure throws —
    // createPost then posts without the thumbnail.
    {
