...
function annualPlanOfferFingerprint(estimate) {
  ...
  const scope = ['waveguard_tier', 'service_interest', 'category', 'source'].map((key) => canonicalRow[key]);
  return crypto.createHash('sha256').update(JSON.stringify([estimateOfferVersion(canonicalRow), scope])).digest('hex');
}
