...
const SocialMediaService = {
  ...
  async publishToAll({ title, description, link, guid, source, imageUrl, customContent }) {
    ...
    // Instagram is the only platform here that uses generatedImageUrl,
    // and it needs both a Facebook token (Graph API) and S3 config
    // (Instagram requires a public URL, so generated bytes must be
    // uploaded first). Without these, generating would spend image
