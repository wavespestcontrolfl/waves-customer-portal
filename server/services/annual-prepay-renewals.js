    ...
    if (nextStatus === 'active' && term.status === PAYMENT_PENDING_STATUS) {
      ...
      if (updated && updated.status === 'cancelled') {
        ...
        if (updated.prepay_invoice_id) {
          ...
          try {
            await require('./invoice').restoreRetiredSetupFeeClaimForPrepay(updated.prepay_invoice_id, conn, { sourceEstimateId: updated.source_estimate_id || null });
          } catch (err) {
  ...
  if (nextStatus === 'cancelled') {
    ...
    for (const decided of decidedCoveredTerms) {
      ...
      if (decided.prepay_invoice_id) {
        ...
        try {
          await require('./invoice').restoreRetiredSetupFeeClaimForPrepay(decided.prepay_invoice_id, conn, { sourceEstimateId: decided.source_estimate_id || null });
        } catch (err) {
