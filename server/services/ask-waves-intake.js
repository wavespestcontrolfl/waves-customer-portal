...
const EAT_EXPOSURE_RE = new RegExp(`\\b${EXPOSURE_SUBJECT}\\s+${NON_PEST_GAP}(?:ate|eaten|eating|drank|drunk|drinking|chewed|chewing|licked|licking|(?:se\\s+)?comi[oó]|(?:se\\s+)?bebi[oó]|mastic[oó]|lami[oó]|consum(?:ed|ing|es)|tast(?:ed|ing)|consumi[oó]|prob[oó]|inhal\\w*|breathed(?:\\s+in)?|breathing(?:\\s+in)?|sniffed|(?:was|were|got|been|is)\\s+exposed\\s+to|exposed\\s+to|touched|splashed|(?:was|were|got|been|gets)\\s+sprayed(?:\\s+with)?|expuest[oa]s?\\s+a|inhal[oó]|respir[oó])(?![a-zñáéíóú])[^.?!]{0,30}?\\b${PRODUCT_NOUN}(?![a-zñáéíóú])|\\b${PRODUCT_NOUN}\\b[^.?!]{0,30}?\\b(?:eaten|drunk|chewed|licked|consumed|tasted|inhaled|breathed\\s+in|sniffed|comid[oa]s?|bebid[oa]s?|inhalad[oa]s?)\\s+(?:by|por)\\s+(?:(?:my|our|the|his|her|their|mi|su|el|la)\\s+)?${EXPOSURE_SUBJECT}\\b`, 'i');
...
const CONTACT_EXPOSURE_RE = new RegExp(`\\b${PRODUCT_NOUN}(?![a-zñáéíóú])[^.?!]{0,40}?\\b(?:in|on|into|en)\\s+(?:(?:his|her|their|my|our|your)(?:\\s+(?:child|son|daughter|kid|baby|toddler|husband|wife|dog|cat|puppy|kitten|pet|bird|parrot|rabbit)'?s)?|the\\s+(?:dog|cat|puppy|kitten|baby|child|kid|toddler|bird|parrot|pet|rabbit)'?s|sus|mis|su|tus)\\s+(?:eyes?|skin|face|mouth|ojos?|piel|cara|boca)(?![a-zñáéíóú])|\\b(?:hij[oa]s?|beb[eé]s?|ni[ñn][oa]s?|perr[oa]s?|gat[oa]s?|mascotas?|esposo|esposa|yo|me|le)\\b[^.?!]{0,40}?\\b${PRODUCT_NOUN}(?![a-zñáéíóú])[^.?!]{0,40}?\\ben\\s+(?:los|la)\\s+(?:ojos?|piel|cara|boca)(?![a-zñáéíóú])`, 'i');
...
const HUMAN_EMERGENCY_DIRECTION_RE = /\b(?:ambulances?|paramedics?|EMS|ambulancias?|param[eé]dicos?)\b|(?:\+?1[-.\s]?)?\(?800\)?[-.\s]?222[-.\s]?1222|\b(?:(?:go|get|head|rush|drive|take|bring|carry)\s+(?:\S+\s+){0,3}?(?:straight\s+|right\s+)?to\s+(?:the\s+|a\s+|an\s+)?(?:nearest\s+|closest\s+|local\s+)?(?:hospital|emergency\s+room)|(?:vaya|vayan|lleve|llével[oa]|lleven|acuda|acudan|corra)\s+(?:\S+\s+){0,3}?(?:al|a\s+un|a\s+la)\s+(?:hospital|sala\s+de\s+emergencias|urgencias)|(?:go|get|head|take\s+\S+)\s+to\s+(?:the\s+)?(?:er|e\.r\.)|urgencias|call(?:ing)?\s+911|dial\s+911|911\s+(?:right\s+away|immediately|now)|(?<!(?:animal|pet)\s)poison\s+(?:control|help)|emergency\s+(?:room|care|services?|department)|urgent\s+care|seek\s+(?:immediate\s+|urgent\s+)?(?:medical|emergency)(?!\s+(?:veterinary|vet|animal))|medical\s+(?:attention|care|help|emergency)|centro\s+de\s+(?:toxicolog[ií]a|envenenamientos?)|control\s+de\s+(?:envenenamientos?|intoxicaciones)|sala\s+de\s+emergencias?|atenci[oó]n\s+m[eé]dica|llam[ea]\s+al\s+911)\b/i;
...
const NEGATED_CARE_RE = /\b(?:does\s+not|doesn'?t|do\s+not|don'?t|won'?t|will\s+not|no|not|never|without|nor)\s+(?:\w+\s+){0,2}?(?:medical\s+(?:attention|care|help|treatment|emergency)|emergency\s+(?:room|care|services?|treatment)|urgent\s+care|(?:a\s+|the\s+)?(?:doctor|hospital|ambulance|er|vet|veterinarian)|911)\b|\bno\s+(?:\w+\s+){0,2}?(?:atenci[oó]n\s+m[eé]dica|m[eé]dico|hospital|urgencias|ambulancia|veterinari[oa])(?![a-zñáéíóú])/gi;
const CLINICIAN_RE = /\b(?:call|contact|see|consult|reach|phone|ask|talk\s+to|speak\s+(?:to|with))\s+(?:a\s+|your\s+|the\s+)?(?:doctor|physician|pediatrician|nurse|medical\s+(?:provider|professional)|health\s*care\s+provider)\b|\b(?:llame|consulte|contacte|vea|acuda)\s+(?:a|al)\s+(?:su\s+)?(?:m[eé]dico|doctor|pediatra)(?![a-zñáéíóú])/i;
const URGENCY_RE = /\b(?:immediately|right\s+away|right\s+now|now|urgent\w*|at\s+once|asap|without\s+delay|de\s+inmediato|inmediatamente|ahora\s+mismo|ya\s+mismo|urgente\w*|cuanto\s+antes)(?![a-zñáéíóú])/i;
