...
const CONTACT_EXPOSURE_RE = new RegExp(`\\b${PRODUCT_NOUN}(?![a-zñáéíóú])[^.?!]{0,40}?\\b(?:in|on|into|en)\\s+(?:his|her|their|my|our|your|the\\s+\\w+'?s|sus|los|la|mis|su)\\s+(?:eyes?|skin|face|mouth|ojos?|piel|cara|boca)(?![a-zñáéíóú])`, 'i');
...
const CLOCK_TIME_RE = /\b\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)(?![a-z])|\b\d{1,2}:\d{2}\b|\b(?:noon|midday|midnight|tonight|tomorrow|this\s+(?:morning|afternoon|evening)|sunset|sundown|sunrise|dawn|dusk|daybreak|nightfall|morning|evening|night|afternoon|amanecer|anochecer|atardecer|dinner\s*time|bedtime|mediod[ií]a|medianoche|esta\s+(?:tarde|noche)|ma[ñn]ana|la\s+(?:tarde|noche))\b|\blas?\s+\d{1,2}(?::\d{2})?\b/i;
