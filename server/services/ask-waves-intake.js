...
const EMERGENCY_RE = /\b(?:911|(?:can'?t|cannot|can\s+not)\s+breathe|(?:not|isn'?t|aren'?t|stopped|stops|quit)\s+breathing|no\s+(?:est[aá]\s+)?respira(?:ndo)?|dej[oó]\s+de\s+respirar|trouble\s+breathing|difficulty\s+breathing|short(?:ness)?\s+of\s+breath|anaphyla\w*|anafila\w*|allergic(?:\s+reaction)?|al[eé]rgic\w*|reacci[oó]n\s+al[eé]rgica|epi\s?pen|throat\s+(?:is\s+)?(?:closing|swelling)|chest\s+pain|passed?\s+out|unconscious|inconsciente|desmay\w*|emergency\s+room|\be\.?r\.?\b|(?:at|in|to|from)\s+(?:the\s+)?hospital|hospitali[sz]\w*|urgencias|sala\s+de\s+emergencias?|poison(?:ed|ing)|envenen\w*|no\s+pued[eo]\s+respirar|dificultad\s+para\s+respirar|falta\s+de\s+aire|dolor\s+de\s+pecho)\b/i;
...
const NEGATED_APPROVAL_RE = /\b(?:doesn'?t|didn'?t|don'?t|does\s+not|did\s+not|do\s+not|won'?t|will\s+not|not|never|isn'?t|aren'?t|wasn'?t|weren'?t|hasn'?t|haven'?t|has\s+not|have\s+not|is\s+not|are\s+not|no\s+(?:est[aá]n?|ha|han|fue)|nunca)\s+(?:been\s+|sido\s+|ever\s+)?(?:approv\w*|endors\w*|certif\w*|sanction\w*|authoriz\w*|aprob\w*|avalad\w*|autoriz\w*)/gi;
