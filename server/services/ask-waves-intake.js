...
const EAT_EXPOSURE_RE = new RegExp(`\\b${EXPOSURE_SUBJECT}\\s+${NON_PEST_GAP}(?:ate|eaten|eating|drank|drunk|drinking|chewed|chewing|licked|licking|(?:se\\s+)?comi[oó]|(?:se\\s+)?bebi[oó]|mastic[oó]|lami[oó]|inhal\\w*|breathed(?:\\s+in)?|breathing(?:\\s+in)?|sniffed|(?:was|were|got|been|is)\\s+exposed\\s+to|exposed\\s+to|touched|splashed|sprayed\\s+with|expuest[oa]s?\\s+a|inhal[oó]|respir[oó])(?![a-zñáéíóú])[^.?!]{0,30}?\\b${PRODUCT_NOUN}(?![a-zñáéíóú])|\\b${PRODUCT_NOUN}\\b[^.?!]{0,30}?\\b(?:eaten|drunk|chewed|licked|inhaled|breathed\\s+in|sniffed|comid[oa]s?|bebid[oa]s?|inhalad[oa]s?)\\s+(?:by|por)\\s+(?:(?:my|our|the|his|her|their|mi|su|el|la)\\s+)?${EXPOSURE_SUBJECT}\\b`, 'i');
...
const CONTACT_EXPOSURE_RE = new RegExp(`\\b${PRODUCT_NOUN}(?![a-zñáéíóú])[^.?!]{0,40}?\\b(?:in|on|into|en)\\s+(?:his|her|their|my|our|your|the\\s+(?:dog|cat|puppy|kitten|baby|child|kid|toddler|bird|parrot|pet|rabbit)'?s|sus|mis|su|tus)\\s+(?:eyes?|skin|face|mouth|ojos?|piel|cara|boca)(?![a-zñáéíóú])|\\b(?:hij[oa]s?|beb[eé]s?|ni[ñn][oa]s?|perr[oa]s?|gat[oa]s?|mascotas?|esposo|esposa|yo|me|le)\\b[^.?!]{0,40}?\\b${PRODUCT_NOUN}(?![a-zñáéíóú])[^.?!]{0,40}?\\ben\\s+(?:los|la)\\s+(?:ojos?|piel|cara|boca)(?![a-zñáéíóú])`, 'i');
...
const NEGATED_ACTION_ON_SUBJECT_RE = /\b(?:won'?t|will\s+not|doesn'?t|does\s+not|don'?t|do\s+not|isn'?t\s+going\s+to|shouldn'?t|should\s+not|can'?t|cannot|never)\s+(?:\w+\s+){1,2}?(?:your\s+|the\s+|any\s+|you\s+or\s+your\s+)?(?:pets?|dogs?|cats?|kids?|children|child|babies|baby|family|people|you|anyone|animals?|pollinators?|bees|fish|birds?|plants?|lawn|garden)\b|\bno\s+(?:les?\s+|lo\s+|la\s+|los\s+|las\s+)?\S+\s+(?:\S+\s+)?a\s+(?:(?:sus|su|tus|tu|los|las|la|el)\s+(?:mascotas?|ni[ñn][oa]s?|hij[oa]s?|perr[oa]s?|gat[oa]s?|familia|familiares|personas|beb[eé]s?|animales|abejas|plantas|c[eé]sped)|nadie)(?![a-zñáéíóú])|\bno\s+(?:les?\s+|lo\s+|la\s+)?(?:hace|hacen|causa|causan|molest\w*|irrit\w*|har[aá]n?|causar[aá]n?|molestar[aá]n?|afectar[aá]n?|perjudicar[aá]n?|lastimar[aá]n?)(?![a-zñáéíóú])/i;
...
const CLOCK_TIME_RE = /\b\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)(?![a-z])|\b\d{1,2}:\d{2}\b|\b(?:noon|midday|midnight|tonight|tomorrow|this\s+(?:morning|afternoon|evening)|sunset|sundown|sunrise|dark|darkness|(?:the\s+)?sun\s+(?:goes|sets|has\s+set|is)\s+down|oscurecer|oscurezca|se\s+ponga\s+el\s+sol|puesta\s+del\s+sol|dawn|dusk|daybreak|nightfall|morning|evening|night|afternoon|amanecer|anochecer|atardecer|dinner\s*time|bedtime|mediod[ií]a|medianoche|esta\s+(?:tarde|noche)|ma[ñn]ana|la\s+(?:tarde|noche))\b|\blas?\s+\d{1,2}(?::\d{2})?\b/i;
...
const HUMAN_EMERGENCY_DIRECTION_RE = /\b(?:ambulances?|paramedics?|EMS|ambulancias?|param[eé]dicos?)\b|(?:\+?1[-.\s]?)?\(?800\)?[-.\s]?222[-.\s]?1222|\b(?:(?:call|contact|see|consult|reach|phone|ask)\s+(?:a\s+|your\s+|the\s+)?(?:doctor|physician|pediatrician|nurse|medical\s+(?:provider|professional)|health\s*care\s+provider)|(?:llame|consulte|contacte|vea|acuda)\s+(?:a|al)\s+(?:su\s+)?(?:m[eé]dico|doctor|pediatra)|(?:go|get|head|rush|drive|take|bring|carry)\s+(?:\S+\s+){0,3}?(?:straight\s+|right\s+)?to\s+(?:the\s+|a\s+|an\s+)?(?:nearest\s+|closest\s+|local\s+)?(?:hospital|emergency\s+room)|(?:vaya|vayan|lleve|llével[oa]|lleven|acuda|acudan|corra)\s+(?:\S+\s+){0,3}?(?:al|a\s+un|a\s+la)\s+(?:hospital|sala\s+de\s+emergencias|urgencias)|(?:go|get|head|take\s+\S+)\s+to\s+(?:the\s+)?(?:er|e\.r\.)|urgencias|call(?:ing)?\s+911|dial\s+911|911\s+(?:right\s+away|immediately|now)|(?<!(?:animal|pet)\s)poison\s+(?:control|help)|emergency\s+(?:room|care|services?|department)|urgent\s+care|seek\s+(?:immediate\s+|urgent\s+)?(?:medical|emergency)(?!\s+(?:veterinary|vet|animal))|medical\s+(?:attention|care|help|emergency)|call\s+(?:a|your)\s+(?:doctor|physician)|centro\s+de\s+(?:toxicolog[ií]a|envenenamientos?)|control\s+de\s+(?:envenenamientos?|intoxicaciones)|sala\s+de\s+emergencias?|atenci[oó]n\s+m[eé]dica|llam[ea]\s+al\s+911)\b/i;
...
const PET_SUBJECT_RE = /\b(?:birds?|parrots?|parakeets?|rabbits?|bunn(?:y|ies)|hamsters?|guinea\s+pigs?|ferrets?|horses?|tortoises?|turtles?|p[aá]jar\w*|aves?|loros?|periquitos?|conejos?|h[aá]msters?|caballos?|tortugas?|dogs?|cats?|pupp(?:y|ies)|kittens?|pets?|perr[oa]s?|gat[oa]s?|mascotas?|cachorr\w*)\b/i;
