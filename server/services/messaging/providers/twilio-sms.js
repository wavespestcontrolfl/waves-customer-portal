...
async function sendViaTwilio(input) {
  ...
  // existing admin-sms-templates kill-switch keys still work AND
  // services/twilio.js's "manual === no auto-attached logo" branch
  // continues to recognize manual sends. The wrapper's purpose enum
  // still drives policy + audit; the messageType string drives the
  // per-template ops kill switch and the logo-attach behavior.
