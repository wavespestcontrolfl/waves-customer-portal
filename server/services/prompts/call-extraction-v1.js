...
- prices: When the call states MORE THAN ONE distinct price (e.g. the agent quotes both a one-time treatment price and a monthly plan price), list EVERY distinct price here, most consequential first, using the same fields as price above for each entry. Copy the PRIMARY one (the accepted one if any, else the first stated) into the top-level price field above as well — price and the matching entry in prices should agree. Leave prices empty/absent when only one price was discussed; price alone still carries it.
