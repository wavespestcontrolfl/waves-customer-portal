...
function variantsForService(serviceKey, prompt = '', generic = false) {
  ...
  if (serviceKey === 'tree_shrub') {
    // 6-visit Standard is the mandated default; Enhanced (9x, un-retired
    // 2026-07-24) is the every-6-weeks upsell — offered, never recommended.
    // Premium (12x) stays retired. Light (4x/quarterly) is likewise retired
    // for new sales as of 2026-09-24 (owner directive: "remove quarterly
    // tree and shrub care from the estimates and services") — dropped from
    // this ladder entirely, never offered under any label.
    const all = [
