...
async function publishWebsiteQuote({ estimateId, leadId, engineInput, engineResult, totals }) {
  ...
  // A duplicate-customer merge can repoint the draft between the owner peek
  // and the estimate lock. That is not ineligibility: roll back (releasing
  // the stale owner's comms lock) and rerun the whole publication, with
  // every eligibility check, under the current owner. Capped so a
  // pathological repeat-merge fails closed (GitHub Codex #4716 P2).
  for (let attempt = 0; ; attempt += 1) {
    try {
      return await db.transaction(trx => publishInTransaction(trx, { estimateId, leadId, engineInput, engineResult, totals }));
    } catch (err) {
      if (err instanceof EstimateOwnerMovedError && attempt < MAX_OWNER_MOVE_RETRIES) continue;
      if (err instanceof EstimateOwnerMovedError) return null;
      throw err;
    }
  }
}

const MAX_OWNER_MOVE_RETRIES = 2;
class EstimateOwnerMovedError extends Error {}

async function publishInTransaction(trx, { estimateId, leadId, engineInput, engineResult, totals }) {
  ...
  // peek and here left this transaction fenced on the wrong key — retry.
  if (String(row.customer_id) !== String(peek.customer_id)) throw new EstimateOwnerMovedError();
