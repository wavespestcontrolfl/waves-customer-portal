...
function applyListicleTreatment({ enabled, actionType, pageType, query, operatorPinned = false, factsAvailable = true, requiredSections, schemaTypes, voiceConstraints }) {
  ...
  const sections = [
    ...
    ...(factsAvailable
      ? ['"how we put this list together" note (2–3 sentences grounded in the brief\'s facts pack, naming the actual sources and linking their supporting pages only when the outbound-link policy permits them; never invent testing, field experience, or a selection methodology)']
      : []),
  ];
  const voice = {
    ...voiceConstraints,
...
// GATE_RETRY_INSTRUCTIONS + buildRetryDirectives moved to the light
// ./gate-retry-directives module — the writer's in-loop self-lint
// (brief-driven-tools emit_draft) issues the same canonical directives, and
// two copies would let the in-loop feedback drift from the run-level redraft
// instructions. Re-required here for the brief composition path.
const { GATE_RETRY_INSTRUCTIONS, buildRetryDirectives } = require('./gate-retry-directives');
