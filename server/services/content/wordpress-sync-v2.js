...
class WordPressSync {
  ...
  async publishToWordPress(blogPostId, siteId = null) {
    const post = await db('blog_posts').where('id', blogPostId).first();
    if (!post) throw new Error('Post not found');

    // Resolve credentials — either from siteId (multi-site) or env vars (legacy)
    let wpUrl, wpUser, wpAppPassword;
    if (siteId) {
      const site = await db('wordpress_sites').where({ id: siteId }).first();
      if (!site) throw new Error(`Site ${siteId} not found`);
      if (!site.wp_username || !site.wp_app_password) throw new Error(`No credentials for ${site.domain}`);
      wpUrl = `https://${site.domain}`;
      ...
      if (!wpUser || !wpAppPassword) throw new Error('WordPress credentials not configured');
    }

    const getAuth = () => ({ Authorization: `Basic ${Buffer.from(`${wpUser}:${wpAppPassword}`).toString('base64')}` });
    ...
    if (post.tag) {
      try {
        const tagResp = await fetch(`${wpUrl}/wp-json/wp/v2/tags?search=${encodeURIComponent(post.tag)}&per_page=5`, {
          headers: getAuth(),
        });
        if (tagResp.ok) {
    ...
    if (post.featured_image_url && post.featured_image_url.startsWith('data:')) {
      try {
        featuredMediaId = await this.uploadFeaturedImage(post.featured_image_url, post.slug, wpUrl, getAuth);
      } catch (imgErr) {
  ...
  async uploadFeaturedImage(dataUrl, slug, wpUrl = WP_URL, getAuth = null) {
    const authHeaders = getAuth ? getAuth() : this.getAuthHeader();
