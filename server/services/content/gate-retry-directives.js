...
const GATE_RETRY_INSTRUCTIONS = {
  ...
  // "pest control calculator", and "estimate your pest control cost" — the
  // collector counts the service only in the words BEFORE the
  // estimate/quote keyword, so the shape must be spelled out.
  FORBIDDEN_CTA_WORDING: 'Rewrite the CTA anchor this finding names — never drop the conversion link. Every link to a conversion path (/pest-control-quote/, /quote/, /pest-control-calculator/, /contact/, a city quote page) and every estimate/quote-worded link to a service or city page must use an anchor that names THIS post\'s own service BEFORE the word estimate/quote, in the shape "Get a <Service> Estimate" / "Request a <Service> Quote" (e.g. "Get a Pest Control Estimate", "Request a Termite Quote"). A generic anchor ("request a quote"), a bare page name ("pest control calculator"), a verb-first anchor where the service follows the keyword ("estimate your pest control cost"), a wrong-service anchor, or any inspection-request anchor ("Request an Inspection") all fail.',
  ...
  AFFILIATE_LINK_REMOVED_ON_REFRESH: 'A refresh may not remove the LAST affiliate link the live page carries — its frozen frontmatter keeps disclosure.type "affiliate", which cannot publish without links; preserve at least one affiliate link (removal happens through the manual lane).',
  ...
  INVALID_SPIDERIDBOARD_PROPS: 'Every <SpiderIdBoard> prop must be one the component accepts (title, eyebrow, species, footnote, caption); species must be a JSON array expression of records with name, risk (beneficial|nuisance|medical), where, hunt, eggSac (optional sciName, glyph, source{label, https url}). Fix or remove the offending prop.',
  INVALID_AFFILIATELINK_PROPS: 'Every <AffiliateLink> carries exactly two props — a quoted literal product="…" and placement="…" — and nothing else; remove any other prop.',
  AFFILIATE_PRODUCT_NOT_IN_BRIEF: 'Link EVERY affiliate product your brief lists under affiliate_products, each EXACTLY once, with EXACTLY the product id, placement and anchor text the brief gives, in the section the brief names — never substitute another product (even an approved one), change the placement, reword the anchor, repeat a link, or leave a listed product out.',
