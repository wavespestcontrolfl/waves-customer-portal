...
const SERVICE_CLAIM_CONTEXT_RE = new RegExp(
  ...
  + `|${SERVICE_KEYWORD_SOURCE}\\b[^.!?]{0,60}?\\b(?:waves\\s+(?:does|do)\\s+(?:too|the same)|so does waves|as does waves|including waves)\\b|no (?:need|reason) to (?:go without|skip|miss|forgo|forego|risk)\\b[^.!?]{0,30}?\\b${SERVICE_KEYWORD_SOURCE}\\b(?!\\s+(?:guides?|tips?|advice|research|facts?|info\\w*|articles?|checklists?|newsletters?)\\b)(?![^.!?]{0,30}\\b(?:without\\s+a\\s+licen\\w*|without\\s+training|yourself|diy)\\b)`
