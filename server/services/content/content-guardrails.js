...
function* eachTag(text) {
  for (let i = 0; i < text.length; i += 1) {
    ...
    // "](</path/>)" is CommonMark's angle-bracket link DESTINATION, not a
    // closing tag named after the path. Reading it as markup made the tag
    // allowlist reject a valid link and stopped the route repair from
    // touching it. Narrow on purpose: the bracket content must BE a
    // destination (site-relative path or http(s) URL) and must close the
    // link, or "[x](<script src=…>)" would inherit the exemption (Codex).
...
function repairInventedInternalRoutes(body, allowedInternalLinks = [], options = {}) {
  ...
  const repaired = text.replace(MD_INTERNAL_LINK_RE, (whole, bang, anchorText, open, dest, close, title, offset) => {
    ...
    // eliminate (Codex r5). The suffix is carried onto the alias — but ONLY
    // when it is plainly safe. Aliasing rewrites the route into an
    // ALLOWLISTED one, which flips the internal-route guard from park to
    // pass, and a refresh runs neither the redaction bundle nor the SEO
    // gate — so a suffix like "?customer=Alice-Smith" would ride a
