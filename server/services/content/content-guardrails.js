...
function offFootprintCityFinding(text) {
  ...
  for (let segIndex = 0; segIndex < allSegments.length; segIndex += 1) {
    ...
    if (/^\|.+/.test(trimmed) || (trimmed.includes('|') && (sepLike(nextTrimmed) || sepLike(trimmed) || (tableIntro && (/^\|/.test(trimmed) || (trimmed.match(/\|/g) || []).length >= 2 || (!/[.!?]/.test(trimmed) || trimmed.length < 60)))))) {
      listIntro = '';
      ...
      if (sepLike(nextTrimmed)) {
        ...
        // "| No | Yes |" denies Naples and affirms Tampa.
        {
          ...
          if (/\?/.test(trimmed) && qCells.length > 1 && qCells.length === aCells.length
            ...
            for (let ci = 0; ci < qCells.length; ci += 1) {
              ...
              if (/^(?:\*\*)?(?:yes|sure|definitely|certainly|yeah|indeed|you bet)\b(?!\s*,?\s*(?:but\s+)?not\b)(?![^.!?]{0,40}\b(?:but\s+not\s+(?:from|by|through|with|us|waves)|not\s+from\s+(?:waves|us)|(?:choose|contact|call|try)\s+a\s+local)\b)/i.test(aCells[ci])) {
        ...
        if (/\?/.test(trimmed)
          ...
          && /(?:^|\|)\s*(?:\*\*)?(?:yes|sure|definitely|certainly|yeah|indeed|you bet)\b(?!\s*,?\s*(?:but\s+)?not\b)(?![^|]{0,40}\b(?:local|another|different)\s+(?:provider|compan\w*|firm|exterminator|option)s?\b)(?![^|]{0,40}\bnot from (?:waves|us)\b)/i.test((allSegments[segIndex + 2] || '').trim())) {
    ...
    for (const clause of semiUnit.split(FOOTPRINT_CLAUSE_SPLIT_RE)) {
      ...
      for (const { city, re, negationRe } of cityRes) {
        ...
        for (const cityMatch of normalized.matchAll(re)) {
          ...
          if (/ County$/.test(city)
            && /\b(?:county-run|government|municipal|state-run|public\s+(?:program|service)|extension|county\s+(?:\w+\s+)?(?:control|programs?|districts?|services?)\b)/i.test(normalized)) {
            continue;
          }
            ...
            if (cityStart < dStart) {
              ...
              // Naples remains outside our service area." stays honest).
              {
                ...
                if (SERVICE_CLAIM_CONTEXT_RE.test(prefix)
                  && !/\b(?:sarasota|bradenton|venice|parrish|palmetto|lakewood ranch|north port|port charlotte|osprey|nokomis|ellenton|myakka)\b/i.test(prefix)) return false;
              }
