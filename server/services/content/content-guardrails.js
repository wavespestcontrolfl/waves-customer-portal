...
const SERVICE_CLAIM_CONTEXT_RE = new RegExp(
  ...
  + '|same.day|we offer|free (?:quote|estimate|inspection)|' + DEMAND_CONTEXT_SOURCE + '|^\\s*(?:and |but |yet )?(?:also |now |still |currently )?(?:includes?|covers?|extends? (?:to|into)|reaches?|serves?|services?|treats?|visits?|sprays?|inspects?|protects?|handles?|helps?)\\b(?!\\s+up\\b)(?![^.!?]{0,30}\\b(?:data|research|weather|statistics|figures|information|charts?|tables?|topics?|sources?|studies)\\b)'
  ...
  + "|(?:waves(?: pest control)?|waveguard)\\s+(?:is |are |can |could |will |do |does |has |have |had )?(?:been )?(?:now |proudly |also |currently |still )?(?:serv(?:e|es|ed)\\b(?!\\s+up\\b)|serving\\b(?!\\s+up\\b)|servic\\w+|treat(?:s|ed|ing)?|cover(?:s|ed|ing)?|exterminat\\w+|remov(?:e|es|ed|ing)\\b|eliminat\\w+|visit(?:s|ed|ing)?\\b|spray(?:s|ed|ing)?\\b|inspect\\w*|handl\\w+|protect\\w*|get(?:s|ting)? rid of\\b|fertiliz(?:e|es|ed|ing)\\b|maintain(?:s|ed|ing)?\\b|mow(?:s|ed|ing)?\\b|aerat\\w+\\b|help(?:s|ing|ed)?\\b(?!\\s+(?:you|readers?|homeowners?)\\s+(?:understand|identify|learn|compare|decide|research|choose|spot))|manag(?:e|es|ed|ing)\\b(?!\\s+to\\b)|proud to (?:serve|service|treat|cover|protect)\\b|work(?:s|ed|ing)? (?:in|throughout|across|around)|operat(?:es|ed|ing)? (?:in|throughout|across|around))"
...
function offFootprintCityFinding(text) {
  ...
  for (let segIndex = 0; segIndex < allSegments.length; segIndex += 1) {
    ...
    if (/^\|.+/.test(trimmed)) {
      ...
      } else if (/\|\s*(?:\*\*)?(?:no\s*[,.!;:—–-][^|]{0,60}|no|not (?:served|available|covered|yet)\b[^|]{0,40}|not (?:in|within|currently|part of)\b[^|]{0,40}|not a (?:service|coverage) area[^|]{0,20}|unavailable[^|]{0,60}|outside\b[^|]{0,40}|✗|✕|x|—|–|-)(?:\*\*)?\s*(?:\||$)/i.test(trimmed)
