...
function offFootprintCityFinding(text) {
  ...
  for (const segment of markdownSegments(s)) {
    ...
    if (/^\|.*\|\s*$/.test(trimmed)) {
      ...
      } else if (/\|\s*(?:\*\*)?(?:no|not (?:served|available|covered|yet)|unavailable|outside(?: our)?(?: service)?(?: area)?|✗|✕|x|—|–|-)(?:\*\*)?\s*\|/i.test(trimmed)) {
