  ...
  for (let segIndex = 0; segIndex < allSegments.length; segIndex += 1) {
    ...
    if (/^\|.+/.test(trimmed) || (trimmed.includes('|') && (sepLike(nextTrimmed) || sepLike(trimmed) || (tableIntro && (/^\|/.test(trimmed) || (trimmed.match(/\|/g) || []).length >= 2 || !/[.!?]/.test(trimmed)))))) {
      ...
      if (sepLike(nextTrimmed)) {
        ...
        // "| No | Yes |" denies Naples and affirms Tampa.
        {
          ...
          if (/\?/.test(trimmed) && qCells.length > 1 && qCells.length === aCells.length
            ...
            for (let ci = 0; ci < qCells.length; ci += 1) {
              if (/^(?:\*\*)?(?:yes|sure|definitely|certainly|yeah|indeed|you bet)\b(?![^.!?]{0,40}\b(?:but\s+not\s+(?:from|by|through|with|us|waves)|not\s+from\s+(?:waves|us)|(?:choose|contact|call|try)\s+a\s+local)\b)/i.test(aCells[ci])) {
        ...
        if (/\?/.test(trimmed)
          && /(?:^|\|)\s*(?:\*\*)?(?:yes|sure|definitely|certainly|yeah|indeed|you bet)\b(?![^|]{0,40}\b(?:local|another|different)\s+(?:provider|compan\w*|firm|exterminator|option)s?\b)(?![^|]{0,40}\bnot from (?:waves|us)\b)/i.test((allSegments[segIndex + 2] || '').trim())) {
