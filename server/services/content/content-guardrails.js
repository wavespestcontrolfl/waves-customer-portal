        ...
        for (const cityMatch of normalized.matchAll(re)) {
          ...
          const disclaimed = disclaimerRanges.some(([dStart, dEnd]) => {
            if (cityStart < dStart) {
              ...
              // by request").
              if (SERVICE_CLAIM_CONTEXT_RE.test(blankDisclaimers(normalized, disclaimerRanges).slice(dEnd))) return false;
