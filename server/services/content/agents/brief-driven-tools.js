      ...
      let metaSelfLintAudit = null;
      {
        ...
        if (metaLintOptions && metaGuardrails?.evaluate) {
          ...
          if (attempts < SELF_LINT_MAX_REDRAFTS) {
            let lintResult = null;
            try {
              // The TITLE is deliberately omitted: protected non-blog pages
              // discard the title proposal at the publisher, and only the
              // run-level gate can know which targets are protected
              // (live-page load) — linting a to-be-discarded title here
              // could burn both redrafts on text that never ships. The
              // description ALWAYS ships; the authoritative gate evaluates
              // the title with its protected-title context. (The sanctioned
              // {{cityPhone}} token is scrubbed inside evaluate() itself.)
              lintResult = metaGuardrails.evaluate({ frontmatter: { meta_description }, body: '' }, metaLintOptions);
            } catch (err) {
