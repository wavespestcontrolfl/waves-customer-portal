...
const PROVIDER_DISPARAGEMENT_RE = new RegExp([
  `\\b(?:${NEG_ADJ})\\b(?:\\s+(?!(?:areas?|zones?|spots?|corners?|patches?|near|around|by|beside|under|behind|at)\\b)\\w+){0,2}\\s+\\b(?:${PROVIDER_NOUN})\\b${NOT_SERVICE_AREA}(?!\\s+(?:monitors?|flags?|stations?|traps?|bait|signs?)\\b)`,
...
const DIRECTED_DISPARAGEMENT_RE = new RegExp([
  ...
  `(?:${DISPARAGEMENT_RE.source})(?:\\s+(?!(?:in|within|across|throughout|among|around|areas?|zones?|spots?|corners?|patches?|near|by|beside|under|behind|at)\\b)\\w+){0,2}\\s+\\b(?:${STRONG_PROVIDER_NOUN})\\b${NOT_SERVICE_AREA}(?!\\s+(?:monitors?|flags?|stations?|traps?|bait|signs?)\\b)`,
  ...
  `\\b(?:${PROVIDER_NOUN})\\b${NOT_SERVICE_AREA}(?:${NOUN_VERB_GAP}(?:has|have|had|uses?|used|includes?|included|comes?\\s+with|(?:known|notorious|infamous)\\s+for|accused\\s+of|blamed\\s+for|cited\\s+for|(?:ha(?:s|ve|d)|face[sd]?|earn(?:s|ed)?|carr(?:y|ies|ied))\\s+a\\s+reputation\\s+for|(?:draws?|drew|gets?|got|receives?|received)\\s+complaints?\\s+(?:about|over|regarding))|\\s+with)\\s+(?:(?:a|an|the|really|very)\\s+){0,2}${POSSESSION_ACCUSATION_SRC}`,
...
const OWN_BRAND_DISPARAGEMENT_RE = new RegExp([
  ...
  `${OWN_BRAND_POSSESSION_SUBJECT}${OWN_BRAND_CLAUSE_GAP}(?:has|have|had|uses?|used|includes?|included|comes?\\s+with|(?:known|notorious|infamous)\\s+for|accused\\s+of|blamed\\s+for|cited\\s+for|(?:ha(?:s|ve|d)|face[sd]?|earn(?:s|ed)?|carr(?:y|ies|ied))\\s+a\\s+reputation\\s+for|(?:draws?|drew|gets?|got|receives?|received)\\s+complaints?\\s+(?:about|over|regarding))\\s+(?:(?:a|an|the|really|very)\\s+){0,2}${POSSESSION_ACCUSATION_SRC}`,
].join('|'), 'i');
...
// Provider-owned educational content is not an accusation — "Acme Pest
// Solutions has a guide to avoid hidden fees" (Codex r43).
const EDITORIAL_TAIL_RE = /\b(?:guides?|advice|tips?|articles?|checklists?|resources?|posts?|blogs?|explainers?)\b[\s\S]{0,40}?\b(?:avoid(?:ing)?|spot(?:ting)?|prevent(?:ing)?|dodg(?:e|ing)|catch(?:ing)?)\b/i;
...
function scopedSelfRankingMatch(text) {
  ...
  while ((sm = selfRe.exec(text)) !== null) {
    ...
    if (/\b(?:uses?|using|used|appl(?:y|ies|ied|ying)|installs?|installed|installing|carr(?:y|ies|ied|ying)|stocks?|stocked|recommends?|recommended|sprays?|sprayed|deploys?|deployed|with|sells?|sold)(?:\s+(?:a|an|the|this|these|those|our|some|really|very)|\s+[\w'’-]+,\s*(?:a|an|the)?)*\s*$/i.test(text.slice(sentStart, sm.index))) continue;
...
function scanOwnBrandRankingArms(scanText) {
  ...
  const trRe = new RegExp(`(?:\\b(?<brandTok>waves)\\b(?:['’]s?)?(?:\\s+[Pp]est\\s+[Cc]ontrol)?(?:(?!\\b(?:that|whether|if|explains?|explained|teach(?:es)?|taught|says?|said|notes?|noted|shows?|showed)\\b)[^.!?\\n]){0,40}?\\b(?:is|are|was|were|remains?)\\s+(?:the\\s+)?(?:${TOP_RATED_SRC})\\b|\\b(?:${TOP_RATED_SRC})\\s+(?:the\\s+)?\\b(?<brandTok2>waves)\\b)`, 'gi');
  let tr;
...
function evaluateProse(draft, body, { operatorBriefText = '' } = {}) {
  ...
  // Name/tone machinery runs on the ASSOCIATED text: URLs resolving to a
  // recognized competitor become that competitor's canonical name
  // (length-preserving), so directed tone scans, unknown-competitor
  // fail-close, and review routing treat every URL form like a name
  // mention (Codex r4/r5). The PROSE-MENTION check below instead uses the
  // BLANKED text — a citation destination is not a prose mention (r2).
  // Deliberately NOT applied to operatorBriefText below — a URL there is a
  // legitimate operator authorization signal.
  const nameScanText = stripQuotesForNames(substituteCompetitorUrlsForNames(scanText));
  // Allowlisted names that appear in the draft TEXT itself (URLs blanked) —
  // the set the in-prose finding may fire on.
  const textNamedKnown = new Set();
  for (const m of competitorFacts.findBusinessMentions(stripQuotesForNames(stripLinkDestinationsForNames(scanText)))) {
    if (m.inAllowlist) textNamedKnown.add(m.name);
  }
  ...
  for (const name of genericNames) {
    ...
    const possessionP0 = new RegExp(
      `${escaped}\\b(?:['’]s?)?(?:${NOUN_VERB_GAP}(?:has|have|had|uses?|used|includes?|included|comes?\\s+with|(?:known|notorious|infamous)\\s+for|accused\\s+of|blamed\\s+for|cited\\s+for|(?:ha(?:s|ve|d)|face[sd]?|earn(?:s|ed)?|carr(?:y|ies|ied))\\s+a\\s+reputation\\s+for|(?:draws?|drew|gets?|got|receives?|received)\\s+complaints?\\s+(?:about|over|regarding))|\\s+with)\\s+(?:(?:a|an|the|really|very)\\s+){0,2}${POSSESSION_ACCUSATION_SRC}`, 'i',
    );
  ...
  for (const nm of known) {
    ...
    // Known only through a link DESTINATION (never named in the text):
    // not a prose mention (r2) — but never invisible either; route to
    // review (tone checks above already read the link's anchor/clauses).
    if (!textNamedKnown.has(nm)) {
      requiresHumanReview = true;
      continue;
    }
...
// Length-preserving URL transform for competitor-NAME detection inputs
// (indices keep aligning with the original text). Two modes:
//  - blank (default): every URL becomes spaces — a link destination is not
//    a PROSE MENTION (its host is validated by the external-link guardrail),
//    so operator-required citations don't self-block (Codex r2).
//  - associate: a URL that RESOLVES to a recognized competitor is replaced
//    by that competitor's canonical name (space-padded to the URL's length);
//    unrecognized URLs still blank. Feeding THIS text to the ordinary
//    name-driven machinery makes every URL form — inline, bare/autolink,
//    HTML href, reference-style — behave exactly like a name mention for
//    tone, unknown-competitor fail-close, the feature gate, and review
//    routing (Codex r4/r5), while the prose-mention and caption checks keep
//    the blanked text.
function maskUrlsForNames(s, { associate = false } = {}) {
  return String(s).replace(new RegExp(NAME_SCAN_URL_RE_SRC, 'gi'), (u) => {
    if (associate) {
      const hits = competitorFacts.findBusinessMentions(u.replace(/[^a-z0-9]+/gi, ' '));
      const hit = hits.find((h) => h.name && h.name.length <= u.length);
      if (hit) return hit.name + ' '.repeat(u.length - hit.name.length);
    }
    return ' '.repeat(u.length);
  });
}
function stripLinkDestinationsForNames(s) {
  return maskUrlsForNames(s);
}
function substituteCompetitorUrlsForNames(s) {
  return maskUrlsForNames(s, { associate: true });
}
...
function linkedCompetitorMentions(text) {
  ...
  while ((m = re.exec(s)) !== null) {
    ...
    for (const hit of competitorFacts.findBusinessMentions(urlTokens)) {
      ...
      out.push({ name: hit.name, inAllowlist: hit.inAllowlist, anchor, context: `${pre} ${anchor} ${post}`.trim() });
    }
  }
  return out;
}
...
function evaluate(draft, { namedCompetitorEnabled = false, operatorBriefText = '' } = {}) {
  ...
  // Name/tone machinery runs on the ASSOCIATED text: URLs resolving to a
  // recognized competitor become that competitor's canonical name
  // (length-preserving), so every URL form behaves like a name mention for
  // tone, the feature gate, and review routing (Codex r4/r5). The
  // prose-mention and unsourced-caption checks use the BLANKED text — a
  // citation destination is not a prose mention (r2). Disparagement/
  // ranking scans keep the original scanText.
  const nameScanText = stripQuotesForNames(substituteCompetitorUrlsForNames(scanText));
  // Allowlisted names present in the draft TEXT itself (URLs blanked) —
  // the only names the unsourced-caption fill below may draw from.
  const textNamedKnown = new Set();
  for (const m of competitorFacts.findBusinessMentions(stripQuotesForNames(stripLinkDestinationsForNames(scanText)))) {
    if (m.inAllowlist) textNamedKnown.add(m.name);
  }
  ...
  if (!disp) {
    ...
    for (const name of extraProseNames) {
      ...
      // "X being <insult>" association — "rumors about acme pest solutions
      // being overpriced" (Codex r46).
      const beingP0 = new RegExp(
        `${escaped}\\b(?:['’]s?)?\\s+being\\s+(?:(?:really|very|just|a|an|the)\\s+){0,2}(?:${DISPARAGEMENT_RE.source}|\\b(?:${NEG_ADJ})\\b(?!-))`, 'i',
      );
      ...
      if (!dm) {
        ...
        dm = firstUnnegatedMatch(proseNameText, negBeforeName)
          || firstUnnegatedMatch(proseNameText, beingP0)
        ...
        // "Bug Busters: hidden fees are not present" (Codex r32); so does
  ...
  for (const block of blocks) {
    ...
    // above is target-scoped, so this keeps table strictness. Denied cells
  ...
  // see it (Codex r5).
  for (const n of known) if (!blockNamedKnown.has(n) && textNamedKnown.has(n)) unsourcedKnown.add(n);
