...
function buildLawnInsightCards({
  ...
  const planState = ['without_plan', 'with_plan'][Number(hasPlan)];
  const waterInState = {
    'false:false': 'not_required',
    'false:true': 'not_required',
    'true:false': 'missing',
    'true:true': 'recorded',
  }[`${waterInRequired === true}:${waterInInstructionRecorded === true}`];
  const waterActionKey = `${planState}:${waterInState}`;
  if (water && water.status === 'surplus') {
    const overwateringEvidenceSource = water.overwateringEvidenceSource
      || (water.overwatering ? 'photo_signal' : 'calculated_estimate');
    const hasPhotoEvidence = overwateringEvidenceSource === 'photo_signal';
    cards.push({
      // Same provenance rule as the damp card below: overwatering is an
      // automated (photo/area) signal, not technician confirmation.
      category: 'water', status: 'needs_attention', confidence: hasPhotoEvidence ? 'ai_supported' : 'area_estimated',
      headline: 'The lawn is likely getting too much water',
      whatWeSaw: hasPhotoEvidence
