...
async function renderServiceReportV1Pdf(data, {
  token, req, logger: callLogger, serviceRecordId, pinnedLawnAssessmentId = null, pinnedWeekPlanAvailableAt,
} = {}) {
  ...
  const url = serviceReportViewerUrl(reportToken, req, 'pdf', { pinnedLawnAssessmentId, pinnedWeekPlanAvailableAt });
  const provider = selectedPdfRenderer();
