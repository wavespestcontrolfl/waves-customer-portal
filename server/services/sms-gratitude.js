...
const MANUAL_QUESTION_RE = /(?:^|[.!,;:\n—-]\s*|\b(?:just|please|also|and|so|then|or|but)\s+)(?:can|could|would|will|do|does|did|is|are|was|were|should|shall|have|has|what|which|when|where|who|whom|how|why|any chance)\b|\b(?:can|could|would|will|do|does|did|is|are|should|shall|have|has|may|might) (?:you|u|ya|we|i|that|this|it|there|anyone|someone|somebody|friday|monday|tuesday|wednesday|thursday|saturday|sunday|tomorrow|today|morning|afternoon)\b|\b(?:wonder(?:ing|ed)?|whether|see if|know if|check(?:ing)? if|curious|want(?:ed)? to (?:see|know|check|ask|confirm)|(?:send|email|text|tell|shoot|forward|upload|attach|snap|show|give|get|share|confirm|remind|advise|update|call|drop|pick|choose|select|reply|respond|fill|sign|complete|return) (?:me|us|it|them|this|that|a|an|the|your|which|what|when|where|how|over|back|with|to|by)\b|let (?:me|us) know|lmk|feel free to (?:reply|text|call|let)|please (?:send|confirm|reply|let|advise|text|call|share)|what(?:'s| is| are| time| day)|which (?:day|time|one)|work(?:s)? for you|good for you|ok(?:ay)? for you)\b/i;
