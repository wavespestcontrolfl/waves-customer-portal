...
async function maybeSendArrivalSms(svc, serviceId, actingTechId, claimArrivedAt = null) {
  ...
  try {
    ...
    const tech = techId
      ...
      : null;
    const techName = tech?.name || 'Your Waves technician';
    const result = await TwilioService.sendTechArrived(svc.customer_id, techName, { scheduledServiceId: serviceId, scheduledDate: svc.scheduled_date ?? null, scheduledWindowStart: svc.window_start ?? null });
