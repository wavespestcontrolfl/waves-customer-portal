...
// GATE_LLM_CALL_TRACES is on (llm-dispatch-metrics recordTrace). The house
// rule (AGENTS.md, PII in logs) keeps full inbound SMS / email bodies, call
// transcripts and review text out of any stored log, and the redactor only
// masks contact fields inside such text — so a lane whose prompt
// interpolates customer-AUTHORED text cannot be traced (codex r1 on #4788).
// Opted in: the service-report copy lanes, whose prompts carry technician
// notes, structured findings and product context; the pre-visit brief
// (AI call summaries, never transcripts); and the invoice summary. Their
// customer-name scalars are scrubbed by redactStructuredFields before the
// prose redactor runs. Left out on purpose: every SMS, call, email, review
// and estimate lane (inbound text in the prompt), vision lanes (image
// bodies), content / SEO / social, embeddings, canaries, evals, and every
// Managed Agents session (session rows carry no bodies). Bodies are capped
// at 8 KB and pruned after 7 days; a low-confidence redaction is never
// stored on any lane.
