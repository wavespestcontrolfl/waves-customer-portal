...
async function mintPacketInvoice({ packet, visit, members, customer, trx }) {
  ...
  if (existing.length) return adoptAcceptanceInvoice({ packet, members, customer, trx, invoices: existing });
  const billed = [];
...
async function adoptAcceptanceInvoice({ packet, members, customer, trx, invoices }) {
  ...
  if (invoices.length !== 1) return rejected;
  const invoice = invoices[0];
  const { isAutoGeneratedPayPerApplicationInvoice, invoiceHasPositiveSetupFeeLine, acceptanceSetupFeeMatchesAuthority } = require('./estimate-first-application-invoice');
  const sourceId = members[0].source_estimate_id;
  if (![sourceId, isAutoGeneratedPayPerApplicationInvoice(invoice),
    String(invoice.notes).includes(`accepted estimate #${sourceId}.`),
  ...
  for (const member of members) {
    if (member.source_estimate_id !== sourceId
        || [member.recurring_parent_id, member.is_callback, isAlwaysFreeServiceType(member.service_type)].some(Boolean)
        || member.status !== 'completed') return rejected;
  ...
  // The acceptance transaction consumed deposits and applied discounts already.
  // Adoption changes only ownership/record links; never mint or reprice it.
  // Match the manual-invoice/obligation stamp and its transaction lock. Try
  // rather than wait while holding member/customer rows owned by closeout.
  const stampLock = await trx.raw('SELECT pg_try_advisory_xact_lock(hashtext(?)) AS locked',
    [`unminted_setup_fee_manual_billing:${sourceId}`]);
  if (!stampLock.rows[0].locked) return rejected;
  const competing = await trx('invoices as i').leftJoin('scheduled_services as s', 's.id', 'i.scheduled_service_id')
    .where('i.customer_id', customer.id).whereNot('i.id', invoice.id)
    .where(function acceptedEstimateInvoice() {
      this.where({ 's.source_estimate_id': sourceId, 's.scheduled_date': owner.scheduled_date })
        .orWhere('i.notes', 'like', `%accepted estimate #${sourceId}%`);
    }).forUpdate('i').noWait().first('i.id');
