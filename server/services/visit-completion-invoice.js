...
async function memberBillingEligibility(member, customer, trx) {
  ...
  if (isAlwaysFreeServiceType(member.service_type)) return { lineItems: [] };
  return null;
}
...
async function buildMemberLines(member, customer, trx) {
  ...
  if (member.is_callback && !eligible) return { lineItems: [] };
...
async function adoptAcceptanceInvoice({ packet, members, customer, trx, invoices }) {
  ...
  if (!sourceId || !isAutoGeneratedPayPerApplicationInvoice(invoice)
      ...
      || invoice.customer_id !== customer.id || invoice.status !== 'draft') return rejected;
  ...
  for (const member of members) {
    if (member.source_estimate_id !== sourceId || [member.recurring_parent_id, member.is_callback].some(Boolean)
  ...
  const competing = await trx('invoices as i').join('scheduled_services as s', 's.id', 'i.scheduled_service_id')
    .where('s.source_estimate_id', sourceId).where('s.scheduled_date', owner.scheduled_date)
    .whereNot('i.id', invoice.id).first('i.id');
