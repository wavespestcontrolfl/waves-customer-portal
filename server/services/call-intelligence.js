...
// The processor's own retry predicate (attempt cap + age window), so the
// phase below never promises a retry the sweep will not make (Codex r9 P2).
...
function describeProcessing(call) {
  ...
  switch (status) {
    ...
    case 'extraction_failed': {
      const attempts = Number(call.extraction_attempts || 0);
      const createdAt = call.created_at ? new Date(call.created_at) : null;
      const tooOld = !!createdAt && !Number.isNaN(createdAt.getTime())
        && (Date.now() - createdAt.getTime()) > EXTRACTION_RETRY_WINDOW_DAYS * 24 * 60 * 60 * 1000;
      const exhausted = attempts >= CALL_EXTRACTION_MAX_ATTEMPTS;
      phase = (exhausted || tooOld) ? 'failed' : 'failed_retrying';
      retryable = phase === 'failed_retrying';
      detail = exhausted
        ? `Extraction failed ${attempts} times; automatic retries are exhausted — Reprocess to try again.`
        : tooOld
          ? `Extraction failed (attempt ${attempts || 1}); the call is older than ${EXTRACTION_RETRY_WINDOW_DAYS} days, so the sweep no longer retries it — Reprocess to try again.`
          : `Extraction failed (attempt ${attempts || 1}); the sweep retries automatically.`;
      break;
    }
...
function buildCallIntelligence({ call, commitments = [], outcomes = null }) {
  // Only a VALID V2 is intelligence. The processor keeps a schema- or
  // normalization-rejected candidate in ai_extraction_enriched for audit
  // and refuses to act on it; the panel must not present it as fact either
  // (Codex r10 P1) — its validation errors show under processing instead.
  const v2 = call.v2_extraction_status === 'valid' ? parseJson(call.ai_extraction_enriched, null) : null;
  // V2 + the transcript only, never the legacy V1 extraction (AGENTS.md:
  // downstream composers read v2 + raw transcript, never v1). When V2 is
  // absent or rejected the field is honestly empty — Codex #3738 r8 P1.
  ...
  return {
    ...
    // V2 only: call_summary / lead_quality / sentiment on call_log are the
    // processor's V1 columns, and composers never read V1 (AGENTS.md;
    // Codex #3738 r14 P1) — an absent or rejected V2 reads as empty.
    summary: v2?.meta?.call_summary || null,
    outcome: {
      disposition: call.disposition || null,
      recommended_disposition: v2?.recommended_disposition || null,
      call_nature: v2?.call_nature || null,
      lead_quality: sal.lead_quality || null,
      sentiment: sal.sentiment || null,
      is_voicemail: v2?.meta?.is_voicemail ?? null,
      is_spam: v2?.meta?.is_spam ?? null,
    ...
    intent: {
      ...
      specific_service_name: sr.specific_service_name || null,
    ...
    caller: {
      name: caller.name_full || [caller.first_name, caller.last_name].filter(Boolean).join(' ') || null,
