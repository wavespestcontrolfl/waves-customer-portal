...
const CONTAINER_MULTIPACK_RE = new RegExp(`(\\d+)\\s+[a-z]+\\s*\\/\\s*(\\d[\\d,]*(?:\\.\\d+)?(?:\\s*\\/\\s*\\d+)?)\\s*(${SIZE_UNITS}|gram|g|l)s?\\b`, 'i');
// Parse "30", "2.5", or a fraction "1/2" into a number (or null).
const numericAmount = (s) => {
...
function extractSizeToken(text) {
  ...
  if (cm) {
    // Compute count x per-unit here (handles a fractional per-unit "1/2") rather
    // than hand parsePackSize "N container / M unit", which misreads a fraction.
    const count = Number(cleanNum(cm[1]));
    const per = numericAmount(cm[2]);
    if (count > 0 && per != null) {
      return `${Math.round(count * per * 1e6) / 1e6} ${cleanUnit(cm[3])}`.trim();
    }
  }
