exports.up = async function(knex) {
  if (!(await knex.schema.hasTable('customer_health_scores'))) {
    await knex.schema.createTable('customer_health_scores', t => {
  ...
  if (!(await knex.schema.hasTable('customer_health_history'))) {
    await knex.schema.createTable('customer_health_history', t => {
  ...
  if (!(await knex.schema.hasTable('customer_health_alerts'))) {
    await knex.schema.createTable('customer_health_alerts', t => {
      t.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
      ...
      t.timestamps(true, true);
      t.index('status'); t.index('severity'); t.index('customer_id'); t.index('alert_type');
    });
  }
