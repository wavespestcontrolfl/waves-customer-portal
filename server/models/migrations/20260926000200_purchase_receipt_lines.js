 ...
 * UNIQUE (vendor, order_number, line_no) constraint is the claim every
 * writer inserts against BEFORE acting on the line, so a re-run of the
 * sweep (cron tick, or the post-sync hook re-offering the same email) can
 * never double-log the same delivered item — see
 ...
 *                     CHECK's four working statuses so a manual override
 *                     has somewhere to land without loosening the schema)
 */
exports.up = async function up(knex) {
  ...
  await knex.schema.createTable('purchase_receipt_lines', (t) => {
    ...
    t.unique(['vendor', 'order_number', 'line_no'], { indexName: 'purchase_receipt_lines_vendor_order_line_uniq' });
    t.index('product_id');
    t.index('email_id');
  });
  await knex.raw(`
    ...
    CHECK (status IN ('logged', 'unmatched', 'size_mismatch', 'needs_size', 'skipped'))
  `);
};
