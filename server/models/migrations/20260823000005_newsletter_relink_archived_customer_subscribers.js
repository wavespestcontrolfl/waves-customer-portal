 ...
 *
 * Twin selection mirrors the helper exactly: same LOWER(TRIM(email)), same
 * scope (deleted_at IS NULL only — deliberately NOT whereLiveCustomer; see
 * liveTwinSubselect's note: customer_id carries link semantics, so a
 * lead-stage profile stays a valid target), ordered is_primary_profile DESC
 ...
 * move; archived links whose email has no non-archived profile are left alone.
 *
...
exports.up = async function up(knex) {
  ...
  // an archived customer move. Mirrors relinkSubscribersFromArchivedCustomer.
  await knex.raw(
