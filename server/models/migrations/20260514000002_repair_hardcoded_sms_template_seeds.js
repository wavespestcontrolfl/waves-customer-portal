/**
 * Repair SMS template rows that used to be hardcoded fallbacks.
 *
 * 20260513000002 exists on main already, so editing that migration would not
 * help databases that have recorded it as applied. This follow-up migration is
 * idempotent and runs the seed/repair work for both already-migrated and fresh
 * environments.
 */
...
const TEMPLATE_REPAIR_KEYS = TEMPLATE_REPAIRS.map((t) => t.template_key);
...
function rewriteLegacyTrackBody(body) {
  if (typeof body !== 'string') return body;
  if (/\{track_clause\}/.test(body) || !/\{track_url\}/.test(body)) return body;
...
function hasVariables(row, variables) {
  const current = new Set(parseVariables(row.variables));
...
exports.up = async function (knex) {
  ...
  for (const template of TEMPLATE_REPAIRS) {
  ...
  for (const rewrite of BODY_REWRITES) {
    const row = await knex('sms_templates')
      .where({ template_key: rewrite.template_key })
      .first();
    if (!row) continue;
    const rewrittenBody = rewriteLegacyServiceDateBody(row.body);
    const shouldUpdateVars = !hasVariables(row, rewrite.variables);
    if (rewrittenBody === row.body && !shouldUpdateVars) continue;
    await knex('sms_templates')
      .where({ template_key: rewrite.template_key })
      .update({
        body: rewrittenBody,
        variables: JSON.stringify(rewrite.variables),
        updated_at: new Date(),
      });
  }
  ...
  const techEnRoute = await knex('sms_templates')
    ...
    .first();
  if (techEnRoute) {
    const rewrittenTrackBody = rewriteLegacyTrackBody(techEnRoute.body);
    const trackVariables = ['first_name', 'tech_name', 'eta_line', 'track_clause'];
    const shouldUpdateVars = !hasVariables(techEnRoute, trackVariables);
    if (rewrittenTrackBody !== techEnRoute.body || shouldUpdateVars) {
      await knex('sms_templates')
        .where({ template_key: 'tech_en_route' })
        .update({
          body: rewrittenTrackBody,
          variables: JSON.stringify(trackVariables),
          updated_at: new Date(),
        });
    }
  }

  for (const key of TEMPLATE_REPAIR_KEYS) {
