/**
 * Add "loop in your family" to the membership.started portal showcase.
 *
 * Follow-up to 20260617120000_membership_started_portal_showcase.js: the
 * customer portal lets members add on-location contacts (a spouse, partner,
 * tenant, or property manager — up to 3 per property) who receive appointment
 * texts. That is one of the more compelling reasons to explore the portal, so
 * it now appears in the welcome email's feature callout (replacing the generic
 * "notification preferences" line).
 *
 * Publishes a new active version of membership.started, swapping only the
 * content blocks. Subject/preview are preserved from the currently ACTIVE
 * version; all template-row metadata / variable contracts are untouched and no
 * new variables are introduced. Mirrors the publish/archive mechanics of the
 * prior showcase migration.
 */
...
const NEW_BLOCKS = [
  ...
  { type: 'callout', content: 'See upcoming visits and reschedule in a tap · View invoices, pay online, and turn on autopay · Request a re-service or add a service · Add a spouse, partner, or tenant to your appointment texts · Refer friends and earn $25 · Find your service reports, invoices, and agreements' },
...
async function publishBlocks(knex, templateKey, blocks) {
  ...
  // highest version_number, which can be an unapproved draft. Prefer the
  // template's own active_version_id, falling back to the newest active-status
  // row.
