 ...
 * All createTable calls wrapped with hasTable checks to prevent
 * "relation already exists" errors that block subsequent migrations.
 */
exports.up = async function (knex) {
  if (!(await knex.schema.hasTable('referral_promoters'))) {
  ...
  if (!(await knex.schema.hasTable('referral_clicks'))) {
  ...
  if (!(await knex.schema.hasTable('referral_payouts'))) {
  ...
  } catch { /* settings table may have different schema */ }
};
