 ...
 * the best the old rows can say. Partial index on the column alone for the
 * session-window scan (the predicate is row_kind + a range on this column,
 * no lane_id — a lane-led index would not serve it).
 */
...
exports.up = async function up(knex) {
  ...
  await knex.raw(`CREATE INDEX IF NOT EXISTS ${INDEX} ON ${TABLE} (${COLUMN}) WHERE row_kind = 'session'`);
};
