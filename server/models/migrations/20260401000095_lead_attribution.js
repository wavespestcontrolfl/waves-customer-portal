exports.up = async function (knex) {
  ...
  await knex.schema.createTable('lead_sources', (t) => {
    ...
    t.string('source_type', 30).notNullable(); // phone_tracking / website_organic / website_paid / social_organic / social_paid / referral / direct / walk_in / marketplace / other
    t.string('channel', 50);
    ...
    t.string('cost_type', 20).defaultTo('free'); // fixed / per_lead / per_month / one_time / free
  ...
  await knex.schema.createTable('leads', (t) => {
    t.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
    t.uuid('lead_source_id').references('id').inTable('lead_sources').onDelete('SET NULL');
    t.string('first_name');
    t.string('last_name');
    t.string('phone');
    t.string('email');
    ...
    t.string('city');
    t.string('zip');
    t.string('lead_type', 30); // inbound_call / inbound_sms / form_submission / chat_widget / walk_in / referral / ai_agent / voicemail / email_inquiry
    ...
    t.string('status', 30).defaultTo('new'); // new / contacted / estimate_sent / estimate_viewed / negotiating / won / lost / unresponsive / disqualified / duplicate
  ...
  await knex.schema.createTable('lead_source_costs', (t) => {
    t.increments('id');
    t.uuid('lead_source_id').notNullable().references('id').inTable('lead_sources').onDelete('CASCADE');
