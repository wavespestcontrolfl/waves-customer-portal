/**
 * Combined "report + invoice" email template for the WDO send-with-invoice
 * flow. project.report_ready only renders the report CTA, so an email-only
 * recipient (no valid SMS destination) of a combined send would receive the
 * invoice PDF with no clickable payment link. This template renders BOTH the
 * report CTA and the pay CTA, plus the invoice number / amount-due rows.
 *
 * Mirrors the seeding pattern in
 * 20260526000011_deepen_service_report_email_templates.js.
 */
...
const TEMPLATE = {
  ...
  blocks: [
    ...
    { type: 'paragraph', content: 'The report includes the documented findings, photos when available, and recommendations. The official report PDF and your invoice PDF are attached to this email.' },
    { type: 'details', rows: [
