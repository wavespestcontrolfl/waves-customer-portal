exports.up = async function (knex) {
  ...
  await knex.schema.createTable('discounts', (t) => {
    ...
    t.text('description');
    t.string('discount_type', 20).notNullable().defaultTo('percentage'); // percentage | fixed_amount | free_service
  ...
  // 3. invoice_discounts — tracks which discounts hit each invoice
  await knex.schema.createTable('invoice_discounts', (t) => {
    ...
    t.uuid('discount_id').references('id').inTable('discounts').onDelete('SET NULL');
  ...
  await knex('discounts').insert(discounts);
};
...
exports.down = async function (knex) {
  ...
  // Don't drop customer columns — other migrations may depend on them
};
