...
/**
 * App-intro email v3 — adds the owner-requested lawn showcase ("Your lawn
 * gets a report card": per-visit graded turf assessment, progress photos,
 * lawn report, Learn-tab wiki, Request upsell for non-lawn customers) below
 * the pay section, and a dedicated Waves AI section (reschedule, add-service
 * quotes, plan coverage, charge explanations) illustrated with an AI-bar
 * crop (app-waves-ai.png).
 *
 * Ships as its OWN migration: v2 landed via 20260707000010 in #2446, which
 * prod records in knex_migrations — an in-place edit to that file would be
 * a silent no-op (knex tracks migrations by filename). down() restores the
 * v2 copy from that migration's TEMPLATE export.
 *
 * Portal-hosted screenshots (client/public/app-email/, PNG for Outlook):
 ...
 *   https://portal.wavespestcontrol.com/app-email/app-waves-ai.png (AI-bar crop)
 */

const { TEMPLATE: TEMPLATE_V2 } = require('./20260707000010_app_intro_email_v2_new_ui');
...
const TEMPLATE = {
  ...
  blocks: [
    ...
    { type: 'paragraph', content: "Tap any completed visit for the full report: what we treated, the products we used, what we found, and photos from your property. Your technician can even record a short video recap on-site, and the report shows when treated areas are dry and safe for kids and pets." },
    ...
    { type: 'paragraph', content: 'You can also loop a spouse or property manager into appointment texts and lock everything behind Face ID.' },

    { type: 'heading', content: 'Your lawn gets a report card' },
    { type: 'paragraph', content: 'On lawn plans, every visit includes a graded assessment of your actual turf — density and coverage, weed cleanliness, color and nutrients, stress and damage — tracked visit over visit with progress photos, a mowing-height check, and recommendations. It all lands in your lawn report, right next to your pest visits.' },
    { type: 'paragraph', content: "Not on a lawn plan? You still get the Learn tab — local tips, articles, and FAQs written for Southwest Florida yards — and if you'd like us to take your lawn on, tap Request and we'll build a plan for your exact yard." },

    { type: 'heading', content: 'Whatever you need — just ask Waves AI' },
    { type: 'paragraph', content: 'The Waves AI bar at the top of every screen answers from your actual account, in plain English. Move a visit, get a quote for adding mosquito or lawn care, check what\'s included in your plan, make sense of a charge — or type "I\'m seeing ants again" and it will sort out what happens next. Prefer a human? Call and Text are one tap away in the same app.' },
    { type: 'image', src: `${IMG}/app-waves-ai.png`, alt: 'Waves AI bar — reschedule, explain a charge, or report a pest in plain English', width: 300, radius: 14 },
...
exports.down = async function down(knex) {
  const hasTables = await knex.schema.hasTable('email_templates')
    && await knex.schema.hasTable('email_template_versions');
  if (!hasTables) return;
  await upsertTemplate(knex, TEMPLATE_V2);
};
