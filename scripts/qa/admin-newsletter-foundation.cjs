...
async function main(){
 ...
 try{for(const width of [1440,390,820]){
  ...
  await page.route('**/*',async route=>{const request=route.request(),url=new URL(request.url()),p=url.pathname;
   ...
   else if(p==='/api/admin/newsletter/subscribers')body={subscribers:[{id:'subscriber-1',email:'avery@example.invalid',first_name:'Avery',last_name:'Example',status:'active',source:'manual',tags:[]}],counts:{active:3,total:3}};
