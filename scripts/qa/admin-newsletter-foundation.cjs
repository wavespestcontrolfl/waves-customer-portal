...
async function main(){
 ...
 try{for(const width of [1440,390,820]){
  ...
  await page.getByRole('button',{name:'Unsubscribe',exact:true}).first().click();const unsubscribeDialog=page.getByRole('dialog');await unsubscribeDialog.getByRole('heading',{name:'Unsubscribe subscriber?',exact:true}).waitFor();await capture('unsubscribe-dialog');await unsubscribeDialog.getByRole('button',{name:'Cancel',exact:true}).click();assert.ok(!report.requests.some(r=>r.method==='DELETE'));
