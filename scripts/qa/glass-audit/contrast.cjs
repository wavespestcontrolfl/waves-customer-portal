...
function sampleContrast(png, dpr, box, color) {
  ...
  const ratios = samples.map((bg) => ratio(fg, bg));
  const min = Math.min(...ratios);
  const avg = samples.reduce((a, s) => ({ r: a.r + s.r / samples.length, g: a.g + s.g / samples.length, b: a.b + s.b / samples.length }), { r: 0, g: 0, b: 0 });
  return { min: Math.round(min * 100) / 100, avg: Math.round(ratio(fg, avg) * 100) / 100, bg: `rgb(${Math.round(avg.r)}, ${Math.round(avg.g)}, ${Math.round(avg.b)})` };
}

module.exports = { decode, sampleContrast, parseColor, ratio };
