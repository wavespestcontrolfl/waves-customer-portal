...
/* global document, window, getComputedStyle, Element, HTMLElement */
...
function collectMetrics(opts) {
  ...
  const sel = (el) => {
    ...
    return `${el.tagName.toLowerCase()}${id}${cls}${dg}${acc}${gt}`;
  };
  const round = (n) => Math.round(n * 100) / 100;
  const box = (el) => { const r = el.getBoundingClientRect(); return { x: round(r.left), y: round(r.top), w: round(r.width), h: round(r.height) }; };
  ...
  const theme = {
    ...
    htmlBackground: html.style.background ? html.style.background.slice(0, 80) + '…' : getComputedStyle(html).backgroundImage.slice(0, 80),
  ...
  // ---- headings ----
  const headings = Array.from(document.querySelectorAll('h1, h2, h3, h4')).filter(visible).map((h) => {
  ...
  const eyebrows = Array.from(document.querySelectorAll('[data-gt="eyebrow"]')).filter(visible).map((e) => { const cs = getComputedStyle(e); return { size: parseFloat(cs.fontSize), weight: parseInt(cs.fontWeight, 10), ls: cs.letterSpacing, color: cs.color, tt: cs.textTransform, text: snippet(e) }; });

  // ---- glass surfaces ----
  const glassEls = Array.from(document.querySelectorAll('[data-glass], [data-glass-accent]')).filter(visible);
  const glass = glassEls.map((g) => {
    ...
    return {
      tier, sel: sel(g), tag: g.tagName.toLowerCase(), radius: cs.borderTopLeftRadius, bg: cs.backgroundColor, bgImage: cs.backgroundImage.slice(0, 60), border: `${cs.borderTopWidth} ${cs.borderTopStyle} ${cs.borderTopColor}`, backdrop: bf, shadow: cs.boxShadow.slice(0, 90), nested: !!parentGlass, nestedBlur: !!parentGlass && bf !== 'none', position: cs.position, box: box(g), padding: `${cs.paddingTop} ${cs.paddingRight} ${cs.paddingBottom} ${cs.paddingLeft}`, minHeight: cs.minHeight, height: round(g.getBoundingClientRect().height),
    };
  });
  const tierHist = {};
  const radiusByTier = {};
  ...
  const inputs = Array.from(document.querySelectorAll('input:not([type=hidden]):not([type=checkbox]):not([type=radio]), select, textarea')).filter(visible).map((i) => {
    ...
    let ph = null;
    try { const p = getComputedStyle(i, '::placeholder'); ph = { size: p.fontSize, color: p.color, style: p.fontStyle }; } catch (e) { /* ignore */ }
  ...
  const stickyTop = Array.from(document.querySelectorAll('*')).filter((el) => { const cs = getComputedStyle(el); return (cs.position === 'sticky' || cs.position === 'fixed') && parseFloat(cs.top) === 0 && visible(el) && el.getBoundingClientRect().width > vw * 0.5; }).map((el) => ({ sel: sel(el), h: round(el.getBoundingClientRect().height), pt: getComputedStyle(el).paddingTop, position: getComputedStyle(el).position }));
  const fixedBottom = Array.from(document.querySelectorAll('*')).filter((el) => { const cs = getComputedStyle(el); return cs.position === 'fixed' && parseFloat(cs.bottom) === 0 && visible(el) && el.getBoundingClientRect().width > vw * 0.5; }).map((el) => ({ sel: sel(el), h: round(el.getBoundingClientRect().height), pb: getComputedStyle(el).paddingBottom }));
  ...
  // ---- overlays ----
  const dialogs = Array.from(document.querySelectorAll('[role="dialog"], [aria-modal="true"]')).filter(visible).map((d) => { const cs = getComputedStyle(d); return { sel: sel(d), label: d.getAttribute('aria-label') || d.getAttribute('aria-labelledby'), glass: d.getAttribute('data-glass'), radius: cs.borderTopLeftRadius, box: box(d), backdrop: cs.backdropFilter || cs.webkitBackdropFilter }; });
  const scrims = Array.from(document.querySelectorAll('[data-glass-scrim]')).filter(visible).map((s) => { const cs = getComputedStyle(s); return { bg: cs.backgroundColor, bgImage: cs.backgroundImage.slice(0, 50), backdrop: cs.backdropFilter || cs.webkitBackdropFilter }; });
  const bodyLocked = getComputedStyle(document.body).overflow === 'hidden' || getComputedStyle(html).overflow === 'hidden';
