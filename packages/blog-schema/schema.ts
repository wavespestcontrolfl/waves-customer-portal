...
const PUBLIC_PROVIDER_ANSWER_RE =
  /^[\s|]*(?:\*\*)?(?:yes|sure|definitely|certainly|yeah|indeed|you bet)\b[^.!?|]{0,40}?\b(?:by|from|through|via|with)\s+(?:(?:only\s+)?(?:an?|the)\s+)?(?:county|city|state|counties|municipal\w*|government\w*|extension|public\s+(?:program|service)s?|other\s+(?:compan|provider|firm)\w*|competitors?|national\s+chains?|local\s+(?:compan|provider|firm)\w*)\b(?![^|]{0,80}?(?:\b(?:waves\w*|we|our\s+(?:team|techs?|technicians?|crews?|company|offices?|branch\w*|staff|pros?|specialists?))\b(?:(?!\b(?:not|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\b)[^|.!?]){0,40}?\b(?:(?:provid|offer|deliver|serv|servic|treat|cover|handl|spray|inspect|protect|perform|manag|run)\w*|do(?:es)?|did)\b\s+(?:it\b|this\b|that\b|them\b|those\b|(?:the|this|that)\s+(?:[\w-]+\s+){0,2}?(?:same|service|work|treatment|coverage|program|spray\w*|applications?)\b|(?:(?!(?:not|no|never)\b)[\w-]+\s+){0,2}?(?:pest|mosquito|termite|rodent|lawn|tree|shrub|bed.?bugs?|wdo|waveguard|exterminat\w+)\b)(?![^|.!?]{0,20}\b(?:in|near|around|throughout|across)\s+(?:(?:the|a|an|this|that|every|each|all|our|its)\s+)?(?!(?:years?|seasons?|summers?|winters?|springs?|falls?|autumns?|months?|weeks?|days?|nights?|mornings?|evenings?|afternoons?|process\w*|meantime|off-?season|peak|holidays?|storms?|rains?)\b)[\w])|\b(?:is|are|was|were|gets?|being|been)\s+(?:\w+\s+){0,2}?(?:administer|coordinat|provid|handl|run|perform|manag|operat|deliver|serv|servic|treat|cover|spray|inspect|protect|done)\w*\s+(?:(?!(?:not|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\b)[\w-]+\s+){0,2}?by\s+(?:waves\w*|us\b|our\s+(?:team|techs?|technicians?|crews?)\b|\{\{\s*brand\w*\s*\}\})))/i;
...
const WAVES_SERVICE_TAIL_RE = new RegExp(
  `\\b(?:waves\\w*|we|our\\s+(?:team|techs?|technicians?|crews?|company|offices?|branch\\w*|staff|pros?|specialists?))\\b(?:(?!\\b(?:not|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\\b)[^.!?|]){0,20}?\\b(?:(?:provid|offer|deliver|serv|servic|treat|cover|handl|spray|inspect|protect|perform|manag|run)\\w*|do(?:es)?|did)\\b\\s+(?:it\\b|this\\b|that\\b|them\\b|those\\b|(?:the|this|that)\\s+(?:[\\w-]+\\s+){0,2}?(?:same|service|work|treatment|coverage|program|spray\\w*|applications?)\\b|(?:(?!(?:not|no|never)\\b)[\\w-]+\\s+){0,2}?${SERVICE_KEYWORD_SOURCE})(?![^.!?|]{0,20}\\b(?:in|near|around|throughout|across)\\s+(?:the\\s+)?(?:${PLACE_NAME_SOURCE})\\b)`
  + `|\\b(?:it|this|that|they|(?:the|this|that)\\s+(?:service|work|treatment|coverage|program|spray\\w*|applications?)|${SERVICE_KEYWORD_SOURCE})\\s+(?:is|are|was|were|gets?|being|been)\\s+(?:(?!(?:not|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\\b)[\\w-]+\\s+){0,2}?(?:administer|coordinat|provid|handl|run|perform|manag|operat|deliver|serv|servic|treat|cover|spray|inspect|protect|done)\\w*\\s+(?:(?!(?:not|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\\b)[\\w-]+\\s+){0,2}?by\\s+(?:waves\\w*|us\\b|our\\s+(?:team|techs?|technicians?|crews?)\\b|\\{\\{\\s*brand\\w*\\s*\\}\\})(?![^.!?|]{0,20}\\b(?:in|near|around|throughout|across)\\s+(?:the\\s+)?(?:${PLACE_NAME_SOURCE})\\b)`,
  'i',
);
