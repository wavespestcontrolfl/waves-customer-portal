...
const SERVICE_CLAIM_PATTERN = new RegExp(
  ...
  + `|(?:we(?:'re| are)?|waves(?: pest control)?(?:'s|')?(?: is| are)?|our (?:team|techs?|technicians?|crews?)(?: is| are)?)\\s*here to help\\b(?!\\s+(?:[\\w.-]+\\s+){0,2}?(?:you|readers?|homeowners?|residents?)\\s+(?:understand|identify|learn|compare|decide|research|choose|spot)\\b)(?!\\s+(?:(?!(?:service|booking|emergency|scheduling|appointment|quote|order|job|ticket|dispatch|billing|payment)s?\\b)[\\w.-]+\\s+){0,2}?(?:questions?\\b|inquir(?:y|ies)\\b|faqs?\\b|answer\\w*\\s+(?:(?!(?:service|booking|emergency|scheduling|appointment|quote|order|job|ticket|dispatch|billing|payment)s?\\b)[\\w-]+\\s+){0,3}?(?:questions?|inquir(?:y|ies)|faqs?|myths?)\\b))`
  ...
  // (codex #406 r6).
  + `|(?:we|waves(?: pest control)?|waveguard|\\{\\{\\s*brand\\w*\\s*\\}\\})\\s+(?:says?|say|reports?|confirms?|notes?|adds?|explains?|claims?)\\s+(?:that\\s+)?(?:it|they|we)\\s+(?:also\\s+|still\\s+|now\\s+)?(?:manag(?:e|es)|runs?|operat(?:e|es)|coordinat(?:e|es)|administers?|provid(?:e|es)|offers?|delivers?|serv(?:e|es)|treats?|covers?|handles?)\\b(?!\\s+(?:(?!(?:not|never)\\b)[\\w-]+\\s+){0,2}?(?:no|none|zero)\\b)`
  + `|(?:we|waves(?: pest control)?|waveguard|\\{\\{\\s*brand\\w*\\s*\\}\\})(?:'re| are|'ll| will| can| could| do| does|'ve| have| has| had)?(?: been)?(?: currently| now| proudly| also| still)?(?: (?:says?|say|confirms?|notes?|adds?|explains?|reports?|claims?)(?: that)? (?:it|they|we)(?: also| still| now| proudly)?)? (?:offer|provid|deliver)\\w*\\s+(?:(?!(?:research|information|info|advice|guidance|tips|insights?|education|educational|resources?|articles?|guides?|content|news|about|on|regarding|of|for|to|no|none|zero|not|never)\\b)[a-z-]+\\s+){0,2}?(?:(?:pest|mosquito|termite|rodent|lawn|tree|shrub|bed.?bugs?|wdo)\\s+)?(?:control|care|treatment|service|plan|program|inspection|removal|exterminat|waveguard)\\w*\\b(?!\\s+(?:(?!(?:and|or|nor|plus|as)\\b)[\\w-]+\\s+){0,4}?(?:research|information|info|advice|guidance|tips|insights?|education|educational|resources?|articles?|guides?|content|news|facts?|myths?|history|reviews?|explainers?|breakdowns?|roundups?|comparisons?|checklists?|overviews?|overviews?|checklists?|comparisons?|roundups?|director(?:y|ies)|summar(?:y|ies)|glossar(?:y|ies)|calendars?|faqs?)\\b)`
  ...
  + `|(?:we|waves(?: pest control)?|waveguard|\\{\\{\\s*brand\\w*\\s*\\}\\})(?:'re| are|'ll| will| can| could| do| does|'ve| have| has| had)?(?: been)?(?: currently| now| proudly| also| still)?(?: (?:says?|say|confirms?|notes?|adds?|explains?|reports?|claims?)(?: that)? (?:it|they|we)(?: also| still| now| proudly)?)? (?:offer|provid|deliver)\\w*\\b(?:(?!\\b(?:about|regarding|concerning|on|for|director(?:y|ies)|lists?|overview|roundup|comparison|index|map|no|none|zero|not|never)\\b)[^.!?;]){0,40}?\\bservices?\\s+(?:in|near|throughout|across)\\b(?![^.!?]{0,30}\\bguides?\\b)`
...
const PUBLIC_PROVIDER_ANSWER_RE =
  /^[\s|]*(?:\*\*)?(?:yes|sure|definitely|certainly|yeah|indeed|you bet)\b[^.!?|]{0,40}?\b(?:by|from|through|via|with)\s+(?:(?:only\s+)?(?:an?|the)\s+)?(?:county|city|state|counties|municipal\w*|government\w*|extension|public\s+(?:program|service)s?|other\s+(?:compan|provider|firm)\w*|competitors?|national\s+chains?|local\s+(?:compan|provider|firm)\w*)\b(?![^|]{0,80}?(?:\b(?:waves\w*|we|our\s+(?:team|techs?|technicians?|crews?|company|offices?|branch\w*|staff|pros?|specialists?))\b(?:(?!\b(?:not(?!\s+(?:only|just)\b)|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\b)(?!\b(?:says?|said|notes?|noted|reports?|reported|explains?|explained|adds?|added|confirms?|confirmed|mentions?|mentioned|points?|clarif\w+|writes?|wrote|suggests?|believes?|thinks?|estimates?)\b(?!\s+(?:that\s+)?(?:it|they|we|waves\w*|\{\{\s*brand\w*\s*\}\})\b))[^|.!?]){0,40}?\b(?:(?:provid|offer|deliver|serv|servic|treat|cover|handl|spray|inspect|protect|perform|manag|run)\w*|do(?:es)?|did)\b\s+(?:it\b|this\b|that\b|them\b|those\b|(?:the|this|that)\s+(?:[\w-]+\s+){0,2}?(?:same|service|work|treatment|coverage|program|spray\w*|applications?)\b|(?:(?!(?:not|no|never)\b)[\w-]+\s+){0,2}?(?:pest|mosquito|termite|rodent|lawn|tree|shrub|bed.?bugs?|wdo|waveguard|exterminat\w+)\b)(?![^|.!?]{0,20}\b(?:in|near|around|throughout|across|within|inside)\s+(?:(?:the|a|an|this|that|every|each|all|our|its)\s+)?(?!(?:years?|seasons?|summers?|winters?|springs?|falls?|autumns?|months?|weeks?|days?|nights?|mornings?|evenings?|afternoons?|process\w*|meantime|off-?season|peak|holidays?|storms?|rains?)\b)[\w])|\b(?:is|are|was|were|gets?|being|been)\s+(?:\w+\s+){0,2}?(?:administer|coordinat|provid|handl|run|perform|manag|operat|deliver|serv|servic|treat|cover|spray|inspect|protect|done)\w*\s+(?:(?!(?:not(?!\s+(?:only|just)\b)|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\b)[\w-]+\s+){0,2}?by\s+(?:waves\w*|us\b|our\s+(?:team|techs?|technicians?|crews?)\b|\{\{\s*brand\w*\s*\}\})))/i;
...
const WAVES_SERVICE_TAIL_RE = new RegExp(
  `\\b(?:waves\\w*|we|our\\s+(?:team|techs?|technicians?|crews?|company|offices?|branch\\w*|staff|pros?|specialists?))\\b(?:(?!\\b(?:not(?!\\s+(?:only|just)\\b)|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\\b)(?!\\b(?:says?|said|notes?|noted|reports?|reported|explains?|explained|adds?|added|confirms?|confirmed|mentions?|mentioned|points?|clarif\\w+|writes?|wrote|suggests?|believes?|thinks?|estimates?)\\b(?!\\s+(?:that\\s+)?(?:it|they|we|waves\\w*|\\{\\{\\s*brand\\w*\\s*\\}\\})\\b))[^.!?|]){0,20}?\\b(?:(?:provid|offer|deliver|serv|servic|treat|cover|handl|spray|inspect|protect|perform|manag|run)\\w*|do(?:es)?|did)\\b\\s+(?:it\\b|this\\b|that\\b|them\\b|those\\b|(?:the|this|that)\\s+(?:[\\w-]+\\s+){0,2}?(?:same|service|work|treatment|coverage|program|spray\\w*|applications?)\\b|(?:(?!(?:not|no|never)\\b)[\\w-]+\\s+){0,2}?${SERVICE_KEYWORD_SOURCE})(?![^.!?|]{0,20}\\b(?:in|near|around|throughout|across|within|inside)\\s+(?:(?:the|our|its)\\s+)?(?:(?!(?:not|no|never)\\b)[\\w-]+\\s+){0,3}?(?:${PLACE_NAME_SOURCE}|(?:(?:service|coverage)\\s+)?(?:areas?|footprints?)(?:\\s+we\\s+serv\\w*)?|areas?\\s+we\\s+serv\\w*)\\b)`
  + `|\\b(?:it|this|that|they|(?:the|this|that)\\s+(?:service|work|treatment|coverage|program|spray\\w*|applications?)|${SERVICE_KEYWORD_SOURCE})\\s+(?:is|are|was|were|gets?|being|been)\\s+(?:(?!(?:not(?!\\s+(?:only|just)\\b)|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\\b)[\\w-]+\\s+){0,2}?(?:administer|coordinat|provid|handl|run|perform|manag|operat|deliver|serv|servic|treat|cover|spray|inspect|protect|done)\\w*\\s+(?:(?!(?:not(?!\\s+(?:only|just)\\b)|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\\b)[\\w-]+\\s+){0,2}?by\\s+(?:waves\\w*|us\\b|our\\s+(?:team|techs?|technicians?|crews?)\\b|\\{\\{\\s*brand\\w*\\s*\\}\\})(?![^.!?|]{0,20}\\b(?:in|near|around|throughout|across|within|inside)\\s+(?:(?:the|our|its)\\s+)?(?:(?!(?:not|no|never)\\b)[\\w-]+\\s+){0,3}?(?:${PLACE_NAME_SOURCE}|(?:(?:service|coverage)\\s+)?(?:areas?|footprints?)(?:\\s+we\\s+serv\\w*)?|areas?\\s+we\\s+serv\\w*)\\b)`,
  'i',
);
...
export function validateServiceAreaClaims(
  ...
  const body = body_mdx
    ...
    // company leadership says …" stays armed) (codex #405 r2).
    .replace(/((?<!\b(?:[Ww]aves|[Oo]ur|[Ww]e)\b[^.!?"\u201c]{0,40})(?<!\}\}[^.!?"\u201c]{0,40})\b(?:[Cc]ompan(?:y|ies)|[Pp]roviders?|[Ff]irms?|[Cc]ompetitors?|[Cc]hains?|[Bb]usiness(?:es)?)\s+(?:(?!(?:Waves|CEOs?|CFOs?|COOs?|CTOs?|VPs?|President|Presidents|Owner|Owners|Founder|Founders|Manager|Managers|Director|Directors|Leadership|Team|Teams|Staff|Spokesperson|Spokespeople|Spokesman|Spokeswoman|Rep|Reps|Representative|Representatives|Executive|Executives|Official|Officials|Leader|Leaders|Management|Headquarters|HQ|Marketing|Sales|Support)\b)(?![Ww][Aa][Vv][Ee][Ss])(?![Ww][Aa][Vv][Ee][Gg][Uu][Aa][Rr][Dd])[A-Z][\w&.'-]*\s+){1,2}?(?:[Ss]ays?|[Ss]aid|[Cc]laims?|[Cc]laimed|[Ww]rites?|[Ww]rote|[Nn]otes?|[Nn]oted|[Aa]dvertises?|[Aa]dvertised|[Bb]oasts?|[Bb]oasted|[Aa]rgues?|[Aa]rgued|[Ii]nsists?|[Ii]nsisted|[Tt]ells?|[Tt]old)\b[^.!?"\u201c]{0,25}["\u201c])([^"\u201d]{0,120})(["\u201d])(?![^.!?]{0,40}\b(?:[Aa]nd so do we|[Ss]o do we|[Ww]e do too|[Aa]s do we|including (?:us|[Ww]aves)|[Ss]ame here|[Ss]o does [Ww]aves|[Aa]s does [Ww]aves|[Ww]aves (?:does|do) (?:too|the same))\b)/g, '$1…$3')
    ...
    if (/^\|.+/.test(trimmed) || (trimmed.includes('|') && (sepLike(nextTrimmed) || sepLike(trimmed) || tableIntro))) {
      ...
      if (sepLike(nextTrimmed)) {
        ...
        if (/\?/.test(trimmed)
          ...
          && !WAVES_SERVICE_TAIL_RE.test((allSegments[segIndex + 2] || '').trim())) {
  ...
  scanUnits.forEach((segment, unitIndex) => {
    for (const part of segment.split(/(?<=[.!?])(?<!\bSt\.)(?<!\bFt\.)(?<!\bMt\.)(?<!\b[eE]\.[gG]\.)(?<!\b[iI]\.[eE]\.)(?<!\bvs\.)\s+/)) {
      sentences.push(part);
  ...
  for (let sentenceIndex = 0; sentenceIndex < sentences.length; sentenceIndex += 1) {
    ...
    if (/^\s*(?:#{1,6}\s+|[-*+]\s+|>\s+|\d+\.\s+|\*\*)?(?:(?:do|does|can|could|will|would|is|are)\s+(?:you|your\s+\w+|waves\w*)\b(?:\b(?:St|Ft|Mt)\.|[^.!?])*\b(?:serv|treat|cover|visit|spray|inspect|protect|handl|exterminat|work|operat|available|run|have|has|carry|offer|provid)\w*|(?:would|will|can|could|do|does|is|are|was|were|should|might|may)\s+(?!(?:(?!\bbe\b)(?:\b(?:St|Ft|Mt)\.|[^.!?]))*?\b(?:municipal|county|city|government\w*|public|state-run|county-run)\b)(?:\b(?:St|Ft|Mt)\.|[^.!?])*?\bbe\s+(?:provid|offer|avail|cover|servic|treat|handl|spray|inspect|protect|deliver)\w*)(?:\b(?:St|Ft|Mt)\.|[^.!?])*\?\**\s*$/i.test(faqProbe)
      && !/\b(?:yourself|your own|diy)\b/i.test(faqProbe)
      && /^\s*(?:(?:yes|sure|definitely|certainly|yeah|indeed|you bet)\b(?!\s*,?\s*(?:but\s+)?not\b)(?![^.!?]{0,40}\b(?:local|another|different)\s+(?:provider|compan\w*|firm|exterminator)\w*\b)(?![^.!?]{0,50}\b(?:but\s+not\s+(?:from|by|through|with|us|waves)\b|not\s+from\s+(?:waves|us)|but\s+(?:choose|contact|call|try)\b|(?:choose|contact|call|try)\s+a\s+local)\b)(?![^.!?]{0,40}\b(?:by|from|through|via|with)\s+(?:(?:only\s+)?(?:an?|the)\s+)?(?:county|city|state|counties|municipal\w*|government\w*|extension|public\s+(?:program|service)s?|other\s+(?:compan|provider|firm)\w*|competitors?|national\s+chains?|local\s+(?:compan|provider|firm)\w*)\b(?![^|]{0,80}?(?:\b(?:waves\w*|we|our\s+(?:team|techs?|technicians?|crews?|company|offices?|branch\w*|staff|pros?|specialists?))\b(?:(?!\b(?:not(?!\s+(?:only|just)\b)|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\b)(?!\b(?:says?|said|notes?|noted|reports?|reported|explains?|explained|adds?|added|confirms?|confirmed|mentions?|mentioned|points?|clarif\w+|writes?|wrote|suggests?|believes?|thinks?|estimates?)\b(?!\s+(?:that\s+)?(?:it|they|we|waves\w*|\{\{\s*brand\w*\s*\}\})\b))[^|.!?]){0,40}?\b(?:(?:provid|offer|deliver|serv|servic|treat|cover|handl|spray|inspect|protect|perform|manag|run)\w*|do(?:es)?|did)\b\s+(?:it\b|this\b|that\b|them\b|those\b|(?:the|this|that)\s+(?:[\w-]+\s+){0,2}?(?:same|service|work|treatment|coverage|program|spray\w*|applications?)\b|(?:(?!(?:not|no|never)\b)[\w-]+\s+){0,2}?(?:pest|mosquito|termite|rodent|lawn|tree|shrub|bed.?bugs?|wdo|waveguard|exterminat\w+)\b)(?![^|.!?]{0,20}\b(?:in|near|around|throughout|across|within|inside)\s+(?:(?:the|a|an|this|that|every|each|all|our|its)\s+)?(?!(?:years?|seasons?|summers?|winters?|springs?|falls?|autumns?|months?|weeks?|days?|nights?|mornings?|evenings?|afternoons?|process\w*|meantime|off-?season|peak|holidays?|storms?|rains?)\b)[\w])|\b(?:is|are|was|were|gets?|being|been)\s+(?:\w+\s+){0,2}?(?:administer|coordinat|provid|handl|run|perform|manag|operat|deliver|serv|servic|treat|cover|spray|inspect|protect|done)\w*\s+(?:(?!(?:not(?!\s+(?:only|just)\b)|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\b)[\w-]+\s+){0,2}?by\s+(?:waves\w*|us\b|our\s+(?:team|techs?|technicians?|crews?)\b|\{\{\s*brand\w*\s*\}\}))))|absolutely\b|of course\b|yep\b|we (?:do|are|can|sure do|sure can)\b|no,?\s+but\s+(?:we|waves\w*)\s+(?:can|do|will|might)\s+(?:help|assist|try)\b(?![^.!?]{0,30}\b(?:in|near|around|throughout)\s+\w)(?![^.!?]{0,40}\b(?:local|another|different)\s+(?:provider|compan\w*|firm|exterminator|pro(?:fessional)?|option)s?\b)(?![^.!?]{0,40}\breferrals?\b)(?![^.!?]{0,40}\b(?:someone|somebody)\b)(?![^.!?]{0,40}\b(?:find|choose|pick|select)\s+(?:you\s+)?a\s+(?:provider|compan\w*|pro(?:fessional)?)\b)(?![^.!?]{0,40}\b(?:understand|learn|compare|decide|research|identify|explain|review|answer\w*|questions?)\b)|no,?\s+but\s+(?:we|waves\w*)\b(?![^.!?]{0,40}\b(?:in|near|around|throughout)\s+\w)[^.!?]{0,40}?\b(?:by request|on request|upon request|if needed|when needed|as needed|case by case)\b|no (?:problem|worries|sweat)\b|no (?:appointment|contract|subscription)s?\s+(?:needed|required|necessary)\b|they (?:do|are)\b)/i.test(normalizeApostrophes(sentences[sentenceIndex + 1] ?? '').replace(/<[^>]+>/g, ' ').replace(/^[\s*_~`>#-]+/, ''))) {
      sentence = faqProbe
        .replace(/\b(?:do|does|can|could|will|would)\s+(?:you|your\s+\w+|waves\w*)\s+(?:have|carry)\s+/i, 'we offer ')
        .replace(/\b(?:do|does|can|could|will|would|is|are)\s+(?:you|your\s+\w+|waves\w*)\s+/i, 'our team ')
        ...
        .replace(/\b(?:would|will|can|could|do|does|is|are|was|were|should|might|may)\s+((?:\b(?:St|Ft|Mt)\.|[^.!?])*?)\s+be\s+((?:provid|offer|avail|cover|servic|treat|handl|spray|inspect|protect|deliver)\w*)/i, '$1 is $2');
    }
    ...
    if (/^\s*(?:#{1,6}\s+|[-*+]\s+|>\s+|\d+\.\s+|\*\*)?(?:do|does|did|can|could|will|would|should|is|are|was|were)\b(?:\b(?:St|Ft|Mt)\.|[^.!?])*\?\**\s*$/i.test(faqProbe)
      ...
      let wavesTail = WAVES_SERVICE_TAIL_RE.test(
        normalizeApostrophes(sentences[sentenceIndex + 1] ?? '').replace(/<[^>]+>/g, ' ').replace(/^[\s*_~`>#-]+/, ''));
      // The whole answer span is scanned — a fixed bound let a Waves
      // attribution hide behind neutral sentences (codex #405 r16).
      for (let k = sentenceIndex + 2; k < sentences.length; k += 1) {
        if (sentenceUnit[k] !== sentenceUnit[sentenceIndex + 1]) break;
        // A topic pivot starts a new subject — the answer span ends there
        // (codex #406 r5).
        ...
        if (WAVES_SERVICE_TAIL_RE.test(follow)) {
          wavesTail = true;
          break;
        }
      }
      ...
      for (const city of NON_SERVICE_AREA_CITIES) {
        ...
        for (const cityMatch of normalized.matchAll(new RegExp(`\\b${source}\\b`, 'gi'))) {
          ...
          if (/ County$/.test(city)
            ...
            // Waves") do not break the attribution (codex #404 r11).
            && !/^[^.!?]{0,60}?\b(?:handled|performed|managed|covered|treated|serviced|delivered|done|provided|run|operated)\s+(?:(?!(?:not(?!\s+(?:only|just)\b)|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\b)[\w-]+\s+){0,2}?by\s+(?:waves\w*|us\b|our\s+(?:team|techs?|technicians?|crews?)\b|\{\{\s*brand\w*\s*\}\})/i.test(normalized.slice(cityEnd))
            ...
            && !/^[^.!?]{0,60}?(?:\b(?:waves\w*|we|our\s+(?:team|techs?|technicians?|crews?))\b|\{\{\s*brand\w*\s*\}\})(?:(?!\b(?:not(?!\s+(?:only|just)\b)|never|no|neither|nor|hardly|rarely|cannot|can'?t|won'?t|wouldn'?t|couldn'?t|shouldn'?t|doesn'?t|don'?t|didn'?t|isn'?t|aren'?t|wasn'?t|weren'?t|unable|fails?|failed)\b)(?!\b(?:says?|said|notes?|noted|reports?|reported|explains?|explained|adds?|added|confirms?|confirmed|mentions?|mentioned|points?|clarif\w+|writes?|wrote|suggests?|believes?|thinks?|estimates?)\b(?!\s+(?:that\s+)?(?:it|they|we|waves\w*|\{\{\s*brand\w*\s*\}\})\b))[^.!?]){0,40}?\b(?:provid|offer|deliver|serv|servic|treat|cover|handl|spray|inspect|protect|perform|manag|run|operat|coordinat|administer)\w*\b(?!\s+(?:(?!(?:not|never)\b)[\w-]+\s+){0,2}?(?:no|none|zero)\b)/i.test(normalized.slice(cityEnd))
