...
Cross-provider routing: `MODEL_OPENAI_BEST` (default `gpt-5.5`), `MODEL_GEMINI_VISION` (default `gemini-3.5-flash`), `GEMINI_VISION_MODEL` (per-service vision override). These best models are LIVE directly (lead-triage, knowledge Q&A, estimate assistant → GPT-5.5; lawn-assessment + satellite-analyzer vision → Gemini 3.5 Flash), each with an automatic Claude/Gemini-2.5 fallback at the call site. No `GATE_OPENAI_*` / `GATE_GEMINI_VISION_PRIMARY` / shadow flags — the competing-model (`ai_model_comparisons`) machinery was removed.
