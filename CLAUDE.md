...
18. **Contracts get deliberate treatment; internals don't.** Identify consumers before touching a boundary: DB schema and CHECK-constrained values, public `/:token` routes, the Stripe webhook, endpoints consumed by the iOS and Android apps (push/app-link payloads included), Astro spoke-fleet form posts and feeds, the retained V1 named exports, persisted identifiers. Inside a route handler or component, improvise freely.
19. **Remove the code you supersede.** When replacing behavior, delete the obsolete helper, flag read, export, or branch in the same PR unless compatibility requires keeping it (whole-file deletions still follow rule 5 — flag them in the PR body instead). State in the PR what became obsolete.
