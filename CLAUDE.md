...
15. **Find the existing mechanism before building one.** Search for the nearest existing implementation first. Canonical paths that must not grow siblings: surcharge math = `computeChargeAmount` (`server/services/stripe-pricing.js`), tracker state = `server/services/track-transitions.js`, ET wall-clock = `server/utils/datetime-et.js`, admin auth = `router.use(adminAuthenticate, …)` from `server/middleware/admin-auth.js`, UI feature flags = `useFeatureFlag` (`client/src/hooks/useFeatureFlag.js`, client layer only), server behavior gates = `GATE_*` env flags via `server/config/feature-gates.js` (separate mechanisms — don't use one for the other's layer), Intelligence Bar tools = `TOOLS` + `executeTool`. Extending the canonical path beats a working local duplicate.
