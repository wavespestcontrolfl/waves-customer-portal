...
Affiliate links (blog monetization pilot): `GATE_AFFILIATE_LINKS` — strict opt-in; blog bodies reference owner-approved products by id through `<AffiliateLink product="…">`, resolved against the vendored `packages/affiliate-registry` (source of truth in the astro repo; `npm run sync:affiliate-registry`). Gate off = every AffiliateLink is a P0 and the lane is dark; raw tracking URLs are always `DISALLOWED_EXTERNAL_LINK`; newsletter/social strip affiliate material regardless of the gate. Risk classes, page eligibility, disclosure, density, and refresh rules are in `content-guardrails.js`; every draft carrying `<AffiliateLink>` parks as `affiliate_review` (approve via `approve-autonomous-run.js`; the PR poller withholds auto-merge without that stamp); playbook in `docs/affiliate-links-pilot.md`.
