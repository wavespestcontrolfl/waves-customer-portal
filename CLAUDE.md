...
- **AI:** Anthropic Claude API. **Never hardcode model IDs** — import `FLAGSHIP` / `WORKHORSE` / `FAST` / `VOICE` / `VISION` from `server/config/models.js`. These are **quality tiers, not cost tiers** (owner directive: best model regardless of cost). The reasoning tiers (FLAGSHIP / WORKHORSE / FAST) resolve to `claude-opus-4-8`. `VOICE` resolves to `claude-sonnet-4-6` — customer-facing copy (SMS replies, service recaps, social posts) where a warm, natural voice beats raw reasoning; high-stakes messages (cancellations, complaints) escalate to FLAGSHIP at the call site. `VISION` also resolves to `claude-sonnet-4-6` because the Opus line removed the `temperature` parameter and image scoring needs it. Tiers can be swapped via `MODEL_FLAGSHIP` / `MODEL_WORKHORSE` / `MODEL_FAST` / `MODEL_VOICE` / `MODEL_VISION` env vars with no code change. **Cross-provider routing:** some features route to OpenAI/Gemini via the `ROUTES` map in `models.js`, dispatched through the shared `server/services/llm/call.js` (one fail-closed place for OpenAI Responses / Gemini generateContent / Anthropic SDK). PR1 wires lead-triage classification (`GATE_OPENAI_LEAD_TRIAGE`) and knowledge-base Q&A (`GATE_OPENAI_KNOWLEDGE`) to GPT-5.5, plus a flag-gated Gemini vision upgrade (`GATE_GEMINI_VISION_PRIMARY`) — **all default OFF, with the Anthropic path retained as fallback**. Cross-provider model defaults: `MODEL_OPENAI_BEST` (gpt-5.5), `MODEL_GEMINI_VISION` (gemini-3.5-flash). The 9 managed agents stay on Anthropic (Managed Agents API has no OpenAI equivalent). **Phase 2** shadow-tests `estimate-assistant` (`GATE_SHADOW_ESTIMATE_OPENAI`) and `call-extraction` (`GATE_SHADOW_CALLEXTRACT_OPENAI`) on OpenAI — the candidate runs in parallel, is logged to the `ai_model_comparisons` table with a deterministic agreement signal (via `server/services/model-comparison-log.js`), and **never drives customer output or call routing**; both default OFF, accruing data for the judge. **Phase 3** adds a nightly LLM judge (`GATE_MODEL_JUDGE`) over `ai_model_comparisons` + a readiness bar (`MODEL_GRAD_*` thresholds in `model-comparison-graduation.js`); when a candidate clears it a **deduped admin bell alert** fires (`model_ready_for_promotion`), and the owner promotes it via the gated `promote_model_provider` Intelligence Bar tool — which writes `model_provider_modes` (DB-backed, audited) so the candidate becomes the live provider with the prior one as **automatic fallback** (resolved by `server/services/live-provider.js`; **no row = baseline = today's behavior**). Promotion is never automatic and re-checks the bar server-side. `tax-advisor` remains deferred (needs an OpenAI tool-use loop + web-search path).
...
Cross-provider routing: `MODEL_OPENAI_BEST` (default `gpt-5.5`), `MODEL_GEMINI_VISION` (default `gemini-3.5-flash`); feature flags `GATE_OPENAI_LEAD_TRIAGE` / `GATE_OPENAI_KNOWLEDGE` / `GATE_GEMINI_VISION_PRIMARY` (all default off — Anthropic path is the fallback). Phase 2 shadow flags (default off; candidate logged to `ai_model_comparisons`, never live): `GATE_SHADOW_ESTIMATE_OPENAI`, `GATE_SHADOW_CALLEXTRACT_OPENAI`. Phase 3 (judge + flip): `GATE_MODEL_JUDGE` (nightly judge; prod opt-in); promotion bar `MODEL_GRAD_MIN_JUDGED` (150) / `MODEL_GRAD_MIN_WINRATE` (0.90) / `MODEL_GRAD_RECENT` (50) / `MODEL_GRAD_MAX_UNSAFE` (0) / `MODEL_JUDGE_BATCH` (30). The live-provider flip is **DB-backed** (`model_provider_modes`), flipped only via the gated `promote_model_provider` IB tool — no env flag.
