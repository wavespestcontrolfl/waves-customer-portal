...
18. **Contracts get deliberate treatment; internals don't.** Identify consumers before touching a boundary: DB schema and CHECK-constrained values, public `/:token` routes, every inbound webhook payload shape — the provider owns the contract, so this covers any webhook route mounted in `server/index.js` (today: Stripe, Twilio, SendGrid, Resend, Bouncie, the ElevenLabs voice-agent), including ones added after this list, endpoints consumed by the iOS and Android apps (push/app-link payloads included), Astro spoke-fleet form posts and feeds, the retained V1 named exports, persisted identifiers. Internal shapes (a handler's private helpers, local state, query structure) may change without ceremony once their consumers are migrated — but every repository-wide correctness and security rule (AGENTS.md P0/P1: auth, parameterized SQL, surcharge math, ET handling, status constraints) still applies inside handlers.
...
Call commitments (dark): `GATE_CALL_COMMITMENTS` — every processed call records what Waves promised and what the caller agreed to as evidence-linked `call_commitments` rows (seeded from the V2 extraction plus one bounded FLAGSHIP model pass over the transcript), links each promise to the later record that fulfils it, and surfaces it all in the Calls tab "Call intelligence" panel. Off (default) → nothing is written; rows already recorded stay visible. The model leg reads `CALL_PROC_EXTRACT_TIMEOUT_MS` and is counted in the claim ceiling. See `docs/call-intelligence-audit-and-expansion.md`.
