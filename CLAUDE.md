...
Cross-provider routing: `MODEL_OPENAI_BEST` (default `gpt-5.5`), `MODEL_GEMINI_VISION` (default `gemini-3.5-flash`); feature flags `GATE_OPENAI_LEAD_TRIAGE` / `GATE_OPENAI_KNOWLEDGE` / `GATE_GEMINI_VISION_PRIMARY` (all default off — Anthropic path is the fallback).
