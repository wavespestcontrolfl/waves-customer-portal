...
Content publish gates: `GATE_COMPLIANCE` — the semantic compliance gate, an LLM second layer for `REENTRY_SAFETY_CLAIM` / `BANNED_TOPIC` catching the paraphrases regex cannot express. **Ships DARK: off unless set to exactly `true`.** It P0-blocks a publish, so calibrate before enabling — `node ops/agents/compliance-gate-eval.js --all` (live API keys required) reports recall/precision over the labelled corpus 23 Codex rounds produced. Once on, `GATE_COMPLIANCE=false` is the revoke. The regex layer in `content-guardrails.js` is unaffected either way and stays fail-closed. Also `MODEL_COMPLIANCE` (swaps the Anthropic leg only; OpenAI fallback stays) and `COMPLIANCE_TIMEOUT_MS` (default 60000, no retry — same ceiling as the fact-check gate).
