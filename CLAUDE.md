...
**Customers — Customer 360 detail (landed PR #4c):** `client/src/components/admin/Customer360ProfileV2.jsx` replaces the V1 slide-out when the `customers-v2` flag is on — `CustomersPageV2` imports it as `Customer360Profile` (alias import, no JSX changes). V1 `CustomersPage` still renders the V1 `Customer360Profile`. Strict 1:1 with V1 on endpoints (GET `/admin/customers/:id`, `/admin/customers/:id/timeline`, `/admin/customers/:id/autopay-state`; POST `/admin/communications/send-sms`, `/admin/customers/:id/refund`, `/admin/customers/:id/charge-now`), state (`data`/`loading`/`activeTab`/`timelineFilter`/`smsReply`/`sendingSms`), 6-tab order (Overview / Services / Billing / Comms / Property / Compliance), slide-out overlay chrome (z-1000, ESC handler, click-outside-to-close), alert chips (overdue balance, expiring card), and autopay panel flow. V2 uses `components/ui` primitives, Tailwind zinc ramp, and reserves `alert-fg` for: health score <40, overdue balance, expiring card, payment refund/failed, invoice unpaid, at_risk/churned StageBadge. `HealthCircle` collapses to zinc-200 track + zinc-900 stroke (alert-fg only when score<40). `RadarChart` uses zinc-200 grid + zinc-900 polygon at 10% fill (no teal). `TierBadgeV2` → neutral Badge (purple/gold/teal dropped). SMS thread: outbound = zinc-900 bg + white text; inbound = zinc-50 + zinc-900. Timeline Badges neutral (no color-coded event types). Mobile still hides `.c360-header-actions` + renders `<CustomerActionBar standalone />` above safe-area.
...
**Mobile-first admin shell (landed PR #Mobile-2):** Admin is now mobile-first — phone is the primary surface, desktop is the fallback. Rolled out behind the `mobile-shell-v2` feature flag. When flag is ON and viewport < 768px, `AdminLayout` swaps the legacy hamburger + 240px drawer for `<MobileAdminShell />` — a fixed 56px top bar (Waves wordmark left, 44px Sparkles command trigger + notification bell right) + fixed 56px persistent bottom tab bar (Dashboard / Schedule / Customers / Messages / More). Total chrome: 112px. Safe-area-inset-bottom honored. Desktop renders exactly as before (no visual change flag-on). The 5-tab rule is strict — everything else lives under `/admin/more` (`client/src/pages/admin/MorePage.jsx`: 7 sections, ~24 items, 56px rows, lucide icons + ChevronRight). Command palette (`GlobalCommandPalette.jsx`) was refactored to `forwardRef` + `useImperativeHandle` so the shell's Sparkles button can call `.open()`; on mobile it renders a full-height bottom sheet (drag-handle, swipe-down-to-close > 120px, auto-focus input, recents-first from `admin_ib_recents` localStorage key max 5, quick-action pills). `⌘K` keybinding + centered modal still work on desktop. `<StickyActionBar />` (`client/src/components/admin/StickyActionBar.jsx`) is the per-detail-view action convention: equal-width icon+label columns, sits above the tab bar (z-30, bottom=56+safe-area) or above overlays when `standalone=true` (z-[1001], bottom=safe-area). Variants: `CustomerActionBar` (Text/Call/Book/Invoice), `EstimateActionBar`, `JobActionBar`, `InvoiceActionBar`. `Customer360Profile` migrated to `CustomerActionBar standalone` — legacy sticky-top pills hidden below 768px to avoid two conventions. Audit-flagged broken routes fixed: `DispatchPageV2` outer padding `p-4 md:p-6`; `WeekViewV2` + `MonthViewV2` in `CalendarViewsV2.jsx` wrapped in horizontal-scroll with `min-w-[700px]` so the 7-col grid contains instead of overflowing; `PricingStrategyPage` bumped its `isMobile` threshold from 640→768 and converted the Conversion Funnel from flex to 5-col grid with mobile-scaled typography; `LawnAssessmentPanel` gained an `isMobile` gate — photo grid 3→2 cols, history metric row converted to 5-col grid. Feature flag `mobile-shell-v2` added to `KNOWN_FLAGS` in `_DesignSystemFlagsPage.jsx`; toggle per-user at `/admin/_design-system/flags`. Flag-off users (including all desktop users) keep the current UX untouched.
...
**Communications — remaining tabs (landed PR #6b):** `CommunicationsPageV2` now renders V2 versions of every tab — `CallLogTabV2` (`client/src/pages/admin/CallLogTabV2.jsx`), `SmsTemplatesTabV2` + `CSRCoachTabV2` (both named-exported from `client/src/pages/admin/CommunicationsTabsV2.jsx`), `EmailAutomationsPanelV2` (`client/src/pages/admin/EmailAutomationsPanelV2.jsx`), and `PushSettingsV2` (`client/src/components/admin/PushSettingsV2.jsx`). Strict 1:1 with V1 on endpoints (GET `/ai/admin/calls`, POST `/admin/communications/call`, PUT `/admin/call-recordings/calls/:id/disposition`; GET/PUT `/admin/sms-templates`; GET `/admin/csr/overview` + `/follow-up-tasks` + `/weekly-recommendation` + `/leaderboard` + `/lead-quality`, PUT `/admin/csr/follow-up-tasks/:id`; GET `/admin/email-automations/automations` + `/stats` + `/log`, POST `/admin/email-automations/trigger`, PUT `/admin/email-automations/automations/:key`; GET/PUT `/admin/push/preferences` + device push helpers), state machines, filter sets, and CRUD flows. `CALL_DISPOSITIONS` and `NUMBER_LABEL_MAP` are now also named-exported from `CommunicationsPage.jsx` so V1 and V2 share the source. V2 uses `components/ui` primitives (`Button`, `Badge`, `Card`, `Switch`, `Table`, `Textarea`, `Input`, `Select`, `cn`), Tailwind zinc ramp, and reserves `alert-fg` for: Missed call stat + row accent + Call Back button (CallLog); CSR booking/score/follow-up rates below threshold + overdue follow-up counts + fixable-loss banner + CSR-attributable loss reason bars (CSRCoach); Beehiiv-not-configured badge + Failed automation status badge (Email); priority=urgent preference dot + push-error banner + Disable button (Push). V1 `CallLogTab` / `SmsTemplatesTab` / `CSRCoachTab` stay inline-defined in `CommunicationsPage.jsx`; `EmailAutomationsPanel` + `PushSettings` still ship for flag-off users.
