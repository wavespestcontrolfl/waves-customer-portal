...
14. **Find the existing mechanism before building one.** Search for the nearest existing implementation first. Canonical paths that must not grow siblings: surcharge math = `computeChargeAmount` (`server/services/stripe-pricing.js`), tracker state = `server/services/track-transitions.js`, ET wall-clock = `server/utils/datetime-et.js`, admin auth = `router.use(adminAuthenticate, …)` from `server/middleware/admin-auth.js`, feature flags = `useFeatureFlag`, Intelligence Bar tools = `TOOLS` + `executeTool`. Extending the canonical path beats a working local duplicate.
15. **Make the smallest coherent change.** Prefer the shared mechanism over a local workaround, but don't generalize past the demonstrated requirement: no single-use wrappers, factories, or config, and no release machinery (compat layers, dual-writes, fallback paths) beyond the standard `GATE_*` dark-ship pattern unless a specific deployment risk is named.
16. **Trace the change end-to-end before claiming done.** UI → route → service → DB → response → UI state, plus side effects (webhooks, crons, SMS, cache, analytics). Verify column names, APIs, and env vars against the code — never invent them.
17. **Contracts get deliberate treatment; internals don't.** Identify consumers before touching a boundary: DB schema and CHECK-constrained values, public `/:token` routes, the Stripe webhook, iOS-consumed endpoints, the retained V1 named exports, persisted identifiers. Inside a route handler or component, improvise freely.
18. **Remove the code you supersede.** When replacing behavior, delete the obsolete helper, flag read, export, or branch in the same PR unless compatibility requires keeping it (whole-file deletions still follow rule 5 — flag them in the PR body instead). State in the PR what became obsolete.
19. **Judge complexity qualitatively, not numerically.** A simplification must remove decisions (dedupe checks, delete dead branches, table-drive repeated conditionals), not relocate them into one-use helpers to lower a metric. Tests follow the same bar: protect behavior, contracts, and regressions — not implementation details or coverage percentages.
20. **Hands-off + exception-based by default.** Deterministic green checks auto-apply with an audit trail and no bell; only exceptions park and surface; every lane keeps an env kill switch + one-click revoke. Where a human decision is genuinely required, use the email-reply approval pattern — a tokenized email to contact@ answered by replying "approved"/"not approved", fail-closed, at-most-once (current in-repo exemplar: the newsletter proof flow in `server/services/newsletter-proof.js`; a generalized module is in flight on PR #3024) — never a park-on-portal-visit queue. Never extend email-approval to customer comms, money movement, or gate flips.
