...
Affiliate links (blog monetization pilot): `CONTENT_AFFILIATE_LINKS` — comma-separated EXACT partner URLs (tracking tag included, never a hostname) permitted in blog bodies only; empty (the default) = lane OFF, and clearing it is the kill switch. Disclosure, meta, and refresh rules are P0s in `content-guardrails.js`; procedure + astro-side follow-ups in `docs/affiliate-links-pilot.md`.
