...
10. **Plan first for non-trivial work.** For anything beyond a small, well-specified change, present a plan and get sign-off before writing code. An already-approved plan does not need another sign-off for unchanged implementation steps. Reconfirm a material change in effect, environment, scope, or risk. Investigation and proposal-only work may proceed within their authorized scope. Sign-off is for product decisions (design, functionality, intended behavior). Technical execution decisions — how to fix a review finding, whether to split a PR, when to re-tag Codex, merging a clean non-blast-radius PR — are the agent's to make under the `waves-ship` gate and are never brought back as questions (owner ruling 2026-09-05).
