...
- Before startup, verify the selected checkout, command hooks, database
  ...
  Use `npm run dev:client` for frontend-only verification. Full startup
  requires a verified dev/preview database and isolated integrations;
  `npm run dev` currently runs migrations through `predev`.
