...
18. **Contracts get deliberate treatment; internals don't.** Identify consumers before touching a boundary: DB schema and CHECK-constrained values, public `/:token` routes, every inbound webhook payload shape (Stripe, Twilio, SendGrid, Resend, Bouncie — the provider owns the contract), endpoints consumed by the iOS and Android apps (push/app-link payloads included), Astro spoke-fleet form posts and feeds, the retained V1 named exports, persisted identifiers. Inside a route handler or component, improvise freely.
