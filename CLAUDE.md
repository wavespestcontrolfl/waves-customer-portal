...
Cross-provider routing: `MODEL_OPENAI_BEST` (default `gpt-5.5`), `MODEL_GEMINI_VISION` (default `gemini-3.5-flash`); feature flags `GATE_OPENAI_LEAD_TRIAGE` / `GATE_OPENAI_KNOWLEDGE` / `GATE_GEMINI_VISION_PRIMARY` (all default off — Anthropic path is the fallback). Phase 2 shadow flags (default off; candidate logged to `ai_model_comparisons`, never live): `GATE_SHADOW_ESTIMATE_OPENAI`, `GATE_SHADOW_CALLEXTRACT_OPENAI`.
