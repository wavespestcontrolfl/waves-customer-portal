...
14. **Find the existing mechanism before building one.** Search for the nearest existing implementation first. Canonical paths that must not grow siblings: surcharge math = `computeChargeAmount` (`server/services/stripe.js`), tracker state = `server/services/track-transitions.js`, ET wall-clock = `server/utils/datetime-et.js`, admin auth = `router.use(adminAuthenticate, …)` from `server/middleware/admin-auth.js`, feature flags = `useFeatureFlag`, Intelligence Bar tools = `TOOLS` + `executeTool`. Extending the canonical path beats a working local duplicate.
