...
Optional model overrides: `MODEL_DEEP` / `MODEL_FLAGSHIP` / `MODEL_WORKHORSE` / `MODEL_FAST` / `MODEL_VOICE` / `MODEL_VISION` (global), `INTELLIGENCE_BAR_MODEL` / `INTELLIGENCE_BAR_TECH_MODEL` (IB-specific). `MODEL_DEEP=claude-opus-4-8` is the DEEP-tier kill switch (reverts the fable-5 lanes to Opus, no deploy).

Cross-provider routing: `MODEL_OPENAI_BEST` (default `gpt-5.5`), `MODEL_GEMINI_VISION` (default `gemini-3.5-flash`), `GEMINI_VISION_MODEL` (per-service vision override), `GEMINI_VISION_FALLBACK_MODEL` (prior Gemini model retried on a live-model miss, default `gemini-2.5-flash`). These best models are LIVE directly (lead-triage, knowledge Q&A, estimate assistant → GPT-5.5; lawn-assessment + satellite-analyzer vision → Gemini 3.5 Flash). Fallback on any miss: the OpenAI features fall back to Claude (then a deterministic template for the estimate assistant); the Gemini vision scorers retry `gemini-2.5-flash` (and still run the parallel Claude-vision fan-out). No `GATE_OPENAI_*` / `GATE_GEMINI_VISION_PRIMARY` / shadow flags — the competing-model (`ai_model_comparisons`) machinery was removed.
