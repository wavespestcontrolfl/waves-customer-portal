...
- **AI:** Model IDs and provider policies live in `server/config/models.js`; never hardcode Anthropic IDs in call sites. Cost-aware defaults: `FLAGSHIP`/`DEEP` → Claude Opus 4.8, `WORKHORSE`/`FAST`/`VOICE` → Claude Sonnet 5, `VISION` → Claude Sonnet 4.6, and `EXTREME` → Fable 5 as an explicit opt-in only. Generated text uses `TEXT_POLICIES` plus `dispatchWithFallback` from `server/services/llm/call.js`, and every policy must cross providers: reports use GPT-5.6 Sol → Claude Opus; routine customer/content writing uses Claude Sonnet → GPT-5.6 Terra; structured high-volume work uses GPT-5.6 Luna → Claude Sonnet; difficult analysis uses Claude Opus → GPT-5.6 Sol. Never run both providers in parallel in production. Managed Agents remain Anthropic-only because their hosted API has no drop-in OpenAI equivalent. Specialized image/video/transcription pipelines retain their provider-specific ladders.
