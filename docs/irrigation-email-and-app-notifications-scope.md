...
The notification preserves a conditional plan's rain condition. It requires usable customer-entered portal numbers: positive weekly inches or a complete minutes/days/head-type schedule, confirmed for the current home. An email asking for setup or move reconfirmation does not qualify. The email audience and the plan shown in My Property retain their existing rules. The weekly customer/event key reuses the existing advisory-lock bell deduplication. The plan, portal-input eligibility, gates, weather preference and quiet hours are checked again before each provider leg. Weekly instructions are excluded from the unvalidated 30-day dashboard alert feed.
