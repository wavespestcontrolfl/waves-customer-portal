...
| 13 | *any* (undecided) | `cancelled` **(a)** | Admin removes the annual-prepay flag from an invoice (`DELETE /:id/annual-prepay`). ADMIN-BUG-R16: now routes through the SAME `cancelTermWithRestorations` pipeline as move 9 — stamps cleared (`throwOnError`), covered visit invoices reopened, pending-window/WaveGuard credits reversed, `customers.billing_mode` reset to the recorded prior mode, switch-superseded invoice / retired setup-fee claim restored, and only non-terminal attached visits are detached. A term with a `renewal_decision` (a decided term) is refused with 409 before the transaction opens — the route no longer destroys it. Re-marking the invoice later re-derives the status via move 1 **only for an undecided term**. | `R` `cancelTermWithRestorations`, called from `AI` `DELETE /:id/annual-prepay` | `renewal_decision IS NULL` |
...
- ~~Move 13 is the one unguarded transition...~~ FIXED (ADMIN-BUG-R16):
  ...
  a decided term with 409 instead of destroying it.
