...
stage list to the migration, pins the written names to the code, and pins the
transition functions' inputs/outputs. Change either side → update this page.
...
| `renewal_pending` | Active term whose customer has been contacted about renewal (`renewal_contacted_at` set). Still covered — behaves as `active` everywhere (`ACTIVE_STATUSES = ['active','renewal_pending']`). | yes |
...
latch: every status-mutating path below guards on `renewal_decision IS NULL`
or on `status IN ACTIVE_STATUSES`, so a decided term never moves again.
...
| 4 | `active` / `renewal_pending` | `renewed` | Operator records decision `renew`. Sets `renewal_decision = 'renew'`. | `R` `recordDecision('renew')` | same as 3 |
| 5 | `active` / `renewal_pending` | `switch_plan` | Operator records decision `switch_plan`. Sets `renewal_decision = 'switch_plan'`. | `R` `recordDecision('switch_plan')` | same as 3 |
| 6 | `active` / `renewal_pending` | `cancelled` **(b)** | Operator records decision `cancel` — a renewal lapse. Sets `renewal_decision = 'cancel'`; coverage for the paid window is kept. | `R` `recordDecision('cancel')` | same as 3 |
| 7 | `payment_pending` / `active` / `renewal_pending` | `cancelled` **(a)** | Prepay invoice voided, refunded, or `payments` refund lands (`syncTermForRefundedPayment`). Clears prepaid stamps, reverses WaveGuard extension credits, reopens covered visit invoices, resets billing mode. | `R` `syncTermForInvoicePayment` (`nextStatus === 'cancelled'`) | `renewal_decision IS NULL` |
| 8 | `active` / `renewal_pending` | `payment_pending` | Dispute opened on the prepay invoice. Stamps `dispute_suspended_at`; coverage suspended (visits bill per-visit) until the dispute resolves. | `R` `suspendActiveTermsForDisputedInvoice` | `status IN ACTIVE_STATUSES` |
| 9 | `payment_pending` *(dispute-suspended)* | `active` | Dispute won → invoice back to paid → move 2 fires; the marker survives until the dues claw-back finishes, then `finishDisputeRecoveryForTerm` clears it. | `R` `syncTermForInvoicePayment` | move 2's guard |
| 10 | `cancelled` **(a)** | `active` | Lost-dispute revival: the dispute-cancelled term's invoice is re-paid in dunning. Restores extension credits. | `R` `syncTermForInvoicePayment` | `status = 'cancelled' AND renewal_decision IS NULL AND dispute_suspended_at IS NOT NULL` |
| 11 | `active` / `renewal_pending` / `payment_pending` | `payment_pending` | Admin reverses an applied credit on a prepaid invoice — the term is "un-paid"; stamps cleared. | `AI` `POST /:id/reverse-prepaid` (apply-credit reversal) | `renewal_decision IS NULL AND status NOT IN ('cancelled','canceled')` |
| 12 | *any* | `cancelled` | Admin removes the annual-prepay flag from an invoice (`DELETE /:id/annual-prepay`). Stamps cleared, attached visits detached; billing mode is NOT reset. **Unguarded** — the only path that can move a decided term. Re-marking the invoice later re-activates the same row via move 1. | `AI` `DELETE /:id/annual-prepay` | none |

Everything not in the table is not a move. In particular there is **no**
`renewed → *`, `switch_plan → *`, or `cancelled(b) → *` (other than move 12),
...
- Move 12 is the one unguarded transition. Whether it should refuse decided
  terms is an owner ruling; documented, not changed.
