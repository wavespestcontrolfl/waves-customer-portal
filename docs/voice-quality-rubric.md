...
A `caller.context` needs `caller.verified: true` — the live resolver returns no
context when verification fails. An `ok: false` response stands in for a thrown tool failure: it performs no fixture
