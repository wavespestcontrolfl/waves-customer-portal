...
| Callback reserve | lawn $2/visit; commercial $3/visit; residential T&S/pest/mosquito/rodent 0 (`constants.js:308,631,657`) | n/a | seed pest with the observed 2.6% callback rate × the recorded 28-min `pest_re_service` span (check-in → check-out, drive already inside it — no separate drive allowance, the same `callbackDriveMinutes: 0` the calculator uses) ≈ $0.42/visit at $35/h, report-only; refine to true on-site timing + drive once check-out-before-driving is enforced (MON-004) |
