...
A revised bid-form original requires a reviewed form profile update: run `node server/scripts/bid-form-fingerprint.js <pdf> <page>` on the new, untouched original (never on a filled export) and record all three values it prints — the content, resource and packet fingerprints — together in the form profile; a stale packet value refuses every upload of the new original. Exports also refuse cropped or shifted pages, annotated pages, filled fields and signed packets.
