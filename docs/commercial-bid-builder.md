...
A revised bid-form original requires a reviewed form profile update: run `node server/scripts/bid-form-fingerprint.js <pdf> <page>` on the new original and record both the content and resource fingerprints. Exports also refuse cropped or shifted pages, annotated pages, filled fields and signed packets.
