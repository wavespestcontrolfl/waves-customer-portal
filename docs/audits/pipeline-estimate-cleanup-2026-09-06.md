...
### Third review: next-estimate ownership and verified preview mode

The third completed review found one P1 and four P2s. The property-link P1 and three local P2s are fixed:

- Next estimate uses the shared property reset before clearing contact identity. The persistence service also rejects a nonempty property link without its owning customer identity. The regression saves before any address edit can independently mask a stale link.
- The existing live-contact messaging action now lives beside the shared conversation panel and serves both estimate-list layouts and the editor; no historical snapshot bypass remains in those three actions.
- The existing phone utility supplies complete match digits to conversation history and both contact-to-lead resolvers. International candidates match in full, while NANP accepts its complete stored domestic/E.164 aliases.
- `/data` returns `verifiedStaffPreview: true` only for an authenticated staff-preview request. The SPA bases read-only behavior on verified response flags, including its referral-card behavior. An unauthenticated copied marker keeps normal published-customer behavior. The additive response field is documented in the public route contract.
...
### Fifth review and CI follow-up

The fifth review of `3d96530ce` returned two P2s and no P0/P1. The scheduler now recognizes a reviewed schedule only when its receipt timestamp matches the claimed row's current `scheduled_at`, matching the existing send-path selection. A historical receipt no longer suppresses the legacy transient-retry path. The regression covers both the matching receipt (stop) and historical receipt (retry).
...
### Scheduled failure replay and unavailable messages

The two scheduling findings on `81f93c1d6` are fixed. Replaying an unstarted queued receipt on a terminal `send_failed` row with no due time returns 422 with `SCHEDULED_SEND_FAILED`, the recorded failure reason, and `scheduled: false`; active schedules and actual delivery receipts retain their prior behavior. An attempt that may have reached a provider still follows the existing uncertainty guard.

The existing send route now validates every selected scheduled channel against the current preview before entering the schedule transaction. Reviewed and legacy requests receive 422 `ESTIMATE_MESSAGE_UNAVAILABLE` when a selected template is unavailable. An unavailable unselected channel does not block the available one; `both` requires both messages. The existing dialog displays the returned error. No client or provider interface changed.

All 60 tests in five reviewed-send, email, schedule-claim, deletion and scheduler suites passed. The isolated real PostgreSQL/API proof returned the terminal replay failure and rejected missing-message schedules for both reviewed and legacy callers without route writes or transport activity; temporary synthetic records were removed. Local SMTP fallback was disabled only in the QA process to represent an unavailable email channel. Targeted lint retained 45 existing warnings and no errors. The restored QA database has no complete migration ledger (doctor reports pending migrations), so this establishes route-specific runtime behavior, not full migration readiness. No migrations or external provider requests ran.
