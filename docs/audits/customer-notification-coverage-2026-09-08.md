...
| Standalone invoice issued / payment link ready | Explicit App choice for automated self-pay invoice delivery; existing email copies continue | Invoices in Billing; exact payable invoice | Charged-profile preference; re-check invoice ownership, payer exclusion and collectible status before App delivery. Staff Text actions stay on Text. |
...
| Payment failed, retry outcome or bank verification needs action | Explicit Payment problems App choice; allowed email copies continue | Payment problems in Billing; opens payment methods | Charged-profile choice, consent-checked text fallback, attempt/event identities and existing deferred-send rechecks. Advance charge/expiry notices remain separate. |
...
| Invoice due / overdue follow-up | Concrete `invoice_followup` sender honors the invoice App choice | Invoices in Billing; existing collection and billing contact rules | Each sequence step has its own retry-stable event key. Existing eligibility, cadence and contact ledger still apply; pending App delivery uses the deferred-send rail. |
...
| Service request received or updated | Existing request lifecycle messages gain an explicit Request updates App choice; email copies and receipt texts continue | Request-owner profile, current request status and existing internal-request exclusions | App opens the exact authorized request and refreshes for each notice. Portal reschedule and promotion-interest receipt writers remain outside the lifecycle receipt trigger. App-only notices never create a text fallback or second support conversation. |
| Cancellation resolution accepted | Existing cancellation-safe lifecycle messages | Existing inactive-account and cancellation rules | Retain these external confirmations; cancellation-only flows do not gain a portal request CTA. |
