...
| Service request received or updated | Explicit Request updates App choice alongside existing lifecycle email copies and receipt texts | Request-owner profile, current request state and existing internal-request exclusions | App opens the exact authorized request, including older/resolved requests. App-only status notices never create a text fallback or second support conversation. |
