...
### Production sequence repair

The first portal release failed before the canonical correction could run: the earlier six-unit migration validated a retired iron predecessor with stale cost evidence. Its initial PostgreSQL fixture covered the canonical correction independently, which missed this execution-order failure. The existing portal continued serving and `/api/health` reported status/database OK. Supplier-price log lines before the failure were not treated as committed data.

The owner explicitly approved an exception to the applied-migration rule. The earlier migration now skips only `active === false` rows; all existing guards still apply to active and null-active rows. The later migration remains responsible for validating exactly one active canonical identity. No retired row is repaired or reinterpreted, and no active cost guard is relaxed. Keeping the original filename preserves compatibility with previews that already recorded it.

Four real development PostgreSQL cases ran through Knex's migration runner: fresh and deduplicated catalogs, each with the earlier filename pending or already recorded. Knex ran exactly the expected files, preserved every field on inactive predecessors (including contradictory cost and stock fixtures), changed only inventory unit/update timestamp on active rows, and performed no work on repetition. All catalog and temporary tracking changes rolled back. Thirteen regression tests passed, including active/null-active stale-liquid rejection and stale dry-cost guards. Scoped lint, domain rules and diff checks passed. These fixtures verify migration behavior; they do not certify unrelated production supplier-price freshness.
