...
> **Status (2026-09-26):** written 2026-04-17 against the v1 and v2 engines. The v2 engine has since
> been deleted, pre-slab pricing moved to `pricePreSlabTermiticide`, and several open items below were
> resolved. Inline updates mark each change; `constants.js` and `server/services/pricing-engine/README.md`
...
**WaveGuard qualification:** bait station monitoring is one of the qualifying services for the WaveGuard tier discount (`WAVEGUARD.qualifyingServices`: lawn care, pest control, tree & shrub, mosquito, termite bait and, since 2026-08-29, rodent bait). Tier discount applies to the monthly rate for recurring customers.
...
**Update 2026-09-26:** superseded. Pre-slab quotes now price through `pricePreSlabTermiticide` (the legacy `pricePreSlabTermidor` wrapper delegates to it) using `SPECIALTY.preSlabTermiticide` and the approved `products_catalog` price for Termidor SC, with a $174.72 / 78 oz code fallback. Migration `20260710000020_preslab_usage_steps_inventory_link.js` retired the $152.10 SiteOne row as stale. The legacy `SPECIALTY.preSlabTermidor` block (including `bottleCost` $152.10) is still validated and synced by `db-bridge.js` but no longer prices quotes, so the reduction above does not apply.
...
- **Trench engine divergence:** **Resolved** — the v2 engine (inline `Math.min(0.50, cp)`) is deleted; the modular cap `SPECIALTY.trenching.concretePctCap` is 0.60 and the client mirror matches. Also clarify pool-feature composition (currently `poolCage` replaces base to 0.35, `pool` replaces to 0.30 — no "pool deck" feature exists despite pre-session draft references; the former `largeDriveway` +0.05 was retired estimator-wide 2026-07-30).
...
- **Pre-slab Termidor full formula:** document labor term (`lhr = min(5, max(1, 0.5 + sqft/1500))`) and discount placement (multiplier applied to rounded post-margin price, not to material cost). **Update 2026-09-26:** this describes the retired v1 pricer; document the current `pricePreSlabTermiticide` instead.
- **`volumeDiscounts` map is dead code:** `SPECIALTY.preSlabTermidor.volumeDiscounts = { '10plus': 0.85, '5plus': 0.90, none: 1.00 }` exists in v1 constants but is unreachable — the only consumer (`service-pricing.js:pricePreSlabTermidor`) is exported but uncalled by v1's estimate-engine, and v2 + client use hardcoded string comparisons (`'10'` / `'5'` / `'NONE'`) rather than the map. Either wire the map into live paths in v4.4 OR delete the map and keep inline string comparisons as canonical. Align key strings (`'10plus'`/`'5plus'`) with UI/live code strings (`'10'`/`'5'`/`'NONE'`) as part of cleanup. **Update 2026-09-26:** no longer accurate — `pricePreSlabTermiticide` applies `SPECIALTY.preSlabTermiticide.volumeDiscounts` (`service-pricing.js`), and `pricePreSlabTermidor` is still called for legacy pre-slab inputs (`estimate-engine.js`) before delegating to it.
...
**Structural/model:**
- **Bait station monitoring model:** Basic and Premier flat pricing doesn't reflect real COGS difference. Port to cartridge-based consumption model. **Resolved 2026-07-28:** replaced by station-count brackets (see §1).
...
- **v2 retirement (Session 11):** **Done** — the v2 engine is deleted. (Original note: v2 emits termite output differently (`tmBait = { ai, ti, bmo, pmo }`). Either align v2 emission with v1's Advance/Trelona split or retire v2 entirely; either path surfaces install pricing cleanly in lookup estimates.)

---

*Audit-verified 2026-04-17 (Session 6 close). All pricing figures in sections 1–3 confirmed against live code via `node -e` simulation. Sections 1–3 have since been updated inline; see the status note at the top.*
