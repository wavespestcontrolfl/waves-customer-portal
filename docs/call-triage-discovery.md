...
route_decisions (
  ...
  created_at                      timestamptz,
  UNIQUE (call_log_id, decision_version, mode, recording_sid)  -- recording_sid: the recording the decision was derived from ('' when none); a replaced recording's pass writes its own row
);
```
