...
SendGrid. GitHub's own run-failure emails depend on each user's notification
...
2. Samples row counts on the sentinel tables (`customers`, `invoices`,
   ...
   `knex_migrations`).
3. `pg_dump --format=custom` (compressed, table-selectable), AES-256 encrypts
   it, uploads to `waves-portal/YYYY/MM/waves-portal-<stamp>.dump.enc` plus a
   writes the `latest.json` manifest locally (key, plaintext sha256, sizes,
   source counts) for publication in step 6.
...
intend to erase — never the live one.
...
4. Spot-check: `psql '<url>' -c 'select count(*) from customers'` against the
   `source_counts` in `latest.json`; `select max(name) from knex_migrations`
   against the newest file in `server/models/migrations/`.
5. Point the portal service's `DATABASE_URL` at the new instance's **private**
   URL (`postgres.railway.internal`) and redeploy. Anything written to the old
   instance after the backup's `taken_at` is lost — Stripe webhooks re-deliver
   on retry, Twilio inbound does not; reconcile the gap from the Stripe
   dashboard and the Twilio message log before reopening the office.
6. Delete the decrypted `.dump` from the laptop.
