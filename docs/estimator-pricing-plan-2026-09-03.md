...
| B4 | `material-budgets.generated.json` (the lawn cost-floor material table) | (1): the file keeps every version under its `pricingVersion` with an effective date, never edited in place; the estimate stamps the version it priced under (reader in `estimate-floor-signal-replay.js`, beside the floor arm and minimum); the package resolves the budget for the injected version; an unstamped floor-priced row resolves the version in force at its `created_at`. |
