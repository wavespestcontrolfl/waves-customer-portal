...
Tapbacks are excluded by the webhook's own reaction detector even when they are stored as ordinary inbound rows. Any explicit time window in the source (relative windows, absences and travel, weekdays, seasons or calendar dates) requires review regardless of the model's duration label. Negated or uncertain pet reports stay review exceptions, because any stored pet detail raises a technician pet alert.
