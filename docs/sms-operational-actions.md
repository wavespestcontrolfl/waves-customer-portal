...
The provider delivery row is excluded from capture and conversation context
when `metadata.scheduled_sms_log_id` identifies a scheduled outbound row for
...
only in database selection, outside model prompts. Send/retry writers are
unchanged. Outbound commitment capture remains a separate gated follow-up.
