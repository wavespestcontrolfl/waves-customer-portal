...
Migration `20260906000001_sms_operational_actions.js` is preserved byte-for-byte from PR #3970 because its preview already ran it. It adds the analysis marker and prepares the existing commitment ledger for the separately reviewed follow-up. Migration `20260907000001_sms_profile_proposal_fields.js` widens the proposals table's create-on-apply allowance (a NULL `resource_id`) to the four free-form fields the data-hygiene extraction phase did not already propose; the approve route's allowlist gains the same four. Approving an irrigation proposal records the companion active-system flip on the proposal and in the apply audit, and revert restores it only while the flag still holds the value apply set. This part never writes SMS commitments. Rollback refuses to discard recorded analysis or commitments; use the gate after activation.
