...
Tapbacks are excluded by the webhook's own reaction detector even when they are stored as ordinary inbound rows. Explicit time windows in the source (relative windows, absences and travel, upcoming-service wording, weekdays, seasons or calendar dates) require review regardless of the model's duration label. Negated or uncertain pet reports stay review exceptions, because any stored pet detail raises a technician pet alert. An access-code value that is a phrase rather than a credential (same as last time, the usual) is refused. Only the message columns the history query uses reach a provider prompt; media metadata never does.
