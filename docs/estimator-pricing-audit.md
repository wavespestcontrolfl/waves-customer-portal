...
- **Independent calculator**: `scripts/audit-estimator-pricing.js` rebuilds pest, lawn (all four tracks, three cadences, every bracket row ±1), mosquito (treatable-area anchors, pressure, water multiplier), rodent bait (brackets + ladder), termite bait (perimeter → stations → install → bracket monitoring), one-time pest, tree & shrub (cost buildup), WDO, German roach, foam drill, top dressing, plugging, palm injection (fixed, tiered, fungal, lethal bronzing, Tree-Age), pest year-built adjustments, the armed lawn cost floor / program minimum / cadence-ladder lift under a simulated overlay, WaveGuard tiers and discounts, cadence identities (termite and recurring foam included), annual-prepay base — from `constants.js` and the documented formulas, without calling any engine pricer — then diffs against `generateEstimate()`. 1,459 scenarios — 1,412 with an independent expected value (1,411 match) plus 47 engine-only observations — **0 price mismatches and 1 cadence mismatch** (CAD-002) on in-code constants; the command exits nonzero while any mismatch stands. The production overlay run (`--db`) predates the cadence assertion and the termite cartridge cost model and is owed a re-run.
...
| Pricing-formula correctness | 8 | 1,412 independently compared scenarios (+47 engine-only observations), 0 price mismatches, 1 cadence mismatch (CAD-002); cadence identities hold; discounts exact | remove T&S double-round; cap FIXED discount; reject unpriceable cadences |
