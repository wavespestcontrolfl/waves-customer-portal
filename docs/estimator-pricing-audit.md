...
**Overall score: 5.5 / 10.** The pricing formulas are correct and reproducible — an independent calculator rebuilt every core price from the constants and the documented formulas and matched the engine on all 1,199 scenarios (0 mismatches), including every bracket boundary, cadence identity, discount stack and tier count. The weakness is everything *behind* the price: the labor minutes are assumed and cannot yet be validated (recorded check-out times include drive time, per the owner's 2026-09-02 review, so the production minutes are not on-site time); material costs are literals copied from invoices months ago and, for one product, 2.1× the live catalog price; no protocol, inventory row, or equipment calibration feeds a price except pre-slab termiticide; the reported margins are therefore optimistic, and the lawn program is below its own 35% floor at list price before any discount. Operationally, ~18 completed billable visits since June left uninvoiced with only a log line as the signal.
...
| 3 | BIL-001 | P1 | 18 completed, billable, non-callback, non-prepaid, non-membership visits since 2026-06-01 have no invoice within ±3 days (est. $1,333 of visit price); the only signal is `logger.warn` in `admin-dispatch.js:9301`. 128 of 182 per-application customers have no `per_application_fee`. | Data + Static |
...
### 5.4 Labor findings
- **LAB-001 (P1, Data + Reproduced)** pest on-site 25 vs 44 median — see §1 #1; numerical example in the calculator run (`docs/audits/...calculator-run.md` §Annual economics): 2,000 sf quarterly $112/app → gross margin 58.6% modeled, 48.7% observed-median, 38.8% observed-p75; at Platinum 48.2% → 35.8% → 23.5%. Contribution margin (all-card, 2.9%) at Platinum observed p75 ≈ 20.6%.
- **LAB-002 (P1)** lawn 9x 4,500 sf St. Augustine $64/app: modeled 30.7% (already `marginFloorOk:false`), observed-median 7.2%, Silver −3.1%, Gold −9.2%, Platinum −16.0%; price needed for 35% at observed labor ≈ $91.36/app (vs $64).
...
- **LAB-006 (P2, Static)** termite install labor (5 min/station) is computed but excluded from the billed base by design (`constants.js:1004-1007`); termite monitoring at 5 min/station + 20 drive costs more than the $19–$39/mo bracket collects (calculator: −65% at Bronze for a 2,000 sf home). Ruling needed (§16).
...
|---|---|---|---|---|---|---|---|
| Pest quarterly | $112 | 4 | $448 | $547 (+$99 setup) | 58.6% | 48.7% | 58.6 → 48.2% |
| Lawn 9x St. Augustine | $64 | 9 | $576 | $576 | 30.7% | 7.2% | 30.7 → 13.3% |
| Mosquito seasonal9 | $77 | 9 | $693 | $693 | 49.2% | UNKNOWN (no minutes stored per mosquito visit yet) | 49.2 → 36.5% |
| Rodent bait | $89 | 4 | $356 | $455 (+$99 setup stand-alone) | 37.2% | UNKNOWN | 37.2 → 21.5% |
| Tree & shrub 6x (1,440 sf beds, 6 trees) | $95.82 | 6 | $575 | $575 | 45.0% | UNKNOWN | 45.0 → 31.3% |
| Termite bait monitoring | $72 | 4 | $288 | $288 (+ $610 install) | −65% (5 min/station model) | UNKNOWN | −65 → −106% |
...
|---|---|---|---|---|---|
| 1 | Completion-without-amount bell + digest; per-application-fee admin writer; re-lane 3 monthly + 18 NULL | BIL-001, BIL-002 | — | S | fee values, lanes |
...
| 3 | Labor recalibration knobs (`on_site_minutes` per service/cadence in `pricing_config`) seeded from medians; reality-check weekly digest | LAB-001/002/006, MON-002 | 1 quarter of clean minutes (already have) | S–M | accept observed basis; lawn reprice |
...
| Pricing-formula correctness | 8 | 1,199 independent scenarios, 0 mismatches; cadence identities hold; discounts exact | remove T&S double-round; cap FIXED discount; reject unpriceable cadences |
...
| Test coverage | 7 | 320 pricing-adjacent suites + 961 new independent cases | commercial golden master; sanitation/exclusion/trenching; invoice cents |
