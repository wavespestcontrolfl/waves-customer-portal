...
| Product quantities / inventory cost | pre-slab only: `db-bridge.js:762-811` reads `products_catalog.best_price` via approved `vendor_pricing`; everything else literal (§6) (**Update 2026-09-26:** termite bait stations joined this link 2026-09-09, `db-bridge.js:899`) | — | `products_catalog`, `vendor_pricing` | catalog (pre-slab); literals (rest) | pre-slab fail-open to constants (silent); audit `product-costing.js:273-277` → `cost: 0` + warning string | no | catalog `best_price_updated_at`; 43 not current |
