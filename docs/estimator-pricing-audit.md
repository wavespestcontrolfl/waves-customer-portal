...
**Overall score: 5.5 / 10.** The pricing formulas are correct and reproducible — an independent calculator rebuilt every core price from the constants and the documented formulas and matched the engine on 1,782 of the 1,784 independently compared scenarios (a further 61 engine-only observations carry no expected value and are listed separately in the appendix), including every bracket boundary, cadence identity, discount stack and tier count; neither open discrepancy is a formula error — one is a prepay coverage cadence (CAD-002: a 9-visit tree & shrub plan resolves to `bimonthly`, six covered visits) and the other an entry-point defect (INP-001: palms supplied at property level never reach the tree & shrub price), both findings in this report (**Update 2026-09-26:** INP-001 has since been fixed in translator v4.8; CAD-002 was not re-checked). The weakness is everything *behind* the price: the labor minutes are assumed and cannot yet be validated (recorded check-out times include drive time, per the owner's 2026-09-02 review, so the production minutes are not on-site time); material costs are literals copied from invoices months ago and, for one product, 2.1× the live catalog price; no protocol, inventory row, or equipment calibration feeds a price except pre-slab termiticide (**Update 2026-09-26:** termite bait stations are catalog-linked too, since 2026-09-09); the reported margins are therefore optimistic, and the lawn program is below its own 35% floor at list price before any discount. Operationally, completed billable visits since June are leaving uninvoiced with only a log line as the signal (the audit's first count was ~18; the Billing Recovery workbench, whose predicate the audit script now mirrors, is the authoritative number).
...
| 3 | BIL-001 | P1 | Completed billable visits since 2026-06-01 with no invoice: the audit's first pass counted 18 within ±3 days (est. $1,333) under a looser predicate than the Billing Recovery workbench's (`admin-billing-recovery.js uninvoicedLeakQuery`: effective price, dispositions, record callbacks, always-free types, payer, autopay). The script now mirrors that predicate but has not been re-run against production, so the workbench figure supersedes the 18. The only completion-time signal is `logger.warn` in `complete-scheduled-service.js:8371-8385` (**Update 2026-09-26:** the completion logic moved here from `admin-dispatch.js`). 128 of 182 per-application customers have no `per_application_fee` (pre-exclusion of internal test accounts; regenerate on the next run). | Data + Static |
| 4 | INP-001 | P1 | Palms entered in the admin builder never reach the tree & shrub price: the translator sends `profile.palmCount` (property level) and the pricer only folds *service-line* palms into the per-tree terms. 30 palms: $53.08/mo (admin) vs $95.17/mo (website form, same property). **Update 2026-09-26:** fixed in v4.8 — the admin builder now forwards palm count on the service line (`property-lookup-v2.js` `translateV2CallToV1Input`). | Reproduced |
...
- **Independent calculator**: `scripts/audit-estimator-pricing.js` rebuilds pest, lawn (all four tracks, three cadences, every bracket row ±1), mosquito (treatable-area anchors, pressure, water multiplier, every hardscape curve and pool addition from frozen literals — single-family, townhome/duplex, condo, commercial — with the treatable area compared directly for every type), rodent bait (brackets + ladder), termite bait (perimeter → stations → install → bracket monitoring, with the property construction multiplier and foundation adjustment on the install), one-time pest, tree & shrub (cost buildup), WDO by resolved footprint across the live bracket array, German roach, foam drill, top dressing (65% treatable assumption, full area when bundled with recurring lawn or entered explicitly), plugging, palm injection (fixed, tiered, fungal, lethal bronzing, Tree-Age with the restricted-use licence guard), pest year-built adjustments (age from the Eastern calendar year — AGE-001), the armed lawn cost floor / program minimum / cadence-ladder lift under a simulated overlay (and, floor armed, the property cost inputs — landscape complexity, heavy shrubs, privacy fence, maintenance condition, pest pressure, explicit route drive minutes), the armed pest program floor beneath a manual discount (simulated overlay), termite bait on both priceable systems (the Trelona default and the Advance replay at its own spacing) and both layout-complexity inputs (service line and property feature), the gated Bermuda-suppression add-on (St. Augustine only, gate enabled in-process, asserted to fail closed when off), WaveGuard tiers and discounts, cadence identities (termite and recurring foam included), the annual-prepay total against its protected floor (foam at full price, the lawn program minimum under a simulated overlay, the operator-waived solo fee mix) — from `constants.js` and the documented formulas, without calling any engine pricer — then diffs against `generateEstimate()`. 1,845 scenarios — 1,784 with an independent expected value (1,782 match) plus 61 engine-only observations — **0 formula mismatches and 2 counted discrepancies** on in-code constants: CAD-002 (cadence) and INP-001 (property-level palms ignored), both already findings; every failed P0/P1 invariant is a counted discrepancy and the command exits nonzero while any stands. The production overlay run (`--db`) predates the cadence assertion and the termite cartridge cost model and is owed a re-run.
...
- **Tests**: new `server/tests/pricing-audit-golden-cases.test.js` — 987 tests green on `origin/main`; they assert the *independent* numbers, so a future engine/formula divergence fails loudly rather than a regenerated baseline hiding it. Existing pricing suites were inventoried (§14) but not re-run beyond the new file.
...
| Product quantities / inventory cost | pre-slab only: `db-bridge.js:762-811` reads `products_catalog.best_price` via approved `vendor_pricing`; everything else literal (§6) (**Update 2026-09-26:** termite bait stations joined this link 2026-09-09, `db-bridge.js:899`) | — | `products_catalog`, `vendor_pricing` | catalog (pre-slab; since 2026-09-09 also the termite station and cartridge rows); literals (rest) | pre-slab fails open to constants (silent); the termite station/cartridge link keeps the configured cost with a refusal warning and stamps `stationCostSource`/`cartridgeCostSource`, surfaced as `materialCostSource` on the priced line; audit `product-costing.js:273-277` → `cost: 0` + warning string | no | catalog `best_price_updated_at`; 43 not current |
...
| Invoice | converter first-application invoice (`:6499-6560`), `InvoiceService.create` (tax, deposit credit, cents), scheduled mint (`scheduled-invoice-mint.js`, price-moved CAS), completion (`billing-lane.js:158 completionInvoiceAmount`) | — | `invoices.line_items/subtotal/tax_amount/total`, `customers.monthly_rate/per_application_fee/billing_mode` | invoice row (`invoiceAmountDue`) | per-application visit with no price and no fee completes **uninvoiced** with `logger.warn` (`complete-scheduled-service.js:8371-8385`) | `activity_log` for accept/opt-out | — |
...
### 6.2 Linkage
- Pricing reads the catalog for **one** service: pre-slab termiticide (`db-bridge.js:762-811`, approved vendor price, `[0.5×, 2×]` sanity band, fail-open, kill switch `link_container_costs_to_catalog`). **Update 2026-09-26:** termite bait stations are now a second linked service (`db-bridge.js:899`, since 2026-09-09).
- `service_product_usage` (32 rows, 24 service types) is the audit's COGS map; **all 32 `service_type` values are display strings, none is a catalog key**; 6 rows point at products with no cost (a lower bound: the 2026-09-03T05:43Z prod run counted only a missing `cost_per_unit` AND `best_price`; the current predicate also counts a `best_price` the COGS fallback cannot use — no `unit_size_oz` or an unconvertible usage unit — and prod was not re-queried); `packets` (Alpine WSG) is not a recognised unit → `convertToOz` null → **$0 cost with a warning string nobody renders** (`product-costing.js:246-277`).
...
| `TERMITE.systems.trelona.stationCost 22.05` (`:1001`) / advance 13.16 | | Trelona ATBS Bait Station **$24.00 (stale, needs_pricing)**; Advance $14.00 (stale) | code behind supplier: 16-ct box is $384.00 = $24.00/station (owner 2026-09-02); replacement cartridges ($6.70–$10.70 each) in no pricer. **Update 2026-09-26:** resolved 2026-09-09 — `stationCost` is $24.00 and catalog-linked (`constants.js:1051`, `db-bridge.js:899`) |
...
|---|---|---|---|
| Palm count | admin (`palmCount`, guarded 1–200), website (`treeShrub.palmCount`), AI (1–200), vision (`estimatedPalmCount`) | **only when on the service line** (`service-pricing.js:2745-2770`); admin translator sends property-level only (`property-lookup-v2.js:3970` sends `{tier:'standard'}`) | **INP-001 P1** — Reproduced: 30 palms $53.08 vs $95.17/mo. **Update 2026-09-26:** fixed in v4.8 (`property-lookup-v2.js` `translateV2CallToV1Input`) |
...
| Non-palm tree count | admin (`treeCount`), website, AI, vision | yes (1.5 min/tree, $4/yr) | **INP-002 P2** blank admin field posts explicit 0 → suppresses density fallback ($45.25 vs $58.75/mo). **Update 2026-09-26:** fixed in v4.8 — blank now stays absent (`property-lookup-v2.js` `firstNonNegativeIntegerOrThrow`) |
...
| Access difficulty | website/AI only; **admin does not collect** | 0/8/15 min | **INP-004 P2**. **Update 2026-09-26:** fixed in v4.8 — admin now collects it (`property-lookup-v2.js` `translateV2CallToV1Input`) |
| 6-visit vs 9-visit | website/AI (`tier`); admin hardcodes `standard` | yes | admin cannot sell 9x T&S through the builder (**INP-004**). **Update 2026-09-26:** fixed in v4.8 — admin now forwards the selected tier (`property-lookup-v2.js` `translateV2CallToV1Input`) |
...
- Stinging insects: admin sends none of species/tier/removal → every admin wasp quote is tier-2 paper wasp, no removal (`property-lookup-v2.js:4232-4241`) — **INP-006 P2**. **Update 2026-09-26:** fixed — admin now collects species, scope tier and removal (`EstimateToolViewV2.jsx` `stingSpecies` / `stingTier` / `stingRemoval`; `property-lookup-v2.js` `translateV2CallToV1Input`).
...
- **MAT-003** termite station cost drift (code $22.05 vs supplier $24.00/station from the $384 16-ct box, owner 2026-09-02; replacement cartridges $6.70–$10.70 each in no pricer — corrected from the first draft's "$384/16 cartridges"); `service_product_usage` spacing 10 ft vs pricing 15 ft (label up to 20, 10–15 recommended). **Update 2026-09-26:** resolved 2026-09-09 — the constant is $24.00 and catalog-linked.
...
- **INV-002** `service_product_usage` keyed by display names; `packets` unit → $0; 6 products without cost (lower bound, see §6) · fix: re-key by `service_key`, add carrier column, reject unknown units.
...
- **AGE-001** (P2) the pest year-built pressure bump and the WDO inspection-time multiplier derive the home's age from the **UTC** calendar year (`modifiers.js:14` and `:65`, `new Date().getFullYear()` on a `TZ=UTC` Railway box): for the five hours after 00:00 UTC on 1 January — still 31 December in Eastern time — a home sitting on an age threshold (20 / 40 / 60 years for pest; 20 / 40 for WDO) is priced one year older than the Eastern calendar says (+$2 / +$3 / +$3 per visit on the pest bump; ×1.10 / ×1.25 on WDO time). Exposure ≈5 h/yr, cents to a few dollars per affected quote · fix: `etParts(new Date()).year` from `server/utils/datetime-et.js` (AGENTS.md America/New_York discipline) · the audit calculator now derives its expectation from the Eastern year, so a run inside that window reports the mismatch instead of agreeing with it (Codex r18). **Update 2026-09-26:** fixed — `modifiers.js:10, 16, 67` now use `etParts().year` (the Eastern year) at both call sites.
...
| 2 | Admin translator: pass T&S `palmCount`/`access`/`tier` on the service line; blank tree count = absent; input bounds at `/calculate-estimate` (integer stories 1–4, sqft caps, ≥0 counts); reject unpriceable cadences (**Update 2026-09-26:** the translator part shipped in v4.8 and the builder now collects stinging inputs; the input bounds, INP-003 and INP-005 remain open) | INP-001..006, CAT-001 | — | S | palm reserve semantics |
...
2. **Labor basis**: the recorded check-in→check-out spans (pest 44, monthly pest 36, lawn 44, WDO 60, trapping 92 min medians) include an unknown amount of drive — check-out often happens on the way to the next stop (§5.3, MON-004) — so they are NOT on-site minutes and must not be adopted as cost-model inputs (alongside a drive allowance they would double-count travel). Decision needed: capture clean on-site timing (a "left site" stamp taken at the truck, or check-out before departure) before any labor recalibration; until then margin reporting stays on the engine's modeled minutes with the recorded spans shown as context only.
3. **Lawn program**: 9x at 4,500 sf lists at 30.7% — below the 35% floor by the engine's own model — and the tier % takes it to 23.0% (Silver), 18.4% (Gold) and 13.3% (Platinum); all positive, all under the floor (appendix, modeled columns; the recorded-span columns are context only, see #2). Reprice the grid, change the cadence discount, re-arm the cost floor, or accept as a loss-leader?
...
| Input completeness | 5 | palms/access/tier missing from the admin T&S path; shrub count absent; stinging inputs absent; blank→0 (**Update 2026-09-26:** palms/access/tier and blank→0 fixed in v4.8, stinging inputs now collected; shrub count remains absent by design) | field-requirements §0–16 met at the API boundary |
| Pricing-formula correctness | 8 | 1,784 independently compared scenarios (+61 engine-only observations), 0 formula mismatches, 2 counted discrepancies (CAD-002 cadence, INP-001 palm entry point — **Update 2026-09-26:** INP-001 since fixed in v4.8); cadence identities hold; discounts exact | remove T&S double-round; cap FIXED discount; reject unpriceable cadences |
...
| Material-cost accuracy | 4 | literals; trenching 2.1×; bed bug 1.35×; only pre-slab linked (**Update 2026-09-26:** termite bait stations linked too, since 2026-09-09) | every product-driven service costed from the catalog with provenance |
...
| Test coverage | 7 | 320 pricing-adjacent suites + 987 new cases (961 formula-parity + 23 frozen literal prices + 3 frozen WaveGuard tier bundles) | commercial golden master; sanitation/exclusion/trenching; invoice cents |
