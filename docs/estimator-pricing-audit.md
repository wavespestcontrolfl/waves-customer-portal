...
- **Independent calculator**: `scripts/audit-estimator-pricing.js` rebuilds pest, lawn (all four tracks, three cadences, every bracket row ±1), mosquito (treatable-area anchors, pressure, water multiplier), rodent bait (brackets + ladder), termite bait (perimeter → stations → install → bracket monitoring, with the property construction multiplier and foundation adjustment on the install), one-time pest, tree & shrub (cost buildup), WDO by resolved footprint across the live bracket array, German roach, foam drill, top dressing (65% treatable assumption, full area when bundled with recurring lawn or entered explicitly), plugging, palm injection (fixed, tiered, fungal, lethal bronzing, Tree-Age with the restricted-use licence guard), pest year-built adjustments (age from the Eastern calendar year — AGE-001), the armed lawn cost floor / program minimum / cadence-ladder lift under a simulated overlay, the armed pest program floor beneath a manual discount (simulated overlay), termite bait on both priceable systems (the Trelona default and the Advance replay at its own spacing) and both layout-complexity inputs (service line and property feature), the gated Bermuda-suppression add-on (St. Augustine only, gate enabled in-process, asserted to fail closed when off), WaveGuard tiers and discounts, cadence identities (termite and recurring foam included), the annual-prepay total against its protected floor (foam at full price, the lawn program minimum under a simulated overlay, the operator-waived solo fee mix) — from `constants.js` and the documented formulas, without calling any engine pricer — then diffs against `generateEstimate()`. 1,620 scenarios — 1,570 with an independent expected value (1,568 match) plus 50 engine-only observations — **0 formula mismatches and 2 counted discrepancies** on in-code constants: CAD-002 (cadence) and INP-001 (property-level palms ignored), both already findings; every failed P0/P1 invariant is a counted discrepancy and the command exits nonzero while any stands. The production overlay run (`--db`) predates the cadence assertion and the termite cartridge cost model and is owed a re-run.
...
| Pricing-formula correctness | 8 | 1,570 independently compared scenarios (+50 engine-only observations), 0 formula mismatches, 2 counted discrepancies (CAD-002 cadence, INP-001 palm entry point); cadence identities hold; discounts exact | remove T&S double-round; cap FIXED discount; reject unpriceable cadences |
