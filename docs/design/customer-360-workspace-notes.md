...
The directory uses the existing Waves Table controls. Columns show customer name with a circular numeric health score, property, WaveGuard Tier, next service, and overdue invoice exceptions. WaveGuard Tier displays the customer's stored tier as Bronze, Silver, Gold, or Platinum; customers without one of those tiers have a blank cell. Email is not shown beneath the customer name. Row actions move into one native disclosure, which supports keyboard access, Escape, and dismissal when focus or a pointer moves outside it. Mobile presents the same rows with visible field labels.
