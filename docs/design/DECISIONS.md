...
3. **DB-bridge economic guardrails.** `validatePestPricingConfig` now rejects `PEST.base < PEST.floor` and `frequencyDiscounts` outside `(0, 1]`, and validates the new `ONE_TIME.pest.{premiumMultiplier, setupEquivalent}` fields. Sync reads new `onetime_pest` keys (`setup_equivalent`, `premium_multiplier`); the legacy `multiplier` key is ignored (different semantics — would mis-map). Migration `20260528000021_onetime_pest_setup_premium_model.js` rewrites existing `pricing_config.onetime_pest` rows to the new shape (preserving any customized floor); the route seed default is updated to match.
