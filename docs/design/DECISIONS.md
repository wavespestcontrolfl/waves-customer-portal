...
**Implementation:**
- **`service_activity_scores`** (migration `20260611000003`): per-visit 0–5 activity for trend types (rodent/wildlife/bed bug/cockroach/flea/termite). Separate from `pest_pressure_scores` by design — the pressure engine's consumers all assume engine-model rows, and separation guarantees recurring Pest Pressure history can never be polluted. `UNIQUE (service_record_id, indicator_key)` (composite, not bare) is the retry/resume idempotency guarantee and leaves room for multi-indicator visits.
- **`delivery_mode`** on profiles (migration `20260611000004`): `auto_send | internal_only | disabled`. `internal_only` renders + stores the report (token/PDF) with no customer SMS/email — the Phase-1b shadow mode. Plus global kill env `SPECIALTY_REPORT_DELIVERY_DISABLED`.
- **`bed_bug_treatment` services row** (migration `20260611000005`): fixes "bed bug has pricing config but no services row." Ships `customer_visible:false, booking_enabled:false` and `project_required` profile — zero behavior change; exposure gated on owner copy approval.
- **Profile pointer fixes** (migration `20260611000006`): `flea_tick`→`flea`, `rodent_exclusion%`→`rodent_exclusion` (stay project_required; only the correct project form opens).
