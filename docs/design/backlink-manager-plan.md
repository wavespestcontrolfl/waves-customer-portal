...
t.text('decision_inputs_hash').notNullable(); // hash of THIS approval's dimension inputs only (same sets as the authority rows): payment = {estimated_cost_cents, renewal_cost_cents, renewal_period, payment_required, legal_attestation, merchant_binding}; communication = {link_type, expected_rel, legal_attestation, recipient, subject/body hash}; execution = {account_required, email_verification, agent_completable, legal_attestation, submission_url}; plus, for every dimension, the shared quality floors {spam_score, score, confidence} AND the `instance_key` (so the hash itself differs per generation). A mismatch at claim time invalidates the approval; a change outside the dimension's set never does
