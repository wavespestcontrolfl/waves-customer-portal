...
t.string('renewal_period_key');                     // for renewals: the period being bought, e.g. '2027' or '2026-11' — null for initial; CHECK ((purchase_kind IN ('initial','reacquisition') AND renewal_period_key IS NULL) OR (purchase_kind = 'renewal' AND renewal_period_key ~ '^[0-9]{4}(-[0-9]{2})?$')); reacquisition rows carry `loss_event_id` (uuid, CHECK ((purchase_kind = 'reacquisition') = (loss_event_id IS NOT NULL))) — a renewal can never carry a null/empty key, so idempotency keys, approvals and instance keys never collapse onto ':renewal:null:'
...
t.uuid('approval_id');                              // → seo_link_approvals (dimension='payment', action = 'purchase' for purchase_kind IN ('initial','reacquisition'), 'renewal' for 'renewal') when authority is OWNER_*; CHECK: required unless authority = AUTO_PAID_WITHIN_POLICY OR state = 'manual_charged' (a manual settlement records a payment the owner already made — the settlement form click IS the owner's act; no pre-payment approval row exists or is required)
