...
- **Reports are bound to the claim.** The lease persists `leased_provider`, `lease_mode`
  ...
  outcomes only (`close_pending`→`charged`, `ambiguous`, `voided`/`no_payment_required` …) with the conversation lifecycle UNCHANGED — the placement stays `contacted`/`negotiating` and moves to `placed` only on the publisher's confirmation captured by the inbound matcher or a follow-up report, never by the payment report; the INITIAL `mode=payment` on a paid EXECUTION path ⇒ the purchase
