...
t.string('currency').notNullable();                 // CHECK (currency = 'USD') — copied from the path at reservation; a reservation cannot exist in any other currency, and the pre-mint/pre-submit read verifies the live checkout currency against it
