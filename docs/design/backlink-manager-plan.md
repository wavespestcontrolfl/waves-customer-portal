...
from the group's decision — and a sibling created LATER (a new GBP location after the group's purchase already settled) is linked to that settled purchase (`purchase_id`) and its payment instance marked `satisfied_at` (`satisfied_reason='group_purchase'`) in the bridge transaction that creates it, since the all-time guard rightly refuses a second initial purchase for the group and `purchase_id` → the group's purchase, and the settlement
