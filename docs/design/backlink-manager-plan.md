...
- **A verified zero total is a no-payment completion, not a purchase.** If the checkout's
  ...
  `outcome='no_payment_required'` BEFORE any mint (no card exists) — but it is still the
  ...
  outcome='no_payment_required' AND settled_at IS NOT NULL)` — used by the late-sibling bridge
