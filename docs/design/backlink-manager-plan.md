...
t.uuid('group_purchase_id');           // payment dimension, fee_scope='account_wide' only: → seo_link_purchases (the group's purchase for THIS kind/generation); CHECK ((dimension = 'payment') OR group_purchase_id IS NULL); settlement satisfies exactly the authority rows whose group_purchase_id = the settled purchase id — never by group alone, so a void/retry/renewal can't satisfy the wrong instance
