...
t.timestamp('conversation_closed_at'); // durable outreach-conversation closure (§13 recipient guard): stamped when the placement reaches live/indexed, goes lost/rejected, or the lane completes with no reply window left (follow-up sent/skipped + 45 ET days since last send, no inbound match); the recipient guard and its partial unique index require it NULL, so a finished conversation releases the inbox; never cleared — a new conversation is a new placement
...
- **Reports are bound to the claim.** The lease persists `leased_provider`, `lease_mode`
  ...
  terms ⇒ `terms_accepted`/`terms_changed`/`mutation_ambiguous`/`failed`/`skipped` only (never `placed`, a purchase outcome or a handoff); credentials/acquire ⇒ the execution outcomes incl. handoffs; a draft
