...
1. **Registry + paths + provenance + statuses** — migrations for §3.1–3.5, `awaiting_owner`/
   `watching`/`ready_for_credentials` (status enum/constraint, `PROSPECT_STATUSES`, domain-guard sets, board filters + tests — all three), `domain_id/path_id/authority` on prospects, per-provider HMAC worker auth (§12), intake endpoint skeleton
