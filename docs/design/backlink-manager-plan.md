...
| Local opportunity feed | `server/services/seo/local-opportunity-promoter.js` | Stays; writes through the guard |
...
4. **Authority policy** — `seo_link_policy`, decision function + tests, owner cards,
   ...
   bridge (gated: selection-only while the gate is off; a domain the weekly enrichment has not reached — enriched_at null — is not selected until it is, so INVALID never bounces it to the investigator for a nightly model pass; ONE bell per run, never per card; every bridged
