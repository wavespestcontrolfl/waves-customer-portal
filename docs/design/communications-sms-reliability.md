...
- Composing starts after choosing a recipient. A thread or saved draft supplies its sending line. Selecting a different line’s reply preserves an existing draft’s sender and reply target together and explains how to change them explicitly. A new message without a saved line requires an explicit sender selection; the draft store never guesses a location number. Malformed recovery JSON is repaired on the next edit.
...
- The active, visible SMS inbox refreshes only its message log every 30 seconds and when the browser tab becomes visible. Inactive channels pause new reads. Statistics and blocklist reads run on initial/manual loads, not background polls; stale statistics filters remain available after a failed refresh. Superseded searches are cancelled, failed reads retain existing messages with a retry action, and updated receipts replace existing messages even when the message count is unchanged.
