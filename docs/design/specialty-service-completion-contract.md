...
+ trend chip; no customer rating prompt) → Visit timeline (D2 amendment,
owner-directed via `universal-onetime-services-plan.md` — trend types with 2+
visits only; dates + each visit's own frozen Today's Result headline, derived
from the same bounded history series as the gauge; one-shot types and first
visits render none) → Findings (customer labels, reportPriority order) → What
we did / products → Photos → Reentry/advisory (when products applied) →
Review CTA → footer. Never: Pest Pressure card, pressure trend, lawn program
cards. Metrics band swaps pressure_index for the activity level (gauge types)
or drops it (non-gauge) and must look intentional with few metrics.
