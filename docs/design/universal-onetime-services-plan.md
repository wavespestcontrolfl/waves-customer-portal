...
2. `ProjectReportViewPage` (`/report/project/:token`) remains for WDO,
   pre-treat cert, Phase-3-pending termite, and historical sends: restyle the
   non-compliance project reports to the current customer-facing shell standard
   (glass navy per `waves-customer-facing-design-brief.md`). WDO inspection reports
   and pre-treatment termite certificates keep the navy/beige paper presentation
   (owner ruling 2026-07-16; `ProjectReportViewPage.jsx`, `isPaperDocument`) and never
   mount the glass scene. Snapshot/as-sent semantics untouched.
