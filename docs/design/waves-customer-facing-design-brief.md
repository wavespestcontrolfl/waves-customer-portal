...
invoice, portal and follow-up. Two deliberate exceptions: official compliance documents (WDO inspection reports and
pre-treatment termite certificates keep the navy/beige paper look — owner ruling 2026-07-16;
`ProjectReportViewPage.jsx`, `isPaperDocument`), and the marketing-site estimate embed
(`?website=1`, generated by `server/routes/public-quote.js`), where `EstimateViewPage.jsx`
renders `WebsiteEstimateFrame` instead of the glass theme. The warm-serif direction sketched above is superseded on
those surfaces.
