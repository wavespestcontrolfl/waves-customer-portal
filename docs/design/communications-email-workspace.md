...
Release verification logs are under `.tmp/email-release/`; the final browser report and screenshots are under `.tmp/email-workspace/after/`. The `browser-previews` CI job runs this harness (Chromium plus WebKit) and uploads `.tmp/email-workspace/` with its evidence. PR descriptions carry native screenshots and the requirement-to-test review map. Child PRs are reviewed against their stated parents; the main-branch integration gate applies after retargeting for release.
