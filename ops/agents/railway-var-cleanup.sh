...
if [ "${#VARS[@]}" -eq 0 ]; then
  echo "usage: $0 [--execute] [--service <name>] VAR [VAR...]" >&2
  exit 2
fi
...
EXISTING="$(railway variables --service "$SERVICE" --json | jq -r 'keys[]')"

MISSING=0
for v in "${VARS[@]}"; do
  if ! printf '%s\n' "$EXISTING" | grep -qxF "$v"; then
    echo "SKIP  $v — not set on $SERVICE"
    MISSING=$((MISSING + 1))
    continue
  fi
  if [ "$EXECUTE" = "1" ]; then
    echo "DELETE $v"
    railway variable delete "$v" --service "$SERVICE"
  else
    echo "WOULD DELETE  $v   (railway variable delete $v --service $SERVICE)"
  fi
done
