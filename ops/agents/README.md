...
| `lawn-visit-assessment-eval.js` | READ-ONLY | Stage-2b eval for the single-call lawn visit assessment (`server/services/lawn-visit-assessment.js`, `GATE_LAWN_VISIT_ASSESSMENT`), two phases: `--export` under `railway run --service Postgres` writes a fixture of CONFIRMED assessments (ids, dates, photo keys, confirmed + legacy AI scores, grass / irrigation / prior-summary context — no names, notes or photo bytes; `--ids`, `--sample N`, `--all`); `--run <fixture>` under `railway run --service waves-customer-portal` reads the photos from S3, makes LIVE model calls, and reports per-metric MAE/bias vs the confirmed and legacy AI scores, not-determinable and unavailable rates, the model's naming discipline, latency p50/p95, tokens and estimated cost (`--thinking LOW\|MEDIUM\|HIGH`, `--repeat N` for run-to-run variance, `--force-fallback` to prove the GPT-6 Astra leg, `--out results.json`). The LLM ledger gates are deleted before load and the run ABORTS if one still resolves enabled — nothing is written anywhere. Scoring is unit-tested in `server/services/eval/lawn-visit-assessment-eval.js`. |
