...
| `lawn-visit-assessment-eval.js` | READ-ONLY | Stage-2b eval for the single-call lawn visit assessment (`server/services/lawn-visit-assessment.js`, `GATE_LAWN_VISIT_ASSESSMENT`), two phases, **stdout only — it never writes a file**: `--export` under `railway run --service Postgres` prints a fixture of CONFIRMED assessments (ids, dates, photo keys, confirmed + legacy AI scores, legacy grass / irrigation / turf-height / prior-summary context omitted with reasons because these rows have no saved prompt snapshot or pre-call timestamp; the fixture records the export's `GATE_LAWN_PROPERTY_HISTORY` setting as `propertyHistory` — no names, notes or photo bytes; `--ids`, `--sample N`, `--all`; every count flag must be a positive whole number or the script stops before any export or paid call); `--run <fixture>` under `railway run --service waves-customer-portal` reads the photos from S3, makes LIVE model calls, and prints per-metric MAE/bias vs the confirmed and legacy AI scores, not-determinable and unavailable rates, the model's naming discipline, latency p50/p95, tokens and estimated cost — every billed leg, a rejected primary answer and both legs of an unavailable run included — as markdown (`--json` for the full results; `--thinking LOW\|MEDIUM\|HIGH`, `--repeat N` for run-to-run variance, `--force-fallback` to prove the GPT-6 Astra leg). The LLM ledger gates are deleted before load and the run ABORTS if one still resolves enabled. Both report formats retain the prompt version, full prompt digest and fixture history setting, and disclose omitted context. These legacy comparisons are not exact prompt replays. Scoring is unit-tested in `server/services/eval/lawn-visit-assessment-eval.js`. |
