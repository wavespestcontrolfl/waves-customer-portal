...
| `cf-pages-flip.js` | MUTATES (dry-run default) | Sets `PUBLIC_*` build variables on ONE Cloudflare Pages project (`--project=<name>`, `--vars=<json file>`) and creates a fresh production deployment — the hub dark-ship flips (`PUBLIC_TEXT_US`, `PUBLIC_ASK_WAVES_AUTO_PROMPT`, `PUBLIC_POSTHOG_HOST`, …) without the Cloudflare dashboard. Dry run prints projects, the target's env KEYS (never values), the latest deploy and the planned adds/overwrites. Needs `CF_API_TOKEN` + `CF_ACCOUNT_ID` via `railway run --service waves-customer-portal`. Preview env untouched. |
