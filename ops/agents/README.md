...
| `listen-transcripts.js` | MUTATES (dry-run default) | `/listen`: mines the operator's own Claude Code + Codex session transcripts (turns inside the last `--hours`, judged per turn) for blog-post ideas. `extract` keeps user/assistant prose only (tool results, thinking and every harness block dropped), runs `pii-redactor` before the FAST-tier LLM pass, applies the canonical geo / compliance gates, withholds turns carrying credentials or low-confidence redactions, and writes a category-seed manifest only with `--out` + `--execute`; `seed --file [--only=ids] --execute` validates it through `category-seed-seeder.loadManifest`, re-runs the targeting rulings on the (possibly hand-edited) manifest, runs every brief through the runner's own topic-targeting gate against the live Astro corpus (required), flags rows already queued or dismissed rows whose old draft is still reviewable, and seeds the rest at `pending_review` so the owner must release each one from the content review queue. Outbound blog content only — never customer comms. |
