...
| `enroll-stranded-consents.js` | MUTATES (dry-run default) | Backfill-enrolls customers whose enrollment-qualifying (v8+) pay-page consent predates the Auto Pay enrollment wiring (owner ruling 2026-08-26). Cohort derived through the runtime consent taxonomy (`payment-method-consents.js`); each enrollment runs the canonical `enrollConsentedMethod` path (expired-card refusal, incumbent arbitration, idempotency, autopay_log, gated confirmation email) with `authorizedAt` = the consent instant so a later explicit disable refuses. `--execute` requires an `--expect=<N>` pin from the dry run and refuses a drifted cohort. |
