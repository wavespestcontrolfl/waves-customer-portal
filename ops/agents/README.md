...
| `affiliate-baseline.js` | READ-ONLY | Captures the **search/traffic half** of the affiliate pilot's pre-activation baseline (`docs/affiliate-links-pilot.md`): per-URL Search Console clicks/impressions/position plus GA4 pageviews over a window (default 90 days, `--end` defaults to a 3-day GSC lag), filtered to the blog lane and flagged against the pilot's excluded topic classes. Queries `searchanalytics` directly rather than the service's `syncPages()`, so it never writes `gsc_pages`. GA4 rows are summed per path (`getTopPages` splits one path across title changes). Warns on row-cap truncation, `--url` values that matched nothing, and GSC-only rows. **PARTIAL — not sufficient to activate links:** estimate starts, calls, CTA clicks and geography are not captured, so the documented stop rule (service-conversion drop) cannot be evaluated from this alone. `--json` stores a reproducible snapshot; re-running the same `--end` reproduces it. |
