...
| `reset-out-of-area-coords.js` | MUTATES (dry-run default) | Clears customer coordinates that lie OUTSIDE the service-area box (`server/services/service-area.js`) so the hourly geocoder backstop sweep re-geocodes them through the PR #3802 guard (coarse / partial / out-of-area Google answers now stay null). Per customer, one transaction: guarded `customers` reset (exact prior coordinates), the matching `customer_properties` mirror rows, and an `audit_log` `customer.geocode.reset` row carrying the previous coordinates (reversible). Pins inside the box are never touched. First run 2026-09-03: 16 rows. |
