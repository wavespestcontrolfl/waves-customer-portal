...
| `listen-transcripts.js` | MUTATES (dry-run default) | `/listen`: mines the operator's own Claude Code + Codex session transcripts (last `--hours`) for blog-post ideas. `extract` keeps user/assistant prose only (tool results + thinking dropped), runs `pii-redactor` before the FAST-tier LLM pass, applies the waves-content targeting rulings, and writes a category-seed manifest; `seed --file [--only=ids] --execute` validates it through `category-seed-seeder.loadManifest`, flags rows already queued or already live, and seeds the rest at `pending_review` so the owner must release each one from the content review queue. Outbound blog content only — never customer comms. |
