...
| `replay-sms-profile.js` | MUTATES (dry-run default) | Re-extracts one analyzed inbound SMS through the existing gated extractor. Preview reports actual field dispositions and rolls back capture writes; `--execute` persists review proposals. Both modes use an LLM call. Prior writes, terminal receipts and handled proposals remain protected. |
