...
| `auto-order-revoke.js` | MUTATES (dry-run default) | Revokes ONE automatic vendor order (`--order=<vendor_orders.id>`, `placed` rows only — a `placing` row is still the dispatcher's and is refused): ledger row → `needs_review`, its restock request `ordered` → `open`, critical `procurement.vendor_order.revoked` audit row. Nothing is sent to the vendor — cancel with Sticker Mule / SiteOne by hand. The unique ledger claim means the request never auto-dispatches again; raise a fresh request to re-enter. `--list` prints this month's placing/placed rows. |
