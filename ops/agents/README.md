...
| `replay-sms-profile.js` | MUTATES (dry-run default) | Explicitly re-extracts one already-analyzed inbound SMS into the existing staff-review proposal queue. Preserves prior writes, original analysis and receipts; requires the SMS gate and activation timestamp. Dry run performs no LLM calls; `--execute` uses the existing extractor and a separate idempotent replay receipt. |
