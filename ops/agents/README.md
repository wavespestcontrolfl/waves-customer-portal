...
| `link-unlinked-visit-invoices.js` | MUTATES (dry-run default) | One-shot, human-reviewed repair for office-raised invoices never attached to the visit they billed (the Invoices page could only link completed visits): pairs each unlinked live invoice with the customer's single live add-on-free visit on its service date when a positive line reads like that visit's catalog service (fee/product lines never count), refuses visits already carrying an invoice, writes the reviewed list to `--plan-out`, and `--execute --plan=<file>` links ONLY those pairs in one transaction under the visit mint-lock chain with the full rule re-evaluated on locked rows (drift aborts). The pairing heuristic lives only here — the runtime never infers a link. Clears the false "unbilled" leak flags those visits raise. |
