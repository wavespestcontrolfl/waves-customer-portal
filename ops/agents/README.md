...
3. No secrets, tokens, customer names, or invoice numbers in filenames or file
   ...
   injects them). Results print to stdout unless the script's index entry and
   usage explicitly document a local JSON, Markdown, or plan artifact; those
   artifacts follow the same restrictions on secrets, customer names, and
   invoice numbers.
...
|---|---|---|
| `replay-no-show-detector.js` | READ-ONLY | Replays operator-supplied timestamped visit and communication evidence at every five-minute decision point; writes local JSON/Markdown reports comparing 45- and 60-minute warnings. No DB or provider access. |
...
| `triage-backlog-sweep.js` | MUTATES (dry-run default) | One-time drain of the Needs Review backlog after the 2026-09-08 call-agent audit: dismisses open cards filed by the two retired flags (`caller_not_authorized` only where the caller was owner / spouse / unknown), cards whose call led to a booking within 3 days, advisories older than `--advisory-days` (30) and any other hold older than `--stale-days` (30); owed-work codes (including explicit third-party authorization holds) only age out; `missing_unit_number` is never swept; re-syncs `call_log.review_status`. Limited to cards filed before 2026-09-09 ET. Apply and `--revert=<run-tag>` take shared per-call/sibling locks, re-read eligibility, and update cards plus call aggregates atomically; reversal skips reasons with a newer open/claimed card. Both default to dry run and list planned card IDs, rules, transitions and call aggregate changes. Invalid flags fail before connecting. |
...
| `replay-sms-profile.js` | MUTATES (dry-run default) | Re-extracts one analyzed inbound SMS through the existing gated extractor. Preview reports actual field dispositions and rolls back capture writes; `--execute --preview-hash=<hash>` persists review proposals only when the extraction and locked decisions exactly match the preview. Both modes use an LLM call; a mismatch rolls back for a fresh preview. Prior writes, terminal receipts and handled proposals remain protected. |
...
| `link-unlinked-visit-invoices.js` | READ-ONLY | Human-reviewed candidate planner for historical unlinked invoices. Refuses callbacks (visit or record), composite programs, add-ons, existing bills/dispositions, grouped closeouts, and mismatched application/Bill-To evidence. `--plan-out=FILE` always replaces a versioned plan with all proposed links, technician ID/name changes, and review digests. Uses an explicit `REPAIR_DATABASE_URL` and a read-only snapshot; `--execute` is refused. Transactional execution is reviewed separately. |
| `execute-visit-invoice-plan.js` | MUTATES (dry-run default) | Applies only a version-2 reviewed invoice-link plan with `--execute --plan=FILE` and an explicit `REPAIR_DATABASE_URL`. The default dry run is a faithful rehearsal: it connects with the same `REPAIR_DATABASE_URL`, takes the same locks, revalidates and applies the plan, prints what execution would do, and rolls back. Execution holds mint, customer/invoice/visit, completion-record, and payer locks, reuses the planner eligibility checks, compares the complete pairing/digest, and atomically applies only the reviewed links and technician ID/name updates. Drift or any error rolls back the whole batch; no communications. |
...
| `primary-property-backfill.js` | MUTATES (dry-run default) | Creates the lazily-backfilled PRIMARY `customer_properties` row for live, addressed customers that have no property row at all (customers created after the 2026-06-29 migration through a quote / lead / webhook path that never read the properties), through the service's own `ensurePrimaryProperty` (source `backfill`). Dry run lists every candidate id + stage. Execute prints the created ids and a lock-then-delete rollback scoped to them and guarded by every FK that references `customer_properties`. First dry run 2026-09-07: 144. `--limit N` caps a run. |
| `twilio-number-audit.js` | READ-ONLY | Twilio number **configuration** audit, per owned number: registered in `server/config/twilio-numbers.js` (an unregistered number has its inbound SMS DROPPED); routing matches `scripts/twilio/routing-contract.js` (voice/fallback/status-callback/SMS URLs + methods, App/trunk overrides — the 2026-08-12 Google Ads → Sandy-sandbox misroute); assigned to every LIVE Trust Hub product (customer profile, SHAKEN/STIR, CNAM, Voice Integrity, Branded Calling — the console's per-number "Not started" items); A2P sender-pool / toll-free-verification coverage. The relay sandbox line (`VOICE_RELAY_SANDBOX_NUMBER`) is reported separately, never against the fleet. Exits 1 on defects. No database. Companion: `npm run twilio:inbound:audit` (`scripts/twilio/audit-inbound-routing.js`) is the call-**traffic** audit on the same contract. Run after every number purchase and before flipping a lane that sends from a new line. |
| `lawn-visit-assessment-eval.js` | READ-ONLY | Versioned legacy assessment export and opt-in live replay against the single-call visit service. Reports missing historical context, unknown scores, usage and prompt provenance; replay disables ledger writes. |
| `uniform-image-audit.js` | READ-ONLY | Vision audit of blog images for a Waves technician OUT of the real uniform (owner directive 2026-09-23: red polo, light-blue or red cap, dark pants). Walks `--dir` recursively (the Astro `public/images/blog` root), keys every image as `category__slug__file.webp` (the key `uniform-image-regenerate.js` consumes), flags a technician with a wrong shirt/cap/pants or a visible bare head, and writes one JSON report (`toFix` + per-image rows). Fail-open per image (unreadable → `error` row, never dropped). LIVE vision calls; no DB, no prod writes. |
| `uniform-image-regenerate.js` | MUTATES (dry-run default) | Regenerates ONLY the images an audit report flagged, through the publisher's own `generatePlannedImage` (same slug/slot plan + `WAVES_UNIFORM_LINE`), re-runs the uniform classifier on the result (a still-wrong replacement retries once with a fresh seed, then is left untouched), re-derives alt from the new picture, and bumps the post's lastmod (ET). Writes into a LINKED astro worktree only (`--astro`, refuses a main checkout); never touches this repo or prod. `--avoid "a; b"` carries a post's brief `image_avoid` exclusions when the post has them (they are not recoverable from the astro repo). Writes `<report>.regen.json` beside the report. |
