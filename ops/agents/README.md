...
| `affiliate-baseline.js` | READ-ONLY | Captures the pre-activation baseline the affiliate pilot requires (`docs/affiliate-links-pilot.md`): per-URL Search Console clicks/impressions/position plus GA4 pageviews over a window (default 90 days, `--end` defaults to a 3-day GSC lag), filtered to the blog lane and flagged against the pilot's excluded topic classes. Queries `searchanalytics` directly rather than the service's `syncPages()`, so it never writes `gsc_pages`. `--json` stores a reproducible snapshot; re-running the same `--end` reproduces it. |
