...
| `completion-charge-why.js` | READ-ONLY | Replays the appointment-card completion-charge lane for ONE visit (`--visit=<scheduled_service_id>`) and names the first condition that blocked the auto-charge — gate off, no `/secure` lane row, hold-rail exclusion, sticky `accepted_amount = 0`, over-cap invoice, or no chargeable Auto Pay method. Also prints the office bells raised and which completion SMS actually sent. Exit 0 = every condition verified and passing, 1 = blocker found, 2 = inconclusive (something — usually the portal-side gate — could not be verified, so the lane is never reported clean on an unchecked condition). Auto Pay is evaluated with the production `autopay-eligibility` helpers so the ET pause window and card-expiry rules cannot drift. |
