...
| `execute-visit-invoice-plan.js` | MUTATES (dry-run default) | Applies only a version-2 reviewed invoice-link plan with `--execute --plan=FILE` and an explicit `REPAIR_DATABASE_URL`. Default prints the saved artifact without connecting. Execution holds mint, customer/invoice/visit, completion-record, and payer locks, reuses the planner eligibility checks, compares the complete pairing/digest, and atomically applies only the reviewed links and technician ID/name updates. Drift or any error rolls back the whole batch; no communications. |
