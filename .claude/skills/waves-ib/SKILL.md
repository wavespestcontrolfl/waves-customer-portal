...
stripe-ops, github-ops, store-ops, growthbook) on EVERY admin context — infra
reads are context-independent, and the shared `INFRA_PROMPT` guidance is
appended for admin requests; technician tokens never see or execute them.
