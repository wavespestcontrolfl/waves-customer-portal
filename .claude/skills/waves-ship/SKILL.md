...
- Fresh portal worktrees need `npm ci` before ANY node code runs out of them — not only tests. `node_modules/` is gitignored, so a new worktree starts empty and a correctly-declared dependency still fails with `MODULE_NOT_FOUND`. This bites hardest when an EXTERNAL tool requires modules out of the worktree PATH instead of running inside it: the error names the dependency, not the missing install, so it reads like a repo bug and sends the session hunting a phantom (Aug 2026: the `waves-ops` rain-engine watch loaded `service-report/application-conditions.js` from `~/wt-rain-watch` and died on `Cannot find module 'winston'` — winston was declared in `server/package.json` the whole time; the worktree just had no install, and a live MRMS verification window burned days on it). Before treating a missing module as a code defect, check `package.json` for the dep and `ls node_modules/<dep>` in the worktree it loaded from. Long-lived probe worktrees need the install re-run after any recreate. `@waves/*` workspace packages are absolute symlinks; an interrupted install makes jest hang silently (fix: `rm -rf node_modules && npm ci`).
