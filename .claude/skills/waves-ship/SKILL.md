...
- Run the full merge gate in CHECKLIST.md. Core rules:
  ...
  - Patch-equivalent clean verdict: a clean issue comment on an earlier commit `R` also completes the final HEAD `H` when all three hold: `R` is an ancestor of `H` (no rebase or force-push since), no Codex finding is open, and `git diff $(git merge-base origin/<base> R) R | git patch-id --verbatim` equals the same command for `H`. That is the shape of a plain merge of the base branch that left the PR's own changes untouched. A conflict resolution or any edit to the PR's files changes the id and needs a fresh round. CI must still pass on `H`. Locally, `~/.claude/bin/codex-status` reports this as `CLEAN_EQUIV` (exit 0); anything it cannot prove stays `STALE`.
  ...
  - NEVER merge until Codex has completed on the final commit, directly or through the patch-equivalent clean verdict above. Green CI + COMMENTED is not enough.
