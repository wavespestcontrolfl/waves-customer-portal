...
- The fallback auditor is sandboxed because the diff it reads is untrusted input (`--restricted --strict-mcp-config` plus an explicit disallow list that includes `ToolSearch`). After any `claude` CLI upgrade run `scripts/verify-fallback-auditor-sandbox.sh` — it proves toollessness against ground truth on disk, not the model's self-report.
