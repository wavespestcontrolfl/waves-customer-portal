...
- The pre-push hook runs a blocking LLM audit and blocks on P0. **Budget for it: Codex alone is usually ~30–60s, but a ref that falls through to the Claude fallback can take up to ~8 minutes** (180s Codex ceiling, then 300s Claude), and a multi-ref push pays that per ref. That is the expected shape when Codex is out of quota, not a hang — do not reach for the bypass because it is slow. Only bypass (`SKIP_CODEX_REVIEW=1 git push --no-verify --no-thin`) when the external branch-hijack strikes — see REFERENCE.md.
