...
- **Review tier — pick it before the PR opens and name it in the PR body.** Full is the default; Light only when every touched file qualifies, and a single touched file on the Full list makes the whole PR Full.
  ...
  - *Full*: anything on the CLAUDE.md rule-18 list or an AGENTS.md P0 domain (money, customer comms, schema/CHECK values, public token routes, webhook payloads, admin auth, app-consumed endpoints, spoke-fleet posts and feeds, persisted identifiers), plus any PR over the size rule. Full ladder below, plus a `live-verify` verdict before merge: an agent that did not write the change runs it on the highest evidence rung the surface allows (a dev/preview stack where one exists) and posts PASS / PASS+NOTES / FAIL / BLOCKED on the PR (`.claude/skills/live-verify/SKILL.md`). Run it once the head is code-ready, before the first `@codex` tag, so behavior bugs surface before review rounds are spent.
...
- Clean = the §4 gate on the final HEAD plus successful final-HEAD CI: zero unresolved P0/P1 (a P0/P1 rebuttal is resolved only after Adam accepts it or a confirmation round on the same head leaves it undisputed; a re-disputed one is unresolved), every P2 fixed, rebutted, or listed as deferred in the PR body; for Full tier, a `PASS` or `PASS+NOTES` `live-verify` verdict whose patch-id matches the final head. A rebutted round qualifies. A round-cap stop (new P0/P1 at round 5+) does NOT — that PR goes back to Adam with a split proposal.
