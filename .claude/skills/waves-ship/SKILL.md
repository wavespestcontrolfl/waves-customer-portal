...
- The pre-push hook runs a blocking LLM audit and blocks on P0. **Budget for it: Codex alone is usually ~30–60s, but a ref that falls through to the Claude fallback can take up to ~13 minutes** (180s Codex ceiling, then a 600s Claude budget covering both of its attempts), and a multi-ref push pays that per ref. The fallback prints its own ceiling when it starts, so the terminal tells you what it is waiting on. That is the expected shape when Codex is out of quota, not a hang — do not reach for the bypass because it is slow. Only bypass (`SKIP_CODEX_REVIEW=1 git push --no-verify --no-thin`) when the external branch-hijack strikes — see REFERENCE.md.
- **Round 0 has two auditors.** Codex is primary; when it cannot produce a verdict (usage limit, timeout, empty or malformed output, not installed) the hook falls back to a sandboxed `claude -p` run using the same schema and the same P0 gate, and the verdict header names which one ran. A pass-through is only real when the output says `UN-AUDITED`, which now means BOTH failed — that is the only case where the GitHub `@codex` round is the sole audit, and it still must be stated in the PR body. A Codex usage limit alone no longer darkens round 0.
- The fallback auditor is sandboxed because the diff it reads is untrusted input (`--restricted --strict-mcp-config` plus an explicit disallow list that includes `ToolSearch`). After any `claude` CLI upgrade run `scripts/verify-fallback-auditor-sandbox.sh` — it proves the auditor cannot read a file, write a file, load a tool through ToolSearch, or reach a local HTTP canary, each decided by ground truth on disk or in the listener's log rather than the model's self-report, and it fails rather than reporting green if its own control probes cannot detect a tool that IS present. You do not have to remember: the hook reads `scripts/.fallback-auditor-verified` on every fallback run and warns when the installed CLI version or the sandbox definition has moved since it was last proved. The warning does not block, because refusing on an unrecognised CLI would re-darken round 0 on the day after every upgrade. `scripts/test-verdict-guards.sh` covers the three functions that decide whether a fallback verdict is trustworthy, and `scripts/test-sandbox-fingerprint.sh` covers the drift tripwire itself — the fingerprint hashes the extracted `claude -p` invocation as well as the flags, env allowlist and disallow list, so deleting `env -i` from the shipping command trips the warning instead of passing silently, while merely re-indenting it does not. `scripts/test-referenced-context.sh` covers the third: the fallback has no tools, so the hook inlines the tracked, in-repo files the diff's added `require`/`import` lines point at (capped at `CLAUDE_CONTEXT_MAX_FILES`/`CLAUDE_CONTEXT_MAX_BYTES`, `CLAUDE_CONTEXT_MAX_FILES=0` kills it) so it stops hedging about files it cannot open. Those paths come out of an untrusted diff, so that suite is a security test: it pins path-escape, symlink-escape, untracked-file (`.env`), node_modules and extension rejection. All three are no model calls, about a second — run them before changing any of the functions they cover.
- **Split advisory:** a diff over 25 files or 120 KB prints a SPLIT ADVISORY at push time. It never blocks. Treat it as the prompt to cut siblings off main *before* the first `@codex` tag — §4's round cap otherwise discovers the same thing at round 5, after those rounds are spent.
...
- **Review tier — pick it before the PR opens and name it in the PR body.** Full is the default; Light only when every touched file qualifies, and a single touched file on the Full list makes the whole PR Full.
  - *Light*: docs, tests, `ops/` scripts, `.claude/` instructions, and code that ships dark behind an unset `GATE_*` flag with no schema change. Pre-push hook + one Codex round on the final head; the P2-only merge rule applies from round 1. A P0/P1 in that round is fixed or rebutted like any other and still needs its confirmation round on the new head — Light changes where the P2-only rule starts, never the final-head or P0/P1 gates below.
  - *Full*: anything on the CLAUDE.md rule-18 list or an AGENTS.md P0 domain (money, customer comms, schema/CHECK values, public token routes, webhook payloads, admin auth, app-consumed endpoints, spoke-fleet posts and feeds, persisted identifiers), plus any PR over the size rule. Full ladder below.
- **Size rule: over ~600 changed lines → split BEFORE opening the PR.** Every PR that needed 5+ rounds in the 2026-09-04..07 window was over 600 lines, and the round cap only catches it after four rounds are spent. Mechanical bulk (fixtures, generated files, lockfiles) does not count toward the line total.
- **Review map in the PR body** (every Full PR; one line per requirement is enough for Light): `requirement → files carrying it → invariant that could break → exact command that proves it`. Codex reads the body before the diff; a round spent on misread intent is the commonest wasted round.
- **Tag only a settled head**: pre-push hook clean, `scripts/verify-pr-checks.sh` passed (portal; astro = the manual mergeable + Pages-build check below), and every fix you already know about is pushed. Never tag mid-fix or on a head you expect to replace — that round is spent on a stale SHA. Tag bare `@codex` on a fresh PR; after each subsequent push, post `@codex review` (a bare re-tag is a no-op; a quote-reply "> @codex" spawns a cloud code-editing task, not a review).
...
- **Siblings off `main` beat a stack. Default to siblings.** A stack re-reviews every level as fixes propagate upward, serialises the merges bottom-up, and pays a rebase on each one — a chain like #4029 → #4260 → #4254 reviews the same code several times over. Cut independent PRs against `main` instead and let them converge in parallel; the re-cut of #4249 into five siblings (#4357–#4361) is the worked example. Stack only when a child genuinely cannot compile or be reviewed without its parent, and then keep it two deep at most.
- Stacked PRs, when you do stack: open children as DRAFT and retarget them to `main` BEFORE the parent squash-merges — squash merges strand children (recurred 4×; GitHub only auto-retargets on head-branch deletion, which squash-merge flows don't guarantee).
- Run the full merge gate in CHECKLIST.md. Core rules:
  ...
  - Carried-over clean verdict: a clean issue comment on an earlier commit `R` also completes the final HEAD `H` when no Codex finding is open and every commit on the first-parent chain from `R` to `H` is a clean automatic merge of the base branch. Each one has exactly two parents, its first parent is the previous commit on the chain (`R` for the first), its second parent is on the base branch, and its tree equals `git merge-tree --write-tree <parent1> <parent2>` with no conflicts. Any hand edit, conflict resolution, rebase, or ordinary commit after `R` fails this and needs a fresh round. CI must still pass on `H`. Owner ruling 2026-09-26: changes on the base branch that the PR depends on are not re-reviewed under this rule; final-head CI is the backstop. Locally, `~/.claude/bin/codex-status` reports this as `CLEAN_EQUIV`; anything it cannot prove stays `STALE`.
  ...
  - Carry earlier findings forward until the fixing diff or accepted rebuttal is verified. A later clean summary or a stale inline anchor does not resolve an unchanged defect; reconcile the paginated review history against the final code before merging.
