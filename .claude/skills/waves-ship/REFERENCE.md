...
- If Codex cannot produce a verdict (including an empty verdict), the hook runs
  its sandboxed Claude fallback auditor. The hook passes fail-open with an
  `UN-AUDITED` warning only when both auditors fail; only then is the GitHub
  `@codex` round the sole audit on that push.
