...
- [ ] `@codex` (fresh PR) or `@codex review` (subsequent push; skip it for a push that only merged the base branch when a clean verdict carries over, see the Codex gate) posted and not bounced — only on a settled head: hook clean, `verify-pr-checks.sh` passed (portal) / mergeable + Pages build checked (astro), no known fix still unpushed
...
- [ ] Codex completed on the final HEAD: either the clean issue comment (Reviewed-commit SHA == final HEAD), a carried-over clean verdict (waves-ship §4: every commit since the reviewed one is a conflict-free automatic merge of the base, no open findings, CI green on the final HEAD), or a review whose findings anchor to the final HEAD
...
- [ ] **Last box before `gh pr merge`, after every wait above** — diff adds or changes a repo-scanning gate (`scripts/check-*.js`, an `npm run check:*` step in the `gates` job) → `git fetch origin && git merge origin/main` in the worktree and re-run that gate NOW. Fast-forward/no-op → merge. Anything changed → push it, then loop back through **Post-push** (remote tip, hijack recheck, `verify-pr-checks.sh`, re-tag), the CI green gate and the Codex gate for the NEW head (pushing only starts CI; Codex must complete on the new SHA, which a conflict-free automatic merge of main satisfies through the carried-over clean verdict, so re-tag only if that check fails), and repeat this box. A green run proves the gate only against main as of that run, and the 15-min Codex wait is itself a window in which main can take the very sites or patterns the gate rejects.
