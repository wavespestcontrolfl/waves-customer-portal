...
- [ ] Diff adds or changes a repo-scanning gate (`scripts/check-*.js`, an `npm run check:*` step in the `gates` job) → re-run that gate on a fresh merge with `origin/main` right before merging (`git fetch origin && git merge origin/main` in the worktree, then the check; push if anything changed so CI proves the same tree). A green `tests` run proves the gate only against main as of that run — later merges to main can carry the very sites or patterns the gate rejects, and main goes red on its first post-merge run.
